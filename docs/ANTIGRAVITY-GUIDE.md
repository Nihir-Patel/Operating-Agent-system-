...
`.agents/` directory. OAS's Antigravity target installs native rules, workflows,
skills, and custom agents into that directory.

Native Antigravity 2.0 installation requires OAS 2.2.0 or newer. OAS 2.1.0 uses
...
npx oas-universal@2.2.0 install --profile minimal --target antigravity
```
...
# Keep the OAS source checkout separate and use its absolute path.
OAS_ROOT="/absolute/path/to/OAS"

# Install the minimal profile
"$OAS_ROOT/install.sh" --profile minimal --target antigravity

# Compatibility syntax: common rules plus only these language packs
"$OAS_ROOT/install.sh" --target antigravity typescript python go
```

PowerShell uses the same project-root working-directory contract:

```powershell
$EccRoot = "C:\absolute\path\to\OAS"
...
| OAS source | Antigravity destination | Purpose |
|---|---|---|
...
OAS does not copy the repository's `.agents/` directory wholesale. That source
...
```bash
node "$OAS_ROOT/scripts/list-installed.js" --target antigravity
node "$OAS_ROOT/scripts/doctor.js" --target antigravity
...
Antigravity still reads legacy `.agent/rules` and `.agent/skills`, but OAS now
uses the canonical `.agents/` layout. Do not rename `.agent` manually because
OAS install-state contains absolute managed paths.

Rerun the same OAS install command after updating. OAS writes and verifies the
new `.agents/oas-install-state.json` first, then removes only unchanged files
...
```bash
node "$OAS_ROOT/scripts/doctor.js" --target antigravity
node "$OAS_ROOT/scripts/repair.js" --target antigravity --dry-run
node "$OAS_ROOT/scripts/uninstall.js" --target antigravity --dry-run
```
...
- `.agent/.agents/skills` is an obsolete nested layout from older OAS builds.
...
See [CONTRIBUTING.md](../CONTRIBUTING.md) for OAS contribution guidance and
