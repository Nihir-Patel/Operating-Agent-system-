...
OAS accepts ideas from outside repos, but shipped skills need to become OAS-native surfaces.
...
- adapt it to OAS's current install surfaces, validation flow, and repo conventions
- remove unnecessary external branding, dependency assumptions, and upstream-specific framing

The goal is reuse without turning OAS into a thin wrapper around someone else's runtime.
...
- there is no better OAS-native name already in the repo
...
Rename the skill when OAS meaningfully expands, narrows, or repackages the original work.

Typical triggers:

- OAS adds substantial new behavior, structure, or guidance
- the original name is vendor-forward or community-brand-forward instead of workflow-forward
- the contribution overlaps an existing OAS surface and needs a clearer boundary
...
- prefer OAS-native names like `product-capability` over vague imported planning labels if the scope changed materially

## Dependency Policy

OAS prefers the narrowest native surface that gets the job done:
...
- vendor or recreate the relevant logic inside OAS when practical
...
1. Is this a real reusable surface in OAS, or just documentation for another tool?
2. Does the current name still match the OAS-shaped surface?
3. Is there already an OAS skill that owns most of this behavior?
4. Are we importing a concept, or importing someone else's product identity?
5. Would an OAS user understand the purpose of this skill without knowing the upstream repo?
