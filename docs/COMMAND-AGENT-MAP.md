...
|-------------|-----------------------|-------|
| `oas memory init` | unified-memory / `scripts/memory.js` | Initialize project, team, or user Markdown vault scopes |
| `oas memory save` | unified-memory / `scripts/memory.js` | Create unreviewed memory; body must come from stdin or a regular file |
| `oas memory handoff` | unified-memory / `scripts/memory.js` | Create a targeted, cross-harness handoff |
| `oas memory search` | unified-memory / `scripts/memory.js` | Bounded lexical search over selected vault scopes |
| `oas memory read` | unified-memory / `scripts/memory.js` | Read one memory plus derived backlinks |
| `oas memory doctor` | unified-memory / `scripts/memory.js` | Audit malformed files, duplicate IDs, broken links, and symlinks |
| `oas-memory-mcp` | unified-memory / `scripts/memory-mcp.mjs` | Optional stdio MCP adapter; exposes save/search/read/doctor only |
...
- **unified-memory**: `oas memory ...` and the opt-in `oas-memory-mcp` server
