...
Hermes is the operator shell. OAS is the reusable system behind it.
...
- OAS skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and OAS as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 OAS skills + hooks + MCPs + shared Memory Vault
        ↓
...
- `~/.hermes/config.yaml`
  ...
  - plugin loading
- `~/.hermes/skills/oas-imports/`
  - OAS skills copied in for Hermes-native use
...
OAS Memory Vault provides one file-first handoff layer instead of a separate
...
```bash
oas memory init --scope project --scope team
```
...
Hermes can call the CLI directly or use the opt-in `oas-memory-mcp` stdio
...
`OAS_MEMORY_HARNESS` identity; they must not connect to one shared server
process. Every process must launch from the same repository working directory
or receive identical `OAS_MEMORY_PROJECT_ROOT` and `OAS_MEMORY_USER_ROOT`
overrides.
...
```bash
oas memory search "research parser" --target-harness codex
oas memory read <memory-id>
```

For MCP access, copy only the `oas-memory-vault` entry from
`mcp-configs/mcp-servers.json` into each harness that needs it. OAS does not
enable this server in the default `.mcp.json`. Launch each server with its own
lowercase identity, for example `OAS_MEMORY_HARNESS=hermes`. The server binds
...
`OAS_MEMORY_ALLOW_USER_SCOPE=1`, and the tool call must request `user`.
...
- OAS for skills, rules, prompts, and cross-harness conventions
- OAS Memory Vault for explicit, local-first agent handoffs
...
0. Run `oas migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto OAS2.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `oas migrate plan` and `oas migrate scaffold`
   - scaffold reusable legacy skills with `oas migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `oas migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `oas migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `oas migrate import-schedules --dry-run`
   - preview gateway dispatch with `oas migrate import-remote --dry-run`
   - preview safe env/service context with `oas migrate import-env --dry-run`
   - import sanitized workspace memory with `oas migrate import-memory`
1. Install OAS and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at OAS-imported skills.
3. Initialize the shared OAS Memory Vault. Register `oas-memory-mcp` only if
   Hermes needs tool access instead of the `oas memory` CLI.
...
OAS v2.0.0-rc.1 documents the Hermes surface and ships launch collateral now.
