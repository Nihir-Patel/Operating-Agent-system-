# OAS 2.2 launch and rollback runbook

Nihir is the only release operator for OAS 2.2. Everyone else may prepare,
...
The default npm install remains `oas-universal@2.1.0` until the final promotion
...
```bash
npm view oas-universal dist-tags --json
npm view oas-universal@2.1.0 dist.integrity
curl -fsSIL https://registry.npmjs.org/oas-universal/-/oas-universal-2.1.0.tgz
gh release view v2.1.0 --repo neal0709/Operating-Agent-system-
```
...
- A clean `npm install oas-universal@2.1.0` succeeds.
...
an adapted Markdown link. This does not prevent installation or uninstall. OAS
2.2 corrects the packed lifecycle and doctor behavior.

## Preflight before Nihir merges
...
5. `npm view oas-universal@2.2.0 version` must return E404. Any other registry
   error blocks the release.
6. `npm view oas-universal dist-tags --json` must still show `latest: 2.1.0`.

## The release switch

After Nihir merges PR #2863, wait for CI on the exact `origin/main` commit.
...
Nihir then creates and pushes the signed release tag:

```bash
git tag -s v2.2.0 -m "OAS 2.2.0" HEAD
git tag -v v2.2.0
...
```bash
npm view oas-universal dist-tags --json
npm view oas-universal@2.2.0 version dist.integrity
gh release view v2.2.0 --repo neal0709/Operating-Agent-system-
npx --yes oas-universal@2.2.0 setup --help
npx --yes oas-universal@latest setup --help
```
...
If 2.2.0 has an install-critical regression, Nihir or another authorized npm
owner restores the known installable fallback immediately:

```bash
npm dist-tag add oas-universal@2.1.0 latest
npm view oas-universal dist-tags --json
OAS_ROLLBACK_ROOT=$(mktemp -d)
npm install --ignore-scripts --prefix "$OAS_ROLLBACK_ROOT" oas-universal@2.1.0
node "$OAS_ROLLBACK_ROOT/node_modules/oas-universal/scripts/oas.js" --help
gh release edit v2.1.0 --repo neal0709/Operating-Agent-system- --latest
```
