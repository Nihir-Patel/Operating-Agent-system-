# OAS v1.10.0 Release Notes

## Positioning

OAS v1.10.0 is a surface-sync and operator-lane release.
...
OAS 2.0 is **real and usable as an alpha**, but it is **not general-availability complete**.

What exists today:

- `oas2/` Rust control-plane codebase in the main repo
- `cargo build --manifest-path oas2/Cargo.toml` passes
- `oas-tui` commands currently available:
  - `dashboard`
  - `start`
  - `sessions`
  - `status`
...
- You should not describe the full OAS 2.0 roadmap as finished.
- The right framing today is **OAS 2.0 alpha / control-plane preview**, not GA.
...
- The selective install / OSS path is still the most reliable full install for teams that want the complete OAS surface.
...
4. Treat OAS 2.0 as an alpha control-plane surface until the open P0/P1 roadmap is materially burned down.
