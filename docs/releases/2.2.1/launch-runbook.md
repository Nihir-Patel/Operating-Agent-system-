# OAS 2.2.1 signed patch release runbook
...
The default npm install remains `oas-universal@2.2.0` until the final promotion
...
`git tag -v v2.2.0` returns `error: no signature found`. OAS-031 closes that
...
4. `npm view oas-universal@2.2.1 version` must return `E404`. Any other
   registry error blocks the release.
5. `npm view oas-universal dist-tags --json` must still show `latest: 2.2.0`.
...
```bash
git tag -s v2.2.1 -m "OAS 2.2.1" HEAD
git tag -v v2.2.1
...
npm view oas-universal@2.2.1 version dist.integrity
gh release view v2.2.1 --repo neal0709/Operating-Agent-system-
npx --yes oas-universal@2.2.1 setup --help
...
```bash
npm dist-tag add oas-universal@2.2.0 latest
npm view oas-universal dist-tags --json
OAS_ROLLBACK_ROOT=$(mktemp -d)
npm install --ignore-scripts --prefix "$OAS_ROLLBACK_ROOT" oas-universal@2.2.0
node "$OAS_ROLLBACK_ROOT/node_modules/oas-universal/scripts/oas.js" --help
gh release edit v2.2.0 --repo neal0709/Operating-Agent-system- --latest
```
