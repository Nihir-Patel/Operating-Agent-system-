# OAS v2.0.0-rc.1 Release Notes

## Positioning

OAS v2.0.0-rc.1 is the first release-candidate surface for OAS as a cross-harness operating system for agentic work.

Claude Code remains a core target. Codex, OpenCode, Cursor, Gemini, and other harnesses are treated as execution surfaces that can share the same skills, rules, MCP conventions, and operator workflows. OAS is the reusable substrate; Hermes is documented as the operator shell that can sit on top of that layer.
...
- Clarified the split between OAS as the reusable substrate and Hermes as the operator shell.
- Documented the cross-harness portability model for skills, hooks, MCPs, rules, and instructions.
- Added a Hermes import playbook for turning local operator patterns into publishable OAS skills.
- Added Zed as a project-local planning/install target while keeping BYOK and OpenRouter secrets outside OAS-managed project files.
...
- Added a local [observability readiness gate](../../architecture/observability-readiness.md) for loop status, session traces, harness audit, and OAS2 tool-risk logs.
- Added the public teaser [Itô prediction-market skill pack](ito-prediction-market-skill-pack.md)
  ...
  and risk review. Live Itô API access remains gated and separate from OAS
  Tools billing.
...
- Published `oas-universal@2.0.0-rc.1` to npm on the `next` dist-tag. The
...
- OAS Tools roadmap mirrors for hosted analysis, fleet-summary consumption,
...
- gated Itô skill distribution as a public workflow teaser, not a live trading
  claim or a merge of OAS Tools and Itô ownership.
- a release URL ledger that separates links which already resolve from links
  that must wait for the plugin tag/directory, video upload, and OAS Tools
  billing readback.

## Why This Matters

OAS is no longer only a Claude Code plugin or config bundle.
...
7. Import only sanitized operator patterns into OAS skills.
8. Treat `oas2/` as an alpha control plane until release packaging and installer
...
- GitHub prerelease:
  <https://github.com/neal0709/Operating-Agent-system-/releases/tag/v2.0.0-rc.1>
- npm rc package:
  <https://www.npmjs.com/package/oas-universal/v/2.0.0-rc.1>
...
OAS Tools billing/native-payments readiness, and final outbound copy.
