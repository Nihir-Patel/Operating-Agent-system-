# OAS v2.0.0-rc.1 Preview Pack Manifest
...
| `skills/hermes-imports/SKILL.md` | Sanitized Hermes-to-OAS import workflow | Includes import rules, sanitization checklist, conversion pattern, and output contract |
...
| `docs/releases/2.0.0-rc.1/naming-and-publication-matrix.md` | Naming, slug, and publication-path decision record | Keeps `OAS`, npm `oas-universal`, and plugin slug `oas` for rc.1 |
...
| `docs/releases/2.0.0-rc.1/ito-prediction-market-skill-pack.md` | Public Itô skill-pack distribution note | Keeps Itô API access gated, non-advisory, and separate from OAS Tools billing |
...
operator patterns feeding back into OAS without merging business ownership.
...
place trades, do not provide investment advice, and do not merge OAS Tools with
...
skills should enter OAS only after they pass the same rules:
...
| Reference pressure | OAS preview-pack interpretation |
| --- | --- |
...
- npm `oas-universal@2.0.0-rc.1` on the `next` dist-tag live readback;
- Claude plugin tag / marketplace propagation for `oas@oas`;
...
- OAS Tools billing/product readiness evidence remains fresh: the May 20
