...
```bash
npm install oas-universal@next
```

`next` currently resolves to `oas-universal@2.0.0-rc.1`; `latest` remains on
...
for loop status, session traces, harness audit, and OAS2 tool-risk logs.

## First Skill

Read `skills/hermes-imports/SKILL.md` first.

It shows the intended OAS 2.0 pattern:
...
- Claude Code consumes OAS through the Claude plugin and native hooks.
- Codex consumes OAS through `AGENTS.md`, `.codex-plugin/plugin.json`, and MCP reference config.
- OpenCode consumes OAS through the OpenCode package/plugin surface.
