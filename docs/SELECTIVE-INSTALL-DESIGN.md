# OAS Selective Install Design

## Purpose

This document defines the user-facing selective-install design for OAS.
...
- how users choose OAS components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current OAS codebase without requiring a rewrite

## Problem

Today OAS still feels like a large payload installer even though the repo now
...
The selective-install system should make OAS feel composable instead of
all-or-nothing.
...
1. Let users install a small default OAS footprint quickly.
...
5. Preserve backward compatibility with the current `oas-install typescript`
   style during rollout.

## Non-Goals

- packaging OAS into multiple npm packages in the first phase
...
A user should be able to get a useful OAS install with one command:

```bash
oas install --target claude --profile core
```
...
```bash
oas install --target cursor --profile developer --with lang:typescript --with framework:nextjs --dry-run
```
...
```bash
oas install --config oas-install.json
```
...
These are the default OAS building blocks:
...
Capability packs are cross-cutting OAS feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying OAS
- `developer`
...
```bash
oas install --target claude --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
```bash
oas install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
oas install --target claude --profile core
oas install --target cursor --profile developer --with lang:typescript --with framework:nextjs
oas install --target antigravity --with capability:security --with lang:python
oas install --config oas-install.json
```
...
```bash
oas plan [same selection flags as install]
```
...
```bash
oas-install typescript
oas-install --target cursor typescript
oas typescript
```
...
```json
{
  "$schema": "./schemas/oas-install-config.schema.json",
  "version": 1,
  "target": "cursor",
  "profile": "developer",
  "include": [
...
The commands differ in action, not in how OAS understands the selected install.
...
- `oas-install.json` config file parsing
...
## Mapping To Current OAS Manifests
...
That is the shortest path from the current OAS codebase to a real selective
install experience that feels like OAS 2.0 instead of a large legacy installer.
