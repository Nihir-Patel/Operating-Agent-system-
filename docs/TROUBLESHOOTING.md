...
Community-reported workarounds for current Claude Code bugs that can affect OAS users.

These are upstream Claude Code behaviors, not OAS bugs. The entries below summarize the production-tested workarounds collected in [issue #644](https://github.com/neal0709/operating-agent-systems/issues/644) on Claude Code `v2.1.79` (macOS, heavy hook usage, MCP connectors enabled). Treat them as pragmatic stopgaps until upstream fixes land.
...
- Use OAS's `strategic-compact` guidance instead of forcing a lower auto-compact threshold.
...
- Advanced users sometimes script a local `/reload` command around `kill -HUP $PPID`, but OAS does not ship that because it is shell-dependent and not universally reliable.
...
## OAS Dashboard Does Not Start

**Symptoms:** `npm run dashboard` or `python3 oas_dashboard.py` fails, often with `ModuleNotFoundError: No module named 'tkinter'`.
...
- Both commands must be run from a full clone of the OAS repo (`git clone https://github.com/neal0709/Operating-Agent-system-`), not from inside the Claude Code plugin directory — plugin installs do not ship `package.json` scripts.

## Anthropic Cyber Safeguards Block Security Audits Of Your Own Code

**Symptoms:** Running security reviews/audits (e.g. `oas:security-reviewer`) fails with an API error citing the Usage Policy and "cyber-related safeguards", even though you are auditing your own codebase.

**What helps:**

- This is an upstream Anthropic model-level safeguard, not GateGuard and not an OAS block. No OAS configuration can bypass it.
...
- [hooks/README.md](../hooks/README.md) for OAS's documented hook lifecycle and exit-code behavior.
- [token-optimization.md](./token-optimization.md) for cost and context management settings.
- [issue #644](https://github.com/neal0709/operating-agent-systems/issues/644) for the original report and tested environment.
