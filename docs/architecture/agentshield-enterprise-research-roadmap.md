...
- OAS-Tools now consumes that fleet primitive in hosted security review:
...
default. OAS-Tools follow-up routing now consumes fleet summaries and surfaces
...
AgentShield is already connected conceptually to the OAS Tools GitHub App.
...
- Billing readiness checks shared with OAS-Tools so payment state never changes
...
- It aligns AgentShield with OAS's harness-agnostic positioning.
...
Connect AgentShield findings to OAS-Tools follow-up routing.

Artifacts:

- Finding fingerprints compatible with OAS-Tools issue caps.
...
- OAS-Tools can consume the finding fingerprints or backlog export without
