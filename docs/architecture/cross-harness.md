...
OAS is the reusable workflow layer. Harnesses are execution surfaces.
...
[OAS Platform Value Loop](platform-value-loop.md).
...
| Memory | `.oas/memory/`, `~/.oas/memory/` | `oas memory` CLI or opt-in `oas-memory-mcp` stdio server | Supported with explicit recall and unreviewed writes |
| Sessions | `oas2/`, session adapters, orchestration scripts | TUI/daemon, tmux/worktree orchestration, harness-specific runners | Alpha |

## What Travels Unchanged

`SKILL.md` is the most portable unit.

A good OAS skill should:
...
- OpenCode has a plugin/event system that can reuse OAS hook logic through an adapter layer.
- Cursor uses its own rule and hook layout, so OAS maintains translated surfaces under `.cursor/`.
...
OAS Memory Vault is the common knowledge-transfer surface for Claude, Codex,
Hermes, Cursor, OpenCode, and other agents. It stores portable
`oas.memory.v1` Markdown documents in three scopes:

- project: `<repo>/.oas/memory/project/`
- team: `<repo>/.oas/memory/team/`
- user: `~/.oas/memory/`

Every harness must use the same repository working directory or the same
`OAS_MEMORY_PROJECT_ROOT` and `OAS_MEMORY_USER_ROOT` overrides. The deterministic
`oas memory` CLI is the baseline interface. Harnesses with MCP support may
instead launch `oas-memory-mcp` and use `memory_save`, `memory_search`,
...
`OAS_MEMORY_HARNESS`; this server-bound identity supplies the source harness
...
Hermes is not the public OAS runtime.

Hermes is an operator shell that can consume OAS assets:

- import selected OAS skills into a Hermes skills directory
- use OAS MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable OAS patterns
- distill repeated local operator work back into sanitized OAS skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse OAS hook logic through the adapter layer, while the skill text stays unchanged.
...
- the full platform loop where external products contribute skill packs,
  gated APIs, evals, and case studies back into OAS

## Rule For New Work

When adding a workflow, put the durable behavior in OAS first.
