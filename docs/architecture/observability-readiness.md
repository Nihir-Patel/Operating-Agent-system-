# OAS 2.0 Observability Readiness

OAS 2.0 should be observable before it becomes more autonomous. The local
...
- Tool activity: `scripts/hooks/session-activity-tracker.js` records local
  `tool-usage.jsonl` events that OAS2 can sync.
- Risk ledger: `oas2/src/observability/mod.rs` scores tool calls and stores a
...
OAS's answer is not a hosted analytics dependency by default. The first
...
2. Run `npm run harness:audit -- --format json` for the broader harness
   scorecard.
3. Run `node scripts/loop-status.js --json --write-dir .oas/loop-status`
...
7. Use OAS2 tool logs for risky operations, conflict analysis, and handoff
...
The end-state is practical: before asking OAS to run larger multi-agent loops,
