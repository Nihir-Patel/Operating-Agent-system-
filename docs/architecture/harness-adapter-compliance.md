...
This matrix is the public onramp for teams that want to use OAS across more
...
OAS's durable units stay in shared sources:
...
| State | Meaning |
| --- | --- |
| Native | OAS can install or verify the surface directly for this harness. |
| Adapter-backed | OAS has a thin adapter, plugin, or package surface, but parity differs by harness. |
| Instruction-backed | OAS can provide the guidance and files, but the harness does not expose the runtime hook/session surface OAS needs for enforcement. |
| Reference-only | The tool is useful as a design pressure or external runtime, but OAS does not yet ship a direct installer or adapter for it. |
...
| Pi | Adapter-backed | Pi package manifest; canonical OAS skills (skills/); canonical OAS commands as prompt templates (commands/); canonical OAS engineering rules (rules/common/) injected into the system prompt; session lifecycle hook adapter; /oas-doctor diagnostics command | Subagents, chains, approval prompts, and persistent todos require companion Pi packages and are not part of this adapter; Pi core has no MCP surface, though OAS MCP configs load verbatim through the community pi-mcp-adapter package, which OAS neither installs nor depends on | `pi install git:github.com/neal0709/Operating-Agent-system-`; `pi install /path/to/OAS` from a local checkout | `node tests/pi/pi-package-manifest.test.js`; `node tests/pi/pi-extension-adapter.test.js`; `npm run harness:adapters -- --check` | Pi extensions execute with full user permissions, and hooks run without a shell and resolve from the installed package rather than the user project; Keep canonical skills and commands as the single source of truth, and never generate copies under .pi/ |
| Cursor | Adapter-backed | Cursor rules; project-local skills; hook adapter; shared scripts | Cursor hook events and rule loading differ from Claude Code | `npx oas-universal install --guided --harness cursor --profile core`; `./install.sh --profile minimal --target cursor` | `node tests/lib/install-targets.test.js`; `npm run harness:audit -- --format json` | Cursor adapters must preserve existing project rules and avoid silent overwrite. |
| Gemini | Instruction-backed | Gemini project-local instructions; shared skills; rules; compatibility docs | No full OAS hook parity; ecosystem ports must document drift from upstream OAS | `./install.sh --profile minimal --target gemini` | `node tests/lib/install-targets.test.js` | Treat Gemini ports as ecosystem adapters until validated end to end inside Gemini CLI. |
...
| Orca | Reference-only | worktree lifecycle; review state; notification; provider-identity design pressure | No OAS installer or direct adapter today | Use as a comparison target for worktree/session state requirements | `npm run observability:ready` | Do not import product-specific assumptions; convert lessons into OAS event fields. |
| Superset | Reference-only | workspace presets; parallel-agent review loops; worktree isolation design pressure | No OAS installer or direct adapter today | Use as a comparison target for workspace preset taxonomy | `npm run observability:ready` | Keep OAS portable; do not require a desktop workspace to get basic value. |
| Ghast | Reference-only | terminal-native pane grouping; cwd grouping; search; notifications | No OAS installer or direct adapter today | Use as a comparison target for terminal-first session grouping | `node scripts/session-inspect.js --list-adapters` | Preserve terminal ergonomics before adding visual UI assumptions. |
...
Use this sequence before asking OAS to make a team or repo setup more
autonomous:
...
node scripts/loop-status.js --json --write-dir .oas/loop-status
```
...
- Treat reference-only tools as design pressure until OAS has a direct adapter.
