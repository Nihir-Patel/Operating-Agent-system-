...
This contract defines the portable status payload OAS uses for local operator
...
Every status payload uses `schema_version: "oas.hud-status.v1"` and keeps these
...
| `sessionControls` | Supported operator actions for the current target | OAS CLI, dmux, git/GitHub |
...
- `oas status --json` exposes readiness, active sessions, skill runs, install
  health, governance, and linked work items from the SQLite state store.
- `oas loop-status --json --write-dir <dir>` writes live transcript snapshots
  and attention signals for long-running loops.
- `oas session-inspect <target> --write <path>` emits canonical session
  snapshots from dmux and Claude-history adapters.
- `scripts/hooks/oas-statusline.js` renders compact model, task, cost, tool,
  file, duration, directory, and context pressure signals inside Claude Code.

The `oas.hud-status.v1` payload is the common outer contract these surfaces can
project into before OAS grows a dedicated full-screen HUD.
