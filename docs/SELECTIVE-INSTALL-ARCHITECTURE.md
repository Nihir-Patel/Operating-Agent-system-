...
- what OAS owns and may safely remove or repair later

That is the missing contract between OAS 1.x installation and an OAS 2.0
control plane.
...
- unified `oas` CLI routing install, planning, and lifecycle commands
...
- legacy `oas-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `oas` and
  `install-apply.js`
...
OAS 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only OAS-owned outputs
...
- `scripts/oas.js` as the unified CLI
...
If the goal is OAS 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today OAS still behaves like a broad payload copier:
...
That means OAS 2.0 needs two contracts, not one:
...
1. Keep `operating-agent-systems` as the canonical source repo.
...
Fields still needed for OAS 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether OAS fully owns the target path or only generated files under it
- `pathMode`
...
That lets OAS 2.0 say things like:
...
OAS 2.0 needs the next layer: operation planning.
...
```json
{
  ...
  "destination": "/Users/example/.claude/rules/oas/common/coding-style.md",
  "ownership": "managed",
...
Install-state is the durable contract that OAS 1.x is missing.

Suggested path conventions:

- Claude target:
  `~/.claude/oas/install-state.json`
- Cursor target:
  `./.cursor/oas-install-state.json`
- Antigravity target:
  `./.agents/oas-install-state.json`
- future Codex target:
  `~/.codex/oas-install-state.json`
...
- enough detail for uninstall to remove only OAS-managed outputs
...
- `oas list-installed` routes to `node scripts/list-installed.js`
- `oas uninstall` routes to `node scripts/uninstall.js`
- `oas doctor` routes to `node scripts/doctor.js`
- `oas repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only OAS-managed destinations recorded in state
...
OAS 2.0 should translate legacy language arguments into a compatibility request.
...
OAS 2.0 should improve this carefully.
...
3. define `oas.install.v1` state schema
...
4. decide when `oas-install` should become a thin alias for `oas install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `oas-install` remains separate or becomes `oas install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `oas` CLI routing and compatibility guarantees
...
That is the shortest path from OAS 1.x installer sprawl to an OAS 2.0
