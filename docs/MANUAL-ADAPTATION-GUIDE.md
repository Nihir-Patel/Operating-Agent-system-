...
Use this guide when you want OAS behavior inside a harness that does not natively load `.claude/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Prefer a first-class OAS target whenever one exists:
...
Use this guide only when you need OAS behavior in a non-native harness.

## What You Are Reproducing

When you adapt OAS manually, you are trying to preserve four things:
...
```bash
cd /path/to/operating-agent-systems

sed -n '1,220p' skills/tdd-workflow/SKILL.md > /tmp/oas-context.md
printf '\n\n---\n\n' >> /tmp/oas-context.md
sed -n '1,220p' skills/backend-patterns/SKILL.md >> /tmp/oas-context.md
printf '\n\n---\n\n' >> /tmp/oas-context.md
sed -n '1,220p' skills/security-review/SKILL.md >> /tmp/oas-context.md
```
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical OAS path.

## Compression Rules

When manually packing OAS for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to OAS behavior.
...
That does not recreate true automation, but it captures the operational discipline of OAS.

## Harness Capability Matrix

| Capability | First-Class OAS Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected OAS skill files into one upload or paste block.
...
```text
You are operating with a manually adapted OAS bundle.
...
- use manual adaptation to carry OAS behavior into non-native harnesses
- use native OAS targets whenever you want the full system

## Related Work

- [Issue #1186](https://github.com/neal0709/operating-agent-systems/issues/1186)
- [Discussion #1077](https://github.com/neal0709/operating-agent-systems/discussions/1077)
