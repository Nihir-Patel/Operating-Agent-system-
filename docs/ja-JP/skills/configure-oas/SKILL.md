---
name: configure-oas
description: Claude Code、Codex、Kimi 内で OAS のインストール、更新、再設定を案内し、各ハーネスが実際に備えるプラグイン、スコープ、フック機能を守ります。
metadata:
  origin: OAS
---

# Operating Agent Systems の設定

現在のハーネス内で対話式ウィザードを実行します。最初にインベントリを調べ、対応する選択肢だけを
収集し、プレビュー後に 1 回だけ確認し、非対話で適用・検証します。ウェルカム表示は成功後だけです。
OAS を一時ディレクトリへ clone したり、プラグインを手作業でコピーしたりしないでください。

ユーザー自身が操作するターミナルの正規エントリは `oas setup` と `npx oas-universal setup` です。
...
- Codex では Codex ネイティブのプラグインライフサイクルを使います。Claude のスコープを提示したり、
  Claude の OAS フック 4 段階を Codex に対応付けたりしません。
- Kimi ではプロジェクトサーフェスを `./.kimi-code` に導入します。Kimi は OAS の Claude ライフサイクル
  フックプロファイルに対応しません。
...
両方のコマンドを実行し、OAS の導入スコープ、有効状態、marketplace ソースを要約します。
...
`oas@oas` が 1 つのみ既存する場合は再設定として扱います。Claude が所有する
"Open home page" コントロールをインストールの根拠にしません。setup が複数の OAS スコープ、
...
- `off` はスキルとコマンドを残し、OAS フック自動化を無効にします。
...
そこで bare な対話式 `oas setup` を実行しません。
...
選択スコープに有効な `oas@oas` が正確に 1 件ある場合のみ続行します。`$CLAUDE_PLUGIN_ROOT` があるときは、
成功した setup の `action`（`installed`、`updated`、`migrated`、`resumed`、
`already-migrated`）を内蔵レンダラーへ渡します。

呼び出し前に、プロバイダーが報告したバージョンが
`scripts/lib/terminal-welcome.js` の `OAS_VERSION_PATTERN` に一致することを
...
OAS marketplace がない場合は追加し、既存ならスナップショットを更新します。
...
JSON が OAS を導入済みと報告し、`installedPath` を提供した場合のみ続行し、検証済みバンドルからウェルカムを表示します。

`installedPath` は Codex JSON が返した絶対パスそのものだけを使い、制御文字を
拒否します。バージョンは `OAS_VERSION_PATTERN` で検証します。`node` を次の
...
確認前に機能サマリーを示します。導入先は `./.kimi-code`、OAS ライフサイクルフックは
...
Kimi が OAS ライフサイクルフックを導入または設定したとは表現しません。
