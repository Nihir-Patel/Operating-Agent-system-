---
description: プロジェクトのスタックを検出し、リポジトリのインストールマニフェストとスタックマッピングを使用してドライランOASオンボーディング計画を生成します。
---

# /project-init

現在のプロジェクト用の安全でレビュー可能なOASオンボーディング計画を作成します。このコマンドはドライランモードで開始し、明示的なユーザー承認後にのみファイルを書き込みます。
...
3. OASのインストーラーとマニフェストツールを使用。インストールのショートカットとしてファイルを手動コピーしたり任意のリモートをクローンしない。
...
- OAS設定: `oas-install.json`
- オプションのスタックマップ: OASリポジトリ内の`config/project-stack-mappings.json`

OASチェックアウトが利用可能な場合、`config/project-stack-mappings.json`をスタックからルール/スキルへの参照として使用。ファイルが利用できない場合、インストール済みのOASマニフェストと明示的なユーザーの選択にフォールバック。
...
3. 最小限の有用なOAS計画を解決:
   - プロジェクトに`oas-install.json`がある: `node scripts/install-plan.js --config oas-install.json --json`
...
- `/oas-guide` — インストール前のインタラクティブな機能ディスカバリー
