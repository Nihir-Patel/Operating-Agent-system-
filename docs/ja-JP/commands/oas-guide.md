---
description: OASの現在のエージェント、スキル、コマンド、フック、インストールプロファイル、ドキュメントをライブリポジトリサーフェスからナビゲートします。
---

# /oas-guide

このコマンドをOperating Agent Systemsの会話型マップとして使用します。完全なREADMEや古いカタログ数をダンプすることなく、タスクに適したOASサーフェスをユーザーが発見するのを支援します。
...
- `scripts/ci/catalog.js --json` — OAS内で実行時のライブカタログカウント
...
- `/project-init` — スタック対応のOASオンボーディング
