...
- `#423` Implement manifest-driven selective install profiles for OAS
- `#421` Add OAS install-state plus uninstall / doctor / repair lifecycle
- `#424` Define canonical session adapter contract for OAS 2.0 control plane
...
Implement manifest-driven selective install profiles for OAS
...
OAS still installs primarily by target and language. The repo now has first-pass
...
Implement manifest-driven install execution for current OAS targets:
...
- `oas-install --profile <name>`
- `oas-install --modules <id,id,...>`
...
Add OAS install-state plus uninstall / doctor / repair lifecycle
...
OAS has no canonical installed-state record. That makes uninstall, repair, and
...
- what paths OAS owns
- how to remove or repair only OAS-managed files
...
- Claude: `~/.claude/oas/install-state.json`
- Cursor: `./.cursor/oas-install-state.json`
- Antigravity: `./.agent/oas-install-state.json`
...
- `uninstall` removes only OAS-managed files and leaves unrelated local files
  alone
...
Define canonical session adapter contract for OAS 2.0 control plane
...
OAS now has real orchestration/session substrate, but it is still
...
Without that adapter contract, any future OAS 2.0 operator shell will be forced
...
- Full OAS 2.0 UI in the same issue
...
OAS now has a large and growing skill surface, but generated/imported/learned
...
As OAS grows, the repo needs explicit rules for where generated skill artifacts
