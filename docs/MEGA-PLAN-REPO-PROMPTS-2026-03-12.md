...
- `operating-agent-systems` has finished the orchestration, Codex baseline, and
  Windows CI recovery lane.
- The next open OAS Phase 1 items are:
  - review `#399`
  - convert recurring discussion pressure into tracked issues
  - define selective-install architecture
  - write the OAS 2.0 discovery doc
- `agentshield`, `OAS-website`, and `skill-creator-app` all have dirty
...
- `applications/` is not a standalone git repo. It lives inside the parent
  workspace repo at `<OAS_ROOT>`.

## Repo: `operating-agent-systems`

### Prompt A — PR `#399` Review and Merge Readiness

```text
Work in: <OAS_ROOT>/operating-agent-systems
...
issues. Focus on high-signal roadmap items that unblock OAS 1.x and OAS 2.0.
...
5. OAS 2.0 discovery doc / adapter contracts
...
### Prompt C — OAS 2.0 Discovery and Adapter Spec

```text
Work in: <OAS_ROOT>/operating-agent-systems

Goal:
Turn the existing OAS 2.0 vision into a first concrete discovery doc focused on
...
4. Define what stays in OAS 1.x versus what belongs in OAS 2.0.
...
- distinguish OAS 1.x toolkit from OAS 2.0 control plane clearly
...
```text
Work in: <OAS_ROOT>/skill-creator-app
...
1. Assess whether the app should support:
   ...
   - OAS convention adaptation workflows
...
- keep the focus on audited skill ingestion and OAS-compatible output
...
## Repo: `OAS` Workspace (`applications/`, `knowledge/`, `tasks/`)
...
Goal:
Use the parent OAS workspace to support the mega-plan hosted/workflow lanes.
...
- keep recommendations compatible with the current multi-repo OAS setup
