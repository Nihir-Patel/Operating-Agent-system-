...
![OAS - das Harness-native Operator-System für agentische Arbeit](../../assets/hero.png)
...
OAS v2.0.0-rc.1 ergänzt diese wiederverwendbare Schicht um die öffentliche Hermes-Operator-Story: Beginne mit dem [Hermes-Setup-Leitfaden](../../docs/HERMES-SETUP.md), prüfe anschließend die [rc.1-Release-Notes](../../docs/releases/2.0.0-rc.1/release-notes.md) und die [Cross-Harness-Architektur](../../docs/architecture/cross-harness.md).
...
<sub>**OSS bleibt kostenlos.** Dieses Repo ist für immer MIT-lizenziert. OAS Pro ist die gehostete GitHub App für private Repos. <a href="https://github.com/sponsors/neal0709">Sponsoren</a> und <a href="https://oas.tools/pricing">Pro-Abonnenten</a> finanzieren die Arbeit — deshalb liefert ein einzelner Maintainer wöchentlich über 7 Harnesses hinweg aus.</sub>
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="The Shorthand Guide to Operating Agent Systems" />
</a>
</td>
<td width="33%">
<a href="https://x.com/nihirpatel/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="The Longform Guide to Operating Agent Systems" />
</a>
</td>
<td width="33%">
...
### v2.0.0-rc.1 — Oberflächen-Refresh, Operator-Workflows und OAS 2.0 Alpha (April 2026)

- **Dashboard-GUI** — Neue Tkinter-basierte Desktop-Anwendung (`oas_dashboard.py` oder `npm run dashboard`) mit Umschalter für dunkles/helles Theme, Schriftanpassung und Projektlogo in Kopfzeile und Taskleiste.
...
- **Erweiterung von Operator- und Outbound-Workflows** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `oas-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` und `workspace-surface-audit` runden die Operator-Spur ab.
...
- **OAS 2.0 Alpha ist im Tree** — der Rust-Control-Plane-Prototyp in `oas2/` baut jetzt lokal und stellt die Befehle `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` und `daemon` bereit. Er ist als Alpha nutzbar, aber noch kein allgemeines Release.
- **Operator-Status-Snapshots** — `oas status --markdown --write status.md` verwandelt den lokalen State Store in eine portable Übergabe, die Bereitschaft, aktive Sessions, Skill-Run-Gesundheit, Install-Gesundheit, ausstehende Governance-Events und verknüpfte Arbeitselemente aus Linear/GitHub/Handovers abdeckt. Nutze `oas work-items upsert ...` für manuelle Einträge, `oas work-items sync-github --repo owner/repo` für den Queue-Status von PRs/Issues und `oas status --exit-code`, um Automatisierung scheitern zu lassen, wenn die Bereitschaft Aufmerksamkeit erfordert.
- **Ökosystem-Härtung** — AgentShield, OAS-Tools-Kostenkontrollen, Arbeiten am Billing-Portal und Website-Refreshes werden weiterhin rund um das Kern-Plugin ausgeliefert, statt in separate Silos abzudriften.
...
- **Harness-First-Release** — OAS ist nun ausdrücklich als Performance-System für Agent-Harnesses positioniert, nicht nur als Config-Paket.
...
- **Hook-Laufzeitsteuerung** — `OAS_HOOK_PROFILE=minimal|standard|strict` und `OAS_DISABLED_HOOKS=...` für Laufzeit-Gating ohne Bearbeitung von Hook-Dateien.
...
- **GitHub Marketplace** — OAS-Tools-GitHub-App live unter [github.com/marketplace/oas-tools](https://github.com/marketplace/oas-tools) mit Free-/Pro-/Enterprise-Stufen
...
- **Inhaltsverlust beim Instinct-Import behoben** — `parse_instinct_file()` verwarf während `/instinct-import` stillschweigend sämtlichen Inhalt nach dem Frontmatter (Abschnitte Action, Evidence, Examples). ([#148](https://github.com/neal0709/Operating-Agent-system-/issues/148), [#161](https://github.com/neal0709/Operating-Agent-system-/pull/161))

### v1.4.0 — Mehrsprachige Rules, Installationsassistent & PM2 (Februar 2026)

- **Interaktiver Installationsassistent** — Der neue `configure-oas`-Skill bietet ein geführtes Setup mit Merge-/Überschreiben-Erkennung
...
Den vollständigen Changelog findest du unter [Releases](https://github.com/neal0709/Operating-Agent-system-/releases).
...
- **Stapele Installationsmethoden nicht.** Das häufigste kaputte Setup ist: zuerst `/plugin install`, danach `install.sh --profile full` oder `npx oas-universal install --profile full`.

Falls du bereits mehrere Installationen übereinandergelegt hast und Dinge doppelt aussehen, springe direkt zu [OAS zurücksetzen / deinstallieren](#oas-zurücksetzen--deinstallieren).

### Low-Context-/No-Hooks-Pfad

Falls sich Hooks zu global anfühlen oder du nur OASs Rules, Agents, Commands und Kern-Workflow-Skills willst, überspringe das Plugin und nutze das minimale manuelle Profil:
...
Falls du nicht sicher bist, welches OAS-Profil oder welche Komponente du installieren sollst, frage den mitgelieferten Advisor aus jedem beliebigen Projekt:
...
OAS hat jetzt drei öffentliche Bezeichner, und sie sind nicht austauschbar:

- GitHub-Quell-Repo: `neal0709/Operating-Agent-system-`
- Claude-Marketplace-/Plugin-Bezeichner: `oas@oas`
- npm-Paket: `oas-universal`

Das ist beabsichtigt. Anthropic-Marketplace-/Plugin-Installationen werden über einen kanonischen Plugin-Bezeichner gekeyt, daher verwendet OAS `oas@oas`, um Tool-Namen und Slash-Command-Namespaces kurz genug für strenge Desktop-/API-Validatoren zu halten. Ältere Beiträge zeigen möglicherweise noch den früheren langen Marketplace-Bezeichner; behandle diesen lediglich als Legacy-Alias. Das npm-Paket blieb davon getrennt bei `oas-universal`, daher verwenden npm-Installationen und Marketplace-Installationen absichtlich unterschiedliche Namen.
...
>
> Falls du OAS bereits über `/plugin install` installiert hast, **führe danach nicht `./install.sh --profile full`, `.\install.ps1 --profile full` oder `npx oas-universal install --profile full` aus**. Das Plugin lädt OAS-Skills, -Commands und -Hooks bereits. Wird der vollständige Installer nach einer Plugin-Installation ausgeführt, kopiert er dieselben Oberflächen in deine Benutzerverzeichnisse und kann doppelte Skills sowie doppeltes Laufzeitverhalten erzeugen.
>
> Kopiere für Plugin-Installationen manuell nur die `rules/`-Verzeichnisse, die du willst, nach `~/.claude/rules/oas/`. Beginne mit `rules/common` plus einem Sprach- oder Framework-Paket, das du tatsächlich verwendest. Kopiere nicht jedes Rules-Verzeichnis, es sei denn, du willst diesen gesamten Kontext ausdrücklich in Claude haben.
>
> Verwende den vollständigen Installer nur dann, wenn du eine vollständig manuelle OAS-Installation statt des Plugin-Pfads durchführst.
>
> Falls dein lokales Claude-Setup gelöscht oder zurückgesetzt wurde, bedeutet das nicht, dass du OAS erneut kaufen musst. Beginne mit `node scripts/oas.js list-installed`, führe dann `node scripts/oas.js doctor` und `node scripts/oas.js repair` aus, bevor du irgendetwas neu installierst. Das stellt OAS-verwaltete Dateien üblicherweise wieder her, ohne dein Setup neu aufzubauen. Falls das Problem im Konto- oder Marketplace-Zugriff für OAS Tools liegt, behandle die Konto-/Abrechnungswiederherstellung separat.
...
# Plugin-Installationspfad: nur OAS-Rules in einen OAS-eigenen Namespace kopieren
mkdir -p ~/.claude/rules/oas
cp -R rules/common ~/.claude/rules/oas/
cp -R rules/typescript ~/.claude/rules/oas/

# Vollständig manueller OAS-Installationspfad (nutze diesen statt /plugin install)
...
### OAS zurücksetzen / deinstallieren

Falls sich OAS doppelt, aufdringlich oder kaputt anfühlt, installiere es nicht weiter über sich selbst.

- **Plugin-Pfad:** Entferne das Plugin aus Claude Code, lösche dann die konkreten Rule-Ordner, die du manuell unter `~/.claude/rules/oas/` kopiert hast.
...
Entferne anschließend OAS-verwaltete Dateien:
...
OAS entfernt nur Dateien, die in seinem Install-State erfasst sind. Es löscht keine fremden Dateien, die es nicht installiert hat.

Falls du Methoden gestapelt hast, räume in dieser Reihenfolge auf:

1. Entferne die Claude-Code-Plugin-Installation.
2. Führe den OAS-Uninstall-Befehl vom Repo-Root aus, um über den Install-State verwaltete Dateien zu entfernen.
...
# Bestehende Slash-artige Command-Namen funktionieren weiterhin, während OAS von commands/ wegmigriert.

# Die Plugin-Installation verwendet die kanonische Namespace-Form
/oas:plan "Benutzerauthentifizierung hinzufügen"
...
Starte das Desktop-Dashboard, um OAS-Komponenten visuell zu erkunden:
...
|   |-- configure-oas/              # Interaktiver Installationsassistent (NEU)
...
|
|-- rules/            # Stets zu befolgende Richtlinien (nach ~/.claude/rules/oas/ kopieren)
...
[GitHub App installieren](https://github.com/apps/skill-creator) | [oas.tools](https://oas.tools)
...
Verwende `/security-scan` in Claude Code, um es auszuführen, oder füge es per [GitHub Action](https://github.com/neal0709/agentshield) zur CI hinzu.
...
**Historie:** Dies hat in diesem Repo wiederholte Fix-/Revert-Zyklen verursacht ([#29](https://github.com/neal0709/Operating-Agent-system-/issues/29), [#52](https://github.com/neal0709/Operating-Agent-system-/issues/52), [#103](https://github.com/neal0709/Operating-Agent-system-/issues/103)). Das Verhalten änderte sich zwischen Claude-Code-Versionen, was zu Verwirrung führte. Wir haben jetzt einen Regressionstest, der verhindert, dass dies erneut eingeführt wird.
...
> mkdir -p ~/.claude/rules/oas
> cp -r rules/common ~/.claude/rules/oas/
> cp -r rules/typescript ~/.claude/rules/oas/   # wähle deinen Stack
...
# Manuelle Installationen nicht unter ~/.claude/skills/oas/ verschachteln.
...
Falls du OAS über `/plugin install` installiert hast, kopiere diese Hooks nicht in `settings.json`. Claude Code v2.1+ lädt Plugin-`hooks/hooks.json` bereits automatisch, und sie in `settings.json` zu duplizieren, verursacht doppelte Ausführung und Cross-Platform-Hook-Konflikte.

Windows-Hinweis: Das Claude-Konfigurationsverzeichnis ist `%USERPROFILE%\\.claude`, nicht `~/claude`.

#### MCPs konfigurieren

Claude-Plugin-Installationen aktivieren die mitgelieferten MCP-Server-Definitionen von OAS absichtlich nicht automatisch. Das vermeidet überlange Plugin-MCP-Tool-Namen auf strengen Drittanbieter-Gateways und hält gleichzeitig das manuelle MCP-Setup verfügbar.
...
Falls du bereits eigene Kopien der von OAS mitgelieferten MCPs betreibst, setze:

```bash
export OAS_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

OAS-verwaltete Install- und Codex-Sync-Flows überspringen oder entfernen diese mitgelieferten Server, statt Duplikate erneut hinzuzufügen. `OAS_DISABLED_MCPS` ist ein OAS-Install-/Sync-Filter, kein Live-Toggle für Claude Code.
...
|--------------|-----------------|------------|
| Ein neues Feature planen | `/oas:plan "Add auth"` | planner |
| Systemarchitektur entwerfen | `/oas:plan` + architect-Agent | architect |
...
Das ist das häufigste Problem. **Füge KEIN `"hooks"`-Feld zu `.claude-plugin/plugin.json` hinzu.** Claude Code v2.1+ lädt `hooks/hooks.json` aus installierten Plugins automatisch. Es explizit zu deklarieren, verursacht Fehler durch Duplikaterkennung. Siehe [#29](https://github.com/neal0709/Operating-Agent-system-/issues/29), [#52](https://github.com/neal0709/Operating-Agent-system-/issues/52), [#103](https://github.com/neal0709/Operating-Agent-system-/issues/103).
</details>

<details>
<summary><b>Kann ich OAS mit Claude Code an einem benutzerdefinierten API-Endpoint oder Modell-Gateway verwenden?</b></summary>

Ja. OAS hat keine Anthropic-gehosteten Transporteinstellungen fest verdrahtet. Es läuft lokal über die normale CLI-/Plugin-Oberfläche von Claude Code, daher funktioniert es mit:
...
Falls dein Gateway Modellnamen umbildet, konfiguriere das in Claude Code statt in OAS. OASs Hooks, Skills, Commands und Rules sind modellanbieter-agnostisch, sobald die `claude`-CLI bereits funktioniert.
...
- **Codex**: Erstklassige Unterstützung sowohl für die macOS-App als auch die CLI, mit Adapter-Drift-Guards und SessionStart-Fallback. Siehe PR [#257](https://github.com/neal0709/Operating-Agent-system-/pull/257).
...
Diese werden nicht mit OAS mitgeliefert und nicht von diesem Repo auditiert, aber sie sind wissenswert, falls du das breitere Claude-Code-Skills-Ökosystem erkundest:
...
OAS bietet Cursor-IDE-Unterstützung mit Hooks, Rules, Agents, Skills, Commands und MCP-Konfigurationen, die an Cursors Projektlayout angepasst sind.
...
| Agents | 48 | `.cursor/agents/oas-*.md` bei Installation; präfixiert, um Kollisionen mit Benutzer- oder Marketplace-Agents zu vermeiden |
...
OAS installiert keine Root-`AGENTS.md` in `.cursor/`. Cursor behandelt verschachtelte `AGENTS.md`-Dateien als Verzeichniskontext, daher würde das Kopieren von OASs Repo-Identität in ein Host-Projekt dieses Projekt verunreinigen.

Das Cursor-native Ladeverhalten kann je nach Cursor-Build variieren. OAS installiert Agents als `.cursor/agents/oas-*.md`; falls dein Cursor-Build keine Projekt-Agents bereitstellt, funktionieren diese Dateien weiterhin als explizite Referenzdefinitionen statt als versteckter globaler Prompt-Kontext.
...
OAS bietet **erstklassige Codex-Unterstützung** sowohl für die macOS-App als auch die CLI, mit einer Referenzkonfiguration, einem Codex-spezifischen AGENTS.md-Zusatz und gemeinsam genutzten Skills.
...
# Automatisches Setup: OAS-Assets (AGENTS.md, Skills, MCP-Server) nach ~/.codex synchronisieren
npm install && bash scripts/sync-oas-to-codex.sh
# oder: pnpm install && bash scripts/sync-oas-to-codex.sh
# oder: yarn install && bash scripts/sync-oas-to-codex.sh
# oder: bun install && bash scripts/sync-oas-to-codex.sh

# Oder manuell: die Referenzkonfiguration in dein Home-Verzeichnis kopieren
cp .codex/config.toml ~/.codex/config.toml
```

Das Sync-Skript merged OAS-MCP-Server sicher in deine bestehende `~/.codex/config.toml` mit einer **add-only**-Strategie — es entfernt oder verändert deine bestehenden Server nie. Führe es mit `--dry-run` aus, um Änderungen in der Vorschau zu sehen, oder mit `--update-mcp`, um ein erzwungenes Refresh der OAS-Server auf die neueste empfohlene Konfiguration zu erzwingen.

Für Context7 verwendet OAS den kanonischen Codex-Abschnittsnamen `[mcp_servers.context7]`, startet aber weiterhin das Paket `@upstash/context7-mcp`. Falls du bereits einen veralteten `[mcp_servers.context7-mcp]`-Eintrag hast, migriert `--update-mcp` ihn auf den kanonischen Abschnittsnamen.
...
| operating-agent-systems | Entwicklungskonventionen und -Patterns für das Projekt |
...
Codex bietet **noch keine Claude-artige Parität bei der Hook-Ausführung**. Die OAS-Durchsetzung dort ist instruction-basiert über `AGENTS.md`, optionale `model_instructions_file`-Overrides sowie Sandbox-/Approval-Einstellungen.
...
OAS liefert drei Beispiel-Rollenkonfigurationen aus:
...
OAS bietet Zed-Projektunterstützung über einen konservativen `.zed`-Adapter für projektlokale Einstellungen, abgeflachte Rules, Agents, Commands und Skills.
...
Der Adapter schreibt OAS-verwaltete Dateien unter `.zed/` und hält BYOK-/OpenRouter-Credentials aus dem Repo heraus. Konfiguriere das Zed-Konto oder API-Keys über Zeds eigene Einstellungs-UI oder deine lokalen Benutzereinstellungen.

---

## OpenCode-Unterstützung

OAS bietet **vollständige OpenCode-Unterstützung** einschließlich Plugins und Hooks.
...
Dieser npm-Plugin-Eintrag aktiviert OASs veröffentlichtes OpenCode-Plugin-Modul (Hooks/Events und Plugin-Tools).
Er fügt **nicht** automatisch OASs vollständigen Command-/Agent-/Instruction-Katalog zu deiner Projektkonfiguration hinzu.

Für das vollständige OAS-OpenCode-Setup entweder:
...
OAS bietet **GitHub-Copilot-Unterstützung** für VS Code über das native Instruction- und Prompt-Datei-System von Copilot Chat — kein zusätzliches Tooling erforderlich.
...
- **`.github/copilot-instructions.md`** — Instructions auf Repository-Ebene, die in jede Copilot-Chat-Anfrage injiziert werden. Enthält OASs Kern-Coding-Standards, Sicherheits-Checkliste, Testanforderungen und Git-Workflow.
- **`.github/prompts/*.prompt.md`** — wiederverwendbare Prompt-Dateien, die Nutzer bei Bedarf aufrufen. Jeder Prompt führt Copilot durch einen bestimmten OAS-Workflow wie Planung, TDD, Security-Review, Build-Fix oder Refactor.
...
| OAS-Feature | Copilot-Entsprechung |
|-------------|-------------------|
...
GitHub Copilot hat kein Hook-System und keine Subagent-API, daher sind OASs Hook-Automatisierungen (Auto-Formatierung, TypeScript-Prüfung, Session-Persistenz, Dev-Server-Guard) sowie die Agent-Delegation nicht verfügbar. Die Instruction- und Prompt-Schicht bringt dennoch die vollständige OAS-Coding-Philosophie — Standards, Sicherheit, TDD und Workflow — in jede Copilot-Chat-Session.

---

## Cross-Tool-Feature-Parität

OAS ist das **erste Plugin, das jedes große KI-Coding-Tool ausreizt**. So vergleicht sich jeder Harness:
...
| `OAS_CONTEXT_MONITOR_COST_WARNINGS` | on | **off für Abonnement-Nutzer** | Unterdrückt agentenseitige API-Rate-Schätzwarnungen, behält aber Kontext-/Scope-/Loop-Warnungen |
...
Falls du ein Claude-Abonnement nutzt und die API-Rate-Schätzungen des Kontext-Monitors nicht nützlich sind, setze `OAS_CONTEXT_MONITOR_COST_WARNINGS=off`. Das unterdrückt nur die agentenseitigen Kostenwarnungen; es deaktiviert keine Warnungen zu Kontexterschöpfung, Scope oder Loops.
...
- Verwende `OAS_DISABLED_MCPS` nur, um OAS-generierte MCP-Konfigurationen während der Install-/Sync-Flows zu filtern
...
Projekte, die auf OAS aufbauen oder davon inspiriert sind:
...
Etwas mit OAS gebaut? Öffne einen PR, um es hier hinzuzufügen.
...
[**Sponsor werden**](https://github.com/sponsors/neal0709) | [Sponsor-Stufen](../../SPONSORS.md) | [Sponsoring-Programm](../../SPONSORING.md)
...
- **Kurzleitfaden (Hier starten):** [The Shorthand Guide to Operating Agent Systems](https://x.com/nihirpatel/status/2012378465664745795)
- **Langleitfaden (fortgeschritten):** [The Longform Guide to Operating Agent Systems](https://x.com/nihirpatel/status/2014040193557471352)
- **Security-Leitfaden:** [Security-Leitfaden](../../the-security-guide.md) | [Thread](https://x.com/nihirpatel/status/2033263813387223421)
- **Folgen:** [@nihirpatel](https://x.com/nihirpatel)
