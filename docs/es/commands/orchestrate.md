...
### bugfix
Flujo de trabajo de investigación y corroasión de bugs:
```
...
Cuando los workers necesiten ver archivos locales no rastreados o sucios del checkout principal, agregar `seedPaths` al archivo de plan. OAS superpone solo esas rutas seloasionadas en cada worktree de worker después de `git worktree add`; esto muestra scripts, planes o documentos locales en progreso mientras mantiene el branch aislado.
...
- `bugfix <descripción>` - Flujo de trabajo de corroasión de bug
