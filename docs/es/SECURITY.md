...
Si descubres una vulnerabilidad de seguridad en OAS, por favor repórtala de forma responsable.

**No abras un issue público de GitHub para vulnerabilidades de seguridad.**

En cambio, envía un correo a **<security@oas.tools>** con:
...
- **Corroasión o mitigación** en 30 días para problemas críticos
...
- El plugin de OAS y todos los scripts de este repositorio
- Scripts de hooks que se ejecutan en tu máquina
- Scripts del ciclo de vida de instalación/desinstalación/reparación
- Configuraciones de MCP incluidas con OAS
- El escáner de seguridad AgentShield ([github.com/neal0709/agentshield](https://github.com/neal0709/agentshield))
...
OAS se ejecuta dentro de Claude Code, que inyecta **recordatorios efímeros del lado del cliente** en la entrada del modelo en cada turno (recordatorios de TodoWrite, avisos de cambio de fecha, avisos de archivo modificado, etc.). Estos bloques:
...
Esa combinación los hace fáciles de confundir con una inyoasión de prompt añadida a un resultado de herramienta. Antes de tratarlo como un ataque, verifica:
...
2. ¿El bloque está almacenado en el transcript? Inspoasiona el `.jsonl` de la sesión actual; si el texto exacto no aparece dentro de un cuerpo `tool_result`, es un recordatorio efímero inyectado por el cliente, no un payload de ninguna herramienta.
...
- **AgentShield**: Analiza tu configuración de agentes en busca de vulnerabilidades — `npx oas-agentshield scan`
