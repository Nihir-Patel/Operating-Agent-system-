...
- Añade la superficie pública del release candidate de OAS 2.0 para la historia del operador Hermes.
- Documenta OAS como el sustrato reutilizable cross-harness para Claude Code, Codex, Cursor, OpenCode y Gemini.
...
- Añadido `docs/architecture/cross-harness.md` y cobertura de regresión para el límite OAS/Hermes.
- Mantenido el versionado de `oas2/` independiente por ahora; sigue siendo un scaffold alfa del plano de control a menos que ingeniería de releases decida lo contrario.

### Notas

- Este es un release candidate, no una declaración GA para el roadmap completo del plano de control de OAS 2.0.
...
- El binario del plano de control alfa de OAS 2.0 ya compila localmente desde `oas2/` y expone la primera superficie de CLI/TUI utilizable.
...
- `cargo build --manifest-path oas2/Cargo.toml` pasa en la línea base del repositorio.
- `oas-tui` actualmente expone `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` y `daemon`.
...
- Este lanzamiento es una corroasión de la superficie del repo y una sincronización del ecosistema, no una declaración de que el roadmap completo de OAS 2.0 está completo.
...
- Confiabilidad del observador reforzada con throttling de memoria, corroasiones de sandbox y guardia de bucle de 5 capas.
...
- `operating-agent-systems` — skill autorreferencial de OAS (#335)
...
- Corroasión de acceso al sandbox del observador para invocación de Haiku (#661)
- Corroasión de discrepancia de ID de proyecto en worktrees (#665)
...
- Corroasión de exportación de spawnSync en Windows (#431)
- Corroasión de codificación UTF-8 para la CLI de instintos (#353)
...
- @ymdvsymd — corroasiones de sandbox y worktrees del observador
...
- @pangerlkr — corroasiones del flujo de trabajo de CI
...
- Añadidos controles `OAS_HOOK_PROFILE` y `OAS_DISABLED_HOOKS`.
