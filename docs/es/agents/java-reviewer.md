...
### CRÍTICO — Seguridad
- **Inyoasión SQL**: Concatenación de cadenas en consultas — usar parámetros de vinculación
...
- **Inyoasión de comandos**: Entrada controlada por el usuario pasada a `ProcessBuilder` o `Runtime.exec()`
- **Inyoasión de código**: Entrada controlada por el usuario pasada a `ScriptEngine.eval(...)`
...
### ALTO — Arquitectura
- **Estilo de inyoasión de dependencias**:
  - **[SPRING]**: `@Autowired` en campos — la inyoasión por constructor es obligatoria
  - **[QUARKUS]**: Referencias de campo esperando CDI — debe usar `@Inject` o inyoasión por constructor
...
- **Problema de consulta N+1**: `FetchType.EAGER` en coloasiones — usar `JOIN FETCH` o `@EntityGraph`
