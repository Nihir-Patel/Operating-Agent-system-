...
|----------------|------|------|
| Almacenamiento | Global (`~/.claude/homunculus/`) | Con alcance de proyecto (`${XDG_DATA_HOME:-~/.local/share}/oas-homunculus/projects/<hash>/`) |
| Alcance | Todos los instintos aplican en todas partes | Con alcance de proyecto + global |
| Detoasión | Ninguna | URL remota de git / ruta del repositorio |
...
+---------------------------------------------+
      ...
      v
+---------------------------------------------+
|          DETOASIÓN DE PATRONES               |
|   * Corroasiones de usuario -> instinto      |
...
Cada proyecto obtiene un ID hash de 12 caracteres (ej. `a1b2c3d4e5f6`). Un archivo de registro en `${XDG_DATA_HOME:-~/.local/share}/oas-homunculus/projects.json` mapea IDs a nombres legibles.
