...
- El flujo de trabajo compacto preferido es:
  ...
  - un commit para la corroasión mínima aplicada y VERDE validado
  - un commit opcional para refactor completo
- No se requieren commits separados solo de evidencia si el commit de prueba claramente corresponde a ROJO y el commit de corroasión claramente corresponde a VERDE
...
Si el repositorio está bajo Git, preparar la corroasión mínima ahora pero diferir el commit de checkpoint hasta que VERDE esté validado en el Paso 5.
...
Volver a ejecutar el mismo objetivo de prueba relevante después de la corroasión y confirmar que la prueba anteriormente fallida ahora está en VERDE.
...
Formato de mensaje de commit recomendado:
- `fix: <feature or bug>`
- El commit de corroasión también puede servir como checkpoint de validación VERDE si el mismo objetivo de prueba relevante fue re-ejecutado y pasó
- Verificar que este commit de checkpoint esté en la rama activa actual antes de continuar
...
test('user can create a new market', async ({ page }) => {
  ...
  // Verificar rediroasión a la página del mercado
