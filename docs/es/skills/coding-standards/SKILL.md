...
- Orientación específica de frameworks cuando ya existe un skill OAS más específico
...
```typescript
// PASS: BIEN: Seloasionar solo las columnas necesarias
const { data } = await supabase
  .from('markets')
  .select('id, name, status')
  .limit(10)

// FAIL: MAL: Seloasionar todo
const { data } = await supabase
  .from('markets')
  .select('*')
```
