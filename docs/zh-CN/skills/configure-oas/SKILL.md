---
name: configure-oas
description: 在 Claude Code、Codex 或 Kimi 内引导 OAS 安装、更新或重新配置，同时严格遵守各家工具真实的插件、范围和 Hook 能力。
metadata:
  origin: OAS
---

# 配置 Operating Agent Systems

在当前工具内运行对话式向导：先检查，只收集受支持的选项，预览，只确认
一次，以非交互方式执行，验证，最后才显示欢迎信息。不要把 OAS 克隆到
临时目录，也不要手动复制插件组件。

在用户自己操作的终端中，规范入口是 `oas setup` 和 `npx oas-universal setup`。
在工具内请改用下方参数完整的非交互命令。
...
- Codex：使用 Codex 原生插件生命周期；不要提供 Claude 范围，也不要映射
  Claude 的四种 OAS Hook 配置。
- Kimi：把项目表面安装到 `./.kimi-code`；Kimi 不支持 OAS 的 Claude 生命周期
  Hook 配置。
...
运行以下两条命令，总结 OAS 的安装范围、启用状态和 marketplace 来源：
...
只有一个现有 `oas@oas` 时，将本次视为重新配置。不要把 Claude 提供商所有的
“Open home page”控件当作安装证据。若 setup 报告多个 OAS 范围、旧版或手动
...
交互式 `oas setup`，因为该 Shell 通常不是 TTY。
...
只有在所选范围中恰好存在一个已启用的 `oas@oas` 条目时才继续。如果
...
`OAS_VERSION_PATTERN`。异常版本文本应被拒绝，不得插入 shell 命令。
...
如果缺少 OAS marketplace，请添加；否则刷新快照：
...
只有 JSON 报告 OAS 已安装并提供 `installedPath` 时才继续，然后渲染已验证组合包的欢迎信息：

`installedPath` 只能使用 Codex JSON 返回的原始绝对路径，并拒绝控制字符。版本必须通过
`OAS_VERSION_PATTERN` 验证。请使用下面的 argument array 直接调用 `node`；这是工具 API
调用，不是 shell 命令：
...
确认前说明能力摘要：目标为 `./.kimi-code`；OAS 生命周期 Hook 为 `hooks=unsupported`。
...
不要声称 Kimi 已安装或配置 OAS 生命周期 Hook。
