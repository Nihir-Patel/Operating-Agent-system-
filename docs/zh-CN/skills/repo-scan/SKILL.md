---
name: repo-scan
description: 用于从固定且可审查的提交安装外部 repo-scan 技能的引导指针。在运行跨栈源代码资产审计前需要安装 repo-scan 时使用；此 OAS 指针本身不执行审计。
origin: community
---
...
安装后，请重新加载智能体运行环境，然后再次调用 `repo-scan`。此 OAS 指针仅安装外部技能，本身不会执行扫描。
