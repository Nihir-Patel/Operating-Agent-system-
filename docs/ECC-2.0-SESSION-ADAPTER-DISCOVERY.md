# OAS 2.0 Session Adapter Discovery

## Purpose

This document turns the March 11 OAS 2.0 control-plane direction into a
...
- `scripts/lib/session-adapters/canonical-session.js`
  defines the canonical `oas.session.v1` normalization layer
...
In practice, OAS can already answer:
...
## Gap Between OAS 1.x And OAS 2.0

OAS 1.x currently has two different "session" surfaces:
...
The missing OAS 2.0 layer is a harness-neutral session adapter boundary that
can normalize:
...
OAS 2.0 should introduce a canonical session adapter contract.
...
This provides a non-orchestrated baseline for OAS 2.0.
...
3. Should event streaming be part of v1, or should OAS 2.0 ship snapshot-only
   first?
...
5. Should the adapter registry live inside this repo long-term, or move into the
   eventual OAS 2.0 control-plane app once the interface stabilizes?
...
The shortest path to OAS 2.0 is:
