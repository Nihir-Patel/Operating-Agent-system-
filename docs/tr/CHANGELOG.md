...
- Hermes operatör hikayesi için genel OAS 2.0 sürüm adayı yüzeyi eklendi.
- OAS, Claude Code, Codex, Cursor, OpenCode ve Gemini genelinde yeniden kullanılabilir cross-harness altyapı olarak belgelendi.
...
- OAS/Hermes sınırı için `docs/architecture/cross-harness.md` ve regresyon kapsamı eklendi.
- `oas2/` sürümlemesi bağımsız tutuldu; release engineering aksi karar vermedikçe alpha control-plane scaffold olarak kalır.

### Notlar

- Bu bir sürüm adayıdır; tam OAS 2.0 control-plane yol haritası için GA iddiası değildir.
...
- OAS 2.0 alpha control-plane binary artık `oas2/` üzerinden yerelde build ediliyor ve ilk kullanılabilir CLI/TUI yüzeyini sunuyor.
...
- Bu sürüm bir repo-yüzeyi düzeltmesi ve ekosistem senkronizasyonudur; tam OAS 2.0 yol haritasının tamamlandığı iddiası değildir.
...
- `operating-agent-systems` — Öz-referansiyel OAS becerisi (#335)
...
- `OAS_HOOK_PROFILE` ve `OAS_DISABLED_HOOKS` kontrolleri eklendi.
