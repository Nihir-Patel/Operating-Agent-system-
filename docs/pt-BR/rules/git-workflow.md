...
Nota: As instalações gerenciadas pelo OAS definem `"includeCoAuthoredBy": false` em `~/.claude/settings.json`, portanto os commits não incluem `Co-Authored-By` por padrão. Para manter a atribuição do Claude, defina `"includeCoAuthoredBy": true` ou configure `attribution`; o OAS nunca sobrescreve uma escolha explícita.
