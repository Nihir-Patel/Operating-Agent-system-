...
### v2.0.0-rc.1 — Sincronização de Superfície, Fluxos Operacionais e OAS 2.0 Alpha (Abr 2026)
...
- **OAS 2.0 alpha já está no repositório** — o plano de controle em Rust dentro de `oas2/` já compila localmente e expõe `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` e `daemon`.
- **Fortalecimento do ecossistema** — AgentShield, controles de custo do OAS Tools, trabalho no portal de billing e a renovação do site continuam sendo entregues ao redor do plugin principal.
...
- **Lançamento focado em harness** — O OAS agora é explicitamente enquadrado como um sistema de desempenho de harness de agentes, não apenas um pacote de configurações.
...
- **Controles de runtime de hooks** — `OAS_HOOK_PROFILE=minimal|standard|strict` e `OAS_DISABLED_HOOKS=...` para controle em tempo de execução sem editar arquivos de hook.
...
npx oas-universal install typescript
```
...
/oas:plan "Adicionar autenticação de usuário"
...
> cp -r operating-agent-systems/rules/typescript ~/.claude/rules/typescript   # escolha sua stack
>
...
|----------|-----------------|--------------|
| Planejar um novo recurso | `/oas:plan "Adicionar auth"` | planner |
| Projetar arquitetura de sistema | `/oas:plan` + agente architect | architect |
...
```
/oas:plan "Adicionar autenticação de usuário com OAuth"
...
<details>
<summary><b>Posso usar o OAS com Cursor / OpenCode / Codex / Antigravity?</b></summary>

Sim. O OAS é multiplataforma:
