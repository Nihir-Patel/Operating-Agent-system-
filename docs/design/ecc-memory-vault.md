...
harnesses can share. Project and team memories live under `.oas/memory/`; user
memories live under `~/.oas/memory/`. The same `oas.memory.v1` documents are
available through the `oas memory` CLI and an opt-in local stdio MCP server, so
...
- **OAS CLI:** deterministic local create/read/search/doctor interface.
- **OAS Memory MCP:** stdio adapter exposing the same create/read/search/doctor
  operations. It has no review or promotion tool.
- **OAS2 context graph:** optional projection populated from the Markdown
...
`OAS_MEMORY_PROJECT_ROOT` and `OAS_MEMORY_USER_ROOT` may override the two vault
...
```text
oas memory init [--scope project|team|user]
oas memory save --title <text> [--body-file <path>|--stdin] [metadata flags]
oas memory handoff --from <harness> --target <harness> --title <text> ...
oas memory search <query> [--scope ...] [--target-harness ...] [--json]
oas memory read <id> [--scope ...] [--json]
oas memory doctor [--json]
```
...
The MCP server requires a lowercase `OAS_MEMORY_HARNESS` identity at launch.
...
the operator also sets `OAS_MEMORY_ALLOW_USER_SCOPE=1`, after which the client
...
the full body. A later event-sourced OAS2 projection may record content hashes
...
- Replacing OAS2 sessions, the context graph, GitHub/Linear work items, or
...
- Whether the team scope should gain a signed promotion manifest that points
  to governed artifacts after the OAS2 append-only event substrate lands.
...
cross-harness tests. OAS2 graph sync, automatic session capture, semantic
