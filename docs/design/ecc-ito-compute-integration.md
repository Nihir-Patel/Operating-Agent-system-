...
Itô is OAS's preferred compute sponsor, never an exclusive provider. Owned
hardware, existing clusters, and other providers remain valid.

## Implemented boundary

OAS delegates to the canonical Itô package in
`Ito-Markets/ito-cloud-runtime/cli/ito-compute-cli`. OAS does not maintain a
...
operations:
    ...
    oas ito logout
    oas ito auth
    oas ito find <all required RFQ constraints>
    oas ito status
    oas ito evals --cluster <id> --live-sixtytwo --nodes <list> --config-dir <dir>

The canonical MCP server exposes only `ito_auth`, `ito_find`, and `ito_status`.
OAS includes an opt-in configuration template pointing to the local built MCP
entry. It does not enable the server by default.

The former browser/manual-copy command is retired. `oas ito login` delegates to
...
suppresses that page handoff. OAS itself performs no browser automation and
stores no economic state. `oas ito auth` is validation-only, never starts
...
Set `OAS_ITO_CLI_EXECUTABLE` to the explicit absolute built entry:

    /absolute/path/to/ito-cloud-runtime/cli/ito-compute-cli/dist/bin/ito.js

OAS does not resolve the credential-bearing client through `PATH`; this avoids
...
directory and 0600 token-file permissions. OAS does not inspect or log secrets.
...
- OAS returns the canonical process's stdout, stderr, and exit code unchanged.
...
- Global OAS dry-run does not create a local success result; the wrapper fails
  closed without invoking the canonical CLI.

All durable RFQ, quote, procurement, and reservation state remains owned by the
Itô platform. OAS adds no shadow store.

## Unsupported in this slice

OAS exposes no quote lock, purchase, workload execution, or inference command.
Node qualification is live-only through the separately gated canonical
adapter; the OAS bridge does not expose its paper fixture mode.

Managed inference remains unavailable. OAS does not claim that Itô created a
...
trigger language, not a second installed skill. The current OAS bridge has no
...
before retrying. OAS must never replace that control plane with root SSH, local
...
or replaced by another verified distribution channel. OAS must not claim npm
availability before a registry read confirms it.

The OAS package version remains unchanged in this worktree. Its version bump,
