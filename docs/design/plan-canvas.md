...
an original, OAS-native implementation of that idea, not a port.
...
- The artifact opens in the browser inside OAS-styled chrome (same design tokens as
...
|---|---|---|
| CLI entry | `scripts/plan-canvas.js` (+ npm bin `oas-plan-canvas`) | `scripts/control-pane.js` |
...
| Mermaid diagrams | `scripts/lib/plan-canvas/ui.js` | ` ```mermaid ` blocks render in the browser, themed to OAS; pinned CDN with offline fallback (`OAS_PLAN_CANVAS_MERMAID_URL` for a local mirror) |
| Session state | `scripts/lib/plan-canvas/sessions.js` | file-path-keyed sessions, state under `~/.claude/plan-canvas/` (`OAS_PLAN_CANVAS_STATE_DIR` override) |
...
from a bare `oas-plan-canvas` invocation).

Surfaces mirror how peer workflow-quality skills ship across OAS's harnesses:
...
- The CLI resolves from any project via the `oas-plan-canvas` bin (global/plugin install) or
...
after 30 min, `OAS_PLAN_CANVAS_IDLE_MS`). Feedback is deliver-and-drain: queued items are
handed to exactly one `await` call and persisted to disk until then, so nothing is lost if
the poll is interrupted.

- `GET /health` — `{ok, app: "oas-plan-canvas", version}` (CLI/server version handshake)
- `GET /` — session list (OAS chrome)
...
- `GET /canvas/<key>` — editor chrome; `GET /artifact/<key>/` — rendered artifact
  (markdown → OAS plan template, HTML passthrough) with the annotation SDK injected;
...
- Verdict actions wired to OAS's plan-confirmation workflow.
- OAS design tokens and chrome; JSON (not TOON) agent output.
- Mermaid renders themed to OAS, but without lavish's pan/zoom or node-id capture —
  whole-element annotation covers pointing at a diagram or node.
- No export/share hosting, no layout-audit gate, no bundled playbooks — OAS's existing
...
`OAS_PLAN_CANVAS_MERMAID_URL`. The server itself still makes no network calls.
