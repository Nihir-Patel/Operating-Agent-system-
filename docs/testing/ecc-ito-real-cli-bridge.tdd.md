# OAS × Itô Real CLI Bridge — TDD Evidence
...
1. As an OAS operator, I can explicitly invoke streaming device `login`, then
...
2. As a security reviewer, I can prove unsupported operations, missing local
   installs, and OAS dry-run requests fail before any child process or network
   operation.
