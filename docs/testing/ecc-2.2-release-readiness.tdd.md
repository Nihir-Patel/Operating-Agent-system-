# OAS 2.2 release-readiness TDD evidence
...
The full suite then exposed three guided Kimi collision checks that rejected OAS's own new bridge checkpoint before reaching the protected destination. Commit `15815eca` advanced the expected fingerprint only for OAS-authored state writes while preserving every external state and destination collision check.
...
- A launch and rollback runbook assigns the merge, signed tag, and release to
  Nihir and uses the npm dist-tag as the reversible availability switch.
