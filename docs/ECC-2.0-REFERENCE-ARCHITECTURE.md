# OAS 2.0 Reference Architecture

Current execution mirror:
[`OAS-2.0-GA-ROADMAP.md`](OAS-2.0-GA-ROADMAP.md).

This document turns the May 2026 reference sweep into concrete OAS backlog
...
| Reference | Primary pressure on OAS 2.0 | Concrete OAS delta |
| --- | --- | --- |
...
| [`jarrodwatts/claude-hud`](https://github.com/jarrodwatts/claude-hud) | Always-visible Claude Code statusline for context, tools, agents, todos, and transcript-backed activity. | Formalize the OAS HUD/status payload for context, cost, tool calls, active agents, todos, queue state, checks, and risk. |
| [`stanford-iris-lab/meta-harness`](https://github.com/stanford-iris-lab/meta-harness) | Automated search over task-specific harness design: what to store, retrieve, and show. | Split OAS improvement loops into scenario spec, proposer trace, verifier result, and promoted playbook. |
...
| [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) | Self-improving operator shell with memories, skills, scheduler, gateways, subagents, terminal backends, and migration tooling. | Keep OAS portable across local, SSH, container, and hosted terminal backends without hiding the underlying commands. |
...
OAS 2.0 should be a harness operating system, not only a catalog of commands,
agents, and skills.
...
│ AgentShield policies/SARIF, OAS Tools checks, billing,       │
...
- `operating-agent-systems`: extend the adapter compliance matrix and public
  scorecard onramp.
- `oas2`: surface session/worktree state through a stable local payload before
  adding hosted telemetry.
- `OAS-Tools`: consume the same lifecycle events for PR checks, issue routing,
  and Linear sync.
...
- Keep `docs/architecture/observability-readiness.md` as the operator-facing
  readiness gate.
- Define a versioned HUD/status JSON contract that both OAS2 and OAS Tools can
  consume.
...
- Use RAG/reference sets for vetted OAS patterns, team history, CI failures,
  review outcomes, harness config quality, and security decisions.

Repo work:

- `operating-agent-systems`: document scenario specs, verifier contracts, and
  playbook promotion rules.
- `OAS-Tools`: map analyzer findings to PR comments, check runs, and Linear
...
### OAS Tools Commercial And Review Platform

OAS Tools should become the GitHub-native layer for billing, deep analysis,
...
- Deep analyzer comparable in scope to the useful parts of GitGuardian,
  ...
  and harness drift.
- RAG/reference set over vetted OAS patterns, historical PR outcomes,
...
5. Audit OAS Tools billing and check-run surfaces.
