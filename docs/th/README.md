...
![Operating Agent Systems — ระบบเพิ่มประสิทธิภาพสำหรับ AI agent harness](../../assets/hero.png)
...
- **อย่าติดตั้งซ้อนกันหลายวิธี** ปัญหาที่พบบ่อยที่สุดคือการรัน `/plugin install` ก่อน แล้วตามด้วย `install.sh --profile full` หรือ `npx oas-universal install --profile full`

หากคุณติดตั้งซ้อนกันไปแล้วและพบว่ามี skill/hook ซ้ำ ดู [Reset / ถอนการติดตั้ง OAS](#reset--ถอนการติดตั้ง-oas)
...
OAS มีชื่อเรียกในระบบสาธารณะ 3 ชื่อที่ต่างกัน:

- GitHub repo: `neal0709/operating-agent-systems`
- Claude marketplace plugin: `oas@oas`
- npm package: `oas-universal`

ชื่อเหล่านี้ตั้งใจให้ต่างกัน Plugin บน Claude Code ใช้ `oas@oas` ส่วน npm ยังคงใช้ `oas-universal`
...
## Reset / ถอนการติดตั้ง OAS

หาก OAS ติดตั้งซ้อนกัน รบกวนระบบ หรือทำงานผิดปกติ อย่ารันติดตั้งทับซ้ำเข้าไปอีก

- **วิธี plugin:** ถอน plugin ออกจาก Claude Code จากนั้นลบโฟลเดอร์ rule ที่คุณคัดลอกเองใน `~/.claude/rules/oas/`
...
OAS จะลบเฉพาะไฟล์ที่อยู่ใน install-state ของตัวเอง ไม่แตะไฟล์อื่นนอกเหนือจากนั้น
...
/oas:plan "เพิ่มระบบยืนยันตัวตนผู้ใช้"
...
ปัจจุบัน OAS มี agent หลายสิบตัว, skill มากกว่า 200 ชุด และ legacy command shim สำหรับ workflow ต่าง ๆ ดูรายการเต็มและคำแนะนำล่าสุดได้ใน [README ภาษาอังกฤษ](../../README.md)
...
```bash
# 1. Fork และ clone
gh repo fork neal0709/operating-agent-systems --clone
cd operating-agent-systems
...
- [GitHub Discussions](https://github.com/neal0709/operating-agent-systems/discussions) — ถาม-ตอบ, โชว์ผลงาน
- [GitHub Sponsors](https://github.com/sponsors/neal0709) — สนับสนุน OSS เริ่มที่ $5/เดือน
- [OAS Pro](https://oas.tools/pricing) — private repo + GitHub App ($19/seat/เดือน)
- [OAS Tools GitHub App](https://github.com/marketplace/oas-tools) — ติดตั้ง, PR audit, มี free tier

**OSS ยังคงฟรีตลอดไป** Repo นี้ใช้สัญญาอนุญาต MIT ตลอดกาล OAS Pro คือ GitHub App ที่ host ไว้สำหรับ private repo ส่วน Sponsors และ Pro subscribers ช่วยสนับสนุนให้ maintainer คนเดียวสามารถส่งงานข้าม 7 harness ได้ทุกสัปดาห์
