# Hermes / OpenClaw -> OAS Migration

This document is the public migration guide for moving a Hermes or OpenClaw-style operator setup into the current OAS model.
...
OAS is the durable public system:
...
- install surfaces
- session adapters
- OAS 2.0 control-plane work

Hermes and OpenClaw are useful inputs because they contain repeated operator workflows that can be distilled into OAS-native surfaces.

That means the shortest safe path is:

1. extract the reusable behavior
2. translate it into OAS-native skills, hooks, docs, or adapter work
...
- OAS hook / command automation for local repeatability
- OAS 2.0 scheduler work under issue `#1050`
...
- OAS 2.0 as the future long-lived scheduling/control plane
...
- OAS session adapter and control-plane work
- orchestration/session inspection commands
- OAS 2.0 control-plane backlog under:
  - `#1045`
  - `#1046`
  - `#1047`
  - `#1048`
...
- OAS-native top-level skills when the workflow is reusable
...
- OAS-native operator skills when the workflow logic is the real asset
- adapter/control-plane work when the missing piece is session/runtime coordination

Do not import opaque third-party runtimes into OAS just because a private workflow depended on them.

If a workflow is valuable:

1. understand the behavior
2. rebuild the minimum OAS-native version
...
- OAS 2.0 adapter/control-plane discovery docs
...
- continuing the OAS 2.0 operator/control-plane buildout

OAS 2.0 now ships a bounded migration audit entrypoint:

- `oas migrate audit --source ~/.hermes`
- `oas migrate plan --source ~/.hermes --output migration-plan.md`
- `oas migrate scaffold --source ~/.hermes --output-dir migration-artifacts`
- `oas migrate import-skills --source ~/.hermes --output-dir migration-artifacts/skills`
- `oas migrate import-tools --source ~/.hermes --output-dir migration-artifacts/tools`
- `oas migrate import-plugins --source ~/.hermes --output-dir migration-artifacts/plugins`
- `oas migrate import-schedules --source ~/.hermes --dry-run`
- `oas migrate import-remote --source ~/.hermes --dry-run`
- `oas migrate import-env --source ~/.hermes --dry-run`
- `oas migrate import-memory --source ~/.hermes`

Use that first to inventory the legacy workspace and map detected surfaces onto the current OAS2 scheduler, remote dispatch, memory graph, templates, and manual-translation lanes.
...
1. Keep the public OAS repo as the canonical reusable layer.
2. Port reusable Hermes/OpenClaw workflows into OAS-native skills one lane at a time.
3. Keep private auth and personal context outside the repo.
4. Use GitHub / Linear / KB systems as durable truth.
5. Treat OAS 2.0 as the path to a native operator shell, not as a finished product.
