...
![Operating Agent Systems — система повышения эффективности сред агентного ИИ](../../assets/hero.png)

[![Stars](https://img.shields.io/github/stars/neal0709/operating-agent-systems?style=flat)](https://github.com/neal0709/operating-agent-systems/stargazers)
[![Forks](https://img.shields.io/github/forks/neal0709/operating-agent-systems?style=flat)](https://github.com/neal0709/operating-agent-systems/network/members)
[![Contributors](https://img.shields.io/github/contributors/neal0709/operating-agent-systems?style=flat)](https://github.com/neal0709/operating-agent-systems/graphs/contributors)
...
OAS v2.0.0-rc.1 добавляет публичную историю оператора Hermes поверх этого переиспользуемого слоя: начните с [руководства по настройке Hermes](../HERMES-SETUP.md), затем прочитайте [примечания к выпуску rc.1](../releases/2.0.0-rc.1/release-notes.md) и [архитектуру для разных сред](../architecture/cross-harness.md).
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="Краткое руководство по Operating Agent Systems" />
</a>
</td>
<td width="33%">
<a href="https://x.com/nihirpatel/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="Подробное руководство по Operating Agent Systems" />
</a>
</td>
<td width="33%">
...
### v2.0.0-rc.1 — Обновление публичного контура, операторские рабочие процессы и OAS 2.0 Alpha (апрель 2026)

- **Dashboard GUI** — новое настольное приложение на Tkinter (`oas_dashboard.py` или `npm run dashboard`) с переключателем тёмной/светлой темы, настройкой шрифта и логотипом проекта в заголовке и панели задач.
...
- **Расширение операторских и outbound-рабочих процессов** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `oas-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` и `workspace-surface-audit` закрывают операторское направление.
...
- **OAS 2.0 alpha находится в дереве репозитория** — прототип control plane на Rust в `oas2/` теперь собирается локально и предоставляет команды `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` и `daemon`. Это пригодная к использованию alpha-версия, но ещё не общий релиз.
- **Укрепление экосистемы** — AgentShield, контроль затрат OAS Tools, работа над billing portal и обновления сайта продолжают поставляться вокруг основного плагина, а не расползаются по отдельным направлениям.
...
- **Релиз с фокусом на средах агентного ИИ** — OAS теперь явно позиционируется как система повышения эффективности таких сред, а не просто набор конфигов.
...
- **Управление хуками во время выполнения** — `OAS_HOOK_PROFILE=minimal|standard|strict` и `OAS_DISABLED_HOOKS=...` для runtime-ограничений без редактирования файлов хуков.
...
- **GitHub Marketplace** — GitHub App OAS Tools доступен на [github.com/marketplace/oas-tools](https://github.com/marketplace/oas-tools) с тарифами free/pro/enterprise
...
- **Исправлена потеря содержимого при импорте инстинктов** — `parse_instinct_file()` незаметно отбрасывал всё содержимое после frontmatter (разделы Action, Evidence, Examples) во время `/instinct-import`. ([#148](https://github.com/neal0709/operating-agent-systems/issues/148), [#161](https://github.com/neal0709/operating-agent-systems/pull/161))

### v1.4.0 — Многоязычные правила, мастер установки и PM2 (февраль 2026)

- **Интерактивный мастер установки** — новый навык `configure-oas` предоставляет пошаговую настройку с обнаружением merge/overwrite
...
Полный журнал изменений смотрите в [Releases](https://github.com/neal0709/operating-agent-systems/releases).
...
- **Не накладывайте методы установки друг на друга.** Самая частая сломанная конфигурация: сначала `/plugin install`, затем `install.sh --profile full` или `npx oas-universal install --profile full`.

Если вы уже наложили несколько установок и видите дублирование, сразу переходите к разделу [Сброс / удаление OAS](#сброс--удаление-oas).

### Путь с малым контекстом / без хуков

Если хуки кажутся слишком глобальными или вам нужны только правила, агенты, команды и основные навыки рабочих процессов OAS, пропустите плагин и используйте минимальный ручной профиль:
...
Если вы не уверены, какой профиль OAS или компонент установить, спросите упакованный advisor из любого проекта:
...
У OAS теперь три публичных идентификатора, и они не взаимозаменяемы:

- исходный репозиторий GitHub: `neal0709/operating-agent-systems`
- идентификатор Claude marketplace/plugin: `oas@oas`
- npm-пакет: `oas-universal`

Это сделано намеренно. Установки Anthropic marketplace/plugin ключуются каноническим идентификатором плагина, поэтому OAS использует `oas@oas`, чтобы имена инструментов и пространства имен slash-команд оставались достаточно короткими для строгих валидаторов Desktop/API. Старые публикации могут всё ещё показывать прежний длинный marketplace-идентификатор; считайте его только устаревшим alias. Отдельно npm-пакет остался `oas-universal`, поэтому npm-установки и marketplace-установки намеренно используют разные имена.

### Шаг 2: Установите правила (обязательно)

> ПРЕДУПРЕЖДЕНИЕ: **Важно:** плагины Claude Code не могут автоматически распространять `rules`.
>
> Если вы уже установили OAS через `/plugin install`, **не запускайте после этого `./install.sh --profile full`, `.\install.ps1 --profile full` или `npx oas-universal install --profile full`**. Плагин уже загружает навыки, команды и хуки OAS. Запуск полного установщика после установки плагина скопирует те же компоненты в пользовательские директории и может создать дублирующиеся навыки и дублирующееся runtime-поведение.
>
> Для установки через плагин вручную скопируйте только нужные директории `rules/` в `~/.claude/rules/oas/`. Начните с `rules/common` плюс один языковой или framework-пакет, который вы действительно используете. Не копируйте все директории правил, если явно не хотите весь этот контекст в Claude.
>
> Используйте полный установщик только если делаете полностью ручную установку OAS вместо пути через плагин.
>
> Если ваша локальная установка Claude была очищена или сброшена, это не значит, что нужно повторно покупать OAS. Начните с `node scripts/oas.js list-installed`, затем запустите `node scripts/oas.js doctor` и `node scripts/oas.js repair` перед любой переустановкой. Обычно это восстанавливает файлы, управляемые OAS, без пересборки всей настройки. Если проблема связана с аккаунтом или marketplace-доступом к OAS Tools, восстановление billing/account нужно делать отдельно.

```bash
# Сначала клонируйте репозиторий
git clone https://github.com/neal0709/operating-agent-systems.git
cd operating-agent-systems

# Установите зависимости (выберите пакетный менеджер)
npm install        # или: pnpm install | yarn install | bun install

# Путь установки через плагин: скопируйте только правила OAS в пространство имён OAS
mkdir -p ~/.claude/rules/oas
cp -R rules/common ~/.claude/rules/oas/
cp -R rules/typescript ~/.claude/rules/oas/

# Полностью ручной путь установки OAS (используйте вместо /plugin install)
...
Если OAS кажется дублированным, навязчивым или сломанным, не переустанавливайте его снова поверх самого себя.

- **Путь через плагин:** удалите плагин из Claude Code, затем удалите конкретные папки правил, которые вы вручную скопировали в `~/.claude/rules/oas/`.
...
Затем удалите файлы, управляемые OAS:
...
OAS удаляет только файлы, записанные в его install-state. Он не удалит посторонние файлы, которые сам не устанавливал.

Если вы смешали методы, очищайте в таком порядке:

1. Удалите установку плагина Claude Code.
2. Запустите команду удаления OAS из корня репозитория, чтобы удалить файлы, управляемые install-state.
...
# Существующие slash-style имена команд продолжают работать, пока OAS мигрирует с commands/.

# Установка через плагин использует каноническую форму с namespace
/oas:plan "Добавить аутентификацию пользователей"
...
Запустите настольную панель управления, чтобы визуально изучить компоненты OAS:
...
|   |-- configure-oas/              # Интерактивный мастер установки (НОВОЕ)
...
|
|-- rules/            # Always-follow guidelines (копируйте в ~/.claude/rules/oas/)
...
|
|-- oas_dashboard.py  # Настольная GUI-панель управления (Tkinter)
|
...
[Установить GitHub App](https://github.com/apps/skill-creator) | [oas.tools](https://oas.tools)
...
Используйте `/security-scan` в Claude Code, чтобы запустить его, или добавьте в CI через [GitHub Action](https://github.com/neal0709/agentshield).
...
**История:** это уже приводило к повторяющимся циклам fix/revert в репозитории ([#29](https://github.com/neal0709/operating-agent-systems/issues/29), [#52](https://github.com/neal0709/operating-agent-systems/issues/52), [#103](https://github.com/neal0709/operating-agent-systems/issues/103)). Поведение менялось между версиями Claude Code, что вызывало путаницу. Теперь есть регрессионный тест, который не даёт вернуть эту ошибку.
...
```json
{
  "extraKnownMarketplaces": {
    "oas": {
      "source": {
        "source": "github",
        "repo": "neal0709/operating-agent-systems"
      }
    }
  },
  "enabledPlugins": {
...
> mkdir -p ~/.claude/rules/oas
> cp -r operating-agent-systems/rules/common ~/.claude/rules/oas/
> cp -r operating-agent-systems/rules/typescript ~/.claude/rules/oas/   # выберите свой стек
> cp -r operating-agent-systems/rules/python ~/.claude/rules/oas/
> cp -r operating-agent-systems/rules/golang ~/.claude/rules/oas/
> cp -r operating-agent-systems/rules/php ~/.claude/rules/oas/
>
...
cp operating-agent-systems/agents/*.md ~/.claude/agents/
...
cp -r operating-agent-systems/.agents/skills/* ~/.claude/skills/
cp -r operating-agent-systems/skills/search-first ~/.claude/skills/
# Claude Code загружает навыки только из прямых подкаталогов ~/.claude/skills.
# Не вкладывайте ручную установку в ~/.claude/skills/oas/.

# Опционально: добавляйте нишевые/framework-specific skills только при необходимости
# for s in django-patterns django-tdd laravel-patterns springboot-patterns; do
# cp -r operating-agent-systems/skills/$s ~/.claude/skills/
# done

# Опционально: сохранить поддерживаемую slash-command совместимость во время миграции
mkdir -p ~/.claude/commands
cp operating-agent-systems/commands/*.md ~/.claude/commands/
...
Не копируйте сырой repo-файл `hooks/hooks.json` в `~/.claude/settings.json` или `~/.claude/hooks/hooks.json`. Этот файл ориентирован на плагин/репозиторий и должен устанавливаться через установщик OAS или загружаться как плагин, поэтому прямое копирование не является поддерживаемым ручным способом установки.
...
Если вы установили OAS через `/plugin install`, не копируйте эти хуки в `settings.json`. Claude Code v2.1+ уже автоматически загружает plugin `hooks/hooks.json`, а дублирование в `settings.json` вызывает двойное выполнение и кроссплатформенные конфликты хуков.

Примечание для Windows: директория конфигурации Claude — `%USERPROFILE%\\.claude`, а не `~/claude`.

#### Настроить MCP

Установки Claude plugin намеренно не включают автоматически bundled MCP server definitions OAS. Это предотвращает слишком длинные имена plugin MCP tools на строгих сторонних gateway, но оставляет доступной ручную настройку MCP.
...
Если у вас уже запущены собственные копии MCP, bundled в OAS, задайте:

```bash
export OAS_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

OAS-managed install и Codex sync flows будут пропускать или удалять эти bundled servers вместо повторного добавления дубликатов. `OAS_DISABLED_MCPS` — это фильтр установки/синхронизации OAS, а не live-переключатель Claude Code.
...
|-----------|---------------|-------|
| Спланировать новую функцию | `/oas:plan "Добавить auth"` | planner |
| Спроектировать архитектуру системы | `/oas:plan` + агент architect | architect |
...
```
/oas:plan "Добавить OAuth-аутентификацию пользователей"
...
Это самая частая проблема. **НЕ добавляйте поле `"hooks"` в `.claude-plugin/plugin.json`.** Claude Code v2.1+ автоматически загружает `hooks/hooks.json` из установленных плагинов. Явное объявление вызывает ошибки обнаружения дубликатов. См. [#29](https://github.com/neal0709/operating-agent-systems/issues/29), [#52](https://github.com/neal0709/operating-agent-systems/issues/52), [#103](https://github.com/neal0709/operating-agent-systems/issues/103).
</details>

<details>
<summary><b>Можно ли использовать OAS с Claude Code на custom API endpoint или model gateway?</b></summary>

Да. OAS не хардкодит транспортные настройки Anthropic-hosted окружения. Он запускается локально через обычный CLI/plugin-интерфейс Claude Code, поэтому работает с:
...
Если ваш gateway переименовывает модели, настраивайте это в Claude Code, а не в OAS. Хуки, навыки, команды и правила OAS не зависят от model provider, если CLI `claude` уже работает.
...
Слишком много MCP-серверов съедают контекст. Каждое описание MCP tool потребляет токены из вашего окна 200k, потенциально сокращая его до ~70k. Контекст SessionStart по умолчанию ограничен 8000 символами; уменьшите его через `OAS_SESSION_START_MAX_CHARS=4000` или отключите через `OAS_SESSION_START_CONTEXT=off` для local-model или low-context setups.
...
- **Codex**: первоклассная поддержка macOS app и CLI, с guards против adapter drift и SessionStart fallback. См. PR [#257](https://github.com/neal0709/operating-agent-systems/pull/257).
...
Они не поставляются вместе с OAS и не аудируются этим репозиторием, но о них стоит знать, если вы изучаете более широкую экосистему Claude Code skills:
...
OAS предоставляет поддержку Cursor IDE с хуками, правилами, агентами, навыками, командами и MCP-конфигами, адаптированными под layout проекта Cursor.
...
| Agents | 50 | `.cursor/agents/oas-*.md` при установке; с префиксом, чтобы избежать конфликтов с user или marketplace agents |
...
OAS не устанавливает root `AGENTS.md` в `.cursor/`. Cursor воспринимает вложенные `AGENTS.md` как directory context, поэтому копирование identity OAS-репозитория в host project загрязняло бы этот проект.

Cursor-native loading behavior может различаться между сборками Cursor. OAS устанавливает агентов как `.cursor/agents/oas-*.md`; если ваша сборка Cursor не показывает project agents, эти файлы всё равно работают как явные reference definitions, а не скрытый global prompt context.
...
OAS предоставляет **первоклассную поддержку Codex** как для macOS app, так и для CLI: reference configuration, Codex-specific supplement `AGENTS.md` и общие skills.
...
# Автоматическая настройка: синхронизировать assets OAS (AGENTS.md, skills, MCP servers) в ~/.codex
npm install && bash scripts/sync-oas-to-codex.sh
# или: pnpm install && bash scripts/sync-oas-to-codex.sh
# или: yarn install && bash scripts/sync-oas-to-codex.sh
# или: bun install && bash scripts/sync-oas-to-codex.sh

# Или вручную: скопировать reference config в домашнюю директорию
cp .codex/config.toml ~/.codex/config.toml
```

Sync script безопасно сливает MCP-серверы OAS в существующий `~/.codex/config.toml` через стратегию **add-only**: он никогда не удаляет и не изменяет ваши существующие серверы. Запускайте с `--dry-run`, чтобы посмотреть изменения, или с `--update-mcp`, чтобы принудительно обновить OAS-серверы до последнего рекомендуемого конфига.

Для Context7 OAS использует каноническое имя секции Codex `[mcp_servers.context7]`, но всё ещё запускает пакет `@upstash/context7-mcp`. Если у вас уже есть legacy-запись `[mcp_servers.context7-mcp]`, `--update-mcp` мигрирует её на каноническое имя секции.
...
| operating-agent-systems | Development conventions и patterns для проекта |
...
Codex **пока не предоставляет parity с Claude-style hook execution**. Принуждение OAS там instruction-based через `AGENTS.md`, опциональные overrides `model_instructions_file` и настройки sandbox/approval.
...
OAS поставляет три sample role configs:
...
OAS предоставляет **полную поддержку OpenCode**, включая плагины и хуки.
...
Эта npm plugin entry включает опубликованный OpenCode plugin module OAS (hooks/events и plugin tools).
Она **не** добавляет автоматически полный catalog команд/агентов/instructions OAS в конфиг вашего проекта.

Для полной настройки OAS OpenCode:
...
OAS — **первый плагин, который помогает максимально использовать каждый крупный инструмент AI-кодинга**. Вот как сравниваются среды:
...
- Используйте `OAS_DISABLED_MCPS` только для фильтрации MCP-конфигов, генерируемых OAS, во время install/sync flows
...
Проекты, построенные на Operating Agent Systems или вдохновлённые им:
...
Построили что-то с OAS? Откройте PR, чтобы добавить это сюда.
...
[**Стать спонсором**](https://github.com/sponsors/neal0709) | [Уровни спонсорства](../../SPONSORS.md) | [Программа спонсорства](../../SPONSORING.md)

---

## История звёзд

[![Star History Chart](https://api.star-history.com/svg?repos=neal0709/operating-agent-systems&type=Date)](https://star-history.com/#neal0709/operating-agent-systems&Date)

---

## Ссылки

- **Краткое руководство (начните здесь):** [The Shorthand Guide to Operating Agent Systems](https://x.com/nihirpatel/status/2012378465664745795)
- **Подробное руководство (продвинутый уровень):** [The Longform Guide to Operating Agent Systems](https://x.com/nihirpatel/status/2014040193557471352)
- **Руководство по безопасности:** [Security Guide](../../the-security-guide.md) | [Тред](https://x.com/nihirpatel/status/2033263813387223421)
- **Подписаться:** [@nihirpatel](https://x.com/nihirpatel)
