...
This document defines the canonical OAS session snapshot contract for
`oas.session.v1`.
...
OAS has multiple session sources:
...
| Field | Type | Notes |
| --- | --- | --- |
| `schemaVersion` | string | MUST be exactly `oas.session.v1` for this contract |
...
Every OAS session adapter MUST:

1. Emit `schemaVersion: "oas.session.v1"` exactly.
...
- rely only on documented fields for `oas.session.v1`
