...
OAS can install its managed command, agent, skill, rule, and MCP surfaces into the Qwen CLI home directory.

## Install

From the OAS repository root:
...
The Qwen adapter writes into `~/.qwen/` and records managed file ownership in `~/.qwen/oas-install-state.json`.
...
Rerun the same install command after pulling OAS updates. The installer uses the install-state file to update OAS-managed files without claiming unrelated user files in `~/.qwen/`.
...
That removes files recorded in `~/.qwen/oas-install-state.json` and leaves unrelated Qwen configuration alone.
