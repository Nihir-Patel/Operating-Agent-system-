...
Common issues and solutions for Operating Agent Systems (OAS) plugin.
...
**Solutions:**
```bash
# First inspect what OAS still knows about this machine
oas list-installed
oas doctor
oas repair
...
# Claude Code → Extensions → Operating Agent Systems → Uninstall
# Then reinstall from marketplace

# If the issue is marketplace/account access, use OAS Tools billing/account recovery separately; do not use reinstall as a proxy for account recovery
...
cp -r operating-agent-systems ~/.claude/plugins/oas
```
...
`[OAS] changed-files tracking disabled` warning appears in the OpenCode
...
- The `~/.opencode` install is missing or incomplete for this machine —
  ...
  and the `oas-hooks` plugin depend on `plugins/lib/changed-files-store.js`;
  since `oas-hooks.ts` is OpenCode's plugin entry point (loaded once at
...
**Solutions:**
```bash
# From the OAS repo, check for and repair missing/incomplete managed files
oas doctor --target opencode
oas repair --target opencode

# If that reports no drift but plugins/ is still missing on the device,
# re-run the OAS installer for the opencode target
```
...
plugin, not OAS — OAS never writes to `~/.config/opencode/`. Fix the model prefix
...
cd ~/.claude/plugins/cache/oas
npm install

# Or for manual install
cd ~/.claude/plugins/oas
npm install
```
...
1. **Check GitHub Issues**: [github.com/neal0709/operating-agent-systems/issues](https://github.com/neal0709/operating-agent-systems/issues)
