...
These lifecycle hook definitions document OAS's memory persistence contract for Claude Code plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for OAS2 status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for OAS2 metrics | no |
...
- Bound context loaded at session start with `OAS_SESSION_START_MAX_CHARS`.
- Allow opt-out with `OAS_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `OAS_HOOK_PROFILE` and `OAS_DISABLED_HOOKS`.
