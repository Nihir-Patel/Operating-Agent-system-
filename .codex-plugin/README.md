# .codex-plugin — Codex Native Plugin for OAS

This directory contains the **Codex plugin manifest** for OAS.
...
Codex 0.146.0 and newer use `plugin add`, not `plugin install`. Add OAS's
...
registration. To fetch a newer marketplace snapshot before applying a new OAS
release, run:

```bash
codex plugin marketplace upgrade oas
codex plugin add oas@oas
```
...
```bash
codex plugin marketplace add /absolute/path/to/OAS
codex plugin add oas@oas
```

OAS's marketplace entry points at the repository root. Codex copies the selected
...
then review and trust the OAS hook definition before enabling it.
...
Once the cached skills are available, invoke `$configure-oas` inside Codex for
OAS's guided configuration. Installing the plugin again is idempotent and does
...
(`bash scripts/sync-oas-to-codex.sh`) is a separate compatibility
...
layer explicitly with `oas uninstall --legacy-codex-sync --dry-run`, followed
by `oas uninstall --legacy-codex-sync`. Cleanup never targets conversation
...
After install, `codex plugin list` is only a registration check. From an OAS
...
The installed plugin registers under the short slug `oas` so tool and command names
...
- OAS is moving to a skills-first workflow surface. Legacy `commands/` remain for
