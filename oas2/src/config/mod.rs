...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            db_path: home.join(".claude").join("oas2.db"),
            worktree_root: PathBuf::from("/tmp/oas-worktrees"),
...
impl Config {
    ...
    pub fn config_path() -> PathBuf {
        Self::config_root().join("oas2").join("config.toml")
    }
    ...
    fn load_from_paths(
        ...
        let mut merged = toml::Value::try_from(Self::default())
            .context("serialize default OAS 2.0 config for layered merge")?;
        ...
        merged
            .try_into()
            .context("deserialize merged OAS 2.0 config")
    }
    ...
    fn project_config_paths_from(start: &std::path::Path) -> Vec<PathBuf> {
        ...
        while let Some(path) = current {
            let legacy = path.join(".claude").join("oas2.toml");
            let primary = path.join("oas2.toml");
    ...
    fn merge_config_file(base: &mut toml::Value, path: &std::path::Path) -> Result<()> {
        let content = std::fs::read_to_string(path)
            .with_context(|| format!("read OAS 2.0 config from {}", path.display()))?;
        let overlay: toml::Value = toml::from_str(&content)
            .with_context(|| format!("parse OAS 2.0 config from {}", path.display()))?;
...
mod tests {
    ...
    fn missing_budget_fields_fall_back_to_defaults() {
        let legacy_config = r#"
db_path = "/tmp/oas2.db"
worktree_root = "/tmp/oas-worktrees"
max_parallel_sessions = 8
max_parallel_worktrees = 6
worktree_retention_secs = 0
session_timeout_secs = 3600
...
"#;
    ...
    fn worktree_branch_prefix_deserializes_from_toml() {
        let config: Config = toml::from_str(r#"worktree_branch_prefix = "bots/oas""#).unwrap();

        assert_eq!(config.worktree_branch_prefix, "bots/oas");
    }

    #[test]
    fn layered_config_merges_global_and_project_overrides() {
        let tempdir = std::env::temp_dir().join(format!("oas2-config-{}", Uuid::new_v4()));
        let legacy_global_path = tempdir.join("legacy-global.toml");
        let global_path = tempdir.join("config.toml");
        let project_path = tempdir.join("oas2.toml");
...
"#,
    ...
    fn project_config_discovery_prefers_nearest_directory_and_new_path() {
        ...
        std::fs::write(project_root.join(".claude").join("oas2.toml"), "").unwrap();
        std::fs::write(project_root.join("oas2.toml"), "").unwrap();

        let paths = Config::project_config_paths_from(&nested_dir);
        assert_eq!(
            paths,
            vec![
                project_root.join(".claude").join("oas2.toml"),
                project_root.join("oas2.toml")
            ]
        );
    ...
    fn primary_config_path_uses_xdg_style_location() {
        ...
        assert!(path.ends_with("oas2/config.toml"));
    }
...
"#,
        ...
        assert_eq!(
            runner.env.get("OAS_HARNESS").map(String::as_str),
            Some("cursor")
        );
    }
...
"#,
        ...
        let vars = BTreeMap::from([
            ...
            ("project".to_string(), "oas-core".to_string()),
        ...
        assert_eq!(template.project.as_deref(), Some("oas-core"));
...
"#,
    ...
    fn save_round_trips_automation_settings() {
        let path = std::env::temp_dir().join(format!("oas2-config-{}.toml", Uuid::new_v4()));
        ...
        config.worktree_branch_prefix = "bots/oas".to_string();
        ...
        assert_eq!(loaded.worktree_branch_prefix, "bots/oas");
        assert_eq!(
