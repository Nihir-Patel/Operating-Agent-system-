...
pub async fn run(db: StateStore, cfg: Config) -> Result<()> {
    tracing::info!("OAS daemon started");
...
mod tests {
    ...
    fn temp_db_path() -> PathBuf {
        std::env::temp_dir().join(format!("oas2-daemon-test-{}.db", uuid::Uuid::new_v4()))
    }
