...
Remove OAS-managed files recorded in install-state for the current context.
When no install-state is found, the uninstaller also detects and removes
legacy sync-oas-to-codex.sh artifacts, but only when a legacy ownership
...
function printHuman(result) {
  if (result.results.length === 0) {
    console.log('No OAS install-state files found for the current home/project context.');
    return;
  }
