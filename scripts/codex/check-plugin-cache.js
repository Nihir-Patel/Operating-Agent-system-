...
function usage() {
  console.log([
    ...
    '  --marketplace <name> Marketplace cache name (default: oas)',
    '  --plugin <name>      Plugin cache name (default: oas)',
...
function checkCache(options) {
  ...
  if (!fs.existsSync(manifestPath)) {
    ...
    if (versions.length > 0) {
      ...
      if (options.marketplace === 'oas' && options.plugin === 'oas') {
        log('Run: codex plugin marketplace add neal0709/Operating-Agent-system-');
      } else {
  ...
  if (failures > 0) {
    ...
    const syncScript = path.join(REPO_ROOT, 'scripts', 'sync-oas-to-codex.sh');
    if (fs.existsSync(syncScript)) {
      log('Use the supported sync path until the cache contains the referenced files:');
      log('npm install && bash scripts/sync-oas-to-codex.sh');
    } else {
      log('Use the supported manual sync workflow from your OAS installation.');
    }
    return 1;
  }
...
function main() {
  ...
  try {
    ...
    console.error(`[oas-codex] ${error.message}`);
    usage();
    process.exit(1);
  }
