...
# OAS Codex global regression sanity check.
# Validates that global ~/.codex state matches expected OAS integration.
...
HOOKS_DIR_EXPECT="${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
...
printf 'OAS GLOBAL SANITY CHECK\n'
printf 'Repo: %s\n' "$REPO_ROOT"
printf 'Codex home: %s\n\n' "$CODEX_HOME"
...
if [[ -f "$AGENTS_FILE" ]]; then
  if search_file '^# Operating Agent Systems \(OAS\)' "$AGENTS_FILE"; then
    ok "AGENTS contains OAS root instructions"
  else
    fail "AGENTS missing OAS root instructions"
  fi

  if search_file '^# Codex Supplement \(From OAS \.codex/AGENTS\.md\)' "$AGENTS_FILE"; then
    ok "AGENTS contains OAS Codex supplement"
  else
    fail "AGENTS missing OAS Codex supplement"
  fi
fi
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  # OAS <= 2.0.0 emitted a url-only exa entry that Codex's stdio-only
  ...
  if search_file '^\[mcp_servers\.exa\]' "$CONFIG_FILE"; then
    ...
    if printf '%s\n' "$exa_block" | grep -Eq '^[[:space:]]*url[[:space:]]*=' \
      ...
      fail "MCP section [mcp_servers.exa] uses a url key, which Codex rejects for stdio servers — re-run oas-sync-codex to repair (#2224)"
    else
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  if [[ "$missing_skills" -eq 0 ]]; then
    ok "All 16 OAS skills are present in $SKILLS_DIR"
  else
    warn "$missing_skills OAS skills missing from $SKILLS_DIR (install via OAS installer or npx skills)"
  fi
else
  warn "Skills directory missing ($SKILLS_DIR) — install via OAS installer or npx skills"
fi

if [[ -f "$PROMPTS_DIR/oas-prompts-manifest.txt" ]]; then
  ok "Command prompts manifest exists"
else
  fail "Command prompts manifest missing"
fi

if [[ -f "$PROMPTS_DIR/oas-extension-prompts-manifest.txt" ]]; then
  ok "Extension prompts manifest exists"
else
  fail "Extension prompts manifest missing"
fi

command_prompts_count="$(find "$PROMPTS_DIR" -maxdepth 1 -type f -name 'oas-*.md' 2>/dev/null | wc -l | tr -d ' ')"
if [[ "$command_prompts_count" -ge 43 ]]; then
  ok "OAS prompts count is $command_prompts_count (expected >= 43)"
else
  fail "OAS prompts count is $command_prompts_count (expected >= 43)"
fi
...
if command -v oas-sync-codex >/dev/null 2>&1; then
  ok "oas-sync-codex command is in PATH"
else
  warn "oas-sync-codex is not in PATH"
fi

if command -v oas-install-git-hooks >/dev/null 2>&1; then
  ok "oas-install-git-hooks command is in PATH"
else
  warn "oas-install-git-hooks is not in PATH"
fi

if command -v oas-check-codex >/dev/null 2>&1; then
  ok "oas-check-codex command is in PATH"
else
  warn "oas-check-codex is not in PATH (this is expected before alias setup)"
fi

printf '\nSummary: checks=%d, warnings=%d, failures=%d\n' "$checks" "$warnings" "$failures"
if [[ "$failures" -eq 0 ]]; then
  printf 'OAS GLOBAL SANITY: PASS\n'
else
  printf 'OAS GLOBAL SANITY: FAIL\n'
  exit 1
fi
