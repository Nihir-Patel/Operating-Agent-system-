...
try {
  ...
  console.error('[oas-codex] Missing dependency: @iarna/toml');
  console.error('[oas-codex] Run: npm install   (from the OAS repo root)');
  process.exit(1);
}
...
function warn(message) {
  console.warn(`[oas-codex] WARNING: ${message}`);
}
...
function main() {
  ...
  if (!fs.existsSync(referencePath)) {
    console.error(`[oas-codex] Reference config not found: ${referencePath}`);
    process.exit(1);
  }

  if (!fs.existsSync(configPath)) {
    console.error(`[oas-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[oas-codex] Failed to parse TOML: ${error.message}`);
    process.exit(1);
  }
