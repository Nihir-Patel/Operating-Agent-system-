...
Pull the latest OAS repo changes and reinstall the current context's managed targets
...
function deriveRepoRootFromState(state) {
  ...
  throw new Error('Unable to infer OAS repo root from install-state operations');
}
...
// Recognized OAS package names. A repo root is only trusted to run its
// install-apply.js if its package.json identifies it as OAS — otherwise a
...
const OAS_PACKAGE_NAMES = new Set(['oas-universal', 'operating-agent-systems']);
...
function validateRepoRoot(repoRoot) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Invalid OAS repo root: missing package.json at ${packageJsonPath}`);
  }

  if (!fs.existsSync(installApplyPath)) {
    throw new Error(`Invalid OAS repo root: missing install script at ${installApplyPath}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid OAS repo root: unreadable package.json at ${packageJsonPath}`);
  }
  if (!OAS_PACKAGE_NAMES.has(pkgName)) {
    throw new Error(`Refusing to run install from untrusted repo root ${normalized}: package.json name '${pkgName}' is not an official OAS package.`);
  }
...
function runAutoUpdate(options = {}, dependencies = {}) {
  ...
  if (!requestedRepoRoot) {
    ...
    if (uniqueRepoRoots.length > 1) {
      throw new Error(`Multiple OAS repo roots detected: ${uniqueRepoRoots.join(', ')}`);
    }
  }
...
function printHuman(result) {
  if (result.results.length === 0) {
    ...
    console.log(hasWarnings
      ? 'No active OAS install-state files found for the current home/project context.'
      : 'No OAS install-state files found for the current home/project context.');
