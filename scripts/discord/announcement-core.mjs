...
export function buildDiscordPayload({ title, body, url, key }) {
  ...
  const footer = `oas:${discussionId}`;
  const description = String(body || '').trim().slice(0, DISCORD_DESCRIPTION_LIMIT);
  const nonce = `oas-${createHash('sha256').update(String(key)).digest('hex').slice(0, 16)}`;
  return {
    allowed_mentions: { parse: [] },
    nonce,
    enforce_nonce: true,
    embeds: [{
      title: String(title || 'OAS announcement').trim().slice(0, 256),
      description,
      url: String(url || ''),
      footer: { text: footer },
    }],
...
export function findDiscordReceipt(messages, key) {
  const discussionId = String(key).split(':').at(-1);
  return messages.find(message => message.embeds?.some(embed => embed.footer?.text === `oas:${discussionId}`)) || null;
}
...
export function discussionReceiptMarker(key) {
  return `<!-- oas-discord-receipt:${createHash('sha256').update(String(key)).digest('hex').slice(0, 32)} -->`;
}
