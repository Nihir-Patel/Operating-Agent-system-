#!/usr/bin/env node
// OAS community Discord bot — dependency-free (Node 22+ native WebSocket).
// Slash commands: /oas /help /skill /docs /release
//
// Env: DISCORD_BOT_TOKEN (required), DISCORD_APP_ID (required),
//      OAS_REPO (path to local clone, default ~/GitHub/OAS/operating-agent-systems),
//      DISCORD_INVITE (optional, shown in /oas)
//
...
const REPO = process.env.OAS_REPO || join(homedir(), 'GitHub/OAS/operating-agent-systems');
const REPO_URL = 'https://github.com/neal0709/Operating-Agent-system-';
...
const HELP = [
  '**OAS bot commands**',
  '- `/oas` — what OAS is + all the links',
  '- `/skill name:<query>` — look up an OAS skill',
  '- `/docs query:<terms>` — search the OAS docs',
  '- `/release` — latest OAS release',
  '- `/help` — this message',
].join('\n');

const handlers = {
  oas: () => [
    '**Operating Agent Systems (OAS)** — the agent harness performance system.',
    ...
    '',
    `- repo: ${REPO_URL}`,
    '- site: https://oas.tools',
    `- install: \`/plugin marketplace add neal0709/operating-agent-systems\` then \`/plugin install oas\``,
  ...
  release: async () => {
    const res = await fetch('https://api.github.com/repos/neal0709/Operating-Agent-system-/releases/latest', {
      headers: { 'User-Agent': 'oas-discord-bot' },
    });
  ...
  ws.onmessage = ev => {
    ...
    switch (msg.op) {
      case 10: { // HELLO
        ...
        send({ op: 2, d: { token: TOKEN, intents: 1, properties: { os: 'darwin', browser: 'oas-bot', device: 'oas-bot' } } });
        break;
      }
