...
function helpText() {
  return `
OAS experimental Nasiko CLI lifecycle bridge

Usage:
  oas nasiko status [--install-dir <absolute-path>] [--json]
  oas nasiko install --version v0.1.0 --yes [--install-dir <absolute-path>] [--json]
  oas nasiko install --version v0.1.0 --dry-run [--install-dir <absolute-path>] [--json]
  oas nasiko uninstall --version v0.1.0 --yes [--install-dir <absolute-path>] [--json]

The installer is opt-in, accepts only OAS-qualified pinned releases, downloads
...
function resolveExecutable(options = {}) {
  const configured = process.env.OAS_NASIKO_CLI_EXECUTABLE;
  const normalized = normalizePlatform();
  ...
  if (!path.isAbsolute(candidate)) {
    throw new Error('OAS_NASIKO_CLI_EXECUTABLE must be an absolute path.');
  }
  return candidate;
}
...
async function main(argumentsList = process.argv.slice(2)) {
  ...
  if (command === 'status') {
    ...
    if (options.json) process.stdout.write(`${JSON.stringify(status, null, 2)}\n`);
      ...
      : 'Nasiko is not installed in the OAS-qualified location.\n');
    return 0;
  }
  if (command === 'install') {
