...
Usage:
  oas memory init [--scope project|team|user] [--json]
  oas memory save --title <text> (--stdin | --body-file <path>) [options]
  oas memory handoff --from <harness> --target <harness> --title <text> (--stdin | --body-file <path>) [options]
  oas memory search [query] [--scope <scope>] [--target-harness <harness>] [--kind <kind>] [--limit <n>] [--json]
  oas memory read <memory-id> [--scope <scope>] [--json]
  oas memory doctor [--scope <scope>] [--json]
...
Write options:
  ...
  --source-harness <name>    Originating harness (default: OAS_MEMORY_HARNESS or unknown)
...
MCP:
  oas-memory-mcp             Start the opt-in local stdio MCP server
...
function printInit(result, json) {
  if (json) return writeJson({ schemaVersion: 'oas.memory.init.v1', ...result });
  process.stdout.write([
    `Initialized OAS memory scopes: ${sanitizeTerminalText(result.scopes.join(', '))}`,
...
function printWrite(result, json) {
  ...
  const payload = {
    schemaVersion: 'oas.memory.write.v1',
    memory,
...
function printSearch(query, result, json) {
  const payload = { schemaVersion: 'oas.memory.search.v1', query, ...result };
  if (json) return writeJson(payload);
...
function printRead(result, json) {
  const payload = { schemaVersion: 'oas.memory.read.v1', ...result };
  if (json) return writeJson(payload);
  process.stdout.write([
...
function printDoctor(report, json) {
  ...
  process.stdout.write([
    `OAS memory doctor: ${report.ok ? 'PASS' : 'ISSUES FOUND'}`,
...
function saveInput(options, kindOverride = null) {
  const sourceHarness = options.from
    ...
    || process.env.OAS_MEMORY_HARNESS
    || 'unknown';
  return {
    title: options.title,
