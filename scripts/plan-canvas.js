...
function usage() {
  return [
    ...
    '',
    'Environment: OAS_PLAN_CANVAS_PORT, OAS_PLAN_CANVAS_STATE_DIR, OAS_PLAN_CANVAS_IDLE_MS'
  ].join('\n');
}
...
async function healthCheck(port) {
  try {
    ...
    return res.body && res.body.app === 'oas-plan-canvas' ? res.body : null;
  } catch {
    return null;
  }
}
...
// mismatch after an OAS update restarts the server so browser and CLI never
...
async function ensureServer({ stateDir, port }) {
  ...
  const child = spawn(process.execPath, [__filename, 'server', '--port', String(port)], {
    ...
    env: { ...process.env, OAS_PLAN_CANVAS_STATE_DIR: stateDir }
  });
  child.unref();
  fs.closeSync(logFd);
  for (let i = 0; i < 50; i++) {
...
async function cmdOpen(file, args, { stateDir, port }) {
  ...
  return {
    ...
    next_step:
      'Run `oas-plan-canvas await <file>` and leave it running; it returns when the human sends feedback, a verdict, or ends the session.'
  };
}
...
async function cmdAwait(file, args, { stateDir, port }) {
  ...
  if (result.status === 'feedback') {
    result.next_step = result.sessionEnded
      ...
      : 'Address the feedback, then run `oas-plan-canvas await <file> --reply "<what you changed>"` to answer in the canvas and keep listening.';
...
function cmdPending({ stateDir }) {
  ...
  return {
    ...
    next_step: waiting.length
      ? 'Run `oas-plan-canvas await <file>` for each file above to receive the messages.'
