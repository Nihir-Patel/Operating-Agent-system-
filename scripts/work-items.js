...
Track Linear, GitHub, handoff, and manual roadmap items in the OAS SQLite state
store so "oas status" can include linked work and blocked operator follow-up.

Options:
  --id <id>                 Stable local work-item id for upsert
  --source <source>         Source system, e.g. linear, github, handoff, manual
  --source-id <id>          Source-local identifier, e.g. OAS-20 or PR number
  ...
  --session-id <id>         Optional OAS session id
...
function buildGithubPrWorkItem(repo, pr, options = {}) {
  return {
    ...
    metadata: {
      ...
      syncedBy: 'oas-work-items-sync-github'
    }
  };
}
...
function closeStaleGithubItems(store, repo, activeIds, options = {}) {
  ...
  for (const item of payload.items) {
    if (!item.metadata || item.metadata.syncedBy !== 'oas-work-items-sync-github') {
      continue;
    }
