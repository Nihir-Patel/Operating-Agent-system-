...
function detectTargetMode(rootDir) {
  ...
  if (packageJson?.name === 'operating-agent-systems') {
    return 'repo';
  }
...
const OAS_PLUGIN_KEY_PATTERNS = [
  /^oas@/i,
  /^operating-agent-systems@/i,
];

const OAS_LEGACY_PLUGIN_DIRS = [
  'oas',
  'oas@oas',
  'operating-agent-systems',
  'operating-agent-systems@operating-agent-systems',
];

const OAS_CACHE_MARKETPLACES = ['operating-agent-systems', 'oas'];
const OAS_CACHE_PLUGIN_NAMES = ['oas', 'operating-agent-systems'];
...
function findPluginInstallFromManifest(installedPluginsPaths) {
  for (const installedPath of installedPluginsPaths) {
    ...
    for (const [key, value] of Object.entries(manifest.plugins)) {
      if (!OAS_PLUGIN_KEY_PATTERNS.some(pattern => pattern.test(key))) {
        continue;
      }
...
function findPluginInstallFlatLayout(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const pluginDir of OAS_LEGACY_PLUGIN_DIRS) {
...
function findPluginInstallMarketplaceCache(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const marketplace of OAS_CACHE_MARKETPLACES) {
      for (const pluginName of OAS_CACHE_PLUGIN_NAMES) {
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.claude/plugins/oas/ (legacy operating-agent-systems paths also supported)',
      description: 'Operating Agent Systems is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the OAS plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      fix: 'Add project-local .claude hooks, commands, skills, or settings that tailor OAS to this repo.',
    },
    {
...
Audits the current working directory by default and auto-detects OAS repo mode vs consumer-project mode.
`);
