...
const EXECUTABLE_OVERRIDE = "OAS_ITO_CLI_EXECUTABLE";
const MAX_OUTPUT_BYTES = 10 * 1024 * 1024;
const NODE_QUALIFICATION_TIMEOUT_MS = 31 * 60 * 1000;

function showHelp() {
  process.stdout.write(`
OAS × Itô local CLI bridge

Usage:
  oas ito login [--no-browser]
  oas ito logout
  oas ito auth
  oas ito find <all required RFQ options>
  oas ito status
  oas ito evals --cluster <id> --live-sixtytwo --nodes <list> --config-dir <dir>
  oas ito <login|logout|auth|find|status|evals> --json

The bridge invokes the separately installed canonical Itô CLI and returns its
real stdout, stderr, and exit code unchanged. "oas ito login" delegates to the
...
suppress that handoff. OAS itself performs no browser automation and adds no
lock, workload, inference, or purchase path.
"oas ito auth" is validation-only and never starts device login.
"oas ito logout" asks the canonical CLI to revoke the current device credential
...
For safety, OAS never discovers this credential-bearing client through PATH.
...
function parseArgs(argv, environment = process.env) {
  ...
  if (environment.OAS_DRY_RUN === "1" || args.includes("--dry-run")) {
    throw new Error(
  ...
  if (!SUPPORTED_COMMANDS.includes(command)) {
    throw new Error(
      `Unsupported Itô command "${command || "(missing)"}"; OAS permits only login, logout, auth, find, status, and evals.`
    );
  }
  if (command === "auth" && withoutJson.includes("--no-browser")) {
    throw new Error("--no-browser is valid only for oas ito login; auth is validation-only.");
  }
  if (command === "evals") {
...
function resolveItoExecutable(environment = process.env) {
  ...
  if (!configured) {
    throw new Error([
      "The canonical ito-compute-cli is unpublished and OAS will not resolve",
