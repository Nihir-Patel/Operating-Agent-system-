...
function assertSafeInstallOperation(plan, operation) {
  ...
  assertWithinTrustedRoot(operation.destinationPath, targetRoot, 'install OAS file');
  ...
  for (const segmentIndex of Array.from({ length: segments.length + 1 }, (_value, index) => index)) {
    ...
    try {
      ...
      if (stats.isSymbolicLink()) {
        throw new Error(
          `Refusing to install OAS file through symlinked path: '${currentPath}'.`
        );
      }
    } catch (error) {
...
function applyInstallPlan(plan, dependencies = {}) {
  ...
  const disabledServers = parseDisabledMcpServers(process.env.OAS_DISABLED_MCPS);
  ...
  try {
    ...
    if (antigravityMigration.detected && !antigravityMigration.complete) {
      antigravityMigrationWarnings = [
        'Legacy Antigravity migration is incomplete. OAS preserved modified, unverifiable, or unmanaged content under .agent; review and move anything you want to keep, then rerun the Antigravity install.',
  ...
  try {
    ...
    if (opencodeMigration.detected && !opencodeMigration.complete) {
      opencodeMigrationWarnings = [
        'Legacy OpenCode migration is incomplete. OAS preserved modified or unverifiable managed content under ~/.opencode; review it and rerun the OpenCode install.',
