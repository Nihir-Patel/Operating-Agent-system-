...
function findManualClaudePlugin(options = {}) {
  ...
  const candidates = [
    ['oas', '.claude-plugin', 'plugin.json'],
    ['oas', 'plugin.json'],
    ['oas@oas', '.claude-plugin', 'plugin.json'],
    ['oas@oas', 'plugin.json'],
    ['operating-agent-systems', '.claude-plugin', 'plugin.json'],
    ['operating-agent-systems', 'plugin.json'],
  ];
...
function validateManagedState(state, statePath, expectedRoot) {
  ...
  if (
    state?.schemaVersion !== 'oas.install.v1'
    || !state.target
...
function findManagedClaudeInstalls(options = {}) {
  ...
  const candidates = [
    {
      statePath: path.join(configDir, 'oas', 'install-state.json'),
      expectedRoot: configDir,
    },
    {
      statePath: path.join(projectRoot, '.claude', 'oas', 'install-state.json'),
