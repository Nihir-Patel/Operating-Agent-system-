...
const INSTALL_STATE_NAME = 'oas-install-state.json';
...
function getVerifiedManagedFile(operation, legacyRoot, sourceRoot) {
  ...
  if (!pathExists(sourcePath)) {
    return {
      ...
      missing: false,
      retainedReason: 'The current OAS source file is unavailable, so its provenance cannot be revalidated.',
    };
  }
  ...
  if (!source || destination.digest !== source.digest) {
    return {
      ...
      fileStat: destination.stat,
      missing: false,
      retainedReason: 'The current OAS source differs from the recorded installed content, so the legacy file was preserved.',
    };
  }
