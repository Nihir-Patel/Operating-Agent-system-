...
const CURRENT_PLUGIN_SLUG = 'oas';
const LEGACY_PLUGIN_SLUG = 'operating-agent-systems';
...
// Artifacts that identify a COMPLETE OAS root when the caller gives no explicit
// probe. A real OAS root ships both the script tree AND OAS's skills; a partial
...
// be present with none of OAS's skills — so we probe for a sentinel skill that
// ships in every OAS root and is exactly what the failing skill commands need.
...
/**
 * Resolve the OAS source root directory.
 *
 * Tries, in order:
 ...
 *   4. Plugin cache auto-detection — scans ~/.claude/plugins/cache/{oas,operating-agent-systems}/
 ...
 *                                    OAS script tree and a sentinel OAS skill,
 *                                    so a partial install (scripts without
 *                                    skills) is rejected for skill consumers.
 * @returns {string} Resolved OAS root path
 */
function resolveEccRoot(options = {}) {
  ...
  // Decide whether a candidate directory is a usable OAS root. An explicit
  ...
  // candidate must contain both OAS's scripts and a sentinel OAS skill —
 ...
 * load the committed resolve-oas-root module, then delegates the authoritative
 ...
 */
const INLINE_RESOLVE = `(function(){var p=require('path'),f=require('fs'),o=require('os');var e=process.env.CLAUDE_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var d=p.join(o.homedir(),'.claude');function L(x){try{return require(p.join(x,'scripts','lib','resolve-oas-root')).resolveEccRoot()}catch(_){return null}}var r=L(d);if(r)return r;var s=['oas','oas@oas','marketplaces/oas','operating-agent-systems','operating-agent-systems@operating-agent-systems','marketplaces/operating-agent-systems'];for(var i=0;i<s.length;i++){r=L(p.join(d,'plugins',s[i]));if(r)return r}try{var g=['oas','operating-agent-systems'];for(var j=0;j<g.length;j++){var c=p.join(d,'plugins','cache',g[j]);var O=f.readdirSync(c);for(var k=0;k<O.length;k++){var q=p.join(c,O[k]);var V=f.readdirSync(q);for(var m=0;m<V.length;m++){r=L(p.join(q,V[m]));if(r)return r}}}}catch(_){}return d})()`;
