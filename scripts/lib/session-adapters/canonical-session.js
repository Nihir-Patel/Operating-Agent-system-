...
const SESSION_SCHEMA_VERSION = 'oas.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'oas.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'oas-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.OAS_SESSION_RECORDING_DIR === 'string' && process.env.OAS_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.OAS_SESSION_RECORDING_DIR);
  }
