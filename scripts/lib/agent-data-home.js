/**
 * Resolve OAS agent data home (memory persistence root) across harnesses.
 *
 ...
 * purpose, consistent with OAS script modules elsewhere. Automated PR reviewers
 ...
 *
 * @see https://github.com/neal0709/Operating-Agent-system-/issues/2065
 */
...
const AGENT_DATA_HOME_ENV = 'OAS_AGENT_DATA_HOME';
const DEFAULT_CLAUDE_DIR_NAME = '.claude';
const DEFAULT_CURSOR_OAS_DIR_SEGMENTS = ['.cursor', 'oas'];
const PROJECT_CONFIG_RELATIVE = path.join('.cursor', 'oas-agent-data.json');
 ...
 * dependency and risk divergent defaults for `~/.cursor/oas` vs `~/.claude`.
 *
...
/**
 * Project root for a config file under .cursor/oas-agent-data.json.
 */
...
function getDefaultCursorAgentDataHome() {
  return path.join(getHomeDirFromEnv(), ...DEFAULT_CURSOR_OAS_DIR_SEGMENTS);
}
...
function warnUnsafeProjectConfig() {
  console.error(
    '[OAS] Ignoring unsafe agent data project config: agentDataHome must stay ' +
    'within the default Cursor or Claude data directories. Use ' +
    'OAS_AGENT_DATA_HOME for an explicit trusted override.'
  );
}
...
function readProjectConfigAt(configPath) {
  ...
  try {
    ...
    const candidate = parsed.agentDataHome || parsed.OAS_AGENT_DATA_HOME;
    ...
    console.error(
      `[OAS] Failed to read or parse agent data config at ${configPath}: ${error.message}`
    );
    return null;
  }
}
...
/**
 * Set OAS_AGENT_DATA_HOME on the current process when unset (hook subprocess safety net).
...
function getCursorSessionEnvPayload(options = {}) {
  ...
  return {
    OAS_AGENT_DATA_HOME: agentDataHome,
  };
}
...
module.exports = {
  ...
  DEFAULT_CURSOR_OAS_DIR_SEGMENTS,
