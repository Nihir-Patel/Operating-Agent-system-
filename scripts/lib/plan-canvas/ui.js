 ...
 *
 * Visual language mirrors the OAS web dashboard (scripts/dashboard-web.js):
...
// air-gapped mirror) via OAS_PLAN_CANVAS_MERMAID_URL. If the fetch fails, the
...
function mermaidUrl(env = process.env) {
  const override = env.OAS_PLAN_CANVAS_MERMAID_URL;
...
// the OAS canvas. Kept import-only so a CDN failure degrades gracefully.
...
(() => {
  ...
  const QKEY = 'oas-plan-canvas:queue:' + key;
  let queue = [];
  ...
  // --- theme ---------------------------------------------------------
  const themeKey = 'oas-plan-canvas:theme';
  ...
  // above and below; match it. See neal0709/Operating-Agent-system-#2702.
  function readTheme() {
...
// OAS-styled document template for rendered markdown plan artifacts.
