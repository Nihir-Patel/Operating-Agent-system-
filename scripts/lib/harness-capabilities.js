...
function hooks(mode, oasConfigured, note) {
  return {
    mode,
    oasConfigured,
    note,
...
const HARNESS_CAPABILITIES = deepFreeze([
  {
    ...
    hooks: hooks(
      'profile-selection',
      true,
      'OAS hooks are configured through the selected off, minimal, standard, or strict profile.'
    ),
    aliases: ['claude-code'],
  },
  {
    ...
    hooks: hooks(
      'native-trust',
      true,
      'OAS hooks use Codex native plugin discovery and remain subject to Codex review and trust.'
    ),
  ...
  },
  {
    ...
    hooks: hooks(
      'not-configured',
      false,
      'OAS hooks are not configured for the Kimi managed-project install.'
    ),
    aliases: ['kimi-code'],
  },
  {
    ...
    hooks: hooks(
      'adapter-configured',
      true,
      'OAS hooks use the Cursor project adapter and Cursor event configuration.'
    ),
    aliases: [],
  },
  {
    ...
    hooks: hooks('not-configured', false, 'OAS hooks are not configured by this adapter.'),
  ...
  },
  {
    ...
    hooks: hooks(
      'adapter-opt-in',
      false,
      'OAS hook runtime support is available through the OpenCode adapter but is not installed by default.'
    ),
    aliases: ['open-code'],
  },
  {
    ...
    hooks: hooks(
      'managed-files',
      true,
      'OAS hook runtime files are installed through the CodeBuddy project adapter.'
    ),
    aliases: ['code-buddy'],
  },
  {
...
function expectedRootForAdapter(adapter) {
  const homeDir = path.resolve('/__oas_catalog_home__');
  const projectRoot = path.resolve('/__oas_catalog_project__');
