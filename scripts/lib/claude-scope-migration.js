...
function recoveryCommands(sourceScope, destinationScope) {
  ...
  commands.push(
    `oas setup --mode claude-plugin --scope ${destinationScope} --move-scope --yes`
  );
  return commands;
}
  ...
  if (installed.length === 0) {
    throw migrationError(
      ...
      `${CURRENT_PLUGIN_ID} is not installed, so there is no source scope to migrate.`,
      {
        observedScopes,
        recovery: [
          `oas setup --mode claude-plugin --scope ${destinationScope} --yes`,
        ],
      }
    );
  }
...
function migrateClaudePluginScope(options = {}, dependencies = {}) {
  ...
  const namedMarketplace = marketplaces.find(entry => entry?.name === 'oas');
