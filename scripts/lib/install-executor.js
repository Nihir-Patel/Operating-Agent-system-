...
const CLAUDE_OAS_NAMESPACE = 'oas';
...
function addCursorAgentDataScaffoldOperations(operations, options) {
  ...
  addFileCopyOperation(operations, {
    ...
    sourceRelativePath: path.join('scaffolds', 'cursor', 'oas-agent-data.json'),
    destinationPath: path.join(options.targetRoot, 'oas-agent-data.json'),
  ...
  addFileCopyOperation(operations, {
    moduleId: options.moduleId,
    sourceRoot: options.sourceRoot,
    sourceRelativePath: path.join('scaffolds', 'cursor', 'rules', 'oas-agent-data-home.mdc'),
    destinationPath: path.join(options.targetRoot, 'rules', 'oas-agent-data-home.mdc'),
    strategy: 'preserve-relative-path'
  });
...
function planClaudeStyleLegacyInstall(context, { adapterId, adapterRootInput, rulesDir: rulesDirOverride }) {
  ...
  const rulesDir = rulesDirOverride || path.join(targetRoot, 'rules', CLAUDE_OAS_NAMESPACE);
