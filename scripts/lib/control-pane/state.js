...
const SNAPSHOT_SCHEMA_VERSION = 'oas.control-pane.snapshot.v1';
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.claude', 'oas', 'state.db');
...
function defaultDbPath(env = process.env) {
  return path.join(homeDir(env), '.claude', 'oas2.db');
}
...
function defaultConfigPaths(cwd = process.cwd(), env = process.env) {
  ...
  const paths = [path.join(home, 'Library', 'Application Support', 'oas2', 'config.toml'), path.join(home, '.config', 'oas2', 'config.toml'), path.join(home, '.claude', 'oas2.toml')];

  let current = path.resolve(cwd);
  while (current && current !== path.dirname(current)) {
    paths.push(path.join(current, '.claude', 'oas2.toml'));
    paths.push(path.join(current, 'oas2.toml'));
...
function resolveControlPaneConfig(options = {}) {
  ...
  return {
    ...normalizeConfig(merged, {
      env,
      dbPath: options.dbPath || env.OAS2_DB_PATH || null,
      stateDbPath: options.stateDbPath || env.OAS_STATE_DB_PATH || null
    }),
