...
const ACTION_DEFINITIONS = new Map([
  [
    'sync-knowledge',
    {
      ...
      description: 'Import all configured OAS2 memory connectors into the context graph.',
      args: ({ limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    'recall-knowledge',
    {
      ...
      description: 'Run OAS2 context recall for the current operator query.',
      args: ({ query, limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    'graph-sync',
    {
      ...
      description: 'Backfill the OAS2 graph from sessions, decisions, file activity, and messages.',
      args: ({ limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    ...
    {
      label: 'Open TUI',
      description: 'Launch the OAS2 terminal dashboard.',
...
function buildControlPaneAction(actionId, options = {}) {
  ...
  const cwd = path.join(repoRoot, 'oas2');
