 ...
 * message through the canonical writer, the `oas-tui messages send` CLI. The CLI
 * owns the oas2 session DB (the `messages` table the control pane reads), so we
...
// Proximity trigger type → oas-tui message kind (value_enum on `--kind`).
...
/**
 * Resolve the oas-tui binary: explicit override, env var, a built target in the
 * repo, then the bare name (hope it's on PATH).
 */
function resolveEccBin(deps = {}) {
  if (deps.binPath) return deps.binPath;
  if (process.env.OAS_TUI_BIN && process.env.OAS_TUI_BIN.trim()) return process.env.OAS_TUI_BIN.trim();
  const repoRoot = deps.repoRoot || path.join(__dirname, '..', '..', '..');
  for (const rel of ['oas2/target/release/oas-tui', 'oas2/target/debug/oas-tui']) {
 ...
 * delivers via `oas-tui messages send`. Inject `runCommand(bin, args)` for tests.
 */
