...
// Host/Origin gating lives in scripts/lib/loopback-guard.js so every OAS
...
function usage() {
  return [
    'Usage:',
    '  node scripts/control-pane.js [--host 127.0.0.1] [--port 8765] [--db <oas2.db>] [--state-db <state.db>] [--config <oas2.toml>] [--query <text>]',
    '',
    'Options:',
    '  --state-db <path>  Read agent work items from an OAS state-store database',
...
function createControlPaneServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && requestUrl.pathname === '/assets/oas-icon.svg') {
        const iconPath = path.join(repoRoot, 'assets', 'oas-icon.svg');
