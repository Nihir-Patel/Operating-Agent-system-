...
<body>
  <div class="shell">
    <header>
      <div class="brand">
        <img src="/assets/oas-icon.svg" alt="">
        <h1>OAS Control Pane</h1>
      </div>
  ...
  <script>
    ...
    function renderSessions(sessions) {
      if (!sessions.length) {
        $('#sessions').innerHTML = '<div class="empty">No OAS2 sessions found.</div>';
        return;
      }
      ...
      document.querySelectorAll('#work-items [data-wi-action]').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (button.getAttribute('data-wi-action') === 'claim') {
            ...
            oasMoveItem(id, button.getAttribute('data-wi-lane'));
          }
        });
      });
    }
    ...
    window.oasClaimItem = function (id) {
    ...
    window.oasMoveItem = function (id, lane) {
      if (!state.allowActions) return;
