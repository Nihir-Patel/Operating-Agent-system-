...
function assertMergeDestination(destinationPath, existingSnapshot = null) {
  ...
  try {
    ...
    throw new Error(`Cannot merge OAS configuration into invalid JSON at ${destinationPath}: ${error.message}`);
  }
  if (!current || typeof current !== 'object' || Array.isArray(current)) {
    throw new Error(`Cannot merge OAS configuration at ${destinationPath}: expected a JSON object.`);
  }
  return current;
}
