 ...
 *
 * Controls:
 * - OAS_HOOKS_ENABLED=true|false (default: true)
 * - OAS_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - OAS_DISABLED_HOOKS=comma,separated,hook,ids
 *
 * Claude plugin options are used when their corresponding OAS variable is
 * absent. A managed install can provide oas/setup.json as the final fallback.
 */
...
function readManagedHookConfig(env = process.env) {
  const pluginRoot = String(
    env.CLAUDE_PLUGIN_ROOT || env.OAS_PLUGIN_ROOT || ''
  ).trim();
  const configPath = String(env.OAS_HOOK_CONFIG || '').trim()
    || (pluginRoot ? path.join(pluginRoot, 'oas', 'setup.json') : '');
  ...
  try {
    ...
    process.stderr.write(`${sanitizeDiagnostic(
      `Warning: unable to read managed OAS hook config at ${configPath}: ${error.message}`
    )}\n`);
    return {};
  }
}
...
function areHooksEnabled(env = process.env, managed = readManagedHookConfig(env)) {
  const raw = env.OAS_HOOKS_ENABLED !== undefined
    ? env.OAS_HOOKS_ENABLED
    : (
...
function getHookProfile(env = process.env, managed = readManagedHookConfig(env)) {
  const selected = env.OAS_HOOK_PROFILE !== undefined
    ? env.OAS_HOOK_PROFILE
    : (
...
function getDisabledHookIds(env = process.env) {
  const raw = String(env.OAS_DISABLED_HOOKS || '');
