...
function getManagedDestination(
  ...
  if (stat && stat.isSymbolicLink() && !allowFinalSymlink) {
    ...
    error.code = 'OAS_FINAL_DESTINATION_SYMLINK';
    throw error;
  }
...
function inspectManagedOperation(repoRoot, trustedRoot, operation, linkIndex = null) {
  ...
  try {
    ...
    return {
      ...
      operation,
      destinationPath,
      reason: error && error.code === 'OAS_FINAL_DESTINATION_SYMLINK'
        ? 'final-symlink'
        : 'outside-root'
    };
  }
...
function analyzeRecord(record, context) {
  ...
  if (record.legacyLayout === 'opencode') {
    issues.push(buildIssue(
      ...
      'Legacy OpenCode install-state remains under ~/.opencode. Rerun the OpenCode install or repair command to migrate unchanged OAS-managed files to ~/.config/opencode; modified files are preserved for review.'
    ));
  }
