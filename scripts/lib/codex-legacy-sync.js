...
const SCHEMA = 'oas.codex-legacy-sync.v1';
const BEGIN_MARKER = '<!-- BEGIN OAS -->';
const END_MARKER = '<!-- END OAS -->';

function getStatePath(codexHome) {
  return path.join(codexHome, 'oas', 'legacy-sync-state.json');
}
...
function removeOpenedRegularFile(filePath, opened) {
  const quarantineDir = fs.mkdtempSync(path.join(path.dirname(filePath), '.oas-remove-'));
...
function listLegacyCandidates(codexHome) {
  ...
  if (fs.existsSync(promptsDir)) {
    for (const entry of fs.readdirSync(promptsDir)) {
      if (entry.startsWith('oas-') || entry.startsWith('oas_') || entry.includes('oas-rules-pack')) {
  ...
  for (const relativePath of [
    ...
    'oas-prompts-manifest.txt',
    'oas-extension-prompts-manifest.txt',
  ]) {
  ...
  if (!state) {
    ...
    try {
      ...
      if (openedAgents) {
        ...
        if (stripped !== content) {
          plannedRemovals.push(`${agentsPath}#oas-marker-block`);
          if (!dryRun) {
