...
function resolveVaultRoots(options = {}) {
  ...
  const projectVault = env.OAS_MEMORY_PROJECT_ROOT
    ? resolveOverride(env.OAS_MEMORY_PROJECT_ROOT, cwd)
    : path.join(projectRoot, '.oas', 'memory');
  const userVault = env.OAS_MEMORY_USER_ROOT
    ? resolveOverride(env.OAS_MEMORY_USER_ROOT, cwd)
    : path.join(homeDir, '.oas', 'memory');
  ...
  Object.defineProperty(roots, VAULT_ROOT_BOUNDARIES, {
    value: Object.freeze({
      project: env.OAS_MEMORY_PROJECT_ROOT
        ? realpathNearestExisting(projectVault)
        : projectRoot,
      team: env.OAS_MEMORY_PROJECT_ROOT
        ? realpathNearestExisting(projectVault)
        : projectRoot,
      user: env.OAS_MEMORY_USER_ROOT
...
function writeCreateOnlyTextFile(filePath, content, trustedRoot) {
  ...
  const temporaryPath = path.join(
    ...
    `.oas-memory-${process.pid}-${crypto.randomUUID()}.tmp`
  );
...
function assertMemoryMatchesLocation(memory, scope, root, filePath) {
  ...
  if (memory.scope !== scope || kindDirectory !== `${memory.kind}s`) {
    ...
    error.code = 'OAS_MEMORY_LOCATION_MISMATCH';
    throw error;
  }
}

function publicMemoryFileError(error) {
  if (error?.code === 'OAS_MEMORY_SECRET') {
    return { code: 'suspected-secret', message: 'Memory document was quarantined.' };
  }
  if (error?.code === 'OAS_MEMORY_LOCATION_MISMATCH') {
    return {
...
function doctorMemoryVault(options = {}) {
  ...
  return {
    schemaVersion: 'oas.memory.doctor.v1',
    ok,
