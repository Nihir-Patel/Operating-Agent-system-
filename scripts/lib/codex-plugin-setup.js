...
function parseMarketplaceInventory(stdout, phase) {
  ...
  const oasEntries = inventory.marketplaces.filter(
    marketplace => marketplace.name === OFFICIAL_MARKETPLACE_NAME
  );
  if (oasEntries.length > 1) {
    fail(
      'INVALID_MARKETPLACE_INVENTORY',
      'Codex marketplace inventory contains duplicate `oas` entries',
      { phase }
    );
  }
...
function parsePluginInventory(stdout, phase) {
  ...
  const oasEntries = inventory.installed.filter(
...
async function runCodexCommand(args, options = {}, dependencies = {}) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      fail(
        'CODEX_NOT_FOUND',
        'Codex CLI is not installed or `codex` is not on PATH. Install Codex, then rerun OAS setup.',
        { argv, phase: options.phase }
      );
    }
...
async function resolveMarketplaceRepository(marketplace, options = {}, dependencies = {}) {
  ...
  try {
    ...
    fail(
      ...
      `Refusing the existing \`oas\` marketplace because its Git provenance could not be verified${detail ? `: ${detail}` : ''}.`,
      { phase: options.phase || 'marketplace-provenance' }
    );
  }
...
async function assertOfficialMarketplace(
  ...
  try {
    ...
    fail(
      'MARKETPLACE_COLLISION',
      `Refusing the existing \`oas\` marketplace because its provenance could not be verified${detail ? `: ${detail}` : ''}.`,
      { phase }
    );
  }
  if (repository !== NORMALIZED_OFFICIAL_MARKETPLACE_REPO) {
    fail(
      'MARKETPLACE_COLLISION',
      'Refusing the existing `oas` marketplace because it is not the official neal0709/Operating-Agent-system- source.',
      { phase }
    );
  }
}
...
function parseMarketplaceUpgradeResult(stdout, marketplace) {
  ...
  if (
    ...
    fail(
      ...
      'Codex did not confirm that the official OAS marketplace was refreshed.',
      { phase, argv }
    );
  }
  return result;
...
async function reconcileCodexPlugin(options = {}, dependencies = {}) {
  ...
  if (!findEccMarketplace(verifiedMarketplaces)) {
    fail(
      ...
      'Could not verify the OAS marketplace after reconciliation.',
