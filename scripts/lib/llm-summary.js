 ...
 * own authentication.
 *
 * Recursion guard: sets OAS_SKIP_LLM_SUMMARY=1 in subprocess env so any Stop
...
function getLLMModel() {
  return process.env.OAS_LLM_SUMMARY_MODEL || 'haiku';
}

function getContextThreshold() {
  const raw = parseInt(process.env.OAS_LLM_SUMMARY_CONTEXT_THRESHOLD || '20', 10);
...
function generateSessionSummary(transcriptPath) {
  if (process.env.OAS_SKIP_LLM_SUMMARY) return null;
