...
/**
 * Shared session bridge utilities for OAS hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `oas-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `oas-metrics-bridge` and the background
 * `oas-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/oas-metrics-bridge.js` (commit 9b1d8918) for the
...
function resolveSessionId() {
  const raw = process.env.OAS_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
