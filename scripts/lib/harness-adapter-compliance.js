...
const ADAPTER_RECORDS = Object.freeze([
  ...
  },
  {
    ...
    supported_assets: [
      'Pi package manifest',
      'canonical OAS skills (skills/)',
      'canonical OAS commands as prompt templates (commands/)',
      'canonical OAS engineering rules (rules/common/) injected into the system prompt',
      'session lifecycle hook adapter',
      '/oas-doctor diagnostics command',
    ],
    unsupported_surfaces: [
      'Subagents, chains, approval prompts, and persistent todos require companion Pi packages and are not part of this adapter',
      'Pi core has no MCP surface, though OAS MCP configs load verbatim through the community pi-mcp-adapter package, which OAS neither installs nor depends on',
    ],
    install_or_onramp: ['`pi install git:github.com/neal0709/Operating-Agent-system-`', '`pi install /path/to/OAS` from a local checkout'],
    verification_commands: [
  ...
  },
  {
    ...
    unsupported_surfaces: ['No full OAS hook parity; ecosystem ports must document drift from upstream OAS'],
  ...
  },
  {
    ...
    unsupported_surfaces: ['No OAS installer or direct adapter today'],
    ...
    risk_notes: ['Do not import product-specific assumptions; convert lessons into OAS event fields.'],
    last_verified_at: '2026-05-12',
    owner: 'OAS maintainers',
  ...
  },
  {
    ...
    verification_commands: ['`npm run observability:ready`'],
    risk_notes: ['Keep OAS portable; do not require a desktop workspace to get basic value.'],
    last_verified_at: '2026-05-12',
    owner: 'OAS maintainers',
    source_docs: ['docs/architecture/cross-harness.md'],
  },
