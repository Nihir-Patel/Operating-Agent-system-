...
function parsePluginList(stdout) {
  ...
  for (const plugin of plugins) {
    const isRelevant = plugin && (
      ...
      || String(plugin.id || '').startsWith('oas@')
      || LEGACY_PLUGIN_IDS.has(plugin.id)
      || String(plugin.id || '').startsWith('operating-agent-systems@')
    );
    ...
    if (
      ...
      fail(
        ...
        'Claude plugin inventory contains an invalid OAS plugin entry'
      );
    }
  }
  return plugins;
  ...
  for (const marketplace of marketplaces) {
    ...
    if (
      ...
      fail(
        ...
        'Claude marketplace inventory contains an invalid `oas` entry'
      );
    }
  }
  return marketplaces;
...
function assertGitAvailable(options = {}, dependencies = {}) {
  ...
  if (result.error?.code === 'ENOENT') {
    fail(
      'GIT_NOT_FOUND',
      'Git is required for Claude marketplace setup but `git` is not on PATH. Install Git, ensure `git` is on PATH, then rerun OAS setup.',
      {
        phase: 'preflight',
        recovery: [
  ...
  if (result.error || result.status !== 0) {
    ...
    fail(
      ...
      {
        phase: 'preflight',
        recovery: ['Repair Git, ensure `git --version` succeeds, then rerun OAS setup.'],
      }
    );
  }
}
...
function runClaude(args, options = {}, dependencies = {}) {
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT') {
      fail(
        'CLAUDE_NOT_FOUND',
        'Claude Code is not installed or `claude` is not on PATH. Install Claude Code, then rerun OAS setup.',
...
function readSettings(settingsPath) {
  ...
  const oasConfig = pluginConfigs?.[CURRENT_PLUGIN_ID];
  if (oasConfig !== undefined && (
    !oasConfig
    || typeof oasConfig !== 'object'
    || Array.isArray(oasConfig)
  )) {
    fail(
      'INVALID_CLAUDE_SETTINGS',
  ...
  if (oasConfig?.options !== undefined && (
    !oasConfig.options
    || typeof oasConfig.options !== 'object'
    || Array.isArray(oasConfig.options)
  )) {
    fail(
      'INVALID_CLAUDE_SETTINGS',
...
function writeClaudePluginOptions(settingsPath, hooks) {
  ...
  const pluginConfigs = settings.pluginConfigs || {};
  const oasConfig = pluginConfigs[CURRENT_PLUGIN_ID] || {};
  const options = oasConfig.options || {};
...
function assertNoConflictingEccPlugins(plugins) {
  ...
  const conflictingEcc = plugins.find(plugin => (
    typeof plugin?.id === 'string'
    && plugin.id.startsWith('oas@')
    && plugin.id !== CURRENT_PLUGIN_ID
  ));
  if (conflictingEcc) {
    fail(
      'DUPLICATE_OAS_PLUGIN',
      `${conflictingEcc.id} is already installed and would duplicate OAS surfaces. Uninstall it before setting up ${CURRENT_PLUGIN_ID}.`,
      {
  ...
  if (installed.length === 1 && installed[0].scope !== scope) {
    fail(
      ...
      `${CURRENT_PLUGIN_ID} is already installed at ${installed[0].scope} scope. Use the scope migration workflow to move it to ${scope}.`,
      {
        observedScopes,
        recovery: [
          `oas setup --mode claude-plugin --scope ${scope} --move-scope --yes`,
        ],
      }
    );
  }
...
function assertSafeLocalInventory(options) {
  ...
  if (manual) {
    fail(
      ...
      `A manual OAS plugin layout exists at ${manual.manifestPath}. Remove or migrate the manual install before setup.`
    );
  }
  let managedInstalls;
  try {
  ...
  if (overlap) {
    fail(
      ...
      `Managed OAS content at ${overlap.statePath} overlaps the Claude plugin. Remove that managed overlap before setup.`
    );
  }
...
function ensureOfficialMarketplace(options) {
  ...
  if (existing && !isOfficialMarketplace(existing)) {
    fail(
      ...
      'Refusing the `oas` marketplace collision because it is not the official neal0709/Operating-Agent-system- source.'
    );
  }
  ...
  if (!verified || !isOfficialMarketplace(verified)) {
    fail(
      ...
      'Could not verify the official OAS marketplace after the marketplace change.',
