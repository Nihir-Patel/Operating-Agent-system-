...
Usage:
  oas setup
  oas setup --mode claude-plugin --scope user|project|local [options]
  oas setup --mode claude-plugin --scope project --move-scope [options]

Install scopes:
  user      Global for this user; OAS is available in every project.
  project   Shared project configuration; the repository can enable OAS for collaborators.
  local     Private project configuration; OAS is enabled here without committing the choice.
...
Re-running setup updates an existing oas@oas installation at its detected scope.
...
async function collectInteractiveOptions(options, defaults = {}, providedTerminal) {
  ...
  try {
    ...
    const scope = options.scope || await askChoice(
      terminal,
      'Where should Claude enable oas@oas?',
      scopeChoices,
      scopeDefaultIndex
    );
    const hookChoices = [
...
function printResult(result, json) {
  ...
  process.stdout.write(`\nOAS ${result.action} ${result.pluginId} at ${result.scope} scope.\n`);
...
function applyClaudePlugin(options, interactive) {
  const spinner = interactive && !options.dryRun && !options.json
    ? startTerminalSpinner('Applying OAS setup...')
    : undefined;
  try {
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (isInteractiveCancellation(error)) {
      process.stdout.write('\nOAS setup cancelled. No changes were made.\n');
      return;
    }
