...
# Sync Operating Agent Systems (OAS) assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Merges OAS AGENTS.md into existing AGENTS.md (marker-based, preserves user content)
...
# - Merges OAS MCP servers into config.toml (add-only via Node TOML parser)
...
BACKUP_DIR="$CODEX_HOME/backups/oas-$STAMP"

log() { printf '[oas-sync] %s\n' "$*"; }
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# OAS Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the OAS `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "OAS AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "OAS Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "OAS Codex agent roles"
require_path "$CODEX_NAV_GUIDE_SRC" "OAS Codex navigation guide"
require_path "$CODEX_COMMAND_AGENT_MAP_SRC" "OAS command-agent map"
require_path "$CODEX_COMMANDS_QUICK_REF_SRC" "OAS commands quick reference"
require_path "$CODEX_CONTRIBUTING_SRC" "OAS contributing guide"
require_path "$CODEX_PR_TEMPLATE_SRC" "OAS PR template"
require_path "$PROMPTS_SRC" "OAS commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "OAS Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "OAS global git hooks installer"
require_path "$SANITY_CHECKER" "OAS global sanity checker"
require_path "$LEGACY_STATE_HELPER" "OAS legacy sync state helper"
require_path "$CURSOR_RULES_DIR" "OAS Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "OAS MCP merge script"
...
if [[ "$MODE" == "apply" ]]; then
  ...
  LEGACY_STATE_PATH="$(
    node "$LEGACY_STATE_HELPER" begin \
      ...
      --installed-hooks-path "${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
  )"
...
OAS_BEGIN_MARKER="<!-- BEGIN OAS -->"
OAS_END_MARKER="<!-- END OAS -->"

compose_oas_block() {
  printf '%s\n' "$OAS_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From OAS .codex/AGENTS.md)\n\n'
  cat "$AGENTS_CODEX_SUPP_SRC"
  printf '\n%s\n' "$OAS_END_MARKER"
}

log "Merging OAS AGENTS into $AGENTS_FILE (preserving user content)"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] merge OAS block into %s from %s + %s\n' "$AGENTS_FILE" "$AGENTS_ROOT_SRC" "$AGENTS_CODEX_SUPP_SRC"
else
  replace_oas_section() {
    # Replace the OAS block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local oas_tmp
    oas_tmp="$(mktemp)"
    compose_oas_block > "$oas_tmp"
    awk -v begin="$OAS_BEGIN_MARKER" -v end="$OAS_END_MARKER" -v oas="$oas_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < oas) > 0) print line; close(oas); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
  ...
  if [[ ! -f "$AGENTS_FILE" ]]; then
    ...
    compose_oas_block > "$AGENTS_FILE"
  elif awk -v b="$OAS_BEGIN_MARKER" -v e="$OAS_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++; if (!fb) fb = NR }
        $0 == e { ec++; if (!fe) fe = NR }
        END { exit !(bc == 1 && ec == 1 && fb < fe) }
      ' "$AGENTS_FILE"; then
    # Exactly one BEGIN/END pair in correct order — replace only the OAS section
    replace_oas_section
  elif awk -v b="$OAS_BEGIN_MARKER" -v e="$OAS_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++ } $0 == e { ec++ }
    ...
    log "WARNING: OAS markers found but not a clean pair — stripping markers and re-appending"
    _fix_tmp="$(mktemp)"
    awk -v b="$OAS_BEGIN_MARKER" -v e="$OAS_END_MARKER" '
      { gsub(/\r$/, "") }
      $0 == b { skip = 1; next }
      $0 == e { skip = 0; next }
      !skip   { print }
    ...
    # Existing file without markers — append OAS block, preserving existing content.
    # Legacy OAS-only files will have duplicate content after this first run, but
    ...
    log "No OAS markers found — appending managed block (backup saved)"
    {
      printf '\n\n'
      compose_oas_block
    } >> "$AGENTS_FILE"
...
log "Merging OAS Codex baseline into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
# ~/.agents/skills/ (installed by OAS installer / npx skills).
# Copying into ~/.codex/skills/ was unnecessary.

log "Generating prompt files from OAS commands"
run_or_echo mkdir -p "$PROMPTS_DEST"
manifest="$PROMPTS_DEST/oas-prompts-manifest.txt"
...
extension_manifest="$PROMPTS_DEST/oas-extension-prompts-manifest.txt"
...
write_extension_prompt "oas-tool-run-tests.md" <<EOF
# OAS Tool Prompt: run-tests
...
write_extension_prompt "oas-tool-check-coverage.md" <<EOF
# OAS Tool Prompt: check-coverage
...
write_extension_prompt "oas-tool-security-audit.md" <<EOF
# OAS Tool Prompt: security-audit
...
write_extension_prompt "oas-rules-pack-common.md" <<EOF
# OAS Rule Pack: common (optional)

Apply OAS common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "oas-rules-pack-typescript.md" <<EOF
# OAS Rule Pack: typescript (optional)

Apply OAS common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/oas-rules-pack-common.md\`.
...
write_extension_prompt "oas-rules-pack-python.md" <<EOF
# OAS Rule Pack: python (optional)

Apply OAS common rules plus Python-specific rules for this session.
...
write_extension_prompt "oas-rules-pack-golang.md" <<EOF
# OAS Rule Pack: golang (optional)

Apply OAS common rules plus Go-specific rules for this session.
...
write_extension_prompt "oas-rules-pack-swift.md" <<EOF
# OAS Rule Pack: swift (optional)

Apply OAS common rules plus Swift-specific rules for this session.
...
log "Merging OAS MCP servers into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  OAS_GLOBAL_HOOKS_DIR="${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
    "$HOOKS_INSTALLER" --dry-run
else
  record_managed_path "${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-commit"
  record_managed_path "${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-push"
  HOME="$HOME" \
  CODEX_HOME="$CODEX_HOME" \
  AGENTS_HOME="${AGENTS_HOME:-$HOME/.agents}" \
  OAS_GLOBAL_HOOKS_DIR="${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
