...
const MAX_LIMIT = 20;
const SCHEMA_VERSION = 'oas.consult.v1';
...
function showHelp(exitCode = 0) {
  console.log(`
Consult OAS install components and profiles from any project
...
function commandFor(kind, id, target) {
  if (kind === 'profile') {
    return `npx oas-universal install --profile ${id} --target ${target}`;
  }

  return `npx oas-universal install --profile minimal --target ${target} --with ${id}`;
}

function planCommandFor(componentId, target) {
  return `npx oas-universal plan --profile minimal --target ${target} --with ${componentId}`;
}
...
function buildConsultation(options) {
  ...
  return {
    ...
    nextSteps: matches.length > 0
      ...
      : [
        'Run `npx oas-universal catalog components` to browse all components.',
...
function formatText(payload) {
  const lines = [
    `OAS consult (${payload.generatedAt})`,
    `Query: ${payload.query}`,
  ...
  if (payload.matches.length === 0) {
    ...
    lines.push('Try: npx oas-universal catalog components');
  } else {
