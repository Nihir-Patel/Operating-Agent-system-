...
function showHelp(output = process.stdout) {
  output.write(`
OAS guided multi-harness install
...
Advanced managed adapters remain available through explicit oas install --target commands:
  ${ADVANCED_HARNESSES}

This command configures OAS. It does not install or authenticate provider CLIs.
`);
}
...
async function askHarnesses(terminal, output) {
  ...
  output.write('\nWhich coding agents should OAS configure?\n');
...
async function confirmPlan(terminal, output) {
  output.write('\n');
  const answer = await terminal.question('Apply OAS to these harnesses? [y/N]: ');
  ...
  try {
    ...
    if (!options.yes) {
      ...
      if (!await confirmPlan(terminal, output)) {
        output.write('\nOAS install cancelled. No changes were made.\n');
        return 0;
      }
    }

    const spinner = interactive && !options.json
      ? makeSpinner('Applying OAS to selected harnesses...')
      : undefined;
    let result;
    try {
    ...
    if (options.json) {
      ...
      output.write(`\nOAS configured for ${result.completed.map(item => getHarnessCapability(item.id).label).join(', ')}.\n`);
      renderWelcome({ action: 'installed', interactive, json: false, output });
    } else {
      const retry = buildRetryArguments(plan, result.retryHarnesses);
      errorOutput.write(
        `OAS stopped at ${sanitizeTerminalText(result.failure.id)}: `
        + `${sanitizeTerminalText(result.failure.message)}\n`
        + `Retry with: oas-universal install --guided ${retry}\n`
      );
    }
