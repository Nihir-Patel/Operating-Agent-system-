...
const OAS_DISABLE_VALUES = new Set(['0', 'false', 'off', 'disabled', 'disable']);
const OAS_ENABLE_VALUES = new Set(['1', 'true', 'on', 'enabled', 'enable', 'yes']);
...
function isRoutineBashGateDisabled() {
  return OAS_ENABLE_VALUES.has(normalizeEnvValue(process.env.GATEGUARD_BASH_ROUTINE_DISABLED));
}
...
function isGateGuardDisabled() {
  ...
  return OAS_DISABLE_VALUES.has(normalizeEnvValue(process.env.OAS_GATEGUARD));
}
...
function resolveSessionKey(data) {
  const directCandidates = [data && data.session_id, data && data.sessionId, data && data.session && data.session.id, process.env.CLAUDE_SESSION_ID, process.env.OAS_SESSION_ID];
...
function condensedGateMsg(action, filePath, ordinal) {
  ...
  return (
    ...
    '(Use GATEGUARD_EXEMPT_GLOBS for path-scoped exemptions; OAS_GATEGUARD=off disables this gate.)'
  );
}
...
function withRecoveryHint(message, hookIds = [EDIT_WRITE_HOOK_ID], narrowRecoveryHint = '') {
  ...
  return [
    ...
    '',
    ...recoveryLines,
    `Recovery: if GateGuard is blocking setup or repair work, run this session with \`OAS_GATEGUARD=off\` or add ${disableTargets} to \`OAS_DISABLED_HOOKS\`.`
  ].join('\n');
}
