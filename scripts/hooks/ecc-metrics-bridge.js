#!/usr/bin/env node
/**
 * OAS Metrics Bridge — PostToolUse hook
 *
 * Maintains a running session aggregate in /tmp/oas-metrics-{session}.json.
 * This bridge file is read by oas-statusline.js and oas-context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'oas-metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[oas-metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[oas-metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.OAS_SESSION_ID) || sanitizeSessionId(process.env.CLAUDE_SESSION_ID);
