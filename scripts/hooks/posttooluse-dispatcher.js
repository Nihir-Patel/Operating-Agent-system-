...
const { run: runMetricsBridge } = require('./oas-metrics-bridge');
const { run: runContextMonitor } = require('./oas-context-monitor');
...
const SYNC_HOOKS = [
  ...
  { id: 'post:oas-metrics-bridge', matcher: '*', profiles: 'minimal,standard,strict', script: 'scripts/hooks/oas-metrics-bridge.js', run: runMetricsBridge },
  { id: 'post:oas-context-monitor', matcher: '*', profiles: 'standard,strict', script: 'scripts/hooks/oas-context-monitor.js', run: runContextMonitor }
];
...
function getPluginRoot(env = process.env) {
  return env.CLAUDE_PLUGIN_ROOT || env.OAS_PLUGIN_ROOT || path.resolve(__dirname, '..', '..');
}
...
async function main() {
  ...
  const stdout = resolveMainStdout(raw, result, {
    passthrough: process.env.OAS_POSTTOOLUSE_PASSTHROUGH === '1',
    truncated
  });
