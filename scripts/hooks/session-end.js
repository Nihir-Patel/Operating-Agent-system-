...
const SUMMARY_START_MARKER = '<!-- OAS:SUMMARY:START -->';
const SUMMARY_END_MARKER = '<!-- OAS:SUMMARY:END -->';
...
async function main() {
  ...
  // OAS's LLM summary helper launches a one-shot Claude subprocess whose Stop
  ...
  if (process.env.OAS_LLM_SUMMARY_SUBPROCESS === '1') {
    log('[SessionEnd] Skipped OAS LLM summary subprocess');
    return;
  }
  ...
  if (transcriptPath && summary && transcriptExists) {
    ...
    const interval = parseInt(process.env.OAS_LLM_SUMMARY_INTERVAL || '50', 10);
