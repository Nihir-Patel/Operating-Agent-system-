...
/**
 * Resolve session retention days from the OAS_SESSION_RETENTION_DAYS env var.
 *
...
function getSessionRetentionDays() {
  const raw = process.env.OAS_SESSION_RETENTION_DAYS;
...
function isSessionStartContextDisabled() {
  const raw = String(process.env.OAS_SESSION_START_CONTEXT || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'none', 'disabled'].includes(raw);
}

function getSessionStartMaxContextChars() {
  const raw = process.env.OAS_SESSION_START_MAX_CHARS;
 ...
 * SessionStart. Overridable via `OAS_INSTINCT_CONFIDENCE_THRESHOLD`
...
function getInstinctConfidenceThreshold() {
  const raw = process.env.OAS_INSTINCT_CONFIDENCE_THRESHOLD;
 ...
 * Overridable via `OAS_MAX_INJECTED_INSTINCTS` (a positive integer);
...
function getMaxInjectedInstincts() {
  const raw = process.env.OAS_MAX_INJECTED_INSTINCTS;
...
function limitSessionStartContext(additionalContext, maxChars = getSessionStartMaxContextChars()) {
  ...
  const marker = '\n\n[SessionStart truncated context. Set OAS_SESSION_START_MAX_CHARS to raise the cap or OAS_SESSION_START_CONTEXT=off to disable injected context.]';
...
function summarizeActiveInstincts(observerContext) {
  ...
  // Gated by OAS_INSTINCT_RELEVANCE_RANKING (default on); when off, or when no
...
async function main() {
  ...
  if (retentionDays === null) {
    log('[SessionStart] Pruning disabled via OAS_SESSION_RETENTION_DAYS');
  } else {
  ...
  if (explicitContextDisabled) {
    log('[SessionStart] Additional context injection disabled by OAS_SESSION_START_CONTEXT');
  } else if (maxContextChars === 0) {
    log('[SessionStart] Additional context injection disabled by OAS_SESSION_START_MAX_CHARS=0');
  }
      ...
      if (recentSessions.length > 0) {
        ...
        if (result) {
          ...
          if (content && !content.includes('[Session context goes here]')) {
            ...
            // https://github.com/neal0709/operating-agent-systems/issues/1534
            const guarded = [
