 ...
 * `oas-plan-canvas await`. The moment a turn ends, nothing is listening, so
 ...
 * OAS_PLAN_CANVAS_STOP_SCOPE=all to consider every open session.
 *
...
function stateDir() {
  const override = process.env.OAS_PLAN_CANVAS_STATE_DIR;
...
function pendingSessions(state, cwd, env = process.env) {
  const scopeAll = String(env.OAS_PLAN_CANVAS_STOP_SCOPE || '').trim().toLowerCase() === 'all';
...
function buildReason(delivered) {
  ...
  for (const entry of delivered) {
    ...
    if (entry.sessionEnded) {
      ...
      lines.push(`    oas-plan-canvas await ${JSON.stringify(entry.file)} --reply "<what you did>"`);
    }
    lines.push('');
  }
