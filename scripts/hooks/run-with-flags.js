#!/usr/bin/env node
/**
 * Executes a hook script only when enabled by OAS hook profile flags.
 *
 * Usage:
...
async function main() {
  ...
  // the truncated flag (run() context / OAS_HOOK_INPUT_TRUNCATED), so
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...
      OAS_HOOK_ID: hookId,
      OAS_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      OAS_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
