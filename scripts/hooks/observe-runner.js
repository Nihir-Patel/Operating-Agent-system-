...
function getPluginRoot(options = {}) {
  ...
  if (process.env.OAS_PLUGIN_ROOT && process.env.OAS_PLUGIN_ROOT.trim()) {
    return process.env.OAS_PLUGIN_ROOT.trim();
  }
...
function getPhaseFromHookId(hookId) {
  const prefix = String(hookId || process.env.OAS_HOOK_ID || '').split(':')[0];
  return prefix === 'pre' || prefix === 'post' ? prefix : null;
}

function getTimeoutMs() {
  const parsed = Number.parseInt(process.env.OAS_OBSERVE_RUNNER_TIMEOUT_MS || '', 10);
...
if (require.main === module) {
  ...
  const output = run(raw, { hookId: process.argv[2] || process.env.OAS_HOOK_ID });
