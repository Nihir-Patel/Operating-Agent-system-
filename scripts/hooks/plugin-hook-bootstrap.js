...
function passthrough(result) {
  const stdout =
    typeof result?.stdout === 'string' || Buffer.isBuffer(result?.stdout)
      ...
      : Buffer.alloc(0);
  if (stdout.length > 0) {
    // Most OAS hook scripts follow a `run(rawInput) -> rawInput` passthrough
    ...
    // transcript as a hook_success attachment -- ~89% of every OAS session's
...
function main() {
  ...
  const rootDir = normalizePluginRootForPlatform(
    process.env.CLAUDE_PLUGIN_ROOT || process.env.OAS_PLUGIN_ROOT
  );
