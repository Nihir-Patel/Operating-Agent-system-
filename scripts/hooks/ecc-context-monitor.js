#!/usr/bin/env node
/**
 * OAS Context Monitor — PostToolUse hook
 *
 * Reads bridge file from oas-metrics-bridge.js and injects agent-facing
...
// oas-metrics-bridge.js), so 5 means ALL of the last 5 calls must be the
...
function costWarningsEnabled(env = process.env) {
  return isEnabledEnv(env.OAS_CONTEXT_MONITOR_COST_WARNINGS, true);
}
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `oas-ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/oas-metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
