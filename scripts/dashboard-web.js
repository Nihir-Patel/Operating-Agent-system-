#!/usr/bin/env node
/**
 * OAS Capabilities Dashboard — agents, skills, commands, MCPs, rules & hooks
 * With multi-language, routing, search suggestions, recently viewed, fine UI
 *
 * Usage: node scripts/dashboard-web.js [port]
 * Open http://localhost:3456
 *
 * Contribution: https://github.com/neal0709/Operating-Agent-system-
 */
...
function resolveDashboardHost(env = process.env) {
  const configured = String(env.OAS_DASHBOARD_HOST || '').trim().toLowerCase();
  if (!configured) return DEFAULT_HOST;
  if (!LOOPBACK_HOSTNAMES.has(configured)) {
    throw new Error(
      '[OAS] OAS_DASHBOARD_HOST must be loopback-only ' +
...
function parsePort(v) {
  ...
  if (isNaN(n) || n < 1 || n > 65535) { console.error('[OAS] Invalid port: ' + v + ' — using 3456'); return 3456; }
  return n;
}
const PORT = parsePort(process.argv[2] || process.env.OAS_DASHBOARD_PORT || '3456');
...
function loadPostToolUseChildren(root) {
  ...
  try {
    ...
    console.error('[OAS] Failed to load PostToolUse dispatcher registry:', error.message);
    return [];
  }
}
...
<div class="header">
  <div class="brand" id="brand-link">
    ...
    <h1><span id="t-title">OAS Capabilities</span> <span class="ver">v2.0.0-rc.1</span></h1>
  </div>
...
<div class="footer">
  <a href="https://github.com/neal0709/Operating-Agent-system-" target="_blank">github.com/neal0709/Operating-Agent-system-</a>
  <span class="dt"></span>
  <span>OAS v2.0.0-rc.1</span>
  <span class="dt"></span>
  <span id="t-contribution">Contribution to OAS</span>
  <span class="dt"></span>
...
let lang = localStorage.getItem('oas-lang') || 'en';
let suggIdx = -1;
...
// Recently viewed
function recents() { try { return JSON.parse(localStorage.getItem('oas-recent') || '[]'); } catch { return []; } }
function addRecent(type, name) {
  ...
  localStorage.setItem('oas-recent', JSON.stringify(r));
}
function clearRecents() { localStorage.removeItem('oas-recent'); location.hash=''; location.reload(); }
...
// Language
function setLang(l) {
  lang = l; localStorage.setItem('oas-lang', l);
...
function toggleTheme() {
  ...
  localStorage.setItem('oas-theme', h.dataset.theme);
}
if (localStorage.getItem('oas-theme')) document.documentElement.dataset.theme = localStorage.getItem('oas-theme');
...
function createDashboardServer({
  ...
  const resolvedHost = resolveDashboardHost({ OAS_DASHBOARD_HOST: host });
  ...
  return http.createServer((req, res) => {
    ...
    if (url.pathname === '/api/data') {
      ...
      try {
        ...
        reportDashboardFailure(
          reportError,
          '[OAS] Failed to load dashboard data:',
          error
        );
    ...
    try {
      ...
      reportDashboardFailure(
        reportError,
        '[OAS] Failed to render dashboard:',
        error
      );
      return sendHtml(
        res,
