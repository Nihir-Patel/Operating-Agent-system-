#!/usr/bin/env node
/**
 * NanoClaw v2 — Barebones Agent REPL for Operating Agent Systems
 *
...
function loadOASContext(skillList) {
...
function main() {
  ...
  let oasContext = loadOASContext(state.skills);
  ...
  const prompt = () => {
    rl.question('claw> ', input => {
      ...
      if (line.startsWith('/load ')) {
        ...
        oasContext = loadOASContext(state.skills);
      ...
      const response = askClaude(oasContext, history, line, state.model);
      console.log(`\n${response}\n`);
