<p align="center">
  <img src="assets/hero.png" alt="OAS - the agent harness operating system" width="100%" />
</p>

<p align="center">
  <a href="https://www.star-history.com/neal0709/oas">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/badge?repo=neal0709/Operating-Agent-system-&type=trending&theme=dark" />
      <img src="https://api.star-history.com/badge?repo=neal0709/Operating-Agent-system-&type=trending" alt="GitHub Trending Repository of the Day" height="46" />
    </picture>
  </a>
  <a href="https://www.star-history.com/neal0709/oas">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/badge?repo=neal0709/Operating-Agent-system-&type=rank&theme=dark" />
      <img src="https://api.star-history.com/badge?repo=neal0709/Operating-Agent-system-&type=rank" alt="Star History Global Rank" height="46" />
    </picture>
  </a>
</p>
...
> [!WARNING]
> **Official sources only.** Install OAS only from verified channels: the GitHub repository [github.com/neal0709/Operating-Agent-system-](https://github.com/neal0709/Operating-Agent-system-), the npm packages [`oas-universal`](https://www.npmjs.com/package/oas-universal) and [`oas-agentshield`](https://www.npmjs.com/package/oas-agentshield), the [GitHub App](https://github.com/apps/oas-tools), the plugin slug `oas@oas`, and the project website [oas.tools](https://oas.tools). Third-party re-uploads and unofficial mirrors are not maintained or reviewed by the project and may contain malware.
...
```bash
npm view oas-universal version
```

This path requires Node.js 18 or newer, Git, and Claude Code 2.1 or newer on
`PATH`. It safely installs, updates, or moves one `oas@oas` plugin scope and
...
The native path installs OAS's skills, agents, commands, and plugin-managed hooks. If you choose it, stop there. Do not also run a full manual install into Claude Code.

> Both paths install the same `oas@oas` plugin. Choose one and do not stack
> another manual Claude install on top.

<div align="center">

<table aria-label="OAS primary links">
<tr>
<td width="33%" align="center">
  ...
  <sub><a href="https://github.com/apps/oas-tools">Install free</a> · <a href="https://oas.tools/pricing">Private repos from $19/seat/mo</a></sub>
</td>
<td width="33%" align="center">
  <a href="https://github.com/sponsors/neal0709">
    <img src="assets/images/community/heart.svg" height="42" alt="" /><br />
    <strong>Sponsor OAS</strong>
  </a><br />
...
<sub>**OSS stays free.** This repo is MIT-licensed forever. OAS Pro is the hosted GitHub App for private repos. <a href="https://github.com/sponsors/neal0709">Sponsors</a> and <a href="https://oas.tools/pricing">Pro subscribers</a> fund the work. That's why a single maintainer ships weekly across 7 harnesses.</sub>
...
<sub><a href="https://github.com/sponsors/neal0709"><strong>Become a Sponsor</strong></a> · <a href="SPONSORS.md">Sponsor Tiers</a> · <a href="SPONSORING.md">Sponsorship Program</a></sub>

</div>

<p align="center"><a href="#install-oas">Jump to install ↓</a></p>

# OAS

Your agent can write code, but OAS gives it a coordinated engineering system and toolbox: it plans before it builds, verifies changes with tests, reviews its own work from a fresh context, remembers what matters, and turns repeated wins into reusable skills and workflows.
...
| Commands         | 94 commands | Convenient entry points while OAS moves to a skills-first surface                    |
...
<p align="center">
  <a href="https://www.star-history.com/neal0709/oas">
    <picture>
      ...
      <img src="assets/star-history-light.svg" alt="OAS star history: first 40,000 stars, January 18 to February 7, 2026" width="100%" />
    </picture>
  </a>
</p>
...
> [!IMPORTANT]
> OAS 2.2 includes guided package setup for Claude Code, Codex, and Kimi Code.
...
You can use OAS with Claude Code, Codex, and other harnesses at the same time. Choose one install method for each harness:
...
**Do not stack install methods.** Installing OAS twice into the same harness can duplicate skills, commands, hooks, or configuration; installing it once into multiple harnesses does not.

If you already layered multiple installs and things look duplicated, skip straight to [Reset / Uninstall OAS](#reset--uninstall-oas).

**Install trouble?** Open the short [install or runtime problem form](https://github.com/neal0709/Operating-Agent-system-/issues/new?template=install-problem.yml), or run `oas feedback`. OAS never uploads diagnostics automatically.

### Claude Code details

Claude Code owns these built-in commands, including their errors when a marketplace, plugin, or conflicting scope already exists. OAS cannot intercept that parser. If either native command reports an existing install or scope conflict, use the 2.2 guided setup or resolve the conflicting Claude plugin scope before retrying; do not layer a manual install on top.

After OAS is installed, `/oas:configure-oas` is the namespaced in-Claude reconfiguration skill. It delegates to the same safe setup flow, but it is available only after the plugin is installed and cannot replace Claude Code's built-in `/plugin` command during a first install.
...
cp -R rules/typescript ~/.claude/rules/oas/  # replace with your stack
```
...
<details>
<summary><strong>Naming + migration note (oas@oas, neal0709/Operating-Agent-system-, oas-universal)</strong></summary>

OAS has three public identifiers, and they are not interchangeable:

- GitHub source repo: `neal0709/Operating-Agent-system-`
- Claude marketplace/plugin identifier: `oas@oas`
- npm package: `oas-universal`

This is intentional. Anthropic marketplace/plugin installs are keyed by a canonical plugin identifier, so OAS uses `oas@oas` to keep tool names and slash-command namespaces short enough for strict Desktop/API validators. Older posts may still show the former long marketplace identifier; treat that as a legacy alias only. Separately, the npm package stayed on `oas-universal`, so npm installs and marketplace installs intentionally use different names.

npm releases are cut per version tag, not per commit, so `oas-universal` tracks releases (2.1, 2.2, ...) rather than every push to `main`. Install from git if you want the bleeding edge.

If your local Claude setup was wiped or reset, that does not mean you need to repurchase anything. Start with `node scripts/oas.js list-installed`, then run `node scripts/oas.js doctor` and `node scripts/oas.js repair` before reinstalling. That usually restores OAS-managed files without rebuilding your setup.
</details>

### Codex App and CLI

Current Codex releases can install OAS as a native repo-marketplace plugin. The marketplace entry uses the repository root so Codex's cache receives the manifest together with all referenced skills, MCP configuration, hook runtime, scripts, and assets:
...
Both add commands are idempotent. To refresh later, run `codex plugin marketplace upgrade oas` followed by `codex plugin add oas@oas`. Codex stores one enabled plugin state in the active `CODEX_HOME`; it does not offer Claude's `user`, `project`, and `local` scopes. Its native hooks require an explicit trust decision and do not use Claude's four OAS hook profiles. Inside Codex, invoke `$configure-oas` for the guided provider-aware flow.

The older `scripts/sync-oas-to-codex.sh` path is a deprecated compatibility option for users who intentionally need copied and merged configuration in `~/.codex`; it is not required for the native plugin. New sync runs write an ownership manifest so cleanup can preserve modified user files. Run Codex once first so `~/.codex/config.toml` exists, then:
...
```bash
node scripts/oas.js uninstall --legacy-codex-sync --dry-run
node scripts/oas.js uninstall --legacy-codex-sync
```

Pre-manifest installations are handled conservatively: OAS removes its marked `AGENTS.md` block but preserves copied files it cannot prove it owns and reports them for review.

You can also open the OAS repository directly in Codex for a project-local setup. Codex reads the root `AGENTS.md` and the trusted project configuration in `.codex/` without a global sync. Do not add the native marketplace plugin on top of the sync flow.

For repo navigation, surface ownership, and PR diff packet guidance, read the [Codex OAS Navigation Map](docs/CODEX-NAVIGATION-GUIDE.md). See the [.codex plugin notes](.codex-plugin/README.md) for native lifecycle details.
...
Clone OAS once, then choose the target that matches your harness:
...
For a harness without a native OAS target, use the [manual adaptation guide](docs/MANUAL-ADAPTATION-GUIDE.md). It explains how to carry a small set of OAS skills and workflow instructions into chat-style tools without pretending hooks or native skill discovery are available.

Cursor installs agent definitions under `.cursor/agents/oas-*.md`. Cursor-native loading behavior can vary by Cursor build. OAS does not install root `AGENTS.md` into `.cursor/`. The adapter keeps Cursor's context scoped to its native rules and agent surfaces.
...
OAS works through each harness's normal configuration, so you can use an official provider, a compatible custom API endpoint or model gateway, or a self-hosted model without changing OAS's workflows.

For Claude Code, OAS does not hardcode Anthropic-hosted transport settings. Minimal gateway example:
...
If your gateway remaps model names, configure that in Claude Code rather than in OAS. OAS's hooks, skills, commands, and rules are model-provider agnostic once the `claude` CLI is already working. See Anthropic's [LLM gateway documentation](https://docs.anthropic.com/en/docs/claude-code/llm-gateway) and [model configuration documentation](https://docs.anthropic.com/en/docs/claude-code/model-config).

Run or self-host any open-source model behind that gateway using separate compute and serving setup. If you need GPU capacity, [Itô](https://compute.itomarkets.com) is OAS's preferred compute sponsor; any GPU provider works. The sponsorship link is passive: it does not invoke an RFQ, reserve capacity, provision compute, or configure serving. Separately, `oas ito find` invokes the explicitly configured canonical Itô CLI and submits a live authenticated RFQ; it does not reserve capacity. Managed inference through Itô is not live yet.

### Self-host Kimi with OAS + Itô compute

The Kimi Code harness and the model-serving layer are separate. OAS configures the agent harness; you bring an API endpoint or self-host an open-weight Kimi model on your own GPU capacity. This adapter is verified against Kimi Code 0.31.x (`@moonshot-ai/kimi-code`):
...
<td width="33%" align="center">
  <a href=".kimi/README.md">
    ...
    <strong>3. Run Kimi Code with OAS</strong>
  </a><br />
...
Configure the endpoint with Kimi Code's <a href="https://moonshotai.github.io/kimi-cli/en/configuration/providers.html">official provider guide</a>, then install OAS:
...
Kimi Code discovers the installed `.kimi-code/AGENTS.md` instructions and `.kimi-code/skills/` workflows natively; project-level `.agents/skills/` is also an official discovery location. OAS safely merges project MCP entries into `.kimi-code/mcp.json` and does not change the user-level `~/.kimi-code/config.toml`. Kimi Code supports native hooks, but OAS's current managed-project adapter does not configure them, so this installer does not offer Kimi hook profiles. The installer dry-run and regression suite verify that every managed Kimi write stays inside the project-local `.kimi-code/` root.

### Itô compute CLI bridge

`oas ito` delegates to the separately installed canonical Itô client; OAS does not maintain a second API client. `oas ito login [--no-browser]` performs device authorization, opens the Itô verification page by default, and persists a device token in macOS Keychain; `--no-browser` suppresses the page handoff. OAS itself does no browser automation. `oas ito auth` is validation-only and rejects `--no-browser`. The available operations are `oas ito login`, `oas ito auth`, `oas ito find`, `oas ito status`, and the separately gated `oas ito evals`. The matching MCP tools remain `ito_auth`, `ito_find`, and `ito_status`; `ito_auth` validates existing credentials and node qualification is CLI-only.

The `ito-compute-cli` package is currently unpublished. Build it locally from the Itô runtime repo (private while the desk hardens; design partners get access) under `cli/ito-compute-cli`, run `npm ci` and `npm run check`, then set `OAS_ITO_CLI_EXECUTABLE` to that build's absolute `dist/bin/ito.js` path. Login never inherits `ITO_API_KEY`; auth, find, and status forward `ITO_API_KEY` directly when configured, and `ITO_AUTH_MODE=legacy` is not required. `oas ito logout` revokes the current device credential and retains its local copy if remote revocation cannot be confirmed. Device tokens use macOS Keychain by default; explicit file fallback must retain owner-only directory/file permissions. OAS does not discover this credential-bearing client through `PATH`. See the [`ito-compute` skill](skills/ito-compute/SKILL.md) for the full RFQ authority and MCP setup contract.

`find` submits a live authenticated RFQ. It does not reserve capacity. `evals` requires both `ITO_ENABLE_SIXTYTWO_LIVE=1` and `--live-sixtytwo`, a separately installed `sixtytwo-cli==0.3.33`, an explicit node list, and an existing absolute configuration directory. It cannot rent, launch, recover, repair, or purchase. OAS exposes no quote lock, purchase, workload, or inference path, and it never replaces a missing client or failed live call with a local result.
...
Use this when you want OAS's rules, agents, commands, platform config, and core workflows without runtime hooks:

```bash
npx oas-universal install --profile minimal --target claude
```
...
Claude manual installs place each skill directly under `~/.claude/skills/<skill-name>/` (or `.claude/skills/<skill-name>/` for `claude-project`) so Claude Code can discover it. When upgrading an older OAS manual install, the installer migrates only nested `skills/oas/` files recorded in OAS install-state. If a flat skill directory is user-owned, OAS preserves it, prints a conflict warning, and keeps any older managed copy tracked for a safe uninstall instead of overwriting user files.
...
installer (`oas install --guided`) asks for this choice interactively.
</details>
...
mkdir -p ~/.claude/rules/oas
cp -r rules/common ~/.claude/rules/oas/
cp -r rules/typescript ~/.claude/rules/oas/   # pick your stack

# Core/general skills only (Claude Code loads skills from direct children
# of ~/.claude/skills; do not nest manual installs under ~/.claude/skills/oas/)
...
Use project-local rules when OAS's standards should apply to one repository rather than every Claude Code session:
...
For hand-picked manual installs, Claude discovers skills as direct children of `~/.claude/skills/`; do not nest them under `~/.claude/skills/oas/`.
...
If you installed OAS via `/plugin install`, do not copy those hooks into `settings.json`. Claude Code v2.1+ already auto-loads plugin `hooks/hooks.json`, and duplicating them in `settings.json` causes duplicate execution and cross-platform hook conflicts.
...
Claude plugin installs intentionally do not auto-enable OAS's bundled MCP server definitions. This avoids overlong plugin MCP tool names on strict third-party gateways while keeping manual MCP setup available.
...
OAS ships exactly one default connector (`chrome-devtools`); everything else is a skill wrapping a CLI/REST API or an opt-in catalog entry. The rule and the June 2026 audit that retired the previous six defaults live in [docs/MCP-CONNECTOR-POLICY.md](docs/MCP-CONNECTOR-POLICY.md).

If you already run your own copies of OAS-bundled MCPs, set:

```bash
export OAS_DISABLED_MCPS="chrome-devtools"
```

OAS-managed install and Codex sync flows will skip or remove those bundled servers instead of re-adding duplicates. `OAS_DISABLED_MCPS` is an OAS install/sync filter, not a live Claude Code toggle.
...
```bash
npx oas-universal list-installed
npx oas-universal doctor
npx oas-universal repair
npx oas-universal uninstall --dry-run
npx oas-universal uninstall
```
...
If you are leaving, the uninstall command prints an optional [20-second feedback form](https://github.com/neal0709/Operating-Agent-system-/issues/new?template=quick-feedback.yml). It is a public GitHub issue, never blocks uninstall, and OAS does not upload diagnostics. You can also run `oas feedback` at any time to see the problem, feedback, and feature routes.

Plugin users should remove the plugin from Claude Code, then delete only the rule folders they manually copied and no longer want. OAS only removes files recorded in its install-state. It does not claim unrelated files in your harness directories.

If you stacked methods, clean up in this order:

1. Remove the Claude Code plugin install.
2. Run the OAS uninstall command from the project directory that contains the managed install-state.
...
> [!IMPORTANT]
> These package-runner commands require `oas-universal` 2.2.0 or newer and
...
OAS 2.2 supports the same guided setup through modern package runners:
...
The wizard inventories the official marketplace and every native Claude install scope before making changes, then installs, updates, or safely moves `oas@oas` to the scope you choose. Rerun the same command whenever you want to update OAS, change scope, or change its hook profile. This setup wizard currently configures the Claude Code plugin; use the multi-harness wizard below for Codex or Kimi Code.
...
|---|---|
| Building a feature | `/oas:plan "describe the feature"`, then `tdd-workflow` |
...
| Auditing agent config | `/security-scan` or `npx -y oas-agentshield scan --path .` |
...
|--------------|-----------------|------------|
| Plan a new feature | `/oas:plan "Add auth"` | planner |
| Design system architecture | `/oas:plan` + architect agent | architect |
...
```
/oas:plan "Add user authentication with OAuth"
...
> [!IMPORTANT]
> **NEW IN OAS 2.1: Plan Canvas · Kimi harness · self-hosted compute on Itô GPUs.**
> [See the full release notes →](https://github.com/neal0709/Operating-Agent-system-/blob/main/docs/releases/2.1.0/release-notes.md)
...
It's harness- and model-agnostic: a plain CLI (`oas-plan-canvas`) speaking JSON, so any agent can drive it. Try it: ask your agent to `/oas:plan` anything, then review from the page instead of the terminal.

[Open the plan used in this demo →](https://github.com/neal0709/Operating-Agent-system-/blob/main/docs/releases/2.1.0/plan-canvas-demo.plan.md)

### Also in 2.1

- **Kimi Code install target** (`--target kimi`): OAS installs natively into [Moonshot AI](https://www.moonshot.ai)'s Kimi Code CLI
- **Self-host on GPUs**: a verified path with [Itô](https://compute.itomarkets.com), OAS's preferred compute sponsor, including the opt-in `oas ito find` RFQ bridge (details and disclosures above in [Self-Hosted Models and Custom Endpoints](#self-hosted-models-and-custom-endpoints))
...
| Version | Highlights |
|---|---|
| [v2.0.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v2.0.0) | The Agent Harness Operating System: cross-harness graduation, control-pane substrate, `orch-*` orchestrators, Discord + OAS bot, single-connector MCP policy |
| [v1.10.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.10.0) | Surface refresh, operator workflows, OAS 2.0 alpha |
| [v1.9.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.9.0) | Selective install, OAS Tools Pro, 12 language ecosystems |
| [v1.8.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.8.0) | Harness performance and cross-platform reliability |
| [v1.7.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.7.0) | Cross-platform expansion and presentation builder |
| [v1.6.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.6.0) | Codex Edition and the OAS Tools GitHub App |
| [v1.5.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.5.0) | Universal Edition |
| [v1.4.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.4.0) | Multi-language rules, installation wizard, PM2 orchestration |
| [v1.3.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.3.0) | Complete OpenCode plugin support |
| [v1.2.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.2.0) | Unified commands and skills |
| [v1.1.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.1.0) | Cross-platform support and community fixes |
| [v1.0.0](https://github.com/neal0709/Operating-Agent-system-/releases/tag/v1.0.0) | Official plugin release |
...
Stable graduation of the 2.0 line: the control-pane substrate (session adapters + MCP inventory), the worktree-lifecycle service, the `orch-*` orchestrator family, and the launch of the [OAS Discord community](https://discord.gg/36yGMHGFbR). Full notes: [docs/releases/2.0.0/release-notes.md](docs/releases/2.0.0/release-notes.md).

### v2.0.0-rc.1: Surface Refresh, Operator Workflows, and OAS 2.0 Alpha (Apr 2026)

- **Dashboard GUI**: New Tkinter-based desktop application (`oas_dashboard.py` or `npm run dashboard`) with dark/light theme toggle, font customization, and project logo in header and taskbar.
...
- **Operator and outbound workflow expansion**: `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `oas-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops`, and `workspace-surface-audit` round out the operator lane.
...
- **Itô prediction-market skill pack**: the consolidated `ito-baskets` skill (read-only basket index, comparison, market briefs, and non-executable planning worksheets — replacing the former `ito-market-intelligence`, `ito-basket-compare`, `ito-trade-planner`, and `ito-data-atlas-agent` skills), plus `prediction-market-oracle-research` and `prediction-market-risk-review`, add public, non-advisory market/basket workflows while keeping live Itô API access gated and separate from OAS Tools billing.
...
- **OAS 2.0 alpha in-tree**: the Rust control-plane prototype in `oas2/` builds locally and exposes `dashboard`, `start`, `sessions`, `status`, `stop`, `resume`, and `daemon` commands.
- **Operator status snapshots**: `oas status --markdown --write status.md` turns the local state store into a portable handoff covering readiness, active sessions, skill-run health, install health, pending governance events, and linked work items from Linear/GitHub/handoffs.
- **Ecosystem hardening**: AgentShield, OAS Tools cost controls, billing portal work, and website refreshes continue to ship around the core plugin instead of drifting into separate silos.
...
- **Harness-first release**: OAS is explicitly framed as an agent harness performance system, not just a config pack.
...
- **Hook runtime controls**: `OAS_HOOK_PROFILE=minimal|standard|strict` and `OAS_DISABLED_HOOKS=...` for runtime gating without editing hook files.
...
- **GitHub Marketplace**: OAS Tools GitHub App live at [github.com/marketplace/oas-tools](https://github.com/marketplace/oas-tools) with free/pro/enterprise tiers
...
- **Fixed instinct import content loss**: `parse_instinct_file()` was silently dropping all content after frontmatter (Action, Evidence, Examples sections) during `/instinct-import`. ([#148](https://github.com/neal0709/Operating-Agent-system-/issues/148), [#161](https://github.com/neal0709/Operating-Agent-system-/pull/161))

### v1.4.0: Multi-Language Rules, Installation Wizard, and PM2 (Feb 2026)

- **Interactive installation wizard**: New `configure-oas` skill provides guided setup with merge/overwrite detection
...
See the full changelog in [Releases](https://github.com/neal0709/Operating-Agent-system-/releases).
</details>

## Why Choose OAS?

| Without a system                                        | With OAS                                                              |
| ------------------------------------------------------- | --------------------------------------------------------------------- |
...
```text
/oas:plan "Add usage-based billing alerts"
...
Rules, skills, agents, and hooks solve different problems. Keeping those jobs separate is how OAS adds capability without dumping the entire repository into every session.
...
OAS's Memory Vault gives Claude, Codex, Hermes, OpenClaw, Kimi, and other harnesses one local, inspectable Markdown format for durable context and handoffs. Project and team memories live under `.oas/memory/`; user memories live under `~/.oas/memory/`.
...
Memory is unreviewed context, not executable policy. Verify important claims against authoritative sources and promote accepted knowledge into governed project documentation. The optional `oas-memory-mcp` server exposes the same bounded save, search, read, and doctor surface without enabling itself by default.
...
The Memory Vault stores portable `oas.memory.v1` Markdown documents instead of copying vendor transcripts or emailing context between agents. Project memories are protected by a fail-closed `.gitignore`; use the team scope only for human-inspected, version-controlled sharing. Team memories remain unreviewed context even after they are committed.
...
For opt-in MCP access, add the `oas-memory-vault` entry from [`mcp-configs/mcp-servers.json`](mcp-configs/mcp-servers.json) to each harness that needs it, then run `oas-memory-mcp`. The server exposes only `memory_save`, `memory_search`, `memory_read`, and `memory_doctor`. Each server must launch with a lowercase `OAS_MEMORY_HARNESS` identity; the identity is server-bound and cannot be supplied by a tool caller. User scope additionally requires the operator-controlled `OAS_MEMORY_ALLOW_USER_SCOPE=1` opt-in. See [`skills/unified-memory/SKILL.md`](skills/unified-memory/SKILL.md) for the workflow and trust boundaries, and [`docs/design/oas-memory-vault.md`](docs/design/oas-memory-vault.md) for the capability contract.
</details>
...
<table aria-label="OAS guides" width="100%">
<tr>
<td width="33%" align="center">
<a href="./the-shortform-guide.md">
<img src="assets/images/guides/shorthand-guide.png" width="213" height="120" alt="The Shorthand Guide to OAS" /><br />
<strong>The Shorthand Guide</strong>
</a>
<br /><sub>Setup, foundations, and day-one use. <b>Read this first.</b> (<a href="https://x.com/nihir/status/2012378465664745795">thread</a>)</sub>
</td>
<td width="33%" align="center">
<a href="./the-longform-guide.md">
<img src="assets/images/guides/longform-guide.png" width="213" height="120" alt="The Longform Guide to OAS" /><br />
<strong>The Longform Guide</strong>
</a>
<br /><sub>Context economics, memory, evals, and parallel agents. (<a href="https://x.com/nihir/status/2014040193557471352">thread</a>)</sub>
</td>
<td width="33%" align="center">
<a href="./the-security-guide.md">
<img src="assets/images/guides/security-guide.png" width="213" height="120" alt="The Security Guide to OAS" /><br />
<strong>The Security Guide</strong>
</a>
<br /><sub>Prompt injection, hooks, MCP, and AgentShield. (<a href="https://x.com/nihir/status/2033263813387223421">thread</a>)</sub>
</td>
</tr>
</table>
...
|   |-- configure-oas/              # Interactive installation wizard
...
|
|-- rules/            # Always-follow guidelines (copy to ~/.claude/rules/oas/)
...
|
|-- oas_dashboard.py  # Desktop GUI dashboard (Tkinter)
|
...
Launch the desktop dashboard to visually explore OAS components:
...
[Install OAS Tools GitHub App](https://github.com/apps/oas-tools) | [oas.tools](https://oas.tools)
...
Use `/security-scan` in Claude Code to run it, or add to CI with the [GitHub Action](https://github.com/neal0709/agentshield).
...
OAS's core Node.js CLI and managed installers run on **Windows, macOS, and Linux**, but optional capabilities are not at full parity. Some continuous-learning, GAN, and orchestration paths still require Bash or Python; harnesses also expose different hook, agent, and skill APIs.
...
| macOS | Supported core | The standalone GAN shell path is not compatible with the system Bash 3.2 and currently has a score-parsing defect ([#2674](https://github.com/neal0709/Operating-Agent-system-/issues/2674)). |
| Windows + WSL | Supported core | WSL follows the Linux paths; Windows host integrations still vary by harness. |
| Windows native | Supported with limitations | Continuous-learning v2's observer daemon and memory-vault writes have open native-Windows defects ([#2489](https://github.com/neal0709/Operating-Agent-system-/issues/2489), [#2626](https://github.com/neal0709/Operating-Agent-system-/issues/2626)). Shell-backed optional features require Git Bash/WSL or are unavailable. |
...
Memory persistence hooks (session summaries, learned skills, session aliases, metrics) store data under a single agent data root. By default that root is `~/.claude`. When you use OAS in both Claude Code and Cursor on the same machine, set a separate root for Cursor so the two environments do not overwrite each other's session files:
...
- `$OAS_AGENT_DATA_HOME/session-data/`: session summaries
- `$OAS_AGENT_DATA_HOME/skills/learned/`: learned skills from evaluate-session
- `$OAS_AGENT_DATA_HOME/session-aliases.json`: session aliases
- `$OAS_AGENT_DATA_HOME/metrics/`: cost and activity metrics

See [neal0709/Operating-Agent-system-#2065](https://github.com/neal0709/Operating-Agent-system-/issues/2065).
</details>
...
| Cursor | Beta project adapter | Selective installer into `.cursor/` | Agent discovery varies by Cursor build, and OAS's installer paths do not yet expose identical hook sets ([#2419](https://github.com/neal0709/Operating-Agent-system-/issues/2419)). |
| OpenCode | Beta built plugin | Build plugin, then selective installer | OAS ships a subset of the catalog; connect a provider and select a model in OpenCode ([#2617](https://github.com/neal0709/Operating-Agent-system-/issues/2617)). |
| GitHub Copilot | Instruction-only | Checked-in instructions and prompt files | No OAS hooks, runtime agents, delegation, or native skill discovery. |
...
| OAS hooks | Native plugin hooks | Native reviewed subset with explicit trust | Cursor hook adapter; install-path differences remain | Plugin events | Not supported |
| MCP configuration | Available, explicit activation | Native plugin manifest; legacy sync can merge TOML | Explicit project/user config | Provider/plugin config | Not supplied by OAS |
...
OAS provides Cursor IDE support with hooks, rules, agents, skills, commands, and MCP configs adapted for Cursor's project layout.
...
| Agents | 48 | `.cursor/agents/oas-*.md` when installed; prefixed to avoid collisions with user or marketplace agents |
...
OAS does not install root `AGENTS.md` into `.cursor/`. Cursor treats nested `AGENTS.md` files as directory context, so copying OAS's repo identity into a host project would pollute that project.

Cursor-native loading behavior can vary by Cursor build. OAS installs agents as `.cursor/agents/oas-*.md`; if your Cursor build does not expose project agents, those files still work as explicit reference definitions instead of hidden global prompt context.

#### Memory and data isolation (Cursor + Claude Code)

OAS memory hooks reuse the same `scripts/hooks/*.js` as Claude Code. For Cursor, OAS tries to keep memory **out of `~/.claude` automatically**:

1. **Cursor `sessionStart` hook** (installed to `.cursor/hooks.json` on `--target cursor`) injects `OAS_AGENT_DATA_HOME` for the whole composer session.
2. **Hook runtime default**: when `CURSOR_VERSION` or `CURSOR_PROJECT_DIR` is present, hooks default to `~/.cursor/oas` if the env var is unset.
3. **Project config**: `.cursor/oas-agent-data.json` documents and overrides the path (`agentDataHome`).
4. **Always-on rule**: `.cursor/rules/oas-agent-data-home.mdc` reminds the agent where memory lives.
...
To **share** memory with Claude Code on purpose, set `OAS_AGENT_DATA_HOME=~/.claude` in the shell or in `.cursor/oas-agent-data.json`.

Continuous learning v2 instincts remain separate under `CLV2_HOMUNCULUS_DIR` (default `~/.local/share/oas-homunculus`).
...
The sync script safely merges OAS MCP servers into your existing `~/.codex/config.toml` using an **add-only** strategy: it never removes or modifies your existing servers. Run with `--dry-run` to preview changes, or `--update-mcp` to force-refresh OAS servers to the latest recommended config.

For Context7, OAS uses the canonical Codex section name `[mcp_servers.context7]` while still launching the `@upstash/context7-mcp` package. If you already have a legacy `[mcp_servers.context7-mcp]` entry, `--update-mcp` migrates it to the canonical section name.
...
Codex does **not provide Claude-style hook execution parity**. The native OAS plugin includes a reviewed hook subset that requires explicit trust in `/hooks`; `AGENTS.md`, optional `model_instructions_file` overrides, and sandbox/approval settings provide the remaining instruction and policy layers.
...
OAS ships three sample role configs:
...
OAS provides Zed project support through a conservative `.zed` adapter for project-local settings, flattened rules, agents, commands, and skills.
...
The adapter writes OAS-managed files under `.zed/` and keeps BYOK/OpenRouter credentials out of the repo. Configure Zed account or API keys through Zed's own settings UI or your local user settings.
</details>

<details>
<summary><strong>OpenCode support in depth</strong></summary>

OAS provides a beta OpenCode plugin integration with instructions, a catalog subset, commands, custom tools, and hook events. It does not provide feature parity with Claude Code. The reference config inherits the user's OpenCode model selection instead of pinning a provider-specific model.
...
That npm plugin entry enables OAS's published OpenCode plugin module (hooks/events and plugin tools). It does **not** automatically add OAS's full command/agent/instruction catalog to your project config.

For the full OAS OpenCode setup, either:
...
OAS provides **GitHub Copilot support** for VS Code via Copilot Chat's native instruction and prompt file system. No extra tooling required.
...
| OAS Feature | Copilot equivalent |
|-------------|-------------------|
...
GitHub Copilot does not have a hook system or a subagent API, so OAS's hook automations (auto-format, TypeScript check, session persistence, dev-server guard) and agent delegation are unavailable. The instruction and prompt layer still brings the full OAS coding philosophy (standards, security, TDD, and workflow) into every Copilot Chat session.
</details>

<details>
<summary><strong>What changed in v2.0.0</strong></summary>

OAS v2.0.0 stabilizes the 2.0 line with the public Hermes operator story, 281 skills, 67 agents, 94 command shims, session adapters, MCP inventory, worktree lifecycle services, orchestrator workflows, and the OAS Discord community.

- [v2.0.0 release notes](docs/releases/2.0.0/release-notes.md)
- [OAS 2.0 reference architecture](docs/OAS-2.0-REFERENCE-ARCHITECTURE.md)
...
| `OAS_CONTEXT_MONITOR_COST_WARNINGS` | on | **off for subscription users** | Suppresses agent-facing API-rate estimate warnings while keeping context/scope/loop warnings |
...
If you use a subscription and the context monitor's API-rate estimates are not useful, set `OAS_CONTEXT_MONITOR_COST_WARNINGS=off`. This only suppresses the agent-facing cost warnings; it does not disable context exhaustion, scope, or loop warnings.
</details>
...
- Use `OAS_DISABLED_MCPS` only to filter OAS-generated MCP configs during install/sync flows
...
**History:** This has caused repeated fix/revert cycles in this repo ([#29](https://github.com/neal0709/Operating-Agent-system-/issues/29), [#52](https://github.com/neal0709/Operating-Agent-system-/issues/52), [#103](https://github.com/neal0709/Operating-Agent-system-/issues/103)). The behavior changed between Claude Code versions, leading to confusion. There is now a regression test to prevent this from being reintroduced.
</details>

## Security

Install OAS only from official sources:

- GitHub repository: <https://github.com/neal0709/Operating-Agent-system->
- Claude Code plugin: `oas@oas`
- npm packages: [`oas-universal`](https://www.npmjs.com/package/oas-universal) and [`oas-agentshield`](https://www.npmjs.com/package/oas-agentshield)
- GitHub App: <https://github.com/apps/oas-tools>
- Website: <https://oas.tools>
...
`OAS_DISABLED_MCPS` is an OAS install/sync filter, not a live Claude Code toggle.
...
<details>
<summary><strong>OAS appears twice or hooks fire twice</strong></summary>

The usual cause is installing the Claude plugin and then running `./install.sh --profile full` on top of it.

1. Remove the Claude Code plugin install.
2. Run `node scripts/oas.js uninstall --dry-run` from the OAS checkout.
...
**Do NOT add a `"hooks"` field to `.claude-plugin/plugin.json`.** Claude Code v2.1+ automatically loads `hooks/hooks.json` from installed plugins. Explicitly declaring it causes duplicate detection errors. See [#29](https://github.com/neal0709/Operating-Agent-system-/issues/29), [#52](https://github.com/neal0709/Operating-Agent-system-/issues/52), [#103](https://github.com/neal0709/Operating-Agent-system-/issues/103).
</details>

<details>
<summary><strong>Codex marketplace installs but skills do not load</strong></summary>

Run the cache check from an OAS checkout:

```bash
node scripts/codex/check-plugin-cache.js
```

If it reports unresolved parent references, refresh the native cache with `codex plugin marketplace upgrade oas`, run `codex plugin add oas@oas` again, and restart Codex. Registration in `codex plugin list` confirms the marketplace entry, while the cache check verifies that the installed manifest can resolve its skills, MCP configuration, and assets. Use `bash scripts/sync-oas-to-codex.sh` only when you intentionally need the legacy copied-configuration compatibility path.
</details>

<details>
<summary><strong>My context window is shrinking</strong></summary>

Too many MCP servers eat your context. Each MCP tool description consumes tokens from your 200k window, potentially reducing it to ~70k. SessionStart context is capped at 8000 characters by default; lower it with `OAS_SESSION_START_MAX_CHARS=4000` or disable it with `OAS_SESSION_START_CONTEXT=off` for local-model or low-context setups.
...
Use the [manual adaptation guide](docs/MANUAL-ADAPTATION-GUIDE.md), or open a [GitHub discussion](https://github.com/neal0709/Operating-Agent-system-/discussions) with the harness name and the file, skill, command, and hook formats it supports.
</details>
...
<details>
<summary><strong>Sponsors and OAS Pro</strong></summary>

OAS stays free because sponsors and Pro users fund the work. Sponsor logos are at the top of this README; the full roster and tiers are in [SPONSORS.md](SPONSORS.md).

OAS Pro adds private-repo analysis, PR-triggered audits, AgentShield-backed scanning, automatic push and PR checks, pooled team usage, and priority support through the hosted GitHub App.

<table>
<tr>
<td width="25%" align="center"><a href="https://oas.tools/pricing"><strong>OAS Pro</strong><br /><sub>Hosted GitHub App for private repos</sub></a></td>
<td width="25%" align="center"><a href="https://github.com/sponsors/neal0709"><strong>Sponsor OAS</strong><br /><sub>Fund the OSS work</sub></a></td>
<td width="25%" align="center"><a href="https://github.com/neal0709/Operating-Agent-system-/discussions"><strong>Community</strong><br /><sub>Q&amp;A, ideas, and Show and Tell</sub></a></td>
<td width="25%" align="center"><a href="https://github.com/apps/oas-tools"><strong>GitHub App</strong><br /><sub>PR audits and hosted workflows</sub></a></td>
</tr>
</table>

[Become a sponsor](https://github.com/sponsors/neal0709) | [Sponsor tiers](SPONSORS.md) | [Sponsorship program](SPONSORING.md)
</details>
...
- **Shorthand Guide (Start Here):** [The Shorthand Guide to OAS](https://x.com/nihir/status/2012378465664745795)
- **Longform Guide (Advanced):** [The Longform Guide to OAS](https://x.com/nihir/status/2014040193557471352)
- **Security Guide:** [Security Guide](./the-security-guide.md) | [Thread](https://x.com/nihir/status/2033263813387223421)
- **Follow:** [@nihir](https://x.com/nihir)
