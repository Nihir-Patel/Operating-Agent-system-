---
name: unified-memory
description: Share durable, inspectable context and handoffs between Claude, Codex, Hermes, Cursor, OpenCode, and other agents through the local OAS Memory Vault. Use when an agent must save work state, transfer context, resume another agent's task, or search shared project knowledge.
origin: OAS
---

# Unified Memory

Use the OAS Memory Vault as the common context layer between harnesses. The
vault stores portable `oas.memory.v1` Markdown documents rather than
...
`PATH`. Install the `oas-universal` npm runtime separately before using the CLI
or MCP examples:
...
`node scripts/oas.js memory ...`, but MCP configurations that name
`oas-memory-mcp` still require that binary on `PATH`.
...
| Scope | Location | Use |
|---|---|---|
| `project` | `<repo>/.oas/memory/project/` | Repo-local context protected by a fail-closed `.gitignore` |
| `team` | `<repo>/.oas/memory/team/` | Context intended for human review and version-controlled sharing |
| `user` | `~/.oas/memory/` | Operator context that follows the user across repositories |

All participating harnesses must use the same repository working directory or
the same `OAS_MEMORY_PROJECT_ROOT` and `OAS_MEMORY_USER_ROOT` overrides.
...
The stdio server is optional and is not enabled by OAS's default `.mcp.json`.
After installing OAS, copy the `oas-memory-vault` entry from
...
```text
OAS_MEMORY_HARNESS=codex oas-memory-mcp
```

The MCP process binds writes and target filtering to
`OAS_MEMORY_HARNESS`; tool callers cannot claim another source identity or
override the target filter. `user` scope remains disabled unless the operator
also launches the server with `OAS_MEMORY_ALLOW_USER_SCOPE=1`, and a tool call
