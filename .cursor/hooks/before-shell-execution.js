  ...
  .then(raw => {
    try {
      ...
      if (hookEnabled('pre:bash:dev-server-block', ['standard', 'strict']) && process.platform !== 'win32') {
        ...
        if (hasBlockedDev) {
          console.error('[OAS] BLOCKED: Dev server must run in tmux for log access');
          console.error('[OAS] Use: tmux new-session -d -s dev "npm run dev"');
          process.exit(2);
        }
      }
      ...
      if (
        ...
        console.error('[OAS] Consider running in tmux for session persistence');
      }

      if (hookEnabled('pre:bash:git-push-reminder', ['strict']) && /\bgit\s+push\b/.test(cmd)) {
        console.error('[OAS] Review changes before push: git diff origin/main...HEAD');
      }
    } catch {
      // noop
    }
