...
const EXPECTED_NAME = 'oas-universal';
const EXPECTED_BIN = 'scripts/oas.js';
...
function validatePackedPackage(packageRoot) {
  ...
  if (!manifest.bin || manifest.bin.oas !== EXPECTED_BIN) {
    fail(`Packed package bin.oas must map to ${EXPECTED_BIN}.`);
  }
  ...
  const binTarget = path.resolve(resolvedRoot, manifest.bin.oas);
  if (!isWithin(resolvedRoot, binTarget)) {
    fail('Packed package bin.oas escapes the extracted package root.');
  }
...
function preparePackedCli(sourceRoot, outputRoot) {
  ...
  if (resolvedSource !== '/oas') {
    fail('Package source must be the read-only /oas checkout.');
  }
  ...
  const publicBin = path.join(binRoot, 'oas');
