...
test('builds pinned Debian and Ubuntu images as a non-root user', () => {
  ...
  assert.match(dockerfile, /ENV HOME=\/tmp\/oas-home/);
  ...
  assert.match(compose, /image:\s*oas-plugin-setup:debian/);
  assert.match(compose, /image:\s*oas-plugin-setup:ubuntu/);
...
test('keeps checkout and source project read-only with hardened defaults', () => {
  ...
  assert.match(compose, /target:\s*\/oas\s*\n\s*read_only:\s*true/);
  assert.match(compose, /target:\s*\/source-project\s*\n\s*read_only:\s*true/);
  assert.match(compose, /CLAUDE_CONFIG_DIR:\s*\/tmp\/oas-claude-config/);
  assert.match(
    compose,
    /\/tmp:rw,nosuid,nodev,exec,size=\$\{OAS_TMPFS_SIZE:-2g\},uid=1000,gid=1000,mode=0700/
  );
  assert.match(
    compose,
    /\/workspace:rw,nosuid,nodev,noexec,size=\$\{OAS_WORKSPACE_SIZE:-1g\},uid=1000,gid=1000,mode=0700/
  );
...
test('real runner copies into tmpfs and exposes only explicit safe modes', () => {
  ...
  assert.match(runner, /OAS_PROJECT_DIR:-\/workspace\/project/);
  ...
  assert.match(runner, /run_oas install/);
  assert.match(runner, /run_oas list-installed --json/);
  assert.match(runner, /run_oas doctor --target claude-project/);
  assert.match(runner, /\[\[ -e "\$project_dir\/\.claude" \]\]/);
  assert.doesNotMatch(
    runner,
    /scripts\/oas\.js" setup|--move-scope|\bmigrate\b/
  );
  assert.doesNotMatch(runner, /scripts\/oas\.js" install/);
...
test('prepares a local npm artifact through the confined public bin contract', () => {
  ...
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-packed-cli-'));
  ...
  function createFixture(name, options = {}) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: options.packageName || 'oas-universal',
        version: '2.1.0',
        bin: options.bin === undefined ? { oas: 'scripts/oas.js' } : options.bin,
      })
    );
    fs.writeFileSync(path.join(packageRoot, 'scripts', 'oas.js'), '#!/usr/bin/env node\n');
    fs.chmodSync(path.join(packageRoot, 'scripts', 'oas.js'), 0o755);
  ...
  try {
    ...
    assert.strictEqual(
      ...
      path.join(validRoot, 'scripts', 'oas.js')
    );

    for (const [name, options, pattern] of [
      ['wrong-name', { packageName: 'not-oas' }, /package name/i],
      ['missing-bin', { bin: {} }, /bin\.oas/i],
      ['escaping-bin', { bin: { oas: '../escape.js' } }, /bin\.oas/i],
...
test('validates dry-run target confinement and nonempty operations', () => {
  ...
  const safePlan = {
    ...
    plan: {
      ...
      installRoot,
      operations: [
        { destinationPath: path.join(installRoot, 'rules', 'oas', 'base.md') },
      ],
    },
  };
  const safe = runNode(
