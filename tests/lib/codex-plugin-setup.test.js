...
const MARKETPLACE_UPGRADE = [
  'plugin', 'marketplace', 'upgrade', 'oas', '--json',
];
const PLUGIN_ADD = ['plugin', 'add', 'oas@oas', '--json'];

function marketplaceInventory(installed = false) {
  return JSON.stringify({
    marketplaces: installed ? [{ name: 'oas', root: '/cache/oas' }] : [],
  });
}
...
function marketplaceUpgradeResult(overrides = {}) {
  return JSON.stringify({
    selectedMarketplaces: ['oas'],
    upgradedRoots: ['/cache/oas'],
    errors: [],
    ...overrides,
  });
}
...
function dependenciesFor(fake, overrides = {}) {
  return {
    ...
    resolveMarketplaceRepository: async () => 'https://github.com/neal0709/Operating-Agent-system-.git',
    ...overrides,
  };
}
  ...
  const cases = [
    ...
    ['resolves marketplace provenance with execFile and exact Git argv', async () => {
      const fake = createExecFile([{
        command: 'git',
        args: ['-C', '/cache/oas', 'remote', 'get-url', 'origin'],
        stdout: 'https://github.com/neal0709/Operating-Agent-system-.git\n',
      }]);
      ...
      assert.strictEqual(repository, 'https://github.com/neal0709/Operating-Agent-system-.git');
    ...
    ['fresh install uses exact native Codex argv and verifies both mutations', async () => {
      const fake = createExecFile([
        ...
        { args: PLUGIN_ADD, stdout: '{"pluginId":"oas@oas"}' },
    ...
    ['accepts the provider root across Windows separator and case differences', () => {
      const result = parseMarketplaceUpgradeResult(
        marketplaceUpgradeResult({
          upgradedRoots: ['c:/users/hira/.codex/marketplaces/oas'],
        }),
        { name: 'oas', root: 'C:\\Users\\Hira\\.codex\\marketplaces\\oas' }
      );

      assert.deepStrictEqual(result.selectedMarketplaces, ['oas']);
    }],
    ['rejects ambiguous native refresh results for a targeted upgrade', () => {
      assert.throws(
        () => parseMarketplaceUpgradeResult(
          marketplaceUpgradeResult({
            selectedMarketplaces: ['oas', 'other'],
            upgradedRoots: ['/cache/oas', '/cache/other'],
          }),
          { name: 'oas', root: '/cache/oas' }
        ),
        error => (
        ...
        reconcileCodexPlugin({}, dependenciesFor(fake, {
          resolveMarketplaceRepository: async () => {
            ...
            return provenanceChecks === 1
              ? 'https://github.com/neal0709/Operating-Agent-system-.git'
              : 'https://github.com/attacker/oas.git';
          },
        })),
        'MARKETPLACE_COLLISION',
        /not the official/i
    ...
    ['fails closed when the oas marketplace has untrusted provenance', async () => {
      const fake = createExecFile([
        { args: MARKETPLACE_LIST, stdout: marketplaceInventory(true) },
        { args: PLUGIN_LIST, stdout: pluginInventory(false) },
      ]);
      ...
      await expectSetupError(
        reconcileCodexPlugin({}, dependenciesFor(fake, {
          resolveMarketplaceRepository: async marketplace => {
            assert.strictEqual(marketplace.root, '/cache/oas');
            return 'https://github.com/attacker/oas.git';
          },
        })),
        'MARKETPLACE_COLLISION',
        /refusing.*oas.*marketplace/i
      );
      assert.deepStrictEqual(fake.calls.map(call => call.args), [
        MARKETPLACE_LIST,
        PLUGIN_LIST,
    ...
    ['rejects malformed marketplace and plugin JSON inventories', async () => {
      ...
      assert.throws(
        () => parseMarketplaceInventory('{"marketplaces":[{"name":"oas","root":""}]}'),
        error => error.code === 'INVALID_MARKETPLACE_INVENTORY'
      );
      assert.throws(
      ...
      assert.throws(
        () => parsePluginInventory(JSON.stringify({
          installed: [
            { pluginId: 'oas@oas', installed: true, enabled: true },
            { pluginId: 'oas@oas', installed: true, enabled: true },
          ],
          available: [],
        })),
      ...
      assert.strictEqual(normalizeGitHubGitOrigin('neal0709/Operating-Agent-system-'), null);
      assert.strictEqual(
        normalizeGitHubGitOrigin('http://github.com/neal0709/Operating-Agent-system-.git'),
        null
      );
    }],
    ...
    ['fails when post-install verification does not find enabled OAS', async () => {
      const fake = createExecFile([
        { args: MARKETPLACE_LIST, stdout: marketplaceInventory(false) },
        { args: PLUGIN_LIST, stdout: pluginInventory(false) },
        { args: MARKETPLACE_ADD, stdout: '{"alreadyAdded":false}' },
    ...
    ['fails when marketplace verification cannot observe OAS', async () => {
      const fake = createExecFile([
        { args: MARKETPLACE_LIST, stdout: marketplaceInventory(false) },
        { args: PLUGIN_LIST, stdout: pluginInventory(false) },
        { args: MARKETPLACE_ADD, stdout: '{"alreadyAdded":false}' },
