...
function writeManifestSourceFixture(root) {
    ...
    modules: [
      {
        ...
        paths: [
          ...
          'missing.txt',
          'skills/demo',
          path.join('runtime', 'oas', 'install-state.json'),
          '.claude-plugin',
        ],
        targets: ['claude'],
        dependencies: [],
  ...
  writeFile(root, path.join('src', 'nested', 'oas-install-state.json'), '{}\n');
  writeFile(root, path.join('rules', 'common', 'coding-style.md'), '# Common\n');
  writeFile(root, path.join('skills', 'demo', 'SKILL.md'), '# Demo\n');
  writeFile(root, 'standalone.txt', 'standalone\n');
  writeFile(root, path.join('runtime', 'oas', 'install-state.json'), '{}\n');
...
function runTests() {
  ...
  if (test('plans Claude legacy rules under the default OAS-managed rules directory', () => {
    const sourceRoot = createTempDir('install-executor-source-');
    const homeDir = createTempDir('install-executor-home-');
    const projectRoot = createTempDir('install-executor-project-');
    try {
      ...
      const managedRulesDir = path.join(homeDir, '.claude', 'rules', 'oas');
      assert.strictEqual(plan.installRoot, managedRulesDir);
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'oas', 'common', 'coding-style.md')));
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'oas', 'typescript', 'testing.md')));
  ...
  if (test('plans Cursor legacy assets and JSON merge payloads', () => {
    ...
    try {
      ...
      assert.ok(operationFor(plan, path.join('.cursor', 'agents', 'oas-planner.md')));
  ...
  if (test('materializes manifest scaffold operations and filters generated runtime state', () => {
    ...
    try {
      ...
      assert.ok(!normalizedSources.includes('runtime/oas/install-state.json'));
      assert.ok(!normalizedSources.includes('src/nested/oas-install-state.json'));
      ...
      assert.ok(plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'oas', 'common', 'coding-style.md')
      )));
      assert.ok(plan.operations.some(operation => (
  ...
  if (test('applyInstallPlan re-export applies a manifest plan and writes install state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'rules', 'oas', 'common', 'coding-style.md')));
      ...
      const state = JSON.parse(fs.readFileSync(path.join(homeDir, '.claude', 'oas', 'install-state.json'), 'utf8'));
  ...
  if (test('per-operation guard runs after mkdir and immediately before a copy write', () => {
    ...
    try {
      ...
      const sourcePath = writeFile(tempDir, path.join('source', 'security.md'), 'oas\n');
      const destinationPath = path.join(targetRoot, 'rules', 'security.md');
      const plan = {
        adapter: { id: 'kimi-project', target: 'kimi', kind: 'project' },
        installStatePath: path.join(targetRoot, 'oas-install-state.json'),
        operations: [{
          kind: 'copy-file',
          moduleId: 'core',
          sourcePath,
