...
const { INLINE_RESOLVE } = require('../../scripts/lib/resolve-oas-root');
...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/scripts','lib','resolve-oas-root/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/scripts','lib','resolve-oas-root/g) || []).length, 3);
});
...
test('instinct-status command uses shared inline resolver (no stale legacy fallback) (#2037)', () => {
  ...
  assert.strictEqual((instinctStatusDoc.match(/scripts','lib','resolve-oas-root/g) || []).length, 1);
...
test('auto-update command probes for the script it runs, not just scripts/lib', () => {
  ...
  // sentinel OAS skill, per #2544/#2577) yet still lack scripts/auto-update.js.
  ...
  for (const docPath of autoUpdateDocs) {
    ...
    assert.strictEqual(
      (doc.match(/scripts','lib','resolve-oas-root/g) || []).length, 1,
...
test('resolveEccRoot module covers current and legacy marketplace plugin roots', () => {
  const { resolveEccRoot } = require('../../scripts/lib/resolve-oas-root');
  assert.ok(typeof resolveEccRoot === 'function');

  const legacyHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-marketplace-legacy-'));
  try {
    const legacyRoot = path.join(legacyHomeDir, '.claude', 'plugins', 'marketplaces', 'oas');
    ...
    fs.mkdirSync(path.join(legacyRoot, OAS_SKILL_SENTINEL), { recursive: true });
  ...
  const cacheHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-marketplace-cache-'));
  try {
    const cacheRoot = path.join(cacheHomeDir, '.claude', 'plugins', 'cache', 'oas', 'neal0709', '1.0.0');
    ...
    fs.mkdirSync(path.join(cacheRoot, OAS_SKILL_SENTINEL), { recursive: true });
  ...
  assert.ok(INLINE_RESOLVE.includes("scripts','lib','resolve-oas-root"));
});
