...
const originalContextWindowEnv = {
  OAS_CONTEXT_WINDOW_TOKENS: process.env.OAS_CONTEXT_WINDOW_TOKENS,
  CLAUDE_CODE_AUTO_COMPACT_WINDOW: process.env.CLAUDE_CODE_AUTO_COMPACT_WINDOW,
};
delete process.env.OAS_CONTEXT_WINDOW_TOKENS;
...
test('honors an explicit OAS_CONTEXT_WINDOW_TOKENS override (e.g. 400k models, #2290)', () => {
  process.env.OAS_CONTEXT_WINDOW_TOKENS = '400000';
  try {
...
test('ignores a non-positive / invalid window override', () => {
  process.env.OAS_CONTEXT_WINDOW_TOKENS = 'not-a-number';
  try {
    assert.strictEqual(resolveContextWindowTokens(50000, 'claude-sonnet-4-6'), STANDARD_CONTEXT_WINDOW_TOKENS);
  } finally {
    delete process.env.OAS_CONTEXT_WINDOW_TOKENS;
...
test('an env override is a detected window, not inferred', () => {
  process.env.OAS_CONTEXT_WINDOW_TOKENS = '1000000';
  try {
