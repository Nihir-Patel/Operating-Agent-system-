...
function runTests() {
  ...
  if (test('manifest uninstall restores previous files, removes owned files, markers, and hooks path', () => {
    ...
    const backupDir = path.join(codexHome, 'backups', 'oas-test');
    const configPath = path.join(codexHome, 'config.toml');
    const agentsPath = path.join(codexHome, 'AGENTS.md');
    const promptPath = path.join(codexHome, 'prompts', 'oas-plan.md');
    ...
    fs.writeFileSync(promptPath, '# OAS generated prompt\n');
  ...
  if (test('uninstall preserves config and AGENTS edits made after legacy sync', () => {
    ...
    const statePath = beginLegacySyncState({
      codexHome,
      backupDir: path.join(codexHome, 'backups', 'oas-test'),
    });
  ...
  if (test('pre-manifest cleanup removes only the OAS marker block and preserves all other artifacts', () => {
    const homeDir = tempDir('legacy-codex-home-');
    const codexHome = path.join(homeDir, '.codex');
    const agentsPath = path.join(codexHome, 'AGENTS.md');
    const promptPath = path.join(codexHome, 'prompts', 'oas-plan.md');
    ...
    const result = uninstallLegacyCodexSync({ codexHome });
    assert.strictEqual(result.status, 'partial');
    assert.ok(!fs.readFileSync(agentsPath, 'utf8').includes('BEGIN OAS'));
  ...
  if (test('pre-manifest cleanup preserves inline, fenced, and symlinked AGENTS markers', () => {
    ...
    fs.mkdirSync(codexHome, { recursive: true });
    const examples = '# User\nInline <!-- BEGIN OAS --> example <!-- END OAS -->\n```md\n<!-- BEGIN OAS -->\n# Example\n<!-- END OAS -->\n```\n````md\n```md\n<!-- BEGIN OAS -->\n# Nested example\n<!-- END OAS -->\n```\n````\n';
    ...
    fs.writeFileSync(outsidePath, '<!-- BEGIN OAS -->\n# Outside\n<!-- END OAS -->\n');
    ...
    assert.strictEqual(fs.readFileSync(outsidePath, 'utf8'), '<!-- BEGIN OAS -->\n# Outside\n<!-- END OAS -->\n');
    fs.rmSync(homeDir, { recursive: true, force: true });
  })) passed += 1; else failed += 1;
  ...
  if (test('interrupted sync rollback restores overwritten files and removes newly created files', () => {
    ...
    const codexHome = path.join(homeDir, '.codex');
    const backupDir = path.join(codexHome, 'backups', 'oas-test');
    const existingPath = path.join(codexHome, 'prompts', 'oas-plan.md');
    const createdPath = path.join(codexHome, 'prompts', 'oas-review.md');
    ...
    const partialContent = Buffer.from('# Partial OAS write\n');
  ...
  if (test('recording refuses symlink targets before the sync can write through them', () => {
    ...
    const linkedPath = path.join(codexHome, 'prompts', 'oas-plan.md');
  ...
  if (test('recording refuses a symlinked parent directory before any managed write', () => {
    ...
    assert.throws(
      () => recordLegacySyncPath({
        statePath,
        filePath: path.join(codexHome, 'prompts', 'oas-plan.md'),
      }),
  ...
  if (test('uninstall preserves a managed path replaced by a symlink', () => {
    ...
    const outsidePath = path.join(homeDir, 'outside.md');
    fs.mkdirSync(path.dirname(promptPath), { recursive: true });
    fs.writeFileSync(outsidePath, '# OAS generated prompt\n');
    const statePath = beginLegacySyncState({
      codexHome,
      backupDir: path.join(codexHome, 'backups', 'oas-test'),
    });
    ...
    assert.strictEqual(fs.readFileSync(outsidePath, 'utf8'), '# OAS generated prompt\n');
    assert.ok(result.retainedPaths.includes(promptPath));
    fs.rmSync(homeDir, { recursive: true, force: true });
  })) passed += 1; else failed += 1;

  if (test('repeat sync preserves the original pre-OAS baseline through uninstall', () => {
    const homeDir = tempDir('legacy-codex-home-');
    const codexHome = path.join(homeDir, '.codex');
    const promptPath = path.join(codexHome, 'prompts', 'oas-plan.md');
    fs.mkdirSync(path.dirname(promptPath), { recursive: true });
    ...
    let statePath = beginLegacySyncState({
      codexHome,
      backupDir: path.join(codexHome, 'backups', 'oas-first'),
    });
    recordLegacySyncPath({ statePath, filePath: promptPath });
    fs.writeFileSync(promptPath, '# OAS v1\n');
    finalizeLegacySyncState({ statePath });

    statePath = beginLegacySyncState({
      codexHome,
      backupDir: path.join(codexHome, 'backups', 'oas-second'),
    });
    recordLegacySyncPath({ statePath, filePath: promptPath });
    fs.writeFileSync(promptPath, '# OAS v2\n');
    finalizeLegacySyncState({ statePath });
  ...
  if (test('failed repeat sync restores the prior installed ownership manifest', () => {
    ...
    fs.writeFileSync(promptPath, '# Partial OAS v2\n');
    const rollback = rollbackLegacyCodexSync({ statePath });
    assert.strictEqual(rollback.status, 'rolled-back');
    assert.strictEqual(fs.readFileSync(promptPath, 'utf8'), '# OAS v1\n');
    assert.strictEqual(fs.readFileSync(statePath, 'utf8'), priorInstalledState);
