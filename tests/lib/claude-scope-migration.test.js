...
function createFixture(initialState = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas scope migration '));
...
function uninstallArgv(scope) {
  return ['plugin', 'uninstall', 'oas@oas', '--scope', scope, '--keep-data'];
}
...
test('zero installs, ambiguous non-destination scopes, and invalid inventories fail closed', () => {
  const cases = [
    ...
    {
      state: { pluginListResponses: [[{ id: 'oas@oas', enabled: true }]] },
      scope: 'project',
      code: 'INVALID_PLUGIN_INVENTORY',
    },
    {
  ...
  const cases = [
    {
      state: {
        ...
        marketplaces: [marketplace('user')],
        failures: [{
          argv: ['plugin', 'marketplace', 'update', 'oas'],
          status: 7,
...
test('migration preserves hook preferences unless --hooks is explicit', () => {
  ...
  withFixture({
    ...
    assert.strictEqual(settings.pluginConfigs['oas@oas'].futureKey, true);
