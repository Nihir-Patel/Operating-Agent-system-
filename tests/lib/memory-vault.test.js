...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-vault-'));
...
test('resolves project, team, and user roots from the nearest project boundary', () => {
  ...
  try {
    assert.strictEqual(
      ...
      path.join(fixture.projectRoot, '.oas', 'memory', 'project')
    );
    assert.strictEqual(
      fixture.roots.team,
      path.join(fixture.projectRoot, '.oas', 'memory', 'team')
    );
    assert.strictEqual(
      fixture.roots.user,
      path.join(fixture.homeDir, '.oas', 'memory')
    );
  } finally {
...
test('uses the working directory for non-git projects instead of a global bucket', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-no-git-'));
  const homeDir = path.join(root, 'home');
  fs.mkdirSync(homeDir);
  try {
    const roots = resolveVaultRoots({ cwd: root, homeDir, env: {} });
    assert.strictEqual(roots.project, path.join(root, '.oas', 'memory', 'project'));
    assert.strictEqual(roots.team, path.join(root, '.oas', 'memory', 'team'));
  } finally {
...
test('honors explicit project and user vault root overrides', () => {
  ...
  try {
    ...
    const roots = resolveVaultRoots({
      ...
      env: {
        OAS_MEMORY_PROJECT_ROOT: projectVault,
        OAS_MEMORY_USER_ROOT: userVault,
      },
    });
...
test('round-trips the strict oas.memory.v1 Markdown frontmatter contract', () => {
...
test('rejects malformed, unknown-schema, and invalid metadata documents', () => {
  ...
  assert.throws(
    () => parseMemoryDocument(
      serializeMemoryDocument(baseMemory()).replace('oas.memory.v1', 'oas.memory.v999'),
      'bad.md'
    ),
...
test('the canonical project guard is honored by git status and check-ignore', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-git-ignore-'));
  const projectRoot = path.join(root, 'project');
  const homeDir = path.join(root, 'home');
...
test('rejects a vault path that traverses a symlink before creating directories', () => {
  ...
  fs.symlinkSync(outside, path.join(fixture.projectRoot, '.oas'));
  try {
    assert.throws(
      () => saveMemory(
...
test('rejects a symlinked ancestor when roots come back from initializeVault', () => {
  ...
  try {
    ...
    fs.rmSync(path.join(fixture.projectRoot, '.oas'), { recursive: true, force: true });
    fs.symlinkSync(outside, path.join(fixture.projectRoot, '.oas'));
...
test('reads a regular file whose handle and path stats are compared', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-identity-'));
...
test('opens regular text files without following a stable symlink', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-file-'));
  const target = path.join(root, 'target.md');
...
test('rejects malformed UTF-8 instead of altering durable text', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-utf8-'));
...
test('opens a file descriptor before inspecting path metadata', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-open-first-'));
  const target = path.join(root, 'target.md');
...
test('detects a swapped file whose inode differs beyond Number precision', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-bigint-ino-'));
  const target = path.join(root, 'target.md');
...
test('rejects a FIFO body path without blocking', () => {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-fifo-'));
...
test('doctor reports malformed files, broken links, duplicate IDs, and skipped symlinks', () => {
  const fixture = createFixture();
  const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-outside-'));
  try {
    saveMemory(
      {
        title: 'Broken link',
