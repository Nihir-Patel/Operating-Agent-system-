/**
 * Tests for the local OAS2 control-pane state projection.
 */
...
async function writeSampleEcc2Database(dbPath) {
  ...
  insertSession.run([
    'lead-hermes',
    'Coordinate Hermes desktop and OAS release work',
    'OAS',
    '2.0-control-pane',
    'claude',
    'claude',
    ...
    'running',
    4242,
    '/tmp/oas-worktrees/hermes',
    'oas/hermes-control-pane',
    'main',
    1200,
    800,
    2000,
  ...
  insertEntity.run([
    ...
    '/notes/hermes.md',
    'How Nihir routes Hermes Desktop, Zellij panes, Devin-style delegation, and OAS release control work.',
  ...
  db.run('INSERT INTO context_graph_observations (session_id, entity_id, observation_type, priority, pinned, summary, details_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
    ...
    3,
    1,
    'Hermes Desktop and OAS should share recall before dispatching work.',
...
async function writeSampleWorkItemsDatabase(dbPath) {
  ...
  insertWorkItem.run([
    ...
    'github-pr',
    '2131',
    'Merge OAS control pane',
    'done',
    'normal',
    'https://github.com/neal0709/Operating-Agent-system-/pull/2131',
    'nihir',
    '/repo/oas',
    null,
    JSON.stringify({
      branch: 'product/oas2-knowledge-control-pane',
  ...
  insertWorkItem.run([
    ...
    'open',
    'high',
    'https://github.com/neal0709/Operating-Agent-system-/issues/2290',
    null,
    '/repo/oas',
    null,
    JSON.stringify({}),
...
async function runTests() {
  ...
  if (
    await test('builds an operator snapshot from OAS2 SQLite and configured connectors', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-state-'));
      const dbPath = path.join(tempDir, 'oas2.db');
      ...
      try {
        ...
        assert.strictEqual(snapshot.schemaVersion, 'oas.control-pane.snapshot.v1');
  ...
  if (
    await test('projects state-store work items into agent Kanban summary', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-work-items-'));
      const dbPath = path.join(tempDir, 'oas2.db');
  ...
  if (
    await test('treats an unreadable optional state-store database as empty work items', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-corrupt-work-items-'));
      const dbPath = path.join(tempDir, 'oas2.db');
  ...
  if (
    await test('resolves config from explicit db path and TOML connector file', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-config-'));
      const dbPath = path.join(tempDir, 'state.db');
      const configPath = path.join(tempDir, 'oas2.toml');
  ...
  if (
    await test('prefers the operator home config over stale app-support config', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-precedence-'));
      const homeDir = path.join(tempDir, 'home');
      const homeConfigDir = path.join(homeDir, '.claude');
      const appConfigDir = path.join(homeDir, 'Library', 'Application Support', 'oas2');
      ...
      try {
        ...
        fs.writeFileSync(path.join(homeConfigDir, 'oas2.toml'), `db_path = "${homeDbPath.replace(/\\/g, '\\\\')}"\n`, 'utf8');
  ...
  if (
    await test('shows configured connectors even when the SQLite database is missing', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-missing-db-'));
  ...
  if (
    await test('handles an existing SQLite database before OAS2 tables are created', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-empty-db-'));
  ...
  if (
    await test('handles malformed JSON rows and all session state counters', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-edge-db-'));
      const dbPath = path.join(tempDir, 'oas2.db');
      ...
      const observations = [
        {
          ...
          priority: 1,
          pinned: false,
          summary: 'Ito should expose market backtesting through OAS tools.'
        }
      ];
