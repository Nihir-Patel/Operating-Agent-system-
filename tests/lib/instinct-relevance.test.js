...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-instinct-relevance-'));
}
...
function runTests() {
  ...
  if (test('isRelevanceRankingEnabled defaults on and honours the opt-out toggle', () => {
    const original = process.env.OAS_INSTINCT_RELEVANCE_RANKING;
    try {
      delete process.env.OAS_INSTINCT_RELEVANCE_RANKING;
      assert.strictEqual(isRelevanceRankingEnabled(), true, 'unset should be on');
      for (const off of ['off', 'OFF', 'false', '0', 'no']) {
        process.env.OAS_INSTINCT_RELEVANCE_RANKING = off;
        assert.strictEqual(isRelevanceRankingEnabled(), false, `${off} should be off`);
      }
      for (const on of ['on', '1', 'true', 'yes', 'anything']) {
        process.env.OAS_INSTINCT_RELEVANCE_RANKING = on;
        assert.strictEqual(isRelevanceRankingEnabled(), true, `${on} should be on`);
      }
    } finally {
      if (original === undefined) delete process.env.OAS_INSTINCT_RELEVANCE_RANKING;
      else process.env.OAS_INSTINCT_RELEVANCE_RANKING = original;
    }
  })) passed++; else failed++;
