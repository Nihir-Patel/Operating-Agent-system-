...
function runTests() {
  ...
  if (test('resolves cursor adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.cursor', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves claude adapter root and install-state path from home dir', () => {
    ...
    const root = adapter.resolveRoot({ homeDir, repoRoot: '/repo/oas' });
    const statePath = adapter.getInstallStatePath({ homeDir, repoRoot: '/repo/oas' });

    assert.strictEqual(root, path.join(homeDir, '.claude'));
    assert.strictEqual(statePath, path.join(homeDir, '.claude', 'oas', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans current Kimi Code project instructions, skills, and MCP config under .kimi-code', () => {
    ...
    assert.strictEqual(
      ...
      path.join(projectRoot, '.kimi-code', 'oas-install-state.json')
    );
    assert.ok(
    ...
    assert.ok(
      ...
      'Should remap OAS Agent Skills into Kimi\'s native skill directory'
    );
    assert.ok(
      plan.operations.some(operation => (
  ...
  if (test('Kimi MCP planning requires an explicit OAS source root', () => {
    assert.throws(
      () => planInstallTargetScaffold({
        target: 'kimi',
  ...
  if (test('plans namespaced Claude rules and flat discoverable skills', () => {
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'oas')
      )),
      'Should install bundled Claude rules under rules/oas'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
  ...
  if (test('plans scaffold operations and flattens native target roots', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.cursor', 'oas-install-state.json'));
  ...
  if (test('does not install root AGENTS.md into Cursor nested context', () => {
    ...
    assert.ok(
      ...
      'Cursor installs should not copy OAS root AGENTS.md into host project context'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('plans cursor agents with oas-prefixed filenames to avoid agent collisions', () => {
    const repoRoot = path.join(__dirname, '..', '..');
    const projectRoot = '/workspace/app';
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(projectRoot, '.cursor', 'agents', 'oas-architect.md')
      )),
      'Should prefix Cursor agent files with oas-'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('exposes validate and planOperations on adapters', () => {
    ...
    assert.deepStrictEqual(
      claudeAdapter.validate({ homeDir: '/Users/example', repoRoot: '/repo/oas' }),
      []
    );
    ...
    assert.deepStrictEqual(
      cursorAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/oas' }),
      []
    );
  })) passed++; else failed++;
  ...
  if (test('resolves codebuddy adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.codebuddy', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves gemini adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.gemini', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves joycode adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.joycode', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves qwen adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.qwen', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('opencode adapter honors config overrides in priority order', () => {
    ...
    assert.strictEqual(
      ...
      path.join(path.resolve(explicitRoot), 'oas-install-state.json')
    );
  })) passed++; else failed++;
  ...
  if (test('opencode adapter isolates an explicit home from ambient config overrides', () => {
    ...
    const child = spawnSync(process.execPath, ['-e', [
      'const { getInstallTargetAdapter } = require(process.env.OAS_TEST_REGISTRY);',
      'const root = getInstallTargetAdapter(\'opencode\').resolveRoot({ homeDir: process.env.OAS_TEST_HOME });',
      'process.stdout.write(JSON.stringify(root));',
    ].join('\n')], {
      encoding: 'utf8',
      env: {
        ...process.env,
        OAS_TEST_REGISTRY: registryPath,
        OAS_TEST_HOME: homeDir,
  ...
  if (test('resolves zed adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.zed', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans codebuddy rules with flat namespaced filenames', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.codebuddy', 'oas-install-state.json'));
  ...
  if (test('plans joycode commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.joycode', 'oas-install-state.json'));
  ...
  if (test('plans qwen commands, agents, skills, and native config under home root', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(homeDir, '.qwen', 'oas-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
  ...
  if (test('plans zed project settings, commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.zed', 'oas-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
  ...
  if (test('resolves adal adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.adal', 'oas-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans adal project rules, skills, and native root sync', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.adal', 'oas-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
  ...
  if (test('exposes validate and planOperations on codebuddy adapter', () => {
    ...
    assert.deepStrictEqual(
      codebuddyAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/oas' }),
      []
    );
  })) passed++; else failed++;

  if (test('every schema target enum value has a matching adapter (regression guard)', () => {
    const schemaPath = path.join(__dirname, '..', '..', 'schemas', 'oas-install-config.schema.json');
  ...
  if (test('resolves claude-project adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.claude', 'oas', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans project-scoped namespaced Claude rules and flat skills', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.claude', 'oas', 'install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'rules', 'oas')
      )),
      'Should install bundled rules under project-scope rules/oas'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
  ...
  if (test('resolves opencode adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(
      statePath,
      path.join(path.resolve(homeDir), '.config', 'opencode', 'oas-install-state.json')
    );
  })) passed++; else failed++;
    ...
    try {
      ...
      require.cache[opencodeHomePath] = {
        ...
        exports: {
          ...
          getInstallStatePath: input => path.join((input.homeDir || '/Users/example'), '.opencode', 'oas-install-state.json'),
          validate: () => ([
