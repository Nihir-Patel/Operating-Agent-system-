...
function tmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-mcp-home-'));
}
...
test('opencode reader isolates an explicit home from ambient config overrides', () => {
  ...
  const child = spawnSync(process.execPath, ['-e', [
    'const { readOpencodeMcp } = require(process.env.OAS_TEST_READER);',
    'const names = readOpencodeMcp({ homeDir: process.env.OAS_TEST_HOME }).map(record => record.name);',
    'process.stdout.write(JSON.stringify(names));',
  ].join('\n')], {
    encoding: 'utf8',
    env: {
      ...process.env,
      OAS_TEST_READER: readerPath,
      OAS_TEST_HOME: home,
