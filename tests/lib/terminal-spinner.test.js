...
test('animator advances frames and exits when its parent disconnects', () => {
  ...
  runAnimator('Applying OAS setup...', {
  ...
  assert.deepStrictEqual(writes, [
    `\r${FRAMES[1]} Applying OAS setup...`,
    `\r${FRAMES[2]} Applying OAS setup...`,
  ]);
  disconnect();
...
test('spinner renders immediately and clears again after animator termination', () => {
  ...
  const spinner = startTerminalSpinner('Applying OAS setup...', {
  ...
  assert.strictEqual(writes[0], `${FRAMES[0]} Applying OAS setup...`);
...
test('real animator advances independently and cannot write after cleanup', () => {
  ...
  assert.match(result.stdout, new RegExp(`${FRAMES[0]} Applying OAS setup`));
  assert.match(result.stdout, new RegExp(`${FRAMES[1]} Applying OAS setup`));
