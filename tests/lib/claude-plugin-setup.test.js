...
function createFixture(initialState = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas plugin setup '));
...
function withFixture(initialState, fn) {
  ...
  const previous = {
    ...
    OAS_TEST_CLAUDE_STATE: process.env.OAS_TEST_CLAUDE_STATE,
    OAS_TEST_CLAUDE_CALLS: process.env.OAS_TEST_CLAUDE_CALLS,
  };
  try {
    ...
    process.env.OAS_TEST_CLAUDE_STATE = fixture.statePath;
    process.env.OAS_TEST_CLAUDE_CALLS = fixture.callsPath;
    return fn(fixture);
  } finally {
...
function writeManagedState(fixture, selectedModules, operations = []) {
  const statePath = path.join(fixture.configDir, 'oas', 'install-state.json');
  fs.mkdirSync(path.dirname(statePath), { recursive: true });
  fs.writeFileSync(statePath, `${JSON.stringify({
    schemaVersion: 'oas.install.v1',
...
test('Windows command-line fallback preserves spaced paths and JSON arguments', () => {
  assert.strictEqual(
    buildWindowsCommandLine(
      ...
      ['plugin', 'install', 'oas@oas', '--config', '{"hooks_enabled":false}']
    ),
    '"C:\\Program Files\\Claude\\claude.cmd" plugin install oas@oas --config "{""hooks_enabled"":false}"'
  );
  assert.throws(
...
test('provider runner times out a hung Claude command with structured context', () => {
  ...
  const spawn = (command, args, options) => {
    ...
    assert.deepStrictEqual(args, ['plugin', 'marketplace', 'update', 'oas']);
...
test('marketplace provenance is validated according to its source type', () => {
  ...
  assert.strictEqual(isOfficialMarketplace({
    name: 'oas',
    source: 'git',
    url: 'https://github.com/neal0709/Operating-Agent-system-.git',
  }), true);
  for (const url of [
    'neal0709/Operating-Agent-system-',
    'http://github.com/neal0709/Operating-Agent-system-.git',
  ]) {
    assert.strictEqual(isOfficialMarketplace({
      name: 'oas',
      source: 'git',
...
test('same-scope repeat setup updates OAS and changes durable user hook preferences', () => {
  withFixture({
    ...
    assert.deepStrictEqual(settings.pluginConfigs['oas@oas'].futureKey, { keep: true });
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.unknown, 'keep');
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hooks_enabled, false);
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hook_profile, 'standard');
...
test('setup preserves an explicit attribution opt-in', () => {
  withFixture({
    ...
    // `attribution` wins over `includeCoAuthoredBy` in Claude Code, so OAS must not
...
test('legacy plugin inventory fails closed before marketplace or plugin mutation', () => {
  withFixture({
    plugins: [{
      id: 'operating-agent-systems@operating-agent-systems',
      scope: 'user',
      enabled: true,
    }],
  }, fixture => {
...
test('skills-directory OAS plugins fail closed before marketplace or plugin mutation', () => {
  withFixture({
    plugins: [{
      id: 'oas@skills-dir',
      scope: 'user',
    ...
    assertThrowsContaining(
      () => setupClaudePlugin(setupOptions(fixture, { scope: 'user' })),
      ['oas@skills-dir', 'duplicate', 'uninstall']
    );
    assert.deepStrictEqual(mutationCalls(readCalls(fixture)), []);
  });
});
...
test('manual plugin layouts fail closed before provider mutation', () => {
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(manualManifest, JSON.stringify({ name: 'oas' }));
    assertThrowsContaining(
      () => setupClaudePlugin(setupOptions(fixture, { scope: 'user' })),
      ['manual', 'oas']
    );
...
test('duplicate OAS plugin scopes fail closed before mutation', () => {
  withFixture({
...
test('malformed plugin JSON and malformed plugin entries fail closed', () => {
  for (const pluginListResponses of [['{not-json'], [[{ id: 'oas@oas', enabled: true }]]]) {
    ...
    },
    {
      initial: {
        marketplaces: [{
          name: 'oas',
          source: 'git',
          url: 'https://github.com/example/not-oas.git',
          scope: 'user',
        }],
      },
...
test('dry-run snapshots local-scope inventory into isolated Claude and project roots', () => {
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(projectSettingsPath, '{"enabledPlugins":{"oas@oas":true}}\n');
    ...
    fs.writeFileSync(marketplacesPath, `${JSON.stringify({
      oas: {
        source: { source: 'github', repo: 'neal0709/Operating-Agent-system-' },
      },
    }, null, 2)}\n`);
    ...
    const runClaude = (args, runOptions) => {
      ...
      assert.strictEqual(
        shadowInstalled.plugins['oas@oas'][0].projectPath,
        runOptions.cwd
      );
      assert.ok(
        shadowInstalled.plugins['oas@oas'][0].installPath
