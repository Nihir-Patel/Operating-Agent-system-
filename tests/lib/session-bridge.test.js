...
function runTests() {
  ...
  if (
    test('returns path containing oas-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('oas-metrics-'), `Expected oas-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses OAS_SESSION_ID env var', () => {
      const original = process.env.OAS_SESSION_ID;
      try {
        process.env.OAS_SESSION_ID = 'env-session-42';
        const result = resolveSessionId();
        assert.strictEqual(result, 'env-session-42');
      } finally {
        if (original === undefined) {
          delete process.env.OAS_SESSION_ID;
        } else {
          process.env.OAS_SESSION_ID = original;
        }
      }
    })
  )
