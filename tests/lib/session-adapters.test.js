...
test('dmux adapter normalizes orchestration snapshots into canonical form', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-recordings-'));
  ...
  try {
    ...
    assert.strictEqual(snapshot.schemaVersion, 'oas.session.v1');
...
test('claude-history adapter loads the latest recorded session', () => {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-adapter-home-'));
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-recordings-'));
  ...
  fs.writeFileSync(sessionPath, [
    ...
    '**Started:** 09:00',
    '**Last Updated:** 11:30',
    '**Project:** operating-agent-systems',
    '**Branch:** feat/session-adapter',
    '**Worktree:** /tmp/oas-worktree',
    '',
    '### Completed',
  ...
  try {
    withHome(homeDir, () => {
      ...
      assert.strictEqual(snapshot.workers[0].worktree, '/tmp/oas-worktree');
...
test('adapter registry routes plan files to dmux and explicit claude targets to history', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-registry-repo-'));
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-registry-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
...
test('adapter registry resolves structured target types into the correct adapter', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-typed-repo-'));
  const planPath = path.join(repoRoot, 'workflow.json');
  fs.writeFileSync(planPath, JSON.stringify({
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-typed-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, '2026-03-13-z9y8x7w6-session.tmp'),
...
test('canonical snapshot validation rejects malformed required fields', () => {
  const invalidCases = [
    ...
    [canonicalSnapshot({ schemaVersion: 'oas.session.v0' }), /Unsupported canonical session schema version/],
...
test('claude history normalization falls back to filename ids and empty metadata defaults', () => {
  ...
  assert.deepStrictEqual(pathOnly.workers[0].intent.seedPaths, ['README.md', 'scripts/oas.js']);
...
test('fallback recordings sanitize paths, use env dirs, and preserve changed history', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-recordings-env-'));
  const previousRecordingDir = process.env.OAS_SESSION_RECORDING_DIR;

  try {
    process.env.OAS_SESSION_RECORDING_DIR = recordingDir;
    ...
    assert.strictEqual(persisted.schemaVersion, 'oas.session.recording.v1');
    ...
    if (typeof previousRecordingDir === 'string') {
      process.env.OAS_SESSION_RECORDING_DIR = previousRecordingDir;
    } else {
      delete process.env.OAS_SESSION_RECORDING_DIR;
    }
    fs.rmSync(recordingDir, { recursive: true, force: true });
  }
});
...
test('persistence supports skip mode, writer variants, and missing state-store fallback', () => {
  ...
  const noWriterDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-no-writer-'));
  const missingModuleDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-missing-module-'));
  try {
