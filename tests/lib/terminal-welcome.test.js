...
test('renders the cfonts block OAS wordmark with a welcome, version, and boxed links', () => {
  ...
  assert.match(welcome, /Welcome to OAS!/);
  assert.ok(welcome.includes(`v${OAS_VERSION}`));
  assert.ok(boxTop > 0);
...
test('renders an explicitly verified installed version when provided', () => {
  ...
  assert.strictEqual(welcome.includes(`v${OAS_VERSION}`), OAS_VERSION === '2.1.0');
});
