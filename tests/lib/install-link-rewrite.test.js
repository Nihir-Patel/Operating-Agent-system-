...
// rules/<x> -> rules/oas/<x>. Mirrors what the real adapter emits.
function claudeNamespaceMappings() {
  return [
    ...
    { sourceRel: 'rules/react/hooks.md', destRel: 'rules/oas/react/hooks.md' },
    { sourceRel: 'rules/react/testing.md', destRel: 'rules/oas/react/testing.md' },
    { sourceRel: 'rules/react/coding-style.md', destRel: 'rules/oas/react/coding-style.md' },
  ];
}
...
function runTests() {
  ...
  for (const skill of ['react-patterns', 'react-performance', 'react-testing']) {
    if (test(`rewrites ../../rules file link for ${skill}`, () => {
      ...
      assert.ok(
        after.includes('](../../rules/oas/react/hooks.md)'),
  ...
  if (test('rewrites a directory link and preserves the trailing slash', () => {
    ...
    assert.ok(after.includes('](../../rules/oas/react/)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('preserves a #fragment on a rewritten link', () => {
    ...
    const after = rewriteRelativeLinks(before, { sourceRel: 'skills/react-patterns/SKILL.md', index });
    assert.ok(after.includes('](../../rules/oas/react/hooks.md#use-effect)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('does not rewrite links inside fenced code blocks', () => {
    ...
    assert.ok(after.includes('[prose](../../rules/oas/react/hooks.md)'), 'prose link must be rewritten');
  })) passed++; else failed++;
  ...
  if (test('computes depth from path math for a nested skill file', () => {
    ...
    assert.ok(after.includes('](../../../rules/oas/react/hooks.md)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('real React skills: rewritten rules links resolve to installed targets', () => {
    ...
    const plan = createManifestInstallPlan({
      ...
      homeDir: '/tmp/oas-link-rewrite-it',
      target: 'claude',
