...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-install-projection-'));
}

function createState(options = {}) {
  const targetRoot = options.targetRoot;
  const installStatePath = options.installStatePath || path.join(targetRoot, 'oas-install-state.json');
    ...
    try {
      ...
      const result = reconcileInstallStateProjections(store, [
        discoveryRecord(state),
        {
          ...
          targetRoot: absentRoot,
          installStatePath: path.join(absentRoot, 'oas-install-state.json'),
          exists: false,
          state: null,
          error: null,
          legacy: false,
