/**
 * Tests for the SQLite-backed OAS state store and CLI commands.
 */
...
const OAS_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'oas.js');
...
const mode = process.env.OAS_FAKE_GH_MODE || 'open';
...
if (args[0] === 'pr' && args[1] === 'list') {
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/neal0709/operating-agent-systems/pull/3',
    ...
    },
    {
      ...
      url: 'https://github.com/neal0709/operating-agent-systems/pull/4',
  ...
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/neal0709/operating-agent-systems/issues/9',
...
async function seedStore(dbPath) {
      ...
      workers: [
        {
          ...
          worktree: '/tmp/oas-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/oas-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline OAS 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for OAS control plane',
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('oas-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.claude', 'oas', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('oas-state-memory-');
  ...
  if (await test('creates private state-store directories and atomically persists a private database file', async () => {
    const testDir = createTempDir('oas-state-private-');
    const privateParent = path.join(testDir, 'new-parent', 'oas');
  ...
  if (await test('refuses a final state database symlink without changing its target', async () => {
    const testDir = createTempDir('oas-state-final-link-');
  ...
  if (await test('refuses an intermediate state database symlink without writing outside the requested tree', async () => {
    const testDir = createTempDir('oas-state-parent-link-');
    const outsideDir = path.join(testDir, 'outside');
    const linkedParent = path.join(testDir, 'linked-parent');
    const dbPath = path.join(linkedParent, 'oas', 'state.db');
    ...
    try {
      ...
      assert.strictEqual(fs.existsSync(path.join(outsideDir, 'oas', 'state.db')), false);
    } finally {
      cleanupTempDir(testDir);
    }
  })) passed += 1; else failed += 1;
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('oas-state-db-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('builds an empty status snapshot with null rates and missing install health', async () => {
    const testDir = createTempDir('oas-state-empty-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('tracks linked work items for Linear, GitHub, and handoff progress', async () => {
    const testDir = createTempDir('oas-state-work-items-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const linearItem = store.upsertWorkItem({
        id: 'linear-oas-20-control-plane',
        source: 'linear',
        sourceId: 'OAS-20',
        title: 'Define harness-neutral session/worktree contract',
        status: 'in-progress',
        priority: 'high',
        url: 'https://linear.app/oastools/issue/OAS-20',
        owner: 'control-plane',
        repoRoot: '/tmp/oas-repo',
        sessionId: 'session-active',
        metadata: {
          project: 'OAS 2.0: Control Plane',
        },
      ...
      store.upsertWorkItem({
        ...
        source: 'handoff',
        sourceId: 'oas-rc1-release-decision-20260511.md',
      ...
      store.upsertWorkItem({
        ...
        status: 'merged',
        priority: 'normal',
        url: 'https://github.com/neal0709/operating-agent-systems/pull/1738',
        owner: 'maintainer',
      ...
      assert.strictEqual(linearItem.id, 'linear-oas-20-control-plane');
      assert.strictEqual(linearItem.metadata.project, 'OAS 2.0: Control Plane');
  ...
  if (await test('normalizes default optional fields and reports warning install health', async () => {
    const testDir = createTempDir('oas-state-defaults-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('rejects invalid limits and unserializable JSON payloads', async () => {
    const testDir = createTempDir('oas-state-errors-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('oas-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI --exit-code reports attention without suppressing output', async () => {
    const attentionDir = createTempDir('oas-state-attention-');
    const okDir = createTempDir('oas-state-ok-');
  ...
  if (await test('status CLI can emit and write markdown operator snapshots', async () => {
    ...
    try {
      ...
      assert.match(written, /^# OAS Status/m);
  ...
  if (await test('work-items CLI supports upsert, list, show, and close', async () => {
    const testDir = createTempDir('oas-work-items-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const upsertResult = runNode(WORK_ITEMS_SCRIPT, [
        ...
        'high',
        '--url',
        'https://linear.app/example/issue/OAS-99',
        '--owner',
        'control-plane',
        '--metadata-json',
        '{"project":"OAS 2.0"}',
      ...
      assert.strictEqual(listPayload.items[0].id, 'linear-oas-99');

      const showResult = runNode(WORK_ITEMS_SCRIPT, ['show', 'linear-oas-99', '--db', dbPath]);
      assert.strictEqual(showResult.status, 0, showResult.stderr);
      assert.match(showResult.stdout, /linear\/#OAS-99 in-progress: Ship work item CLI/);

      const closeResult = runNode(WORK_ITEMS_SCRIPT, ['close', 'linear-oas-99', '--db', dbPath, '--json']);
  ...
  if (await test('work-items CLI syncs GitHub PRs and issues into readiness', async () => {
    const testDir = createTempDir('oas-work-items-github-');
    const dbPath = path.join(testDir, 'state.db');
    const binDir = path.join(testDir, 'bin');
    const repo = 'neal0709/operating-agent-systems';

    try {
      const env = {
        OAS_GH_SHIM: createGhShim(binDir),
      };
      ...
      assert.strictEqual(syncPayload.items[0].id, 'github-neal0709-operating-agent-systems-pr-3');
  ...
  if (await test('oas CLI delegates the new status, sessions, and work-items subcommands', async () => {
    const testDir = createTempDir('oas-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const statusResult = runNode(OAS_SCRIPT, ['status', '--db', dbPath, '--json']);
      ...
      const sessionsResult = runNode(OAS_SCRIPT, ['sessions', 'session-active', '--db', dbPath, '--json']);
      ...
      const workItemResult = runNode(OAS_SCRIPT, [
        'work-items',
        'upsert',
        'handoff-roadmap',
        '--db',
      ...
      const delegatedStatusResult = runNode(OAS_SCRIPT, ['status', '--db', dbPath, '--json']);
