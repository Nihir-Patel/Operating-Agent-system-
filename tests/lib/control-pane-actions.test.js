/**
 * Tests for allowlisted OAS2 control-pane actions.
 */
...
function runTests() {
  ...
  if (test('builds copyable and executable allowlisted OAS2 actions', () => {
    ...
    assert.strictEqual(sync.cwd, path.join(repoRoot, 'oas2'));
  ...
  if (test('rejects unknown action identifiers', () => {
    assert.throws(
      () => buildControlPaneAction('rm -rf', { repoRoot: '/repo/oas' }),
