...
function runTests() {
  ...
  if (test('preserves recorded legacy content when the current OAS source has changed', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'antigravity-source-drift-'));
    try {
      const legacy = seedLegacyState(projectRoot, [{
        relativePath: 'rules/common-coding-style.md',
        sourceRelativePath: 'rules/common/coding-style.md',
        recordedContent: 'historical OAS content\n',
      }]);
      const sourcePath = path.join(projectRoot, 'source.md');
      fs.writeFileSync(sourcePath, 'canonical managed\n', 'utf8');
      ...
      assert.ok(result.warnings.some(warning => warning.includes(
        'current OAS source differs from the recorded installed content'
      )));
    } finally {
      fs.rmSync(projectRoot, { recursive: true, force: true });
    }
  ...
  if (test('list discovery and doctor report both canonical and remaining legacy states', () => {
    ...
    try {
      ...
      const canonicalStatePath = path.join(canonicalRoot, 'oas-install-state.json');
      writeInstallState(
        canonicalStatePath,
  ...
  if (test('does not discover mismatched or symlinked legacy state', () => {
    ...
    try {
      ...
      const mismatchedStatePath = path.join(mismatchedRoot, 'oas-install-state.json');
      ...
      if (process.platform !== 'win32') {
        const symlinkStatePath = path.join(externalRoot, 'oas-install-state.json');
        const linkedRoot = path.join(symlinkProject, '.agent');
        fs.symlinkSync(externalRoot, linkedRoot, 'dir');
        writeInstallState(
          symlinkStatePath,
          createAntigravityState(linkedRoot, path.join(linkedRoot, 'oas-install-state.json'))
        );
  ...
  if (test('reports corrupt legacy state instead of treating it as absent', () => {
    ...
    try {
      ...
      const legacyStatePath = path.join(legacyRoot, 'oas-install-state.json');
