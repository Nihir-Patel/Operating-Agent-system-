...
function runTests() {
  ...
  if (test('marks unified-memory install surfaces as requiring the separate OAS runtime', () => {
    const component = getInstallComponent('skill:unified-memory');
    assert.deepStrictEqual(component.moduleIds, ['skill-unified-memory']);
    assert.match(component.description, /oas-universal/i);
    ...
    assert.match(singleSkillModule.description, /oas-universal/i);
    assert.match(singleSkillModule.description, /separate|external/i);
    assert.match(workflowModule.description, /oas-universal/i);
