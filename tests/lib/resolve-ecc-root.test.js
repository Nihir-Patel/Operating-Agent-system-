/**
 * Tests for scripts/lib/resolve-oas-root.js
 *
 * Covers the OAS root resolution fallback chain:
...
const { resolveEccRoot, INLINE_RESOLVE } = require('../../scripts/lib/resolve-oas-root');

// Sentinel OAS skill that resolveEccRoot() requires (alongside the script tree)
...
const OAS_SKILL_SENTINEL = path.join('skills', 'continuous-learning-v2');
...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-root-test-'));
}
...
function setupStandardInstall(homeDir) {
  ...
  fs.mkdirSync(path.join(claudeDir, OAS_SKILL_SENTINEL), { recursive: true });
  return claudeDir;
}
...
function setupLegacyPluginInstall(homeDir, segments) {
  ...
  fs.mkdirSync(scriptDir, { recursive: true });
  fs.writeFileSync(path.join(scriptDir, 'utils.js'), '// stub');
  fs.mkdirSync(path.join(legacyDir, OAS_SKILL_SENTINEL), { recursive: true });
  return legacyDir;
}
function setupPluginCache(homeDir, pluginSlug, orgName, version) {
  ...
  fs.mkdirSync(scriptDir, { recursive: true });
  fs.writeFileSync(path.join(scriptDir, 'utils.js'), '// stub');
  fs.mkdirSync(path.join(cacheDir, OAS_SKILL_SENTINEL), { recursive: true });
  return cacheDir;
}

function runTests() {
  console.log('\n=== Testing resolve-oas-root.js ===\n');
  ...
  if (test('finds current plugin install at ~/.claude/plugins/oas', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['oas']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('finds current plugin install at ~/.claude/plugins/oas@oas', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['oas@oas']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/operating-agent-systems', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['operating-agent-systems']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/operating-agent-systems@operating-agent-systems', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['operating-agent-systems@operating-agent-systems']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds marketplace current plugin install at ~/.claude/plugins/marketplaces/oas', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'oas']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds marketplace legacy plugin install at ~/.claude/plugins/marketplaces/operating-agent-systems', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'operating-agent-systems']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('prefers exact legacy plugin install over plugin cache', () => {
    ...
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'oas']);
      setupPluginCache(homeDir, 'oas', 'neal0709', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'oas', 'neal0709', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('handles multiple versions in plugin cache', () => {
    const homeDir = createTempDir();
    try {
      setupPluginCache(homeDir, 'operating-agent-systems', 'legacy-org', '1.7.0');
      const expected = setupPluginCache(homeDir, 'oas', 'neal0709', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      // Should find one of them (either is valid)
      assert.ok(
        result === expected ||
        result === path.join(homeDir, '.claude', 'plugins', 'cache', 'operating-agent-systems', 'legacy-org', '1.7.0'),
  ...
  if (test('rejects a partial ~/.claude (scripts + non-OAS skills) and prefers a complete root (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // ~/.claude has OAS's scripts and a user's OWN skills/ dir, but not OAS's
      ...
      // A COMPLETE OAS root exists in the plugin cache (scripts + OAS skill).
      const expected = setupPluginCache(homeDir, 'oas', 'neal0709', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected,
  ...
  if (test('rejects a partial exact plugin root (scripts, no OAS skill) and prefers a complete root (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // An exact plugin root under ~/.claude/plugins/oas ships OAS's scripts but
      // not OAS's skills. The stricter predicate must reject it on the
      // exact-plugin branch too, not only for ~/.claude.
      const partialScripts = path.join(homeDir, '.claude', 'plugins', 'oas', 'scripts', 'lib');
  ...
  if (test('rejects a partial plugin-cache root (scripts, no OAS skill) and falls back to ~/.claude (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // A versioned plugin-cache root ships OAS's scripts but not OAS's skills.
      ...
      const cacheScripts = path.join(
        homeDir, '.claude', 'plugins', 'cache', 'oas', 'neal0709', CURRENT_PACKAGE_VERSION,
        'scripts', 'lib'
      );
  ...
  if (test('custom probe skips a qualifying root that lacks the probed script (auto-update)', () => {
    ...
    // carry full resolver evidence (script tree + sentinel OAS skill) yet
    ...
    try {
      const claudeDir = setupStandardInstall(homeDir);
      const marketplaceRoot = setupLegacyPluginInstall(homeDir, ['marketplaces', 'oas']);
  ...
  if (test('INLINE_RESOLVE delegates to committed resolver when env var is unset', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(resolverDir, 'resolve-oas-root.js'), `module.exports = { resolveEccRoot() { return 'delegated:' + process.env.INLINE_RESOLVE_MARKER; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
  ...
  if (test('INLINE_RESOLVE loads the committed resolver module from home base', () => {
    ...
    try {
      const resolverDir = path.join(homeDir, '.claude', 'scripts', 'lib');
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-oas-root.js'), `const assert = require('assert');
module.exports = { resolveEccRoot() { assert.strictEqual(process.env.HOME, ${JSON.stringify(homeDir)}); return 'module-loaded'; } };`);
  ...
  if (test('INLINE_RESOLVE bootstraps module from an exact plugin root when env unset', () => {
    const homeDir = createTempDir();
    try {
      const resolverDir = path.join(homeDir, '.claude', 'plugins', 'oas', 'scripts', 'lib');
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-oas-root.js'), `module.exports = { resolveEccRoot() { return 'plugin-root'; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
  ...
  if (test('INLINE_RESOLVE bootstraps module from the versioned plugin cache when env unset', () => {
    ...
    try {
      ...
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-oas-root.js'), `module.exports = { resolveEccRoot() { return 'cache-root'; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
