 ...
 * manifest points directly at OAS's canonical `skills/` and `commands/`
...
function main() {
  ...
  const tests = [
    ...
    ["pi.extensions is exactly the single OAS adapter entry file, and it exists on disk", () => {
      assert.deepStrictEqual(
    ...
    ["pi.skills and pi.prompts point at OAS's canonical top-level directories, never at .pi/", () => {
      assert.deepStrictEqual(
        packageJson.pi.skills,
        ["./skills"],
        `pi.skills must be exactly ["./skills"] (OAS's canonical skills directory) — got ${JSON.stringify(packageJson.pi.skills)}`
      )
      assert.deepStrictEqual(
        packageJson.pi.prompts,
        ["./commands"],
        `pi.prompts must be exactly ["./commands"] (OAS's canonical commands directory) — got ${JSON.stringify(packageJson.pi.prompts)}`
      )
      for (const entry of [...packageJson.pi.skills, ...packageJson.pi.prompts]) {
        assert.ok(
          !entry.startsWith("./.pi") && !entry.includes(".pi/"),
          `pi.skills/pi.prompts entry "${entry}" must not point under .pi/ — Pi must mount OAS's canonical assets directly, never a copy generated into the adapter directory`
        )
      }
    }],
