/**
 * Tests for the OAS <-> Pi coding agent thin adapter (.pi/extensions/index.ts).
 *
 ...
 *       OAS package root, which breaks global installs;
 ...
 *
 * Group 2 exercises OAS's real hook runner (`scripts/hooks/run-with-flags.js`)
 ...
 *
 * Group 4 covers the adapter's injection of OAS's canonical engineering rules
 ...
 */
function runHookRunner(oasRoot, hookId, relScript, profiles, payload, extraEnv, cwd) {
  const runner = path.join(oasRoot, "scripts", "hooks", "run-with-flags.js")
  return spawnSync(process.execPath, [runner, hookId, relScript, profiles], {
    input: JSON.stringify(payload),
    encoding: "utf8",
    cwd: cwd || oasRoot,
    timeout: 30000,
    env: { ...process.env, CLAUDE_PLUGIN_ROOT: oasRoot, OAS_PLUGIN_ROOT: oasRoot, ...extraEnv },
  })
}

/**
 * Builds a minimal, standalone OAS package skeleton under a fresh temp
...
function buildEccSkeleton(repoRoot) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "oas pi test-"))
 ...
 * `"\n\n---\n\n"` join). Deliberately omits the `OAS_PI_RULES` disable check
 ...
 * below reads the real function and the real `OAS_PI_RULES` env var name out
...
async function main() {
  ...
  const tests = [
    ...
    ["resolves the OAS package root from __dirname, never from process.cwd()", () => {
      assert.ok(
        ...
        "expected the adapter to derive its package root with path.resolve(__dirname, ...); " +
          "resolving from __dirname is what makes a globally installed OAS find its own hooks " +
      ...
      assert.ok(
        ...
        "found process.cwd() used as executable code in .pi/extensions/index.ts; " +
          ...
          "because it looks for OAS's hooks inside the user's project instead of the " +
          "installed OAS package (this is the exact defect PR #2352 was rejected for)"
      )
    }],
    ...
    ["registers the oas-doctor diagnostics command", () => {
      assert.ok(
        extensionSource.includes(`registerCommand("oas-doctor"`),
        "expected the adapter to register an 'oas-doctor' command via pi.registerCommand(...) " +
    ...
    ["propagates the OAS package root to hooks via CLAUDE_PLUGIN_ROOT and OAS_PLUGIN_ROOT", () => {
      assert.ok(
        extensionSource.includes("CLAUDE_PLUGIN_ROOT"),
        "expected the adapter to set CLAUDE_PLUGIN_ROOT in the hook environment; OAS's " +
          "shared hook scripts read this to locate the package root"
      )
      assert.ok(
        extensionSource.includes("OAS_PLUGIN_ROOT"),
        "expected the adapter to set OAS_PLUGIN_ROOT in the hook environment; this is " +
          "the OAS-specific fallback the same hook scripts also read"
      )
    }],
    ...
    ["hook resolution is package-relative, not cwd-relative: still succeeds when cwd points elsewhere", () => {
      ...
      try {
        ...
        assert.strictEqual(
          result.error,
          undefined,
          "hook runner failed to spawn when cwd pointed away from the OAS package root; " +
            "a globally installed OAS must resolve its own hooks regardless of which " +
            `project directory the user is in (error: ${result.error && result.error.message})`
        )
        assert.strictEqual(
          result.status,
          0,
          "hook runner exited non-zero when cwd pointed away from the OAS package root " +
    ...
    ["profile gating is honored: a disabled hook and a restrictive profile both degrade cleanly", () => {
      ...
      try {
        const disabledResult = runHookRunner(
          ...
          "minimal,standard,strict",
          { hook_event_name: "SessionEnd", reason: "quit", cwd: skeletonRoot, session_id: "pi-adapter-test" },
          { OAS_DISABLED_HOOKS: "session:end:marker" }
        )
        ...
        assert.strictEqual(
          ...
          "hook runner failed to spawn when session:end:marker was listed in " +
            `OAS_DISABLED_HOOKS (error: ${disabledResult.error && disabledResult.error.message})`
        )
        assert.strictEqual(
          disabledResult.status,
          0,
          "hook runner exited non-zero for a hook disabled via OAS_DISABLED_HOOKS; a " +
        ...
        const minimalResult = runHookRunner(
          ...
          "minimal,standard,strict",
          { hook_event_name: "SessionEnd", reason: "quit", cwd: skeletonRoot, session_id: "pi-adapter-test" },
          { OAS_HOOK_PROFILE: "minimal" }
        )

        assert.strictEqual(
          minimalResult.error,
          undefined,
          "hook runner failed to spawn under OAS_HOOK_PROFILE=minimal " +
            `(error: ${minimalResult.error && minimalResult.error.message})`
        )
        assert.strictEqual(
          minimalResult.status,
          0,
          "hook runner exited non-zero under OAS_HOOK_PROFILE=minimal; hook-profile " +
    ...
    ["additionalContext extraction tolerates non-JSON hook passthrough", () => {
      ...
      assert.strictEqual(
        ...
        "expected an empty-string additionalContext to yield undefined rather than an " +
          "empty <oas-session-context> block being spliced into the system prompt"
      )
    ...
    ["stale context is cleared at session_start before awaiting the hook, and again after injection in before_agent_start", () => {
      ...
      // inline inside its <oas-session-context> block instead of copying it to
      // a local first; both orders are equivalent in a synchronous handler.
      const captureIdx = beforeAgentStartSource.indexOf("<oas-session-context>")
      ...
      assert.ok(
        ...
        "expected the before_agent_start handler in .pi/extensions/index.ts to read " +
          "pendingContext into an <oas-session-context> block before clearing it"
      )
      assert.ok(
        clearIdx2 !== -1,
    ...
    ["companion package detection reads Pi's package list (source contract): require.resolve is gone, PI_CODING_AGENT_DIR is honored, and normalizePiPackageName's version-stripping guard is pinned", () => {
      ...
      assert.ok(
        ...
        "expected normalizePiPackageName in .pi/extensions/index.ts to read `source` off " +
          ...
          "object form as unrecognized makes /oas-doctor report an installed companion as " +
          "missing"
      )
      assert.ok(
    ...
    ["companion package name normalization (behavioral mirror): an object entry with resource filters resolves to the same name as the bare source string", () => {
      assert.strictEqual(
        ...
        "expected the object form Pi documents for filtered packages to resolve to the " +
          ...
          "contributes still has it installed, and /oas-doctor exists to report exactly that"
      )
      assert.strictEqual(
    ...
    ["PORTABLE_RULE_FILES lists exactly OAS's 7 Pi-portable rule files and excludes the 3 Claude-Code-only ones", () => {
      ...
      assert.deepStrictEqual(
        ...
        "expected PORTABLE_RULE_FILES in .pi/extensions/index.ts to contain exactly these " +
          `7 files (got: ${ruleFiles.join(", ")}); a drift here silently changes which OAS ` +
    ...
    ["engineering rules are read from rules/common/ joined onto the package root at runtime, and nothing is copied into .pi/", () => {
      const withoutComments = stripComments(extensionSource)
      assert.ok(
        /path\.join\(\s*OAS_ROOT\s*,\s*["'`]rules["'`]\s*,\s*["'`]common["'`]/.test(withoutComments),
        "expected .pi/extensions/index.ts to build the rules directory via " +
          'path.join(OAS_ROOT, "rules", "common", ...); rules must be read at runtime from ' +
          "the canonical rules/common/ directory of the installed OAS package, which is " +
      ...
      assert.ok(
        ...
        `found ${piRulesDir} on disk; OAS's engineering rules must be read at runtime from ` +
    ...
    ["loadPortableRules() behavioral mirror: concatenates the real rules/common/ files, stays under the cap, and contains markers from several rule files", () => {
      ...
      assert.ok(
        ...
        "expected loadPortableRules() to return a non-empty string when run against this " +
          "repo's real rules/common/ files; an empty result means the <oas-engineering-rules> " +
    ...
    ["/oas-doctor reports rule files actually loaded, not the allowlist length (source contract)", () => {
      assert.ok(
      ...
      const disabledBranch = extensionSource.slice(
        extensionSource.indexOf("isDisabledByEnv(process.env.OAS_PI_RULES)"),
        extensionSource.indexOf("const sections: string[] = []")
      )
      assert.ok(
        /cachedRuleFileCount\s*=\s*0/.test(disabledBranch),
        "expected the OAS_PI_RULES disable branch of loadPortableRules in " +
      ...
      assert.ok(
        ...
        "expected describeRulesStatus in .pi/extensions/index.ts to report the loaded count " +
          ...
          "alone makes an install that loaded 3 of 7 report 7 -- and /oas-doctor is the one " +
          "place a user looks to find a partial install"
      )
    }],

    ["isDisabledByEnv() behavioral mirror: recognizes 0/false/off/none/disabled case- and whitespace-insensitively, and the real function reads OAS_PI_RULES", () => {
      ...
      assert.strictEqual(
        ...
        'expected isDisabledByEnv("  OFF  ") to be true (mixed case AND surrounding whitespace ' +
          "at once); a user pasting OAS_PI_RULES=\"  OFF  \" into a shell profile must still " +
          "disable injection"
      )
      ...
      for (const enabledValue of [undefined, "", "1", "true", "on", "yes", "TRUE ISH"]) {
        assert.strictEqual(
          ...
          `expected isDisabledByEnv(${JSON.stringify(enabledValue)}) to be false; treating an ` +
            ...
            "who sets OAS_PI_RULES to something other than the 5 documented off-values"
        )
      }

      const withoutComments = stripComments(extensionSource)
      assert.ok(
        withoutComments.includes("process.env.OAS_PI_RULES"),
        "expected .pi/extensions/index.ts to read process.env.OAS_PI_RULES as the env var " +
          "that turns rule injection off; a different or renamed env var would silently break " +
          "anyone's existing OAS_PI_RULES=off configuration"
      )
    }],
    ...
    ["before_agent_start wraps rules and context in their tags, consumes pendingContext but never the rules, and returns early with no override when there is nothing to add", () => {
      ...
      assert.ok(
        handlerSource.includes("<oas-engineering-rules>"),
        "expected the before_agent_start handler in .pi/extensions/index.ts to wrap " +
          "injected rules in an <oas-engineering-rules> tag"
      )
      assert.ok(
        handlerSource.includes("<oas-session-context>"),
        "expected the before_agent_start handler in .pi/extensions/index.ts to wrap the " +
          "session context in an <oas-session-context> tag"
      )

      const contextPushIdx = handlerSource.indexOf("<oas-session-context>")
      ...
      assert.ok(
        ...
        "expected before_agent_start to clear pendingContext = undefined after using it to " +
          "build the <oas-session-context> block; without this, the same one-shot session " +
