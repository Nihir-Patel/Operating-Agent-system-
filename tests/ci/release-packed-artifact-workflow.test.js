...
for (const workflowPath of workflowPaths) {
  ...
  test(`${workflowPath} uploads the one packed tgz as the release artifact`, () => {
    ...
    assert.match(verify, /name:\s*oas-release-artifacts/);
  ...
  test(`${workflowPath} verifies the same tgz on Node 20 across three operating systems`, () => {
    ...
    assert.match(lifecycle, /name:\s*oas-release-artifacts/);
    assert.match(lifecycle, /OAS_RELEASE_PACKAGE:\s*release-artifacts\/\$\{\{ needs\.verify\.outputs\.package_file \}\}/);
    assert.match(lifecycle, /OAS_RELEASE_SHA256:\s*\$\{\{ needs\.verify\.outputs\.package_sha256 \}\}/);
  ...
  test(`${workflowPath} blocks publishing on packed-artifact lifecycle success`, () => {
    ...
    assert.match(publish, /OAS_RELEASE_PACKAGE:\s*\$\{\{ needs\.verify\.outputs\.package_file \}\}/);
    assert.match(publish, /npm publish "\.\/\$\{OAS_RELEASE_PACKAGE\}"/);
    assert.match(publish, /name:\s*Verify artifact before publish/);
    assert.match(publish, /OAS_RELEASE_SHA256:\s*\$\{\{ needs\.verify\.outputs\.package_sha256 \}\}/);
    assert.match(publish, /createHash\(['"]sha256['"]\)/);
    assert.match(publish, /oas-universal-\[0-9A-Za-z\.\+-\]/);
    assert.ok(
...
test('packed installer matrix stays defined for Linux, macOS, and Windows', () => {
  ...
  assert.match(lifecycle, /os:\s*\[ubuntu-latest, macos-latest, windows-latest\]/);
  assert.match(lifecycle, /node-version:\s*['"]20\.x['"]/);
  assert.match(lifecycle, /name:\s*oas-ci-installer-artifact/);
...
test('packed lifecycle invokes installed public bins, including setup help', () => {
  ...
  assert.match(lifecycleRunnerSource, /\['oas-universal', 'setup', '--help'\]/);
  assert.match(lifecycleRunnerSource, /\['oas', \.\.\.args\]/);
  assert.doesNotMatch(lifecycleRunnerSource, /node_modules.*scripts.*oas\.js/);
});
...
test('packed lifecycle applies and updates README-primary Claude setup with a fake provider', () => {
  ...
  assert.match(
    ...
    /const claudeSetupArgs = \[\s*'oas-universal', 'setup',\s*'--mode', 'claude-plugin',\s*'--scope', 'user',\s*\]/
  );
  assert.match(
    lifecycleRunnerSource,
  ...
  assert.match(lifecycleRunnerSource, /plugin update oas@oas/);
});

test('packed lifecycle mutates through the fully explicit guided Kimi install', () => {
  assert.match(
    lifecycleRunnerSource,
    /const guidedKimiInstallArgs = \[\s*'oas-universal', 'install', '--guided',\s*'--harness', 'kimi',\s*'--profile', 'core',\s*\]/
  );
  assert.match(
    lifecycleRunnerSource,
