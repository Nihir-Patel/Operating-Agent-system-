...
const PACKAGE_NAME = 'oas-universal';
const HASH_PATTERN = /^[a-f0-9]{64}$/i;
const PACKAGE_PATH_PATTERN = /^release-artifacts\/oas-universal-[0-9A-Za-z.+-]+\.tgz$/;

function parseEnvironment(environment = process.env, cwd = process.cwd()) {
  const packageValue = environment.OAS_RELEASE_PACKAGE;
  const hashValue = environment.OAS_RELEASE_SHA256;

  if (!packageValue) {
    throw new Error('OAS_RELEASE_PACKAGE must name the downloaded release .tgz');
  }
  if (!PACKAGE_PATH_PATTERN.test(String(packageValue))) {
    throw new Error('OAS_RELEASE_PACKAGE must name one OAS .tgz under release-artifacts');
  }
  if (!HASH_PATTERN.test(hashValue || '')) {
    throw new Error('OAS_RELEASE_SHA256 must be a 64-character SHA-256 digest');
  }
...
function installPackage(projectDir, packagePath, environment) {
  const projectManifest = {
    name: 'oas-packed-artifact-lifecycle',
    version: '1.0.0',
    private: true,
    dependencies: {
...
function fakeClaudeProviderMain() {
  ...
  const statePath = process.env.OAS_TEST_CLAUDE_STATE;
  const callsPath = process.env.OAS_TEST_CLAUDE_CALLS;
  ...
  const createReadArtifacts = () => {
    if (process.env.OAS_TEST_CLAUDE_CREATE_READ_ARTIFACTS !== '1') return;
  ...
  if (joined === 'plugin marketplace update oas') {
    return;
  }
  if (joined.startsWith('plugin install oas@oas ')) {
    writeState({
      ...state,
      plugins: [{ enabled: true, id: 'oas@oas', scope: 'user', version: '2.2.0' }],
    });
    return;
  }
  if (joined.startsWith('plugin update oas@oas ')) {
    writeState({
      ...state,
      plugins: (state.plugins || []).map(plugin => (
        plugin.id === 'oas@oas' && plugin.scope === 'user'
...
function runTargetSmoke(options) {
  ...
  const statePath = path.join(options.targetRoot, 'oas-install-state.json');
...
function runLifecycle(options) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-packed-lifecycle-'));
  ...
  try {
    ...
    const statePath = path.join(cursorRoot, 'oas-install-state.json');
    ...
    const setupHelp = runPublicCli(['oas-universal', 'setup', '--help']);
    assert.match(setupHelp.stdout, /OAS guided setup/);
    assert.match(setupHelp.stdout, /oas setup --mode claude-plugin/);
    ...
    const claudeSetupEnvironment = {
      ...
      OAS_TEST_CLAUDE_CALLS: fakeClaudeCallsPath,
      OAS_TEST_CLAUDE_CREATE_READ_ARTIFACTS: '1',
      OAS_TEST_CLAUDE_STATE: fakeClaudeStatePath,
    ...
    assert.deepStrictEqual(fakeClaudeState.plugins, [
      { enabled: true, id: 'oas@oas', scope: 'user', version: '2.2.0' },
    ]);
    assert.deepStrictEqual(fakeClaudeState.marketplaces, [
      { name: 'oas', repo: 'neal0709/Operating-Agent-system-', scope: 'user', source: 'github' },
    ]);
    ...
    assert.ok(
      fakeClaudeCalls.includes('plugin marketplace update oas'),
      'repeat packed Claude setup must update the official marketplace'
    );
    assert.ok(
      fakeClaudeCalls.some(call => call.startsWith('plugin install oas@oas ')),
      'initial packed Claude setup must install oas@oas'
    );
    assert.ok(
      fakeClaudeCalls.includes('plugin update oas@oas --scope user'),
      'repeat packed Claude setup must update oas@oas'
    );
    const claudeSettings = JSON.parse(
      fs.readFileSync(path.join(claudeConfigDir, 'settings.json'), 'utf8')
    );
    assert.strictEqual(
      claudeSettings.pluginConfigs['oas@oas'].options.hook_profile,
      'strict'
    );
    assert.strictEqual(
      fs.readFileSync(claudeSetupSentinel, 'utf8'),
    ...
    const guidedKimiStatePath = path.join(guidedKimiRoot, 'oas-install-state.json');
    ...
    const guidedKimiDoctor = parseJsonOutput(
      runPublicCli(['oas', 'doctor', '--target', 'kimi', '--json']),
      'guided Kimi doctor'
    );
    ...
    const guidedKimiUninstall = parseJsonOutput(
      runPublicCli(['oas', 'uninstall', '--target', 'kimi', '--json']),
