...
test("has valid discoverable frontmatter and trigger phrases", () => {
  ...
  assert.match(skill, /OAS implements no training stack of its own/i);
});
...
test("labels backend stages as future and keeps eval gates human-honest", () => {
  const skill = read("skills/ito-training/SKILL.md");
  assert.match(skill, /describe the future backend \(Layer 0\.3\), not code that exists in\s+OAS/i);
...
test("keeps unsupported training outside the executable bridge", () => {
  ...
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oas-ito-train-reject-"));
  try {
    ...
    const result = spawnSync(process.execPath, [
      path.join(REPO_ROOT, "scripts", "oas.js"), "ito", "train",
      "--booking", "booking_test", "--model-size", "8B",
    ], {
      encoding: "utf8",
      env: { ...process.env, OAS_ITO_CLI_EXECUTABLE: executable },
    });
