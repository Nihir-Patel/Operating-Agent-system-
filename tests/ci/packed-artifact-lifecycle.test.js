...
test('resolves package and hash from explicit environment variables', () => {
  const options = lifecycle.parseEnvironment({
    OAS_RELEASE_PACKAGE: 'release-artifacts/oas-universal-2.2.0.tgz',
    OAS_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace');

  assert.strictEqual(
    options.packagePath,
    path.resolve('/workspace', 'release-artifacts/oas-universal-2.2.0.tgz')
  );
  assert.strictEqual(options.expectedSha256, 'a'.repeat(64));
});

test('rejects missing, malformed, and non-tgz release inputs', () => {
  assert.throws(() => lifecycle.parseEnvironment({}, '/workspace'), /OAS_RELEASE_PACKAGE/);
  assert.throws(() => lifecycle.parseEnvironment({
    OAS_RELEASE_PACKAGE: 'package.zip',
    OAS_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace'), /\.tgz/);
  assert.throws(() => lifecycle.parseEnvironment({
    OAS_RELEASE_PACKAGE: 'release-artifacts/oas-universal-2.2.0.tgz',
    OAS_RELEASE_SHA256: 'not-a-hash',
  }, '/workspace'), /SHA-256/);
  assert.throws(() => lifecycle.parseEnvironment({
    OAS_RELEASE_PACKAGE: '../release-artifacts/oas-universal-2.2.0.tgz',
    OAS_RELEASE_SHA256: 'a'.repeat(64),
  ...
  assert.throws(() => lifecycle.parseEnvironment({
    OAS_RELEASE_PACKAGE: '/tmp/oas-universal-2.2.0.tgz',
    OAS_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace'), /release-artifacts/);
});

test('hashFile computes a lowercase SHA-256 digest', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-packed-hash-'));
...
test('public CLI invocations use npm exec instead of internal package paths', () => {
  ...
  assert.deepStrictEqual(invocation.args, [
    ...
    '/s',
    '/c',
    'npm exec --offline --yes=false -- oas-universal setup --help',
  ]);

  const unixInvocation = lifecycle.getNpmExecInvocation(
    ['oas', 'doctor', '--target', 'cursor', '--json'],
    {},
    'linux'
  );
  ...
  assert.strictEqual(unixInvocation.args[4], 'oas');
...
test('Windows public CLI invocation accepts the exact Itô capability selection', () => {
  ...
  assert.strictEqual(
    invocation.args[3],
    'npm exec --offline --yes=false -- oas install --profile core --with capability:ito-compute --with capability:prediction-markets --target cursor --enable-hooks --json'
  );
});
