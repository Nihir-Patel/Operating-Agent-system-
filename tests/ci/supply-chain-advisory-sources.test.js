...
async function run() {
  ...
  if (await test('offline report emits passing coverage checks and Linear-ready ITO-57 payload', async () => {
    ...
    assert.strictEqual(report.schema_version, 'oas.supply-chain-advisory-sources.v1');
  ...
  if (await test('CLI JSON can be written as a scheduled workflow artifact', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-advisory-sources-'));
    ...
    try {
      ...
      assert.strictEqual(parsed.schema_version, 'oas.supply-chain-advisory-sources.v1');
