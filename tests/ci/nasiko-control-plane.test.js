...
async function main() {
  ...
  const tests = [
    ...
    ['cleans an exclusively created lifecycle lock when initialization fails', () => {
      ...
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-lock-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
      try {
    ...
    ['recovers only locks whose recorded owner is confirmed dead', () => {
      const { acquireLifecycleLock } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-stale-lock-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
      try {
    ...
    ['refuses to recover malformed lifecycle-lock ownership', () => {
      const { acquireLifecycleLock } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-malformed-lock-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
      try {
    ...
    ['recovers a lock abandoned by a finished process', () => {
      const { acquireLifecycleLock } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-dead-process-lock-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
    ...
    ['a prior release callback never removes a replacement lifecycle lock', () => {
      const { acquireLifecycleLock } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-replaced-lock-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
    ...
    ['uses descriptor identity when Windows path stats disagree', () => {
      const { acquireLifecycleLock } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-windows-identity-'));
      const lockPath = path.join(installRoot, '.oas-nasiko-lifecycle.lock');
    ...
    ['verifies manifest and blob digests before an atomic install', async () => {
      const { installNasiko } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-green-'));
      ...
      try {
        ...
        assert.strictEqual(fs.existsSync(path.join(installRoot, '.oas-nasiko-install.json')), true);
        ...
        assert.strictEqual(fs.existsSync(path.join(installRoot, '.oas-nasiko-install.json')), false);
      } finally {
    ...
    ['rejects digest mismatch and unsafe archive entries without installing', async () => {
      ...
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-reject-'));
    ...
    ['read-only status never executes an unqualified explicit executable', () => {
      const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-status-'));
      const executable = path.join(fixtureRoot, 'nasiko');
      const marker = path.join(fixtureRoot, 'executed');
      fs.writeFileSync(executable, `#!/bin/sh\ntouch ${JSON.stringify(marker)}\nprintf "nasiko 0.1.0\\n"\n`, { mode: 0o755 });
      try {
        const result = spawnSync(process.execPath, [
          path.join(REPO_ROOT, 'scripts', 'oas.js'),
          'nasiko',
          'status',
          '--json',
        ], {
          encoding: 'utf8',
          env: { ...process.env, OAS_NASIKO_CLI_EXECUTABLE: executable },
        });
    ...
    ['read-only status has a stable absent result shape', () => {
      ...
      const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-absent-'));
      try {
    ...
    ['rejects and never executes an unqualified pre-existing binary', async () => {
      const { installNasiko } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-existing-'));
    ...
    ['rolls back a published binary when metadata persistence fails', async () => {
      const { installNasiko } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-rollback-'));
    ...
    ['never overwrites or deletes a destination created during publication', async () => {
      const { installNasiko } = require('../../scripts/lib/nasiko-release');
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-race-'));
      const binary = Buffer.from('qualified binary');
    ...
    ['fails uninstall when staged tombstones cannot be removed', () => {
      ...
      const installRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nasiko-cleanup-failure-'));
      const executable = path.join(installRoot, 'nasiko');
      const metadataPath = path.join(installRoot, '.oas-nasiko-install.json');
    ...
    ['ships a canonical opt-in skill without silently bundling Nasiko', () => {
      ...
      assert.match(skill, /oas nasiko status/i);
      ...
      assert.match(skill, /oas nasiko uninstall/i);
