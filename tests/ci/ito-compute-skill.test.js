...
function main() {
  ...
  const tests = [
    ["documents only the real CLI commands and MCP tools", () => {
      ...
      assert.match(skill, /OAS_ITO_CLI_EXECUTABLE/);
      assert.match(skill, /explicit absolute built entry/);
      assert.match(skill, /never discovers[^\n]*through `PATH`/);
      assert.match(skill, /oas ito login --no-browser/);
      ...
      assert.match(skill, /OAS (?:itself )?(?:does|performs) no browser automation/i);
