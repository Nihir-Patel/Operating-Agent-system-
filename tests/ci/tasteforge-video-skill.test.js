...
test("distinguishes local deterministic operations from provider generation", () => {
  ...
  assert.match(skill, /OAS never calls Fal/i);
  assert.match(
    skill,
...
// OAS_TEST_NPM_PACK=1 (release/CI verification); the default suite relies on
// the files-array assertions above.
test("ships inside the real npm tarball (opt-in)", () => {
  if (process.env.OAS_TEST_NPM_PACK !== "1") {
    return { skipped: "set OAS_TEST_NPM_PACK=1 to run real npm pack inclusion" };
  }
