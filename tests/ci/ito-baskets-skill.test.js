...
test("preserves the non-advisory, non-executing boundary from all four predecessors", () => {
  ...
  assert.match(skill, /`oas ito find` submits an\s+authenticated RFQ/);
  assert.match(skill, /`oas ito status` reads RFQ\/procurement status, not\s+basket data/);
  assert.match(skill, /UNSUPPORTED_OPERATION/);
  assert.match(skill, /prediction-market-risk-review/);
  assert.doesNotMatch(skill, /(?:run|invoke|call) `?oas ito (?:find|status)/i);
...
test("documents anonymous, keyed, and SDK surfaces with scope and credential separation", () => {
  ...
  assert.match(skill, /never uses device authorization or `oas ito login`/i);
