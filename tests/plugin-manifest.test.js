...
test('agent.yaml uses canonical OAS identity', () => {
  const agentYamlSource = fs.readFileSync(agentYamlPath, 'utf8');
  assert.ok(/^name:\s*oas$/m.test(agentYamlSource), 'Expected agent.yaml to use the oas name');
});
...
test('claude plugin.json uses short plugin slug', () => {
  assert.strictEqual(claudePlugin.name, 'oas');
});
...
test('claude plugin.json disables bundled MCP servers for provider tool-name compatibility', () => {
  const legacyPluginName = 'operating-agent-systems';
...
test('claude marketplace.json has plugins array with the published plugin entry', () => {
  ...
  assert.strictEqual(claudeMarketplace.name, 'oas');
  assert.strictEqual(claudeMarketplace.plugins[0].name, 'oas');
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'oas');
});
...
test('codex plugin.json explicitly declares the supported lifecycle hook bundle', () => {
  assert.strictEqual(
    ...
    'Codex supports a top-level hooks path; keep the OAS hook bundle explicit instead of inventing provider-specific settings'
  );
...
test('codex plugin.json uses canonical OAS repo and display name', () => {
  assert.strictEqual(codexPlugin.repository, 'https://github.com/neal0709/Operating-Agent-system-');
  assert.strictEqual(codexPlugin.interface.displayName, 'OAS');
});
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'oas');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'oas');
});
...
test('marketplace local plugin source is a self-contained native Codex bundle', () => {
  ...
  for (const plugin of marketplace.plugins) {
    ...
    assert.strictEqual(sourceRoot, repoRoot, `OAS's native Codex bundle must use the self-contained repository root, got: ${plugin.source.path}`);
    ...
    assert.ok(fs.existsSync(path.join(sourceRoot, 'skills', 'configure-oas', 'SKILL.md')), 'Codex configure-oas skill must ship inside the installed source root');
  }
});

// ── plugins/oas marketplace plugin folder ─────────────────────────────────────
...
test('plugins/oas manifest version matches package.json', () => {
  assert.strictEqual(marketplacePluginManifest.version, expectedVersion);
});

test('plugins/oas manifest version matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.version, rootCodexManifest.version);
});

test('plugins/oas manifest reuses root skills and MCP config without vendoring', () => {
  ...
  assert.ok(!fs.existsSync(path.join(pluginDir, 'skills')), 'plugins/oas must not vendor a second skills/ copy (see #2097 review)');
  assert.ok(!fs.existsSync(path.join(pluginDir, '.mcp.json')), 'plugins/oas must not vendor a second .mcp.json (see #2097 review)');
});

test('plugins/oas manifest interface assets resolve to root assets', () => {
  const pluginDir = path.dirname(path.dirname(marketplacePluginManifestPath));
...
test('plugins/oas README marks the thin folder as a legacy compatibility artifact', () => {
  ...
  assert.ok(source.includes('check-plugin-cache.js'), 'plugins/oas README must point at the cache health check');
...
test('user-facing docs do not use overlong legacy marketplace install commands', () => {
  const markdownFiles = [
    ...
    path.join(repoRoot, 'skills', 'configure-oas', 'SKILL.md'),
  ...
  for (const filePath of markdownFiles) {
    ...
    if (/\/plugin\s+(install|list)\s+operating-agent-systems(?:@operating-agent-systems)?\b/.test(source)) {
...
test('user-facing docs do not use the legacy non-URL marketplace add form', () => {
  ...
  for (const filePath of markdownFiles) {
    const source = fs.readFileSync(filePath, 'utf8');
    if (source.includes('/plugin marketplace add neal0709/operating-agent-systems')) {
      offenders.push(path.relative(repoRoot, filePath));
    }
  }
...
test('.codex-plugin README uses current marketplace add flow', () => {
  ...
  assert.ok(readme.includes('codex plugin marketplace add neal0709/Operating-Agent-system-'), 'Expected .codex-plugin README to document the canonical OAS repo marketplace source');
  assert.ok(readme.includes('codex plugin add oas@oas'), 'Expected .codex-plugin README to document the current Codex install command');
