...
function assertHonestComputeCopy(content) {
  ...
  assert.match(content, /oas ito find/i);
  ...
  assert.doesNotMatch(content, /OAS only (?:links|provides this link)/i);
}
...
function main() {
  ...
  const tests = [
    ...
    ['README keeps the three primary choices and all three guides inline', () => {
      const readme = read('README.md');
      const primaryLinks = extractNamedTable(readme, 'OAS primary links');
      const guides = extractNamedTable(readme, 'OAS guides');
      const centeredPrimaryLinks = readme.match(
        /<div align="center">\s*<table[^>]*aria-label="OAS primary links"[^>]*>[\s\S]*?<\/table>\s*<\/div>/
      );
      ...
      assert.ok(primaryLinks.includes('assets/images/community/oas-tools-mark.svg'));
      assertExactHref(primaryLinks, 'https://github.com/apps/oas-tools');
      assertExactHref(primaryLinks, 'https://oas.tools/pricing');
      assertExactHref(primaryLinks, 'https://github.com/sponsors/neal0709');
      ...
      const oasToolsMark = read('assets/images/community/oas-tools-mark.svg');
      assert.match(oasToolsMark, /viewBox="0 0 96 96"/);
      assert.match(oasToolsMark, /id="favicon-frame"/);
      assert.match(oasToolsMark, /id="favicon-node"/);
      assert.match(oasToolsMark, /circle cx="62" cy="44"/);
      assert.doesNotMatch(
        oasToolsMark,
        /<script|<foreignObject|\son[a-z]+=|(?:href|xlink:href)=/i
      );
    ...
    ['README shows the verified local Kimi via Ito path without claiming managed serving', () => {
      ...
      assert.ok(localModelPath.includes('assets/images/community/oas-tools-mark.svg'));
      assert.match(readme, /install\.sh --target kimi --profile minimal/);
      assert.match(readme, /npx oas-universal doctor --target kimi/);
    ...
    ['Kimi install stays inside its project root and passes doctor with native instruction surfaces', () => {
      const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-kimi-home-'));
      const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-kimi-project-'));
    ...
    ['integration record keeps the thesis and real client boundary honest', () => {
      ...
      assert.doesNotMatch(record, /manual_copy|ito\.compute\.handoff|oas ito rent/i);
    }],
    ['top-level CLI help exposes the provider-neutral compute route', () => {
      const result = spawnSync('node', ['scripts/oas.js', '--help'], {
        cwd: REPO_ROOT,
        encoding: 'utf8',
      });
      assert.strictEqual(result.status, 0, result.stderr);
    ...
    ['installer help and human dry-run expose the compute route', () => {
      ...
      const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-ito-home-'));
      const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-ito-project-'));
      try {
    ...
    ['npm package publishes the Ito mark and welcome route', () => {
      ...
      assert.match(packageJson.scripts.welcome, /oas ito find/i);
