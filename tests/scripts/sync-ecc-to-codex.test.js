/**
 * Source-level tests for scripts/sync-oas-to-codex.sh
 */
...
function runTests() {
  ...
  if (test('sync script carries the Codex navigation guide referenced by AGENTS', () => {
    ...
    assert.ok(source.includes('require_path "$CODEX_NAV_GUIDE_SRC" "OAS Codex navigation guide"'), 'Expected sync preflight for Codex navigation guide');
  ...
  if (test('sync script avoids GNU-only grep -P parsing', () => {
    assert.ok(!source.includes('grep -oP'), 'sync-oas-to-codex.sh should remain portable across BSD and GNU environments');
  })) passed++; else failed++;
  ...
  if (test('sync records a versioned ownership manifest before mutating Codex state', () => {
    ...
    assert.ok(source.includes('record_managed_path "${OAS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-commit"'));
  })) passed++; else failed++;
