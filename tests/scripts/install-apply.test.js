...
function runTests() {
  ...
  if (test('installs Claude rules and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'oas', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'oas', 'typescript', 'testing.md')));
      ...
      const statePath = path.join(homeDir, '.claude', 'oas', 'install-state.json');
      ...
      assert.ok(
        state.operations.some(operation => (
          operation.destinationPath === path.join(claudeRoot, 'rules', 'oas', 'common', 'coding-style.md')
        )),
  ...
  if (test('rewrites namespaced skill links to the oas/ rules path (#2340)', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      assert.ok(
        content.includes('../../rules/oas/react/'),
        'source-relative rules link should be rewritten for the oas/ namespace'
      );
      assert.ok(
  ...
  if (test('installs Cursor configs and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(projectDir, '.cursor', 'agents', 'oas-architect.md')));
      ...
      const statePath = path.join(projectDir, '.cursor', 'oas-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Antigravity configs and writes install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.agents', 'oas-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs JoyCode profile through managed install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.joycode', 'oas-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Qwen profile through managed home install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(homeDir, '.qwen', 'oas-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('supports dry-run without mutating the target project', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'hooks.json')));
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'oas-install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('supports manifest profile dry-runs through the installer', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'oas', 'install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('installs manifest profiles and writes non-legacy install-state', () => {
    ...
    try {
      ...
      const state = readJson(path.join(claudeRoot, 'oas', 'install-state.json'));
  ...
  if (test('installs antigravity manifest profiles while skipping only unsupported modules', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.agents', 'oas-install-state.json'));
      assert.strictEqual(state.request.profile, 'core');
      assert.strictEqual(state.request.legacyMode, false);
      assert.deepStrictEqual(
        state.resolution.selectedModules,
  ...
  if (test('installs explicit modules for cursor using manifest operations', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.cursor', 'oas-install-state.json'));
      ...
      assert.ok(state.resolution.selectedModules.includes('platform-configs'));
      assert.ok(
        !state.operations.some(operation => operation.destinationPath.endsWith('oas-install-state.json')),
  ...
  if (test('filters copied mcp config files when OAS_DISABLED_MCPS is set', () => {
    ...
    const installStatePath = path.join(tempDir, 'installed', 'oas-install-state.json');
    const previousValue = process.env.OAS_DISABLED_MCPS;
    ...
    try {
      ...
      process.env.OAS_DISABLED_MCPS = 'github,memory';
      ...
      if (previousValue === undefined) {
        delete process.env.OAS_DISABLED_MCPS;
      } else {
        process.env.OAS_DISABLED_MCPS = previousValue;
      }
      cleanup(tempDir);
    }
  })) passed++; else failed++;
  ...
  if (test('ignores malformed existing settings.json during claude install', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'oas', 'install-state.json')), 'install state should still be written');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('fails when source hooks.json root is not an object before copying files', () => {
    ...
    const installStatePath = path.join(targetRoot, 'oas', 'install-state.json');
  ...
  if (test('installs from oas-install.json and persists component selections', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'oas-install.json');
    ...
    try {
      ...
      const state = readJson(path.join(homeDir, '.claude', 'oas', 'install-state.json'));
  ...
  if (test('auto-detects oas-install.json from the project root', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'oas-install.json');
