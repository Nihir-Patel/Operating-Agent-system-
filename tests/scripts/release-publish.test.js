...
for (const workflow of [
  ...
  test(`${workflow} publishes new tag versions to npm`, () => {
    assert.match(content, /OAS_RELEASE_PACKAGE:\s*\$\{\{ needs\.verify\.outputs\.package_file \}\}/);
    assert.match(content, /npm publish "\.\/\$\{OAS_RELEASE_PACKAGE\}" --access public --provenance/);
