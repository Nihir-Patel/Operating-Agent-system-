...
function makeRecord({ repoRoot, homeDir, projectRoot, adapter, request, resolution, operations }) {
  ...
  const installStatePath = adapter.kind === 'project'
    ? path.join(targetRoot, 'oas-install-state.json')
    : path.join(targetRoot, 'oas', 'install-state.json');
...
function ensureFakeRepo(repoRoot) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'operating-agent-systems', version: '1.10.0' }, null, 2)
  );
...
function runTests() {
  ...
  if (test('parseArgs reads repo-root, target, dry-run, and json flags', () => {
    ...
    assert.strictEqual(parsed.repoRoot, '/tmp/oas');
  ...
  if (test('deriveRepoRootFromState uses sourcePath and sourceRelativePath', () => {
    const state = {
      operations: [
        {
          sourcePath: path.join('/tmp', 'oas', 'scripts', 'setup-package-manager.js'),
    ...
    assert.strictEqual(
      ...
      path.resolve(path.join('/tmp', 'oas'))
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('determineInstallCwd uses the project root for project installs', () => {
    ...
    assert.strictEqual(determineInstallCwd(record, '/tmp/oas'), path.join('/tmp', 'project'));
  })) passed += 1; else failed += 1;
  ...
  if (test('runAutoUpdate rejects mixed inferred repo roots', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        /Multiple OAS repo roots detected/
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectRoot);
            ...
            if (command === process.execPath) {
              return {
                stdout: JSON.stringify({
                  dryRun: false,
                  result: {
                    installStatePath: path.join(projectRoot, '.cursor', 'oas-install-state.json'),
                  },
                }),
                stderr: '',
              };
  ...
  if (test('runAutoUpdate excludes residual legacy Antigravity records', () => {
    ...
    try {
      ...
      const legacy = {
        ...canonical,
        installStatePath: path.join(projectRoot, '.agent', 'oas-install-state.json'),
        legacy: true,
      };
      const commands = [];
  ...
  if (test('runAutoUpdate gives legacy-only OpenCode migration guidance', () => {
    ...
    try {
      ...
      const legacy = {
        ...
        installStatePath: path.join(homeDir, '.opencode', 'oas-install-state.json'),
        legacy: true,
