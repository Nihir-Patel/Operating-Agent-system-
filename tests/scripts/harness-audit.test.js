...
function runTests() {
  ...
  if (test('GitHub Integration category scores against a fully-wired consumer fixture', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'oas', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'oas', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'oas' }, null, 2)
      );
  ...
  if (test('detects repo mode from structural markers when package name differs', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'oas' }, null, 2));
      fs.writeFileSync(
        path.join(projectRoot, 'package.json'),
  ...
  if (test('audits consumer projects from cwd instead of the OAS repo root', () => {
  ...
  if (test('prints no top actions when consumer checks all pass', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'oas@oas'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'oas@oas', 'plugin.json'),
        JSON.stringify({ name: 'oas' }, null, 2)
      );
      fs.mkdirSync(path.join(projectRoot, '.claude'), { recursive: true });
  ...
  if (test('detects marketplace-installed Claude plugins under home marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'operating-agent-systems', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'operating-agent-systems', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'operating-agent-systems' }, null, 2)
      );
  ...
  if (test('detects marketplace-installed Claude plugins under project marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'operating-agent-systems', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'operating-agent-systems', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'operating-agent-systems' }, null, 2)
      );
  ...
  if (test('detects Claude plugin installs from installed_plugins.json', () => {
    ...
    const installRoot = path.join(pluginsDir, 'cache', 'operating-agent-systems', 'oas', '2.0.0');

    try {
      fs.mkdirSync(path.join(installRoot, '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(installRoot, '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'oas', version: '2.0.0' }, null, 2)
      );
      fs.writeFileSync(
        path.join(pluginsDir, 'installed_plugins.json'),
        JSON.stringify({
          plugins: {
            'oas@operating-agent-systems': [
              { installPath: path.join('cache', 'operating-agent-systems', 'oas', '2.0.0') },
            ],
          },
        }, null, 2)
      );
      ...
      try {
        ...
        assert.ok(found);
        assert.ok(found.includes(`${path.sep}cache${path.sep}operating-agent-systems${path.sep}oas${path.sep}2.0.0${path.sep}`));
      } finally {
  ...
  if (test('detects newest Claude plugin install from cache marketplace layout', () => {
    ...
    const pluginRoot = path.join(homeDir, '.claude', 'plugins', 'cache', 'operating-agent-systems', 'oas');
    ...
    try {
      for (const version of ['1.8.0', '1.10.0']) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({ name: 'oas', version }, null, 2)
        );
      }
