...
function runEcc(args, env = {}) {
  return spawnSync(process.execPath, [oasScript, ...args], {
    cwd: repoRoot,
    encoding: 'utf8',
...
console.log('\n=== OAS welcome command tests ===\n');

test('oas welcome renders the install artwork for captured agent output', () => {
...
test('oas welcome disables ANSI color when stdout is redirected', () => {
  const env = { ...process.env, TERM: 'xterm-256color' };
  delete env.NO_COLOR;
  const result = spawnSync(process.execPath, [oasScript, 'welcome'], {
    cwd: repoRoot,
    encoding: 'utf8',
    env,
  });
...
test('oas welcome supports explicit update and configured outcomes', () => {
  const cases = [
    ['updated', /OAS is updated/],
    ['configured', /OAS is configured/],
    ['migrated', /OAS is configured/],
    ['resumed', /OAS is configured/],
    ['already-migrated', /OAS is configured/],
  ];
...
test('oas welcome renders a provider-verified installed version', () => {
...
test('oas welcome rejects unsafe version text', () => {
...
test('oas welcome keeps parser error output free of terminal control bytes', () => {
...
test('oas welcome rejects unknown actions before rendering', () => {
