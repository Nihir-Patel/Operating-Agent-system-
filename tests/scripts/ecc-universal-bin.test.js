/**
 * Published npm binary aliases for the primary OAS CLI.
 *
...
function getPackedFixture() {
  ...
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-universal-bin-'));
...
function prepareLocalPackedProject(packageManager) {
  ...
  const packageDirectory = path.join(modulesDirectory, 'oas-universal');
  const binDirectory = path.join(modulesDirectory, '.bin');

  fs.mkdirSync(projectDirectory, { recursive: true });
  fs.writeFileSync(
    path.join(projectDirectory, 'package.json'),
    `${JSON.stringify({ name: 'oas-packed-smoke', private: true }, null, 2)}\n`
  );
  ...
  for (const executable of ['oas', 'oas-universal']) {
    ...
    if (process.platform === 'win32') {
      ...
      fs.writeFileSync(
        cmdPath,
        `@ECHO off\r\nnode "%~dp0\\..\\oas-universal\\${target}" %*\r\n`
      );
    } else {
      fs.symlinkSync(
        path.join('..', 'oas-universal', packageJson.bin[executable]),
    ...
    case 'npm':
      {
        ...
        return {
          ...
          args: [
            'exec',
            '--offline',
            '--package=./node_modules/oas-universal',
            '--',
            executable,
            ...args,
          ],
...
console.log(`\n=== OAS universal packed binary tests (${activePackageManager}) ===\n`);
...
test('published package exposes oas and oas-universal through scripts/oas.js', () => {
  assert.strictEqual(packageJson.bin.oas, 'scripts/oas.js');
  assert.strictEqual(packageJson.bin['oas-universal'], 'scripts/oas.js');
  assert.deepStrictEqual(packageLock.packages[''].bin, packageJson.bin);

  const fixture = getPackedFixture();
  assert.ok(
    fixture.publishedPaths.has('scripts/oas.js'),
    'npm package should publish the shared CLI target'
  );
});

test('packed oas-universal launches the guided Claude setup help', () => {
  const result = launchPackedBinary('oas-universal', ['setup', '--help']);
  assert.match(result.stdout, /OAS guided setup/);
});

test('packed oas-universal launches the guided multi-harness help', () => {
  const result = launchPackedBinary(
    'oas-universal',
    ['install', '--guided', '--help']
  );
  assert.match(result.stdout, /OAS guided multi-harness install/);
  assert.match(result.stdout, /Claude Code/);
  assert.match(result.stdout, /Codex/);
  assert.match(result.stdout, /Kimi/);
});

test('packed oas alias launches the primary dispatcher', () => {
  const result = launchPackedBinary('oas', ['--help']);
  assert.match(result.stdout, /OAS selective-install CLI/);
  assert.match(result.stdout, /oas install --guided/);
});
