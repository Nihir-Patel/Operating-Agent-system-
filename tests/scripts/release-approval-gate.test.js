...
function approvedPacketContent(overrides = {}, release = CURRENT_RELEASE) {
  ...
  return [
    `# OAS v${release} Owner Approval Packet`,
    '',
    '## Decision Register',
    '',
...
function finalLedgerContent(extra = '', release = CURRENT_RELEASE) {
  return [
    `# OAS v${release} Release URL Ledger`,
    '',
...
function manifestContent(release = CURRENT_RELEASE) {
  return [
    `# OAS v${release} Preview Pack Manifest`,
    '',
    '| Artifact | Role | Gate |',
    '| --- | --- | --- |',
...
function runTests() {
  ...
  if (test('seeded approved release passes every publication approval check', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'oas.release-approval-gate.v1');
    ...
    try {
      ...
      seedRepo(rootDir, {
        [`${releaseDir}/release-url-ledger-2026-05-19.md`]: [
          `# OAS v${CURRENT_RELEASE} Release URL Ledger`,
          '',
          '## Approval-Gated URLs',
          '',
          '| Surface | Intended URL or command | Gate before use |',
          '| --- | --- | --- |',
          `| GitHub prerelease | https://github.com/neal0709/Operating-Agent-system-/releases/tag/v${CURRENT_RELEASE} | must return the prerelease |`,
        ].join('\n'),
      });
  ...
  if (test('announcement drafts fail on unresolved placeholders and private paths', () => {
    ...
    try {
      const releaseDir = releaseDirFor(CURRENT_RELEASE);
      seedRepo(rootDir, {
        [`${releaseDir}/x-thread.md`]: 'Ship copy with <video-url> and /Users/nihir/raw-footage.',
      });
