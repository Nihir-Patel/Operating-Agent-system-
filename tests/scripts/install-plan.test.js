...
function runTests() {
  ...
  if (test('shows help with no arguments', () => {
    ...
    assert.ok(result.stdout.includes('Inspect OAS selective-install manifests'));
  })) passed++; else failed++;
  ...
  if (test('loads planning intent from oas-install.json', () => {
    const configDir = path.join(__dirname, '..', 'fixtures', 'tmp-install-plan-config');
    const configPath = path.join(configDir, 'oas-install.json');
  ...
  if (test('auto-detects planning intent from project oas-install.json', () => {
