...
function seedMinimalRepo(rootDir, overrides = {}) {
  const files = {
    ...
    'scripts/work-items.js': 'sync-github github-pr github-issue sourceClosedAt oas-work-items-sync-github',
    'scripts/hooks/session-activity-tracker.js': 'tool-usage.jsonl session_id tool_name',
    'oas2/src/observability/mod.rs': 'ToolCallEvent RiskAssessment ToolLogger',
    'oas2/src/session/store.rs': 'insert_tool_log query_tool_logs',
    'oas2/src/session/manager.rs': 'sync_tool_activity_metrics tool-usage.jsonl',
    ...
    'tests/docs/oas2-release-surface.test.js': 'publication-readiness.md',
  };
...
function runTests() {
  ...
  if (test('current repo reports a complete readiness score', () => {
    ...
    assert.strictEqual(parsed.schema_version, 'oas.observability-readiness.v1');
  ...
  if (test('missing tool logger surfaces become prioritized top actions', () => {
    ...
    try {
      seedMinimalRepo(projectRoot, {
        'oas2/src/observability/mod.rs': 'ToolCallEvent only'
      });
      const report = buildReport(projectRoot);

      assert.strictEqual(report.ready, false);
      assert.ok(report.top_actions.some(action => action.id === 'oas2-tool-risk-ledger'));
      assert.ok(report.checks.some(check => check.id === 'oas2-tool-risk-ledger' && !check.pass));
    } finally {
      cleanup(projectRoot);
    }
  })) passed++; else failed++;
