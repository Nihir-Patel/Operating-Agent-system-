...
function createTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-home-'));
}
...
function runTests() {
  ...
  if (test('reports overdue ScheduleWakeup calls from Claude transcripts', () => {
    ...
    try {
      ...
      assert.strictEqual(payload.schemaVersion, 'oas.loop-status.v1');
  ...
  if (test('parses write-dir snapshot option', () => {
    const options = parseArgs([
      ...
      'scripts/loop-status.js',
      '--write-dir',
      '/tmp/oas-loop-snapshots',
    ]);

    assert.strictEqual(options.writeDir, '/tmp/oas-loop-snapshots');
  })) passed++; else failed++;
  ...
  if (test('exit-code mode returns 1 for scan errors without attention signals', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-missing-'));
  ...
  if (test('watch mode emits repeated JSON status frames', () => {
    ...
    try {
      ...
      assert.strictEqual(frames[0].schemaVersion, 'oas.loop-status.v1');
      assert.strictEqual(frames[1].schemaVersion, 'oas.loop-status.v1');
  ...
  if (test('writes per-session status snapshots and index when write-dir is set', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-snapshots-'));
    ...
    try {
      ...
      assert.strictEqual(stdoutPayload.schemaVersion, 'oas.loop-status.v1');
      ...
      assert.strictEqual(indexPayload.schemaVersion, 'oas.loop-status.index.v1');
      ...
      assert.strictEqual(snapshotPayload.schemaVersion, 'oas.loop-status.session.v1');
  ...
  if (test('keeps index.json reserved when session id sanitizes to index', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-index-collision-'));
  ...
  if (test('avoids Windows reserved basenames for session snapshots', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-windows-name-'));
  ...
  if (test('cleans temporary snapshot files when atomic rename fails', () => {
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-loop-status-rename-failure-'));
