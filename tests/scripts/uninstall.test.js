...
function runTests() {
  ...
  if (test('uninstalls files from a real install-apply state and preserves unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedProjectRoot, '.cursor', 'oas-install-state.json');
  ...
  if (test('reverses non-copy operations and keeps unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedTargetRoot, 'oas-install-state.json');
  ...
  if (test('auto-detects legacy sync-oas-to-codex.sh install and removes artifacts without touching conversations or unrelated config keys', () => {
    ...
    try {
      ...
      assert.strictEqual(uninstallResult.code, 0, uninstallResult.stderr);
      assert.ok(!uninstallResult.stdout.includes('No OAS install-state files found'), uninstallResult.stdout);
