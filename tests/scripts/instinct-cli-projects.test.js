...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-instinct-cli-projects-'));
}
...
test('status migrates legacy no-remote linked worktree project dirs to main worktree id', () => {
  ...
  try {
    ...
    runGit(mainWorktree, ['config', 'user.email', 'oas@example.test']);
    runGit(mainWorktree, ['config', 'user.name', 'OAS Test']);
