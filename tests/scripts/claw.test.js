...
// Test helper — matches OAS's custom test pattern
...
function runTests() {
  ...
  if (test('loadOASContext() returns "" when no skills specified', () => {
    const result = loadOASContext('');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadOASContext() skips missing skill directories gracefully', () => {
    const result = loadOASContext('nonexistent-skill-xyz');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadOASContext() concatenates multiple skill files', () => {
    // Use real skills from the OAS repo if they exist
    ...
    const result = loadOASContext(twoSkills);
  ...
  if (test('module exports all required functions', () => {
    ...
    const required = [
      ...
      'appendTurn', 'loadOASContext', 'askClaude', 'main'
    ];
