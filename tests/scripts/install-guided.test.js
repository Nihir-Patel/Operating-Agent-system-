...
(async () => {
  ...
  await test('interactive prompts keep spacing, recommended defaults, and one visible confirmation', async () => {
    ...
    assert.deepStrictEqual(
      ...
      ['Apply OAS to these harnesses? [y/N]: ']
    );
  });
  ...
  await test('runs one preflight, one confirmation, and one apply for all selected harnesses', async () => {
    ...
    assert.strictEqual(
      terminal.prompts.filter(prompt => /Apply OAS to these harnesses\?/.test(prompt)).length,
      1
    );
  });
  ...
  await test('help and failed apply paths are actionable', async () => {
    ...
    assert.match(
      errorOutput.read(),
      /Retry with: oas-universal install --guided --harness codex/
    );
  ...
  await test('retry command preserves unfinished provider-specific choices', async () => {
    ...
    assert.match(
      errorOutput.read(),
      /Retry with: oas-universal install --guided --harness claude --harness kimi --claude-scope local --claude-hooks strict --profile developer/
    );
  });
