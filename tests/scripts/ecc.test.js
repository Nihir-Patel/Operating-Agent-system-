...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'oas.js');
...
function main() {
  ...
  const tests = [
    ...
    ['delegates explicit install command', () => {
      const homeDir = createTempDir('oas-cli-install-home-');
      try {
    ...
    ['delegates consult command', () => {
      ...
      assert.strictEqual(result.status, 0, result.stderr);
      const payload = parseJson(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'oas.consult.v1');
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('oas-cli-home-');
      const projectRoot = createTempDir('oas-cli-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/oas-cli');
    }],
    ['delegates loop-status command', () => {
      const homeDir = createTempDir('oas-cli-home-');
      const transcriptDir = path.join(homeDir, '.claude', 'projects', '-tmp-oas');
    ...
    ['delegates feedback command', () => {
      ...
      assert.strictEqual(result.status, 0, result.stderr);
      const payload = parseJson(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'oas.feedback.v1');
    ...
    ['delegates security-ioc-scan command', () => {
      const projectRoot = createTempDir('oas-cli-ioc-scan-');
      fs.writeFileSync(
