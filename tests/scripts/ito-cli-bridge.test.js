/**
 * End-to-end contract tests for OAS's real local Itô CLI bridge.
 *
...
const OAS_SCRIPT = path.join(REPO_ROOT, "scripts", "oas.js");
...
function runCli(args, environment = {}) {
  return spawnSync(process.execPath, [OAS_SCRIPT, ...args], {
    cwd: REPO_ROOT,
    encoding: "utf8",
    env: {
      ...process.env,
...
function runCliAndObserveFirstOutput(args, environment = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [OAS_SCRIPT, ...args], {
      cwd: REPO_ROOT,
...
function makeItoProbe(exitCode = 0) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "oas-ito-cli-"));
...
async function main() {
  console.log("\n=== Testing OAS × Itô real CLI bridge ===\n");
  ...
  const tests = [
      ...
      for (const command of ["login", "logout", "auth", "find", "status"]) {
        ...
        try {
          const result = runCli(["ito", command], {
            OAS_ITO_CLI_EXECUTABLE: probe.executable,
          });
    ...
    ["login never inherits ITO_API_KEY but preserves secure token settings", () => {
      ...
      try {
        ...
        assert.strictEqual(childEnvironment.OAS_ITO_CLI_EXECUTABLE, undefined);
      } finally {
        fs.rmSync(probe.directory, { recursive: true, force: true });
      }
    }],
    ...
    ["isolates live node qualification from Itô and unrelated credentials", () => {
      ...
      try {
        ...
        const result = runCli([
          ...
          SSH_AUTH_SOCK: "/tmp/oas-test-agent.sock",
          ITO_CLI_DEMO: "1",
        ...
        assert.strictEqual(invocation.env.SSH_AUTH_SOCK, "/tmp/oas-test-agent.sock");
        assert.strictEqual(invocation.env.ITO_API_KEY, undefined);
      ...
      const safe = createSafeItoInvocationEnvironment(
        {
          ...
          OAS_ITO_CLI_EXECUTABLE: "/operator/canonical/ito.js",
          ITO_API_KEY: "must-not-cross",
          SIXTYTWO_TOKEN: "must-not-cross",
        },
        ["--future-oas-flag", "evals"],
        { includeControls: true },
      );
      assert.strictEqual(safe.OAS_ITO_CLI_EXECUTABLE, "/operator/canonical/ito.js");
    ...
    ["fails closed with exact local install guidance when the explicit CLI is absent", () => {
      const emptyPath = fs.mkdtempSync(path.join(os.tmpdir(), "oas-empty-path-"));
      try {
        const result = runCli(["ito", "status"], {
          OAS_ITO_CLI_EXECUTABLE: "",
          PATH: emptyPath,
        });
        assert.notStrictEqual(result.status, 0);
        ...
        assert.match(result.stderr, /OAS_ITO_CLI_EXECUTABLE/);
    ...
    ["never forwards Itô credentials to an unverified PATH collision", () => {
      const collisionDirectory = fs.mkdtempSync(
        path.join(os.tmpdir(), "oas-hostile-ito-path-")
      );
      ...
      try {
        ...
        assert.notStrictEqual(result.status, 0);
        assert.match(result.stderr, /explicit absolute|OAS_ITO_CLI_EXECUTABLE/i);
        assert.ok(
    ...
    ["rejects an absolute POSIX shim before it can resolve an interpreter through PATH", () => {
      ...
      const directory = fs.mkdtempSync(path.join(os.tmpdir(), "oas-hostile-ito-shim-"));
    ...
    ["rejects a readable JavaScript decoy outside the canonical package entry", () => {
      const directory = fs.mkdtempSync(path.join(os.tmpdir(), "oas-hostile-ito-js-"));
    ...
    ["help separates device login from auth validation", () => {
      ...
      try {
        ...
        assert.strictEqual(result.status, 0, result.stderr);
        assert.match(result.stdout, /oas ito login \[--no-browser\]/);
        assert.match(result.stdout, /oas ito logout/);
        assert.match(result.stdout, /oas ito auth/);
        assert.match(result.stdout, /oas ito find/);
        assert.match(result.stdout, /oas ito status/);
        assert.match(result.stdout, /oas ito evals/);
        ...
        assert.match(result.stdout, /OAS itself performs no browser automation/i);
