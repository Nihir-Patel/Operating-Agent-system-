...
function runTests() {
  ...
  if (test('parseArgs accepts dashboard flags and rejects invalid values', () => {
    ...
    try {
      ...
      assert.deepStrictEqual(parsed.repos, ['neal0709/Operating-Agent-system-']);
  ...
  if (test('seeded repo emits an objective audit with remaining work', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'oas.operator-readiness-dashboard.v1');
      ...
      assert.ok(report.requirements.some(item => item.id === 'oas-tools-next-level' && item.status === 'in_progress'));
      assert.ok(report.requirements.some(item => (
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'oas-tools-next-level'
          ...
          && item.evidence.includes('OAS-Tools Dependabot alert remediation')
      )));
      assert.ok(report.requirements.some(item => (
      ...
      assert.ok(!report.top_actions.some(item => item.id === 'oas-preview-pack'));
  ...
  if (test('preview pack and Hermes gates stay in progress until smoke gate is wired', () => {
    ...
    try {
      ...
      const report = buildSeededReport(rootDir);
      const previewPack = report.requirements.find(item => item.id === 'oas-preview-pack');
  ...
  if (test('markdown output can be written as the dashboard artifact', () => {
    ...
    try {
      ...
      assert.ok(written.includes('# OAS Operator Readiness Dashboard'));
  ...
  if (test('text output renders compact status and top actions', () => {
    ...
    try {
      ...
      assert.ok(stdout.includes('OAS Operator Readiness Dashboard'));
