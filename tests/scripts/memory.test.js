...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-cli-'));
  ...
  return {
    ...
    env: {
      ...
      HOME: homeDir,
      USERPROFILE: homeDir,
      OAS_MEMORY_PROJECT_ROOT: path.join(projectRoot, '.oas', 'memory'),
      OAS_MEMORY_USER_ROOT: path.join(homeDir, '.oas', 'memory'),
    },
  };
}
...
test('shows memory command help directly and through the oas router', () => {
  const fixture = createFixture();
  try {
    const direct = run(MEMORY_SCRIPT, ['--help'], fixture);
    assert.strictEqual(direct.status, 0, direct.stderr);
    assert.ok(direct.stdout.includes('oas memory save'));
    assert.ok(direct.stdout.includes('oas-memory-mcp'));

    const routed = run(OAS_SCRIPT, ['memory', '--help'], fixture);
    assert.strictEqual(routed.status, 0, routed.stderr);
    assert.ok(routed.stdout.includes('oas memory search'));
...
test('routes stdin through oas memory without dropping the body', () => {
  const fixture = createFixture();
  try {
    const saved = json(run(OAS_SCRIPT, [
      'memory',
      'save',
      '--title', 'Routed stdin',
      '--stdin',
...
test('initializes selected scopes and reports their roots as JSON', () => {
  ...
  try {
    ...
    assert.strictEqual(payload.schemaVersion, 'oas.memory.init.v1');
...
test('saves and reads a targeted handoff without a harness-specific inbox', () => {
  ...
  try {
    ...
    assert.strictEqual(saved.schemaVersion, 'oas.memory.write.v1');
    ...
    assert.strictEqual(read.schemaVersion, 'oas.memory.read.v1');
...
test('accepts body content over stdin and finds it through bounded JSON search', () => {
  ...
  try {
    ...
    assert.strictEqual(search.schemaVersion, 'oas.memory.search.v1');
...
test('doctor is machine-readable and clean for a valid vault', () => {
  ...
  try {
    ...
    assert.strictEqual(report.schemaVersion, 'oas.memory.doctor.v1');
...
test('rejects ambiguous body sources and does not expose a trust promotion flag', () => {
  ...
  try {
    ...
    assert.strictEqual(
      fs.existsSync(path.join(fixture.projectRoot, '.oas', 'memory')),
      false
    );
  } finally {
    fs.rmSync(fixture.root, { recursive: true, force: true });
...
test('global dry-run rejects every mutating memory command without creating a vault', () => {
  ...
  cases.forEach(args => {
    ...
    try {
      ...
      assert.strictEqual(
        fs.existsSync(path.join(fixture.homeDir, '.oas', 'memory')),
        false,
