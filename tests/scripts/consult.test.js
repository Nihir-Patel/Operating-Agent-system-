...
function runTests() {
  ...
  if (test('shows help with an explicit help flag', () => {
    ...
    assert.match(result.stdout, /Consult OAS install components/);
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(
      ...
      'npx oas-universal install --profile minimal --target claude --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.strictEqual(result.status, 0, result.stderr);
    assert.match(result.stdout, /OAS consult/);
    assert.match(result.stdout, /capability:security/);
    assert.match(result.stdout, /npx oas-universal install --profile minimal --target claude --with capability:security/);
    assert.match(result.stdout, /npx oas-universal plan --profile minimal --target claude --with capability:security/);
  })) passed++; else failed++;
  ...
  if (test('works from outside the OAS repository', () => {
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-consult-project-'));
    try {
