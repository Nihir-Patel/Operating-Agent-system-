...
function runTests() {
  ...
  if (test('skip-github report checks local release and security evidence', () => {
    ...
    try {
      ...
      assert.strictEqual(parsed.schema_version, 'oas.platform-audit.v1');
  ...
  if (test('markdown output can be written as an operator artifact', () => {
    ...
    try {
      ...
      assert.ok(written.includes('# OAS Platform Audit'));
  ...
  if (test('discussion-disabled repos skip the heavy discussion query', () => {
    ...
    try {
      seedRepo(projectRoot);
      const shimPath = writeGhShim(projectRoot, {
        'pr list --repo OAS-Tools/OAS-website --state open --json number,title,isDraft,mergeStateStatus,updatedAt,url,author': [],
        'issue list --repo OAS-Tools/OAS-website --state open --json number,title,updatedAt,url,author,labels': [],
