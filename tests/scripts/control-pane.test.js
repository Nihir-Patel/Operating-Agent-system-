...
async function writeMinimalDatabase(dbPath) {
  ...
  db.run(`
    ...
    INSERT INTO sessions (
      ...
      'session-a', 'Build the control pane', 'codex', 'codex', '["codex"]', '/repo/oas',
...
function waitForCliReady(child) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (!settled && stdout.includes('OAS Control Pane:') && stdout.includes('Actions:')) {
        settled = true;
        clearTimeout(timer);
...
async function runTests() {
  ...
  if (
    await test('parses CLI arguments for local-only serving', async () => {
      ...
      assert.strictEqual(parsed.dbPath, '/tmp/oas2.db');
      assert.strictEqual(parsed.stateDbPath, '/tmp/oas-state.db');
  ...
  if (
    await test('serves HTML and snapshot JSON from a temp OAS2 database', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-server-'));
      ...
      try {
        ...
        try {
          ...
          assert.ok(html.includes('OAS Control Pane'));
          ...
          assert.ok(!/onclick="oas(Claim|Move)Item\(/.test(html), 'no inline onclick handlers with interpolated ids');
  ...
  if (
    await test('serves the 3D agent-airspace page and the proximity JSON feed', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-proximity-'));
      const dbPath = path.join(tempDir, 'oas2.db');
  ...
  if (
    await test('serves health, asset, not-found, invalid body, and read-only action responses', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-routes-'));
      ...
      try {
        ...
        try {
          ...
          try {
            const realAsset = await fetchLocal(`${realAssetApp.url}/assets/oas-icon.svg`);
          ...
          const missingAsset = await fetchLocal(`${app.url}/assets/oas-icon.svg`);
  ...
  if (
    await test('runAction captures success, failure, and bounded output', async () => {
      ...
      const spawnError = await runAction({
        id: 'spawn-error',
        command: 'definitely-not-oas-control-pane-command',
        args: [],
        cwd: repoRoot
      });
  ...
  if (
    await test('CLI starts a read-only local server and shuts down on SIGTERM', async () => {
      const child = spawn(process.execPath, [SCRIPT, '--host', '127.0.0.1', '--port', '0', '--read-only', '--no-open'], {
        ...
        env: {
          ...process.env,
          OAS2_DB_PATH: path.join(os.tmpdir(), 'missing-oas2-cli.db')
        }
      });
      ...
      try {
        ...
        assert.match(ready.stdout, /OAS Control Pane: http:\/\/127\.0\.0\.1:\d+/);
  ...
  if (
    await test('interactive board: claim and move work items via POST endpoints', async () => {
      ...
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-control-pane-board-'));
