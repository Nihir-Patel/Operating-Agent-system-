...
assert.equal(releaseMarker('v2.2.0'), '<!-- oas-release:v2.2.0 -->');
...
assert.equal(announcementKey({ repository: 'neal0709/Operating-Agent-system-', discussionId: 'D_kw123' }), 'neal0709/Operating-Agent-system-:discussion:D_kw123');

const payload = buildDiscordPayload({
  title: '@everyone OAS 2.2.0',
  body: 'A'.repeat(5000),
  url: 'https://github.com/neal0709/Operating-Agent-system-/discussions/3000',
  key: 'neal0709/Operating-Agent-system-:discussion:D_kw123',
});
assert.deepEqual(payload.allowed_mentions, { parse: [] });
assert.equal(payload.embeds.length, 1);
assert.ok(payload.embeds[0].description.length <= 4000);
assert.equal(payload.embeds[0].footer.text, 'oas:D_kw123');
assert.equal(payload.embeds[0].url, 'https://github.com/neal0709/Operating-Agent-system-/discussions/3000');
assert.equal(payload.enforce_nonce, true);
assert.match(payload.nonce, /^oas-[a-f0-9]{16}$/);
...
const receiptMarker = discussionReceiptMarker('neal0709/Operating-Agent-system-:discussion:D_kw123');
assert.match(receiptMarker, /^<!-- oas-discord-receipt:[a-f0-9]{32} -->$/);
