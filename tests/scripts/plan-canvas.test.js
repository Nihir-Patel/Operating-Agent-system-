...
async function main() {
  ...
  if (await test('GET /health identifies the app and version', async () => {
    ...
    assert.deepStrictEqual(jsonBody(res), { ok: true, app: 'oas-plan-canvas', version: '9.9.9-test' });
  })) passed++; else failed++;
  ...
  if (await test('GET /canvas/:key serves the OAS chrome with CSP', async () => {
  ...
  if (await test('markdown artifacts render in the OAS plan template with the SDK', async () => {
  ...
  if (await test('GET / lists sessions in the OAS shell', async () => {
