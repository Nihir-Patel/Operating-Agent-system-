...
const oasScript = path.join(repoRoot, 'scripts', 'oas.js');
...
function createFixture(state = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas setup cli '));
...
function runSetup(fixture, args, options = {}) {
  const env = {
    ...
    OAS_TEST_CLAUDE_STATE: fixture.statePath,
...
const SETUP_SPINNER_PATTERN = /[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]\s+Applying OAS setup/;
...
test('an existing install without --scope updates its detected scope', () => {
  withFixture({
    plugins: [{ id: 'oas@oas', scope: 'project', enabled: true, version: '1.9.0' }],
    marketplaces: [{
      name: 'oas',
      source: 'github',
    ...
    assert.ok(readCalls(fixture).some(argv => (
      JSON.stringify(argv) === JSON.stringify([
        'plugin', 'update', 'oas@oas', '--scope', 'project',
      ])
    )));
  });
});
...
test('dry-run leaves a pristine HOME unchanged when Claude inventory creates backups', () => {
  withFixture({}, fixture => {
    ...
    const result = runSetup(fixture, [
      ...
      env: { OAS_TEST_CLAUDE_CREATE_READ_ARTIFACTS: '1' },
    });
    assert.strictEqual(result.status, 0, result.stderr);
    assert.deepStrictEqual(fs.readdirSync(fixture.homeDir), []);
    assert.deepStrictEqual(fs.readdirSync(fixture.projectRoot), []);
...
test('dry-run isolates pre-existing Claude backups and symlinked project settings', () => {
  ...
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(settingsTarget, '{"enabledPlugins":{"oas@oas":true}}\n');
    ...
    const result = runSetup(fixture, [
      ...
      defaultClaudeConfig: true,
      env: {
        OAS_TEST_CLAUDE_CREATE_READ_ARTIFACTS: '1',
        OAS_TEST_CLAUDE_OVERWRITE_READ_ARTIFACTS: '1',
        OAS_TEST_CLAUDE_WRITE_XDG_DATA: '1',
...
test('setup automatically migrates an existing install to the selected scope and hooks', () => {
  withFixture({
    plugins: [{ id: 'oas@oas', scope: 'local', enabled: true, version: '1.9.0' }],
    marketplaces: [{
      name: 'oas',
      source: 'github',
      repo: 'neal0709/Operating-Agent-system-',
    ...
    assert.ok(calls.some(argv => (
      argv.join(' ') === 'plugin install oas@oas --scope user'
        + ' --config hooks_enabled=true --config hook_profile=minimal'
    )));
    assert.ok(calls.some(argv => (
      argv.join(' ') === 'plugin uninstall oas@oas --scope local --keep-data'
    )));
    ...
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hooks_enabled, true);
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hook_profile, 'minimal');
  });
});
...
test('setup resumes a safe two-scope migration without requiring --move-scope', () => {
  withFixture({
    plugins: [
      { id: 'oas@oas', scope: 'local', enabled: true, version: '1.9.0' },
      { id: 'oas@oas', scope: 'user', enabled: true, version: '2.0.0' },
    ],
    marketplaces: [{
      name: 'oas',
      source: 'github',
  ...
  for (const sourceScope of scopes) {
    for (const destinationScope of scopes.filter(scope => scope !== sourceScope)) {
      for (const hookMode of hooks) {
        withFixture({
          plugins: [
            { id: 'oas@oas', scope: sourceScope, enabled: true, version: '1.9.0' },
            { id: 'oas@oas', scope: destinationScope, enabled: true, version: '2.0.0' },
          ],
          marketplaces: [{
            name: 'oas',
            source: 'github',
          ...
          assert.ok(calls.some(argv => (
            argv.join(' ') === `plugin uninstall oas@oas --scope ${sourceScope} --keep-data`
          )));
          const state = JSON.parse(fs.readFileSync(fixture.statePath, 'utf8'));
          assert.deepStrictEqual(state.plugins, [{
            id: 'oas@oas',
          ...
          const stored = settings.pluginConfigs['oas@oas'].options;
...
test('--move-scope remains explicit about its destination', () => {
  withFixture({
    plugins: [{ id: 'oas@oas', scope: 'user', enabled: true, version: '1.9.0' }],
  }, fixture => {
    const result = runSetup(fixture, [
      '--mode', 'claude-plugin',
      '--move-scope',
...
test('destination-only --move-scope is an idempotent first call', () => {
  withFixture({
    plugins: [{ id: 'oas@oas', scope: 'local', enabled: true, version: '2.0.0' }],
  }, fixture => {
    const result = runSetup(fixture, [
      '--mode', 'claude-plugin',
      '--scope', 'local',
...
test('destination-only --move-scope applies explicit hook preferences', () => {
  withFixture({
    ...
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hooks_enabled, true);
    assert.strictEqual(settings.pluginConfigs['oas@oas'].options.hook_profile, 'strict');
  });
});
...
test('migration dry-run JSON exposes ordered actions without mutation', () => {
  withFixture({
    ...
    assert.deepStrictEqual(payload.plannedActions.slice(-4), [
      ['plugin', 'list', '--json'],
      ['plugin', 'list', '--json'],
      ['plugin', 'uninstall', 'oas@oas', '--scope', 'user', '--keep-data'],
      ['plugin', 'list', '--json'],
    ]);
    assert.strictEqual(hasMutation(fixture), false);
  });
...
test('migration JSON failures retain phase, scopes, and exact recovery', () => {
  withFixture({
    ...
    failures: [{
      argv: ['plugin', 'uninstall', 'oas@oas', '--scope', 'user', '--keep-data'],
      status: 9,
    ...
    assert.deepStrictEqual(payload.error.recovery, [
      'claude plugin uninstall oas@oas --scope user --keep-data',
      'oas setup --mode claude-plugin --scope project --move-scope --yes',
    ]);
  });
});
...
test('oas setup delegates to the focused setup command', () => {
  const result = spawnSync(process.execPath, [oasScript, 'setup', '--help'], {
    cwd: repoRoot,
    encoding: 'utf8',
    timeout: 15000,
  });
  assert.strictEqual(result.status, 0, result.stderr);
  assert.match(result.stdout, /OAS (guided )?setup/i);
  assert.match(result.stdout, /claude-plugin/);
});

test('oas setup preserves a real terminal for the interactive wizard', () => {
  if (process.platform === 'win32') return;
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /Where should Claude enable oas@oas\?/);
    assert.match(result.stdout, /How should OAS hooks run\?/);
...
test('confirmed interactive apply starts immediately and clears the spinner on success', () => {
  ...
  withFixture({}, fixture => {
    ...
    assertSetupSpinnerLifecycle(
      ...
      /OAS installed oas@oas at project scope/
    );
  });
});
  ...
  withFixture({
    failures: [{
      argv: [
        ...
        'https://github.com/neal0709/Operating-Agent-system-',
        '--scope', 'user',
      ],
      status: 8,
  ...
  for (const [scopeIndex, scope] of scopes.entries()) {
    for (const [hookIndex, hookMode] of hooks.entries()) {
      withFixture({}, fixture => {
        ...
        assert.ifError(result.error);
        assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
        assert.match(result.stdout, new RegExp(`OAS installed oas@oas at ${scope} scope`));
...
test('all interactive choices from an existing install update or migrate to the selected configuration', () => {
  ...
  for (const [sourceIndex, sourceScope] of scopes.entries()) {
    for (const [selectedIndex, selectedScope] of scopes.entries()) {
      for (const [hookIndex, hookMode] of hooks.entries()) {
        withFixture({
          plugins: [{ id: 'oas@oas', scope: sourceScope, enabled: true, version: '1.9.0' }],
          marketplaces: [{
            name: 'oas',
            source: 'github',
            repo: 'neal0709/Operating-Agent-system-',
          ...
          assert.match(
            result.stdout,
            new RegExp(`OAS ${expectedAction} oas@oas at ${selectedScope} scope`)
          );
          assert.match(result.stdout, new RegExp(`Hook preference: ${hookMode}`));
...
test('interactive named choices install and persist the selected configuration', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.ifError(result.error);
    assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
    assert.match(result.stdout, /OAS installed oas@oas at project scope/);
...
test('invalid interactive choices explain the problem and allow a retry', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /OAS would-install oas@oas at project scope/);
...
test('interactive cancellation after non-default choices performs no mutation', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.ifError(result.error);
    assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
    assert.match(result.stdout, /OAS cancelled oas@oas at project scope/);
    assertNoSetupSpinner(`${result.stdout}${result.stderr}`);
    assert.strictEqual(hasMutation(fixture), false);
...
test('interactive mode flag still prompts for missing scope and hook choices', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /Where should Claude enable oas@oas\?/);
    assert.match(result.stdout, /How should OAS hooks run\?/);
    assert.match(result.stdout, /OAS would-install oas@oas at local scope/);
    assert.match(result.stdout, /Hook preference: strict/);
  });
});
...
test('interactive defaults preserve an existing install scope and hook preference', () => {
  ...
  withFixture({
    ...
    assert.match(result.stdout, /OAS would-update oas@oas at local scope/);
    assert.match(result.stdout, /Hook preference: minimal/);
    assert.strictEqual(hasMutation(fixture), false);
  });
});
...
test('partial migration requires an explicit destination and preserves stored hook defaults', () => {
  ...
  withFixture({
    plugins: [
      { id: 'oas@oas', scope: 'user', enabled: true, version: '1.9.0' },
      { id: 'oas@oas', scope: 'project', enabled: true, version: '2.0.0' },
    ],
    marketplaces: [{
      name: 'oas',
      source: 'github',
    ...
    assert.match(result.stdout, /Please choose 1, 2, or 3/);
    assert.match(result.stdout, /Choose \[2\]:/);
    assert.match(result.stdout, /OAS would-resume oas@oas at project scope/);
