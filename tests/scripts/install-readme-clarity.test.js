...
function runTests() {
  ...
  if (test('README leads with the idempotent guided plugin setup path', () => {
    ...
    const topGuidedCommandIndex = readme.indexOf('npx oas-universal setup', topClaudeSectionIndex);
    const nativePluginCommandIndex = readme.indexOf('/plugin marketplace add', topClaudeSectionIndex);
    const installSectionIndex = readme.indexOf('## Install OAS');
    const guidedCommandIndex = readme.indexOf('npx oas-universal setup', installSectionIndex);
    ...
    assert.ok(
      readme.includes('installs, updates, or safely moves `oas@oas`'),
    ...
    assert.ok(
      ...
      'README should distinguish provider-owned slash behavior from OAS setup behavior'
    );
    assert.ok(
      readme.includes('`/oas:configure-oas`'),
    ...
    assert.ok(
      ...
      'README should not imply that the current setup wizard installs every OAS harness'
    );
  })) passed++; else failed++;

  if (test('README documents modern package-runner alternatives', () => {
    assert.ok(readme.includes('pnpm dlx oas-universal setup'));
    assert.ok(readme.includes('yarn dlx oas-universal setup'));
    assert.ok(readme.includes('bunx oas-universal setup'));
    assert.ok(
  ...
  if (test('README documents reset and uninstall flow', () => {
    assert.ok(
      readme.includes('### Reset / Uninstall OAS'),
    ...
    assert.ok(
      readme.includes('node scripts/oas.js list-installed'),
      'README should document install-state inspection before reinstalling'
    );
    assert.ok(
      readme.includes('node scripts/oas.js doctor'),
    ...
    assert.ok(
      readme.includes('OAS only removes files recorded in its install-state.'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('npx oas-universal install --profile minimal --target claude'),
      'README should document the published universal-package minimal profile command'
    );
    assert.ok(
      !/^\s*npx oas-install\b/m.test(readme),
      'README code examples must not invoke the unpublished oas-install package'
    );
    assert.ok(
  ...
  if (test('README documents consult-based component discovery', () => {
    ...
    assert.ok(
      readme.includes('npx oas-universal consult "security reviews" --target claude'),
  ...
  if (test('README never invokes the unrelated oas npm package', () => {
    assert.ok(
      !/\bnpx oas\s/.test(readme),
      'README one-shot commands should use the published oas-universal package name'
    );
  })) passed++; else failed++;

  if (test('README gives the native guided Codex and managed Kimi dry-run paths', () => {
    assert.ok(
      readme.includes('npx oas-universal install --guided --harness codex --dry-run'),
      'README should verify Codex through the native guided reconciler'
    );
    assert.ok(
      !readme.includes('npx oas-universal install --profile core --target codex --dry-run'),
      'README should not present the legacy managed Codex adapter as the native lifecycle'
    );
    assert.ok(
      readme.includes('npx oas-universal install --profile core --target kimi --dry-run')
    );
  ...
  if (test('README describes the post-release universal install contract', () => {
    assert.ok(
      ...
      'README should state the runtime required by oas-universal'
    );
    assert.ok(
    ...
    assert.ok(
      readme.includes('| OAS hooks | Native plugin hooks | Native reviewed subset with explicit trust |'),
    ...
    assert.ok(
      readme.includes("# Recommended current install: add OAS's native plugin from the repo marketplace"),
    ...
    assert.ok(
      !readme.includes('# Automatic setup: sync OAS assets'),
  ...
  if (test('README documents Cursor agent namespace and loading caveat', () => {
    assert.ok(
      readme.includes('`.cursor/agents/oas-*.md`'),
    ...
    assert.ok(
      readme.includes('OAS does not install root `AGENTS.md` into `.cursor/`.'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    ...
    assert.ok(
      readme.includes('~/.claude/rules/oas/'),
      'README should steer plugin-path rules into an OAS-owned namespace'
    );
  })) passed++; else failed++;

  if (test('rules README mirrors OAS namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.claude/rules/oas'),
      'rules README should create the OAS-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.claude/rules/oas/'),
      'rules README should copy common rules under ~/.claude/rules/oas/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.claude/rules/oas/'),
      'rules README should copy language rules under ~/.claude/rules/oas/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .claude/rules/oas'),
      'rules README should document the project-local OAS namespace'
    );
    assert.ok(
