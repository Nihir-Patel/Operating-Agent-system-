...
function createFixture(extraEnv = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-mcp-'));
...
async function withClient(fn, options = {}) {
  ...
  const initialized = await request('initialize', {
    ...
    capabilities: {},
    clientInfo: { name: 'oas-memory-test', version: '1.0.0' },
  });
...
async function main() {
  ...
  await test('starts when the npm bin invokes the server through a symlink', async () => {
    const binRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-memory-bin-'));
    const binPath = path.join(binRoot, 'oas-memory-mcp');
  ...
  await test('denies user scope unless the server explicitly grants it', async () => {
    ...
    }, { env: { OAS_MEMORY_ALLOW_USER_SCOPE: '1' } });
  });
  ...
  await test('requires server identity and strictly validates JSON-RPC envelopes', async () => {
    ...
    const fixture = createFixture({ OAS_MEMORY_HARNESS: undefined });
    try {
      ...
      assert.match(started.stderr, /OAS_MEMORY_HARNESS/);
