...
function runTests() {
  ...
  if (test('runs from a temporary directory, reads the final response, and cleans up', () => {
    ...
    let removed;
    let verified = false;
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-council-test-'));
  ...
  if (process.env.OAS_CODEX_ISOLATION_INTEGRATION === '1') {
    if (test('real Codex cannot read an adversarial sentinel outside the review directory', () => {
      const sentinelDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-council-sentinel-'));
      const sentinelPath = path.join(sentinelDir, 'outside-review.txt');
      const secret = `OAS-SENTINEL-${Date.now()}-${process.pid}`;
