...
const bashBinary = process.env.OAS_TEST_BASH || (process.platform === 'win32' ? null : 'bash');
...
function executeInstallation(block, scenario) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-repo-scan-install-'));
  try {
...
if (bashBinary) {
  ...
  console.log('  Integration coverage skipped on Windows without OAS_TEST_BASH');
}
