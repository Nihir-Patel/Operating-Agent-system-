...
test('documents the OAS plugin setup harness and safe operating modes', () => {
...
test('documents the private named-container lifecycle and terminal boundary', () => {
  assert.match(skill, /OAS_TMPFS_SIZE/);
  ...
  assert.match(skill, /docker rm.*oas-plugin-shell/is);
...
test('requires the offline smoke to execute the locally packed public bin', () => {
  ...
  assert.match(skill, /package\.json.*bin\.oas/is);
