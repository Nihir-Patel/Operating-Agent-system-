 ...
 *
 * Environment:
 * - OAS_TEST_CLAUDE_STATE: JSON state file (required)
 * - OAS_TEST_CLAUDE_CALLS: JSONL argv log (optional)
 *
 * State supports:
 * {
...
if (!statePath) {
  process.stderr.write('OAS_TEST_CLAUDE_STATE is required\n');
  process.exit(2);
}
...
function createProviderReadArtifacts() {
  ...
  if (process.env.OAS_TEST_CLAUDE_OVERWRITE_READ_ARTIFACTS === '1') {
  ...
  if (
    process.env.OAS_TEST_CLAUDE_WRITE_XDG_DATA === '1'
...
if (args[0] === 'plugin' && args[1] === 'marketplace' && args[2] === 'add') {
  ...
  const marketplaces = [
    ...(state.marketplaces || []).filter(entry => entry.name !== 'oas'),
    {
      name: 'oas',
      source: 'github',
...
if (args[0] === 'plugin' && args[1] === 'install' && args[2] === 'oas@oas') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
  const plugins = [
    ...(state.plugins || []).filter(plugin => (
      plugin.id !== 'oas@oas' || plugin.scope !== scope
    )),
    { id: 'oas@oas', scope, enabled: true, version: '2.0.0' },
  ];
  writeState({ ...state, plugins });
  process.exit(0);
}

if (args[0] === 'plugin' && args[1] === 'update' && args[2] === 'oas@oas') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
