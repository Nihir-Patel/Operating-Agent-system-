...
console.log('\n=== Testing OAS 2.0 release surface ===\n');
...
test('business launch copy stays aligned with the rc.1 public surface', () => {
  ...
  assert.ok(source.includes('OAS v2.0.0-rc.1'), 'business launch copy should use the rc.1 release');
  ...
  assert.ok(source.includes('https://github.com/neal0709/Operating-Agent-system-'), 'business launch copy should include the public repo URL');
  assert.ok(source.includes('https://github.com/neal0709/Operating-Agent-system-/blob/main/docs/releases/2.0.0-rc.1/release-notes.md'), 'business launch copy should link to the rc.1 release notes');
...
test('announcement drafts avoid live-release claims before publication', () => {
  ...
  for (const relativePath of announcementFiles) {
    ...
    assert.ok(!/OAS v2\.0\.0-rc\.1 is live\./.test(source), `${relativePath} must not claim rc.1 is live before the release gate completes`);
  }
});
...
test('Hermes setup uses release-candidate wording for the rc.1 surface', () => {
  ...
  assert.ok(source.includes('OAS v2.0.0-rc.1 documents the Hermes surface'));
...
test('Hermes setup cross-links adjacent migration and architecture docs', () => {
  ...
  assert.ok(!source.includes('0.5. Generate and review artifacts with `oas migrate plan` /'));
});

test('release docs preserve the OAS/Hermes boundary', () => {
  const releaseNotes = read('docs/releases/2.0.0-rc.1/release-notes.md');
  assert.ok(releaseNotes.includes('OAS is the reusable substrate'));
...
test('preview pack manifest assembles release, Hermes, and publication gates', () => {
  ...
  for (const blocker of [
    ...
    'npm `oas-universal@2.0.0-rc.1`',
    'Claude plugin tag',
    'Codex repo-marketplace distribution evidence',
    'OAS Tools billing/product readiness'
  ]) {
...
test('owner approval packet consolidates the final gated decisions', () => {
  ...
  for (const urlSurface of ['GitHub prerelease URL', 'npm rc package URL', 'Claude plugin tag URL', 'Primary launch video URL', 'OAS Tools billing/readiness URL']) {
...
test('GA roadmap mirrors the current May 19 release evidence', () => {
  const roadmap = read('docs/OAS-2.0-GA-ROADMAP.md');
...
test('rc.1 quickstart gives a clone-to-cross-harness path', () => {
  ...
  assert.ok(quickstart.includes('git clone https://github.com/neal0709/Operating-Agent-system-.git'));
  assert.ok(quickstart.includes('cd OAS'));
...
test('launch checklist records the oas2 alpha version policy', () => {
  const cargoToml = read('oas2/Cargo.toml');
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  assert.ok(cargoToml.includes('version = "0.1.0"'));
  assert.ok(launchChecklist.includes('`oas2/Cargo.toml` stays at `0.1.0`'));
  assert.ok(!launchChecklist.includes('confirm whether `oas2/Cargo.toml` moves'));
});
...
test('release video suite manifest gates the content launch lane', () => {
  ...
  for (const marker of [
    ...
    'OAS_VIDEO_SOURCE_ROOT',
    'OAS_VIDEO_RELEASE_SUITE_ROOT',
  ...
  for (const asset of ['longform-full-wide.mp4', 'sf-thread-2-whatisoas.mp4', 'thread-2-ghapp-money.mp4', 'coverage-montage-wide.mp4', 'star_history.png', 'x_analytics.png']) {
...
test('publication readiness checklist gates public release actions on evidence', () => {
  ...
  for (const surface of ['GitHub release', 'npm package', 'Claude plugin', 'Codex plugin', 'Codex repo marketplace', 'OpenCode package', 'OAS Tools billing reference', 'Announcement copy']) {
  ...
  assert.ok(may15Evidence.includes('OAS Tools PR #73'));
  assert.ok(may15Evidence.includes('OAS-Tools PR #75'));
  ...
  assert.ok(source.includes('OAS-Tools #92 main CI'));
  assert.ok(source.includes('OAS-Tools #93 main CI'));
...
test('release name and plugin publication checklist freezes rc.1 surfaces', () => {
  ...
  for (const value of [
    ...
    '`oas-universal`',
    '`oas` on npm is occupied',
    '`@neal0709/oas` is unclaimed on npm',
    'Claude plugin',
    'Codex plugin',
...
test('active release identity surfaces use canonical OAS repo URLs', () => {
  const activeFiles = [
    'README.md',
  ...
  for (const relativePath of activeFiles) {
    const source = read(relativePath);
    if (source.includes('neal0709/operating-agent-systems')) {
      offenders.push(relativePath);
    }
  }
