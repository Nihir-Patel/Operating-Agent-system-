...
console.log('\n=== Testing configure-oas install path guidance ===\n');
...
for (const relativePath of configureEccDocs) {
  test(`${relativePath} delegates to guided plugin setup`, () => {
    ...
    assert.ok(content.includes('oas setup'));
    assert.ok(content.includes('npx oas-universal setup'));
    ...
    assert.ok(!content.includes('rm -rf /tmp/operating-agent-systems'));
    assert.ok(!content.includes('cp -R "$OAS_ROOT'));
  });
  ...
  test(`${relativePath} defines the Claude in-harness wizard contract`, () => {
    ...
    assert.ok(content.includes('OAS_VERSION_PATTERN'));
  ...
  test(`${relativePath} keeps provider capabilities truthful`, () => {
    ...
    assert.ok(content.includes('codex plugin add oas@oas --json'));
  ...
  test(`${relativePath} verifies Codex and Kimi before their concrete welcomes`, () => {
    ...
    const kimiVerifyIndex = content.indexOf('oas doctor --target kimi');
    const kimiWelcomeIndex = content.indexOf('oas welcome --action configured');
...
test('Codex legacy sync docs do not require an unrelated package install', () => {
  ...
  assert.ok(content.includes('bash scripts/sync-oas-to-codex.sh'));
  assert.ok(!content.includes('npm install && bash scripts/sync-oas-to-codex.sh'));
});
