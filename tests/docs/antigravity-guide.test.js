...
test('guide requires an installer with native Antigravity 2.0 support', () => {
  assert.ok(
    guide.includes('OAS 2.2.0 or newer'),
    'Guide should state the minimum OAS version that installs into .agents'
  );
});

test('guide uses the published 2.2 package without stale pre-release copy', () => {
  assert.ok(
    guide.includes('npm view oas-universal version'),
    'Guide should let operators verify registry propagation before installation'
  );
  assert.ok(
    guide.includes('npx oas-universal@2.2.0 install --profile minimal --target antigravity'),
    'Guide should provide the pinned published-package installation path'
  );
  assert.ok(
    !guide.includes('OAS 2.2.0 has not been published to npm yet'),
    'The immutable 2.2 guide must not claim that 2.2 is unpublished'
  );
  assert.ok(
    !guide.includes('npm latest is currently `oas-universal@2.1.0`'),
...
test('guide keeps the target project as the working directory', () => {
  ...
  assert.ok(
    guide.includes('OAS_ROOT="/absolute/path/to/OAS"'),
    'Guide should define an absolute OAS source path separately from the target project'
  );
  assert.ok(
    guide.includes('$EccRoot = "C:\\absolute\\path\\to\\OAS"'),
...
test('guide installs through dependency-bootstrapping wrappers', () => {
  assert.ok(guide.includes('"$OAS_ROOT/install.sh" --profile minimal --target antigravity'));
  assert.ok(guide.includes('& "$EccRoot\\install.ps1" --profile minimal --target antigravity'));
  assert.ok(
    !guide.includes('node "$OAS_ROOT/scripts/install-apply.js"'),
    'Fresh source installs should not bypass the wrapper dependency bootstrap'
  );
});

test('guide invokes every post-install lifecycle script through the absolute OAS source path', () => {
  for (const script of ['list-installed.js', 'doctor.js', 'repair.js', 'uninstall.js']) {
    assert.ok(
      guide.includes(`node "$OAS_ROOT/scripts/${script}"`),
      `Guide should invoke ${script} through OAS_ROOT`
    );
    assert.ok(
  ...
  assert.ok(!guide.includes('./install.sh'), 'Guide should not target the current project through a relative OAS installer path');
  assert.ok(!/node scripts\/(?:list-installed|doctor|repair|uninstall)\.js/.test(guide));
});

test('repository has no accidental nested OAS gitlink', () => {
  assert.ok(
    !fs.existsSync(path.join(repoRoot, 'OAS')),
    'The documentation PR should not add an OAS gitlink without .gitmodules metadata'
  );
});
