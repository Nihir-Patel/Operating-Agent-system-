...
test('token optimization guide separates Claude MCP disables from OAS config filters', () => {
  ...
  assert.ok(
    source.includes('`OAS_DISABLED_MCPS` only affects OAS-generated MCP config output'),
    'Token guide should scope OAS_DISABLED_MCPS to config generation'
  );
  assert.ok(
...
test('README MCP guidance avoids settings.json disable instructions', () => {
  ...
  assert.ok(
    source.includes('`OAS_DISABLED_MCPS` is an OAS install/sync filter, not a live Claude Code toggle.'),
    'README should explain OAS_DISABLED_MCPS scope'
  );
  assert.ok(
