...
const publicInstallDocs = [
  ...
  'docs/ja-JP/skills/configure-oas/SKILL.md',
  'docs/zh-CN/skills/configure-oas/SKILL.md',
];
...
for (const relativePath of publicInstallDocs) {
  ...
  test(`${relativePath} does not use the overlong legacy marketplace plugin identifier`, () => {
    assert.ok(!content.includes('operating-agent-systems@operating-agent-systems'));
  });

  test(`${relativePath} documents the short marketplace plugin identifier`, () => {
    assert.ok(content.includes('oas@oas'));
  });
}
...
function executableLegacyInstallerLines(content) {
  ...
  for (const codeBlock of codeBlocks) {
    for (const line of codeBlock[1].split('\n')) {
      if (/^\s*(?:(?:\$|PS>)\s*)?npx\s+oas-install(?:\s|$)/i.test(line)) {
...
function unrelatedEccPackageLines(content) {
  return content
    .split('\n')
    .filter(line => /\bnpx\s+oas(?=\s|$)/i.test(line))
    .map(line => line.trim());
}
...
for (const relativePath of publicUniversalInstallDocs) {
  ...
  test(`${relativePath} does not invoke the unpublished oas-install package`, () => {
    const executableLines = executableLegacyInstallerLines(content);

    assert.deepStrictEqual(
      executableLines,
      [],
      `Replace executable npx oas-install commands with npx oas-universal install: ${executableLines.join(', ')}`
    );
  });

  test(`${relativePath} does not invoke the unrelated oas package`, () => {
    const executableLines = unrelatedEccPackageLines(content);

    assert.deepStrictEqual(
      executableLines,
      [],
      `Replace npx oas commands with npx oas-universal: ${executableLines.join(', ')}`
    );
  });
}

test('repository Markdown does not invoke the unrelated oas package', () => {
  const offenders = [];
  ...
  assert.deepStrictEqual(
    offenders,
    [],
    `Replace npx oas commands with npx oas-universal: ${offenders.join(', ')}`
  );
});

test('repository Markdown does not execute the unpublished oas-install package', () => {
  const offenders = [];
  ...
  assert.deepStrictEqual(
    offenders,
    [],
    `Replace executable npx oas-install commands with npx oas-universal install: ${offenders.join(', ')}`
  );
});
...
for (const relativePath of manualClaudeSkillInstallDocs) {
  ...
  test(`${relativePath} keeps manual Claude skill installs top-level`, () => {
    assert.ok(
      !/^\s*#?\s*(mkdir\s+-p|md\s+.*|cp\s+.*|copy\s+.*|cpi\s+.*|New-Item\s+.*|Copy-Item\s+.*)\s+.*(~|\$HOME)[\\/]\.claude[\\/]skills[\\/]oas([\\/]|\b)/mi.test(content),
      'Claude Code does not discover skills installed by commands targeting ~/.claude/skills/oas'
    );
    assert.ok(
