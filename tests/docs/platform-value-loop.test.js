...
console.log('\n=== Testing OAS platform value loop docs ===\n');

test('platform value loop doc defines the three-layer OAS 2.0 direction', () => {
  ...
  for (const marker of [
    ...
    'reproducible demo',
    'OAS can be used full-stack as a meta-harness + agent + control pane',
  ]) {
...
test('Ito example preserves non-advisory and gated-access boundaries', () => {
  ...
  for (const marker of [
    ...
    'do not merge OAS Tools billing with Ito billing',
  ]) {
...
test('platform value loop does not overclaim release status or trading ability', () => {
  ...
  const forbidden = [
    ...
    'public OAS skills place trades',
  ];
