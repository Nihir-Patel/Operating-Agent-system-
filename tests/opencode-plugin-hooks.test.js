...
async function withTempProject(files, fn) {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "oas-opencode-plugin-"))
  try {
...
async function main() {
  ...
  const { OASHooksPlugin } = await loadPlugin()
  const tests = [
    [
      ...
      async () => withTempProject([], async (projectDir) => {
        ...
        try {
          ...
          const hooks = await OASHooksPlugin({ client, $, directory: projectDir })

          const disabledWarnings = client.logs.filter(
            (entry) =>
              entry.level === "warn" &&
              entry.message.includes("[OAS] changed-files tracking disabled") &&
              entry.message.includes("oas repair --target opencode")
          )
          assert.strictEqual(
    ...
    [
      ...
      async () => withTempProject(
        ...
        async (projectDir) => {
          ...
          // Verify OAS_VERSION is not hardcoded
          assert.ok(env.OAS_VERSION !== "1.8.0", "OAS_VERSION should not be hardcoded to 1.8.0")
          assert.ok(env.OAS_VERSION.match(/^\d+\.\d+\.\d+$/), "OAS_VERSION should be a valid semver version")
        }
      ),
    ],
    [
      ...
      async () => withTempProject(["CLAUDE.md"], async (projectDir) => {
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[OAS] Found CLAUDE.md - loading project context"),
