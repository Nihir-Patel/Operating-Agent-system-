/**
 * Tests for scripts/hooks/oas-metrics-bridge.js
 *
 * Run with: node tests/hooks/oas-metrics-bridge.test.js
 */
...
const { run, hashToolCall, extractFilePaths, readSessionCost } = require('../../scripts/hooks/oas-metrics-bridge');
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-metrics-bridge-test-'));
}

function runTests() {
  console.log('\n=== Testing oas-metrics-bridge.js ===\n');
  ...
  if (
    test('readSessionCost suppresses repeated malformed breadcrumbs across hook subprocesses', () => {
      ...
      try {
        ...
        const bridgePath = path.resolve(__dirname, '../../scripts/hooks/oas-metrics-bridge');
