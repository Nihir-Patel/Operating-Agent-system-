 ...
 * OAS_OBSERVER_NOSURVIVE_WARN_AFTER (default 3). Finding the observer alive
 * resets the streak.
 *
...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-nosurvive-'));
}
...
function buildSandbox() {
  ...
  const configDir = path.join(projectDir, '.local', 'share', 'oas-homunculus');
...
test('the non-survival warning is threshold-gated, not logged every call', () => {
  const content = fs.readFileSync(observeShPath, 'utf8');
  assert.ok(
    /OAS_OBSERVER_NOSURVIVE_WARN_AFTER/.test(content),
    'the threshold should be overridable via OAS_OBSERVER_NOSURVIVE_WARN_AFTER'
  );
  assert.ok(
...
async function runWarnsAtThreshold() {
  ...
  try {
    ...
    await runObserve(testObserve, projectDir, { OAS_OBSERVER_NOSURVIVE_WARN_AFTER: '1' });
    ...
    assert.ok(
      log.includes('OAS_OBSERVER_NOSURVIVE_WARN_AFTER'),
...
async function runSilentBelowThreshold() {
  ...
  try {
    fs.writeFileSync(path.join(projectDir, '.observer.pid'), `${deadPid()}\n`);
    await runObserve(testObserve, projectDir, { OAS_OBSERVER_NOSURVIVE_WARN_AFTER: '3' });
...
async function runRejectsZeroThreshold() {
  ...
  try {
    for (let i = 0; i < 3; i++) {
      fs.writeFileSync(path.join(projectDir, '.observer.pid'), `${deadPid()}\n`);
      await runObserve(testObserve, projectDir, { OAS_OBSERVER_NOSURVIVE_WARN_AFTER: '00' });
    }
...
async function runLeadingZeroThreshold() {
  ...
  try {
    ...
    await runObserve(testObserve, projectDir, { OAS_OBSERVER_NOSURVIVE_WARN_AFTER: '08' });
