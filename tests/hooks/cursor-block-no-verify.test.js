...
function runWrapper(input, env = {}) {
  ...
  const result = spawnSync('node', [wrapper], {
    input: rawInput,
    encoding: 'utf8',
    env: { ...process.env, OAS_HOOK_PROFILE: 'standard', ...env },
    timeout: 15000,
...
// --- Disable via OAS_DISABLED_HOOKS ---

if (test('respects OAS_DISABLED_HOOKS=pre:bash:block-no-verify', () => {
  const r = runWrapper(
    { command: 'git commit --no-verify -m "msg"' },
    { OAS_DISABLED_HOOKS: 'pre:bash:block-no-verify' }
  );
