 ...
 * (the actual entry point used by OAS plugin hooks, NOT run-with-flags.js)
...
const FIXTURE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-pr2380-fixtures-'));
...
function assertNestedChildCleanup(label, childSource, options) {
  ...
  const result = runSupervised(
    ...
    '',
    { OAS_TEST_CHILD_PID_FILE: pidPath },
    options
  );
...
const persistentChildSource = `
  const fs = require('fs');
  fs.writeFileSync(process.env.OAS_TEST_CHILD_PID_FILE, String(process.pid));
  setInterval(() => {}, 1000);
`;
...
// --- THE CORE OAS PATTERN: most OAS hooks do `process.stdout.write(run(data))`
...
if (
  test('CORE OAS PATTERN: hook returning raw input as stdout is suppressed', () => {
    ...
    // transcript bloat — 12+ OAS hook scripts use this exact pattern.
