...
function run(args = [], options = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      OAS_PLUGIN_ROOT: options.oasRoot || '',
      ...(options.env || {}),
    },
...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/oas', 'win32'),
      'C:/Users/x/.claude/plugins/oas'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/z/Work/OAS/scripts/hooks/check-console-log.js', 'win32'),
      'Z:/Work/OAS/scripts/hooks/check-console-log.js'
    );
  })) passed++; else failed++;
  ...
  if (test('leaves already-Windows roots unchanged', () => {
    ...
    assert.strictEqual(
      normalizePluginRootForPlatform('D:\\Users\\x\\.claude\\plugins\\oas', 'win32'),
      'D:\\Users\\x\\.claude\\plugins\\oas'
    );
  })) passed++; else failed++;

  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/oas', 'darwin'),
      '/c/Users/x/.claude/plugins/oas'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/oas', 'linux'),
      '/c/Users/x/.claude/plugins/oas'
    );
  })) passed++; else failed++;
  ...
  if (test('does not mangle UNC or non-drive absolute paths on Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('\\\\server\\share\\oas', 'win32'),
      '\\\\server\\share\\oas'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/workspace/oas', 'win32'),
      '/workspace/oas'
    );
  })) passed++; else failed++;
...
process.stdout.write(JSON.stringify({
  ...
  oasRoot: process.env.OAS_PLUGIN_ROOT,
}));
`);
      ...
      assert.strictEqual(parsed.oasRoot, root);
    } finally {
      cleanup(root);
    }
  })) passed++; else failed++;
