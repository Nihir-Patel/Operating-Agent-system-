...
function loadHook(stateDir) {
  ...
  process.env.OAS_PLAN_CANVAS_STATE_DIR = stateDir;
  return require(HOOK);
}
...
async function runTests() {
  ...
  let passed = 0;
  let failed = 0;
  const originalStateDir = process.env.OAS_PLAN_CANVAS_STATE_DIR;
  ...
  if (await test('ignores sessions outside the project, unless scope=all', async () => {
    ...
    assert.strictEqual(
      hook.pendingSessions({ sessions: state }, projectDir, { OAS_PLAN_CANVAS_STOP_SCOPE: 'all' }).length,
      1
    );
  })) passed++; else failed++;
  ...
  if (originalStateDir === undefined) delete process.env.OAS_PLAN_CANVAS_STATE_DIR;
  else process.env.OAS_PLAN_CANVAS_STATE_DIR = originalStateDir;
