...
function runTests() {
  ...
  if (test('hooks default enabled when no preference source exists', () => {
    withEnv({
      OAS_HOOKS_ENABLED: undefined,
      CLAUDE_PLUGIN_OPTION_HOOKS_ENABLED: undefined,
      OAS_HOOK_CONFIG: undefined,
      CLAUDE_PLUGIN_ROOT: undefined,
      OAS_PLUGIN_ROOT: undefined,
    }, () => {
  ...
  if (test('explicit OAS environment overrides Claude plugin options', () => {
    withEnv({
      OAS_HOOKS_ENABLED: 'true',
      OAS_HOOK_PROFILE: 'strict',
      CLAUDE_PLUGIN_OPTION_HOOKS_ENABLED: 'false',
      CLAUDE_PLUGIN_OPTION_HOOK_PROFILE: 'minimal',
    }, () => {
      assert.strictEqual(areHooksEnabled(), true);
    ...
    assert.strictEqual(
      ...
      'standard',
      'an explicit empty OAS profile must not fall through to plugin config'
    );
  })) passed++; else failed++;

  if (test('managed hook config is used after explicit and plugin preferences', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-hook-flags-'));
    const configPath = path.join(root, 'oas', 'setup.json');
    try {
  ...
  if (test('a hook evaluation reads managed config only once', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-hook-flags-read-once-'));
    ...
    try {
      ...
      assert.strictEqual(isHookEnabled('pre:test', {
        env: { OAS_HOOK_CONFIG: configPath },
        profiles: ['minimal'],
      }), true);
  ...
  if (test('malformed managed config emits one sanitized diagnostic', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-hook-flags-invalid-'));
    const configPath = path.join(root, 'setup.json');
    ...
    try {
      ...
      assert.deepStrictEqual(readManagedHookConfig({ OAS_HOOK_CONFIG: configPath }), {});
      assert.strictEqual(diagnostics.length, 1);
      assert.match(diagnostics[0], /Warning: unable to read managed OAS hook config/);
  ...
  if (test('run-with-flags suppresses wrapper hooks when plugin hooks are off', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-hook-wrapper-'));
  ...
  if (test('returns minimal when set to minimal', () => {
    withEnv({ OAS_HOOK_PROFILE: 'minimal' }, () => {
      assert.strictEqual(getHookProfile(), 'minimal');
    });
  })) passed++; else failed++;

  if (test('returns standard when set to standard', () => {
    withEnv({ OAS_HOOK_PROFILE: 'standard' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;

  if (test('returns strict when set to strict', () => {
    withEnv({ OAS_HOOK_PROFILE: 'strict' }, () => {
      assert.strictEqual(getHookProfile(), 'strict');
    });
  })) passed++; else failed++;
  ...
  if (test('defaults to standard for invalid value', () => {
    withEnv({ OAS_HOOK_PROFILE: 'invalid' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;

  if (test('defaults to standard for empty string', () => {
    withEnv({ OAS_HOOK_PROFILE: '' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;
  ...
  if (test('returns empty Set when env var not set', () => {
    withEnv({ OAS_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('parses single hook id', () => {
    withEnv({ OAS_DISABLED_HOOKS: 'my-hook' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('parses multiple comma-separated hook ids', () => {
    withEnv({ OAS_DISABLED_HOOKS: 'hook-a,hook-b,hook-c' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('trims whitespace around hook ids', () => {
    withEnv({ OAS_DISABLED_HOOKS: ' hook-a , hook-b ' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('normalizes hook ids to lowercase', () => {
    withEnv({ OAS_DISABLED_HOOKS: 'MyHook,ANOTHER' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('returns true by default for a hook (standard profile)', () => {
    withEnv({ OAS_HOOK_PROFILE: undefined, OAS_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns false when hook is in disabled list', () => {
    withEnv({ OAS_HOOK_PROFILE: undefined, OAS_DISABLED_HOOKS: 'my-hook' }, () => {
  ...
  if (test('returns true when hook is not in disabled list', () => {
    withEnv({ OAS_HOOK_PROFILE: undefined, OAS_DISABLED_HOOKS: 'other-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;

  if (test('returns false when current profile is not in allowed profiles', () => {
    withEnv({ OAS_HOOK_PROFILE: 'minimal', OAS_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns true when current profile is in allowed profiles', () => {
    withEnv({ OAS_HOOK_PROFILE: 'strict', OAS_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('disabled hooks take precedence over profile match', () => {
    withEnv({ OAS_HOOK_PROFILE: 'strict', OAS_DISABLED_HOOKS: 'my-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook', { profiles: 'strict' }), false);
    });
  })) passed++; else failed++;
  ...
  if (test('allows standard profile by default', () => {
    withEnv({ OAS_HOOK_PROFILE: 'standard', OAS_DISABLED_HOOKS: undefined }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;
