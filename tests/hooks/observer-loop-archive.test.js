...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-observer-archive-'));
}
...
function runAnalyzeOnce(claudeExitCode) {
  ...
  try {
    ...
    // Defensive: never leak CLAUDE_PLUGIN_ROOT into the OAS test shell (it
    ...
    childEnv.OAS_OBSERVER_TIMEOUT_SECONDS = '2';
