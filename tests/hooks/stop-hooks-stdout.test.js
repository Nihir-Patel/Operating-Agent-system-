 ...
 *
 * Stop hooks follow the OAS pass-through convention (echo stdin on stdout).
...
const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-stop-stdout-')); // non-git cwd
const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-stop-data-'));
...
function hookEnv() {
  const env = {
    ...
    OAS_AGENT_DATA_HOME: dataHome,
    CLAUDE_SESSION_ID: `stop-stdout-test-${process.pid}`
  };
  delete env.OAS_GATEGUARD;
  delete env.OAS_DISABLED_HOOKS;
