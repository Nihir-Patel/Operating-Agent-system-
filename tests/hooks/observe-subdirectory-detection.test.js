...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-observe-subdir-test-'));
}
...
function gitInit(dir) {
  ...
  const remoteResult = spawnSync(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/example/oas-test.git'],
...
function readSingleProjectMetadata(homeDir) {
  const projectsDir = path.join(homeDir, '.local', 'share', 'oas-homunculus', 'projects');
