...
function runDispatcher(mode, toolName, env = {}) {
  const raw = JSON.stringify({
    ...
    tool_name: toolName,
    tool_input: toolName === 'Bash' ? { command: 'true' } : { file_path: path.join(os.tmpdir(), 'oas-posttooluse-test.txt') },
    tool_response: {}
  });
...
function runTests() {
  ...
  if (
    test('hooks.json exposes one sync and one async PostToolUse entry', () => {
      ...
      assert.ok(entries.every(entry => entry.hooks[0].command.includes('resolve-oas-root')));
      assert.ok(
      ...
      const cases = [
        {
          ...
          sync: [
            ...
            'post:oas-metrics-bridge',
            'post:oas-context-monitor'
          ],
          async: ['post:quality-gate', 'post:observe:continuous-learning']
        },
        {
          tool: 'Write',
          sync: ['post:edit:design-quality-check', 'post:edit:accumulator', 'post:governance-capture', 'post:session-activity-tracker', 'post:oas-metrics-bridge', 'post:oas-context-monitor'],
          async: ['post:quality-gate', 'post:observe:continuous-learning']
        },
        {
          tool: 'Bash',
          sync: ['post:governance-capture', 'post:session-activity-tracker', 'post:oas-metrics-bridge', 'post:oas-context-monitor'],
          async: ['post:bash:dispatcher', 'post:observe:continuous-learning']
        },
        {
          tool: 'Read',
          sync: ['post:session-activity-tracker', 'post:oas-metrics-bridge', 'post:oas-context-monitor'],
          async: ['post:observe:continuous-learning']
        }
      ];

      for (const expected of cases) {
        const sync = runDispatcher('sync', expected.tool, { OAS_DRY_RUN: '1' });
        const asyncResult = runDispatcher('async', expected.tool, { OAS_DRY_RUN: '1' });
  ...
  if (
    test('actual hooks.json commands preserve Edit dry-run output and IDs', () => {
      ...
      const raw = JSON.stringify({
        hook_event_name: 'PostToolUse',
        tool_name: 'Edit',
        tool_input: { file_path: path.join(os.tmpdir(), 'oas-posttooluse-test.txt') },
        tool_response: {}
      });
      const results = entries.map(entry => runConfiguredCommand(entry, raw, { OAS_DRY_RUN: '1' }));
    ...
    test('actual hooks.json commands never echo truncated oversized input', () => {
      ...
      for (const value of values) {
        ...
        for (const entry of entries) {
          const result = runConfiguredCommand(entry, raw, { OAS_DRY_RUN: '1' });
          assert.strictEqual(result.status, 0, result.stderr);
  ...
  if (
    test('profiles and disabled IDs remain scoped to each original hook', () => {
      ...
      assert.deepStrictEqual(previewedIds(minimalSync.stderr), ['post:oas-metrics-bridge']);
      ...
      const disabled = runDispatcher('sync', 'Edit', {
        OAS_DRY_RUN: '1',
        OAS_DISABLED_HOOKS: 'post:edit:accumulator'
      });
      ...
      assert.ok(ids.includes('post:oas-context-monitor'));
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('dry-run has no PostToolUse side effects', () => {
      const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-posttooluse-dry-run-'));
      try {
  ...
  if (
    test('dispatcher isolates failures and preserves explicit output and exit status', () => {
      ...
      const result = runHooks(raw, hooks, { toolName: 'Read', env: { OAS_HOOK_PROFILE: 'standard' } });
      assert.deepStrictEqual(
        calls,
        [
          ['first', raw],
  ...
  if (
    test('failing hook exit code propagates to the real dispatcher process status', () => {
      ...
      const result = spawnSync(process.execPath, ['-e', script], {
        ...
        encoding: 'utf8',
        env: { ...process.env, CLAUDE_PLUGIN_ROOT: repoRoot, OAS_POSTTOOLUSE_PASSTHROUGH: '1' },
        timeout: 10000
      });
