 ...
 * Layer 2 (the OAS_HOOK_PROFILE check) while denied entrypoints stop at
 * Layer 1's `exit 0`. We force Layer 2 to short-circuit via
 * OAS_HOOK_PROFILE=minimal so the test is fast and side-effect-free
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'oas-observe-allowlist-'));
}
 ...
 * Layer 2 is forced to short-circuit (OAS_HOOK_PROFILE=minimal) so the only
...
// `[ "${OAS_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0` therefore
