...
function runTests() {
  ...
  // 6. Prefers OAS_SESSION_ID for OAS2 session correlation
  (test('prefers OAS_SESSION_ID over CLAUDE_SESSION_ID when both are present', () => {
    const tmpHome = makeTempDir();
    const input = {
      model: 'claude-sonnet-4-20250514',
    ...
    const metricsFile = path.join(tmpHome, '.claude', 'metrics', 'costs.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'oas-session-1234', 'Expected OAS_SESSION_ID to win');
