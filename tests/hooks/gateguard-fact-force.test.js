...
function runTests() {
  ...
  // --- Test 9: respects OAS_DISABLED_HOOKS ---
  clearState();
  if (
    test('respects OAS_DISABLED_HOOKS (skips when disabled)', () => {
      const input = {
        tool_name: 'Edit',
        tool_input: { file_path: '/src/disabled.js', old_string: 'a', new_string: 'b' }
      };
      const result = runHook(input, {
        OAS_DISABLED_HOOKS: 'pre:edit-write:gateguard-fact-force'
      });
  ...
  clearState();
  if (
    test('respects OAS_GATEGUARD=off without writing gate state', () => {
      const input = {
        tool_name: 'Write',
        tool_input: { file_path: '/src/env-disabled.js', content: 'export const ok = true;' }
      };
      const result = runHook(input, { OAS_GATEGUARD: 'off' });
      const output = parseOutput(result.stdout);
  ...
  if (
    test('denial messages include direct recovery escape hatch', () => {
      ...
      assert.strictEqual(output.hookSpecificOutput.permissionDecision, 'deny');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('OAS_GATEGUARD=off'), 'denial reason should show the direct recovery env toggle');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('OAS_DISABLED_HOOKS'), 'denial reason should mention the existing hook-id disable control');
  ...
  if (
    test('destructive Bash denials omit recovery escape hatch', () => {
      ...
      assert.ok(!output.hookSpecificOutput.permissionDecisionReason.includes('OAS_GATEGUARD=off'), 'destructive gate should not advertise disabling GateGuard');
      assert.ok(!output.hookSpecificOutput.permissionDecisionReason.includes('OAS_DISABLED_HOOKS'), 'destructive gate should not advertise disabling its hook');
  ...
  function runFreshSessionEdit(filePath, extra = {}) {
    return runHook(
      ...
      { CLAUDE_SESSION_ID: '', OAS_SESSION_ID: '' }
    );
  }
  ...
  if (
    test('emits a condensed single-line denial once the full-block budget is spent', () => {
      ...
      assert.ok(reason.includes('OAS_GATEGUARD=off'), 'condensed message keeps a recovery hint');
