...
function assertNoProjectDetectionSideEffects(homeDir, testName) {
  const homunculusDir = path.join(homeDir, '.local', 'share', 'oas-homunculus');
...
async function runTests() {
  ...
  if (
    await asyncTest('exits 0 even with isolated empty HOME', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-iso-start-'));
  ...
  if (
    await asyncTest('skips template session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-tpl-start-'));
  ...
  if (
    await asyncTest('injects real session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-real-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('caps very large session-start context by default', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-large-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('honors OAS_SESSION_START_MAX_CHARS for injected context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-max-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          OAS_SESSION_START_MAX_CHARS: '700'
        });
        assert.strictEqual(result.code, 0);
        const additionalContext = getSessionStartAdditionalContext(result.stdout);
  ...
  if (
    await asyncTest('honors OAS_MAX_INJECTED_INSTINCTS for injected instincts', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-max-instincts-'));
      ...
      try {
        ...
        const capped = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_MAX_INJECTED_INSTINCTS: '3' });
        ...
        const garbage = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_MAX_INJECTED_INSTINCTS: 'not-a-number' });
        ...
        const partial = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_MAX_INJECTED_INSTINCTS: '3.9' });
        ...
        const exponent = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_MAX_INJECTED_INSTINCTS: '1e2' });
  ...
  if (
    await asyncTest('honors OAS_INSTINCT_CONFIDENCE_THRESHOLD for injected instincts', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-instinct-threshold-'));
      const homunculusDir = path.join(isoHome, 'homunculus');
      const instinctsDir = path.join(homunculusDir, 'instincts', 'personal');
      fs.mkdirSync(instinctsDir, { recursive: true });
      ...
      try {
        ...
        const raised = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_INSTINCT_CONFIDENCE_THRESHOLD: '0.95' });
        ...
        const lowered = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_INSTINCT_CONFIDENCE_THRESHOLD: '0.5' });
        ...
        const hex = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, OAS_INSTINCT_CONFIDENCE_THRESHOLD: '0x1' });
  ...
  if (
    await asyncTest('ranks stack-relevant instincts above higher-confidence unrelated ones (#2371)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-instinct-relevance-'));
      const homunculusDir = path.join(isoHome, 'homunculus');
      const instinctsDir = path.join(homunculusDir, 'instincts', 'personal');
      fs.mkdirSync(instinctsDir, { recursive: true });
      ...
      const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-tf-project-'));
      ...
      const baseEnv = {
        ...
        OAS_INSTINCT_RELEVANCE_RANKING: 'on',
        OAS_INSTINCT_CONFIDENCE_THRESHOLD: '0.7',
        OAS_MAX_INJECTED_INSTINCTS: '6',
      };
      ...
      try {
        ...
        const off = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          ...baseEnv,
          OAS_INSTINCT_RELEVANCE_RANKING: 'off',
        });
  ...
  if (
    await asyncTest('disables session-start additional context when requested', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-disabled-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('prefers canonical session-data content over legacy duplicates', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-canonical-start-'));
  ...
  if (
    await asyncTest('strips ANSI escape codes from injected session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-ansi-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on Desktop SessionStart resume', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-resume-start-'));
  ...
  if (
    await asyncTest('skips prior session summary on CLI SessionStart resume', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-cli-resume-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on clear SessionStart payloads', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-clear-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not log malformed SessionStart stdin content while skipping prior summary', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-invalid-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not fall back to unrelated recent session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-cross-project-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject same-project sessions from a different explicit worktree', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-cross-worktree-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('allows project fallback only for legacy sessions without worktree metadata', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-project-only-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reports learned skills count', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-skills-start-'));
  ...
  if (
    await asyncTest('injects learned skills into session-start additional context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-skills-context-'));
      const learnedDir = path.join(isoHome, '.claude', 'skills', 'learned');
      fs.mkdirSync(learnedDir, { recursive: true });
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-create-'));
  ...
  if (
    await asyncTest('includes session ID in filename', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-id-'));
  ...
  if (
    await asyncTest('derives shortId from transcript_path UUID when available', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-transcript-'));
  ...
  if (
    await asyncTest('normalizes transcript UUID shortId to lowercase', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-transcript-upper-'));
      const transcriptUuid = 'ABCDEF12-3456-4789-A012-BCDEF3456789';
  ...
  if (
    await asyncTest('matches getSessionIdShort when transcript UUID equals CLAUDE_SESSION_ID', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-transcript-match-'));
  ...
  if (
    await asyncTest('writes project, branch, and worktree metadata into new session files', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-session-metadata-'));
  ...
  if (
    await asyncTest('annotates active session file with compaction marker', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-annotate-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('compaction log contains timestamp', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-ts-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    test('SessionStart hook uses safe inline resolver without plugin-tree scanning', () => {
      ...
      assert.ok(bootstrapSrc.includes('resolve-oas-root'), 'Bootstrap should delegate to the committed resolver module');
  ...
  if (
    test('Stop and SessionEnd hooks use the safe inline resolver when plugin root may be unset', () => {
      ...
      for (const hook of [...stopHooks, ...sessionEndHooks]) {
        ...
        assert.ok(commandText.includes('resolve-oas-root'), 'Lifecycle hook should delegate to the committed resolver module');
        ...
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            ...
            if (hook.type === 'command' && commandText.includes('scripts/hooks/')) {
              ...
              const usesDirectPostDispatcher = commandStart.startsWith('node -e') && commandText.includes('posttooluse-dispatcher.js') && commandText.includes('resolve-oas-root');
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/neal0709/operating-agent-systems/issues/103
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('OAS_OBSERVER_MAX_TURNS'), 'observer-loop should allow max-turn overrides');
      ...
      assert.ok(observerLoopSource.includes('OAS_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions');
      assert.ok(observerLoopSource.includes('OAS_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        spawnSync('git', ['remote', 'add', 'origin', 'https://github.com/example/oas-test.git'], { cwd: repoDir, stdio: 'ignore' });
        ...
        const registryPath = path.join(homeDir, '.local', 'share', 'oas-homunculus', 'projects.json');
        const expectedProjectDir = path.join(homeDir, '.local', 'share', 'oas-homunculus', 'projects', projectId);
        ...
        assert.strictEqual(metadata.remote, 'https://github.com/example/oas-test.git', 'project.json should include the sanitized remote');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('observe.sh skips minimal hook profile before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', OAS_HOOK_PROFILE: 'minimal' }
      });
    })
  )
    passed++;
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('observe.sh skips cooperative skip env before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', OAS_SKIP_OBSERVE: '1' }
      });
    })
  )
    passed++;
    ...
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: {
          ...
          OAS_OBSERVE_SKIP_PATHS: ' observer-sessions , .claude-mem '
        },
  ...
  if (
    await asyncTest('only annotates *-session.tmp files, not other .tmp files', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-glob-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('handles no active session files gracefully', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-nosession-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-empty-'));
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-blank-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('annotates only the newest session when multiple match the current worktree', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-multi-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('does not inject empty session file content into context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-empty-file-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-notools-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-alias-'));
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('parallel compaction runs all append to log without loss', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-par-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when sessions path is a file (not a directory)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-blocked-'));
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-7day-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('prunes session files older than the retention window', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-prune-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('disables pruning when OAS_SESSION_RETENTION_DAYS=0', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-prune-off-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          OAS_SESSION_RETENTION_DAYS: '0'
        });

        assert.strictEqual(result.code, 0);
        assert.ok(fs.existsSync(expiredFile), 'Should keep all sessions when retention is opt-out=0');
        assert.ok(result.stderr.includes('Pruning disabled via OAS_SESSION_RETENTION_DAYS'), `Should log pruning disabled, stderr: ${result.stderr}`);
  ...
  if (
    await asyncTest('disables pruning when OAS_SESSION_RETENTION_DAYS=off', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-prune-offstr-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          OAS_SESSION_RETENTION_DAYS: 'off'
        });
  ...
  if (
    await asyncTest('falls back to default retention when OAS_SESSION_RETENTION_DAYS is garbage', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-prune-garbage-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          OAS_SESSION_RETENTION_DAYS: 'bogus-value'
        });
  ...
  if (
    await asyncTest('injects newest session when multiple recent sessions exist', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-multi-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-start-unreadable-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-compact-ro-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-end-nots-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-role-only-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs "Transcript not found" for nonexistent transcript_path', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-notfound-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-r70-entryname-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-r71-ss-default-'));
  ...
  if (
    await asyncTest('extracts user messages from entries where only message.role is user (not type or role)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-msgrole-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-r81-numcontent-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      const transcriptPath = path.join(isoHome, 'transcript.jsonl');
  ...
  if (
    await asyncTest('collects tool name from entry with tool_name but non-tool_use type', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-r82-toolname-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('preserves file when marker present but regex does not match corrupted template', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'oas-r82-tmpl-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
...
`;
  ...
  // ── Round 95: pre-compact.js — OAS_SKIP_LLM_SUMMARY guard ──
  console.log('\nRound 95: pre-compact.js (transcript_path provided + OAS_SKIP_LLM_SUMMARY=1 — LLM skipped):');

  if (
    await asyncTest('pre-compact falls back to compaction log entry when OAS_SKIP_LLM_SUMMARY=1', async () => {
      const testDir = createTestDir();
  ...
  // ── Round 95: session-end.js — OAS_LLM_SUMMARY_INTERVAL controls trigger ──
  console.log('\nRound 95: session-end.js (OAS_LLM_SUMMARY_INTERVAL — controls LLM trigger cadence):');
  ...
  if (
    await asyncTest('session-end triggers LLM when totalMessages % interval === 0', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'session-end.js'), stdinJson, {
        HOME: testDir,
        OAS_LLM_SUMMARY_INTERVAL: '3',
        OAS_SKIP_LLM_SUMMARY: '1' // prevent actual claude -p invocation in tests
      });
