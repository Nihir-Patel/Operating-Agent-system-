export const meta = {
  name: 'orch-review',
  description:
    'OAS Review phase as a native Claude Code workflow: multi-dimension review (quality + language + conditional security) then adversarial verification of every CRITICAL/HIGH finding. Returns blocking + advisory findings for Gate 2.',
  phases: [
...
// Language → OAS reviewer agent. Mirrors the agents present in agents/.
const LANGUAGE_REVIEWER = {
  typescript: 'oas:typescript-reviewer',
  javascript: 'oas:typescript-reviewer',
  python: 'oas:python-reviewer',
  go: 'oas:go-reviewer',
  rust: 'oas:rust-reviewer',
  java: 'oas:java-reviewer',
  kotlin: 'oas:kotlin-reviewer',
  swift: 'oas:swift-reviewer',
  php: 'oas:php-reviewer',
  csharp: 'oas:csharp-reviewer',
  fsharp: 'oas:fsharp-reviewer',
  react: 'oas:react-reviewer',
  vue: 'oas:vue-reviewer',
  flutter: 'oas:flutter-reviewer',
  dart: 'oas:flutter-reviewer',
  django: 'oas:django-reviewer',
  fastapi: 'oas:fastapi-reviewer',
  cpp: 'oas:cpp-reviewer'
};
...
const dimensions = [
  { key: 'quality', label: 'correctness & quality', agentType: 'oas:code-reviewer' },
  ...(langReviewer ? [{ key: `lang:${input.language}`, label: `${input.language} idioms & pitfalls`, agentType: langReviewer }] : []),
  ...(securityNeeded ? [{ key: 'security', label: 'security (OWASP, secrets, injection)', agentType: 'oas:security-reviewer' }] : [])
];
if (securityNeeded) {
