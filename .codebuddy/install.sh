#!/bin/bash
#
# OAS CodeBuddy Installer
# Installs Operating Agent Systems workflows into a CodeBuddy project.
#
# Usage:
...
# Locate the OAS repo root by walking up from SCRIPT_DIR to find the marker
...
if [ -z "$REPO_ROOT" ]; then
    echo "Error: Cannot locate the OAS repository root."
    echo "This script must be run from within the OAS repository's .codebuddy/ directory."
    exit 1
fi
...
do_install() {
    ...
    MANIFEST="$codebuddy_full_path/.oas-manifest"
    touch "$MANIFEST"
    ...
    ensure_manifest_entry "$MANIFEST" ".oas-manifest"
    ...
    echo "  3. Enjoy the OAS workflows!"
    echo ""
