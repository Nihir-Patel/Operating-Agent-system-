#!/usr/bin/env node
/**
 * OAS CodeBuddy Uninstaller (Cross-platform Node.js version)
 * Uninstalls Operating Agent Systems workflows from a CodeBuddy project.
 *
 * Usage:
...
async function doUninstall() {
  ...
  console.log('OAS CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  const manifest = path.join(codebuddyFullPath, '.oas-manifest');

  // Handle missing manifest
  if (!fs.existsSync(manifest)) {
    console.log('Warning: No manifest file found (.oas-manifest)');
    console.log('');
    console.log('This could mean:');
    console.log('  1. OAS was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by OAS');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall OAS from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
