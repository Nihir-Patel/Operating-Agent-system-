/**
 * OAS adapter for the Pi coding agent.
 *
 * This is the ONLY adapter logic OAS ships for Pi. OAS's canonical assets stay
 ...
 *   - Pi lifecycle events -> OAS's existing hook runner (`run-with-flags.js`),
 *     so OAS hook profiles and disable flags keep working under Pi.
 *   - OAS's SessionStart `additionalContext` payload -> Pi's system prompt.
 *   - A `/oas-doctor` command for install diagnostics.
 *
...
/**
 * OAS package root. This file lives at `<root>/.pi/extensions/index.ts`, so the
 ...
 */
const OAS_ROOT = path.resolve(__dirname, "..", "..")

/** OAS's universal hook runner. It applies hook-profile and disable flags. */
const HOOK_RUNNER = path.join(OAS_ROOT, "scripts", "hooks", "run-with-flags.js")

const HOOK_TIMEOUT_MS = 30_000
const MAX_HOOK_OUTPUT_BYTES = 1024 * 1024

/**
 * OAS rules injected into Pi's system prompt, read from the canonical
...
/** Values OAS treats as "off" across its existing environment switches. */
const DISABLED_VALUES = new Set(["0", "false", "off", "none", "disabled"])

/**
 * Optional Pi companion packages. OAS works without every one of these; they
 * are reported by `/oas-doctor` so users can see which extras are available.
 */
...
interface HookSpec {
  /** OAS hook id, used for profile gating and disable flags. */
  id: string
  /** Hook script path relative to the OAS package root. */
  script: string
...
/**
 * Run an OAS hook through OAS's own runner.
 *
 ...
 * OAS package root if Pi reports a directory that no longer exists, so a stale
...
/**
 * Build the environment OAS hooks expect.
 *
 * `CLAUDE_PLUGIN_ROOT` / `OAS_PLUGIN_ROOT` are how every OAS hook locates the
 * package; setting them from `OAS_ROOT` is what makes a global install resolve
 * correctly instead of probing the user's project. The `CLAUDE_*` session vars
 * are the names OAS's shared hook scripts already read across harnesses.
 */
function buildHookEnv(ctx: ExtensionContext): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    CLAUDE_PLUGIN_ROOT: OAS_ROOT,
    OAS_PLUGIN_ROOT: OAS_ROOT,
...
/**
 * Map Pi's session reason onto the `source` values OAS's SessionStart hook
 ...
 *
 * OAS hooks emit a JSON envelope, but the runner passes stdin straight through
 ...
 * allowlist length would overstate a partial install in `/oas-doctor`, which is
 * the one place a user looks to find exactly that.
 */
let cachedRuleFileCount = 0

/**
 * OAS's portable engineering rules, concatenated from the canonical
 * `rules/common/` directory of the installed package.
 *
 * Returns null when disabled via `OAS_PI_RULES` or when no rule file could be
...
function loadPortableRules(): string | null {
  ...
  if (isDisabledByEnv(process.env.OAS_PI_RULES)) {
    cachedRules = null
  ...
  for (const file of PORTABLE_RULE_FILES) {
    let text: string
    try {
      text = fs.readFileSync(path.join(OAS_ROOT, "rules", "common", file), "utf8").trim()
    } catch {
      continue
    }
...
function readEccVersion(): string {
  try {
    const manifest = JSON.parse(fs.readFileSync(path.join(OAS_ROOT, "package.json"), "utf8")) as {
      version?: string
    }
...
function describeRulesStatus(): string {
  if (isDisabledByEnv(process.env.OAS_PI_RULES)) {
    return "disabled via OAS_PI_RULES"
  }

  const rules = loadPortableRules()
  if (!rules) {
    return `NOT FOUND (${path.join(OAS_ROOT, "rules", "common")})`
  }
...
function buildDoctorReport(ctx: ExtensionContext): string {
  const skillsDir = path.join(OAS_ROOT, "skills")
  const commandsDir = path.join(OAS_ROOT, "commands")
  ...
  const lines = [
    "OAS adapter for Pi",
    "",
    `  OAS version:   ${readEccVersion()}`,
    `  Package root:  ${OAS_ROOT}`,
    `  Project cwd:   ${ctx.cwd}`,
    "",
    "Canonical resources",
    ...
    `  profile:       ${process.env.OAS_HOOK_PROFILE || "standard (default)"}`,
    `  disabled:      ${process.env.OAS_DISABLED_HOOKS || "none"}`,
    "",
  ...
  lines.push(
    "",
    "Companion packages are optional; OAS skills, commands, and session hooks",
...
export default function (pi: ExtensionAPI): void {
  /**
   * OAS's SessionStart hook returns context for the model, but Pi has no
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (result.failure) {
      ctx.ui.notify(`OAS session-start hook skipped (${result.failure})`, "warning")
      return
    }
  ...
  pi.on("before_agent_start", event => {
    ...
    const rules = loadPortableRules()
    if (rules) {
      additions.push(`<oas-engineering-rules>\n${rules}\n</oas-engineering-rules>`)
    }

    if (pendingContext) {
      additions.push(`<oas-session-context>\n${pendingContext}\n</oas-session-context>`)
      pendingContext = undefined
    }
  ...
  pi.on("session_shutdown", async (event, ctx) => {
    ...
    if (result.failure) {
      ctx.ui.notify(`OAS session-end hook skipped (${result.failure})`, "warning")
    }
  })

  pi.registerCommand("oas-doctor", {
    description: "Report OAS adapter status: package root, canonical resources, hooks, companions",
    handler: async (_args, ctx) => {
      pi.sendMessage(
        {
          customType: "oas-doctor",
