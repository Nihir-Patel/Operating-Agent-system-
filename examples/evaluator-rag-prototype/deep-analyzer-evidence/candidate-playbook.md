...
2. Retrieve the Deep Analyzer Evidence contract from `../OAS-Tools/README.md`
   and the follow-up logic in `../OAS-Tools/src/lib/analyzer.ts`.
3. Match the change to maintained corpus or reference evidence:
   - `../OAS-Tools/src/analyzers/fixtures/deep-analyzer-corpus.ts`
   - `../OAS-Tools/src/analyzers/deep-analyzer-corpus.test.ts`
   - `../OAS-Tools/src/lib/analyzer.compare.test.ts`
...
5. Add or update analyzer corpus, expected-output snapshots, fixtures,
   ...
   surface.
6. Run the relevant validation gate from `../OAS-Tools/`:
