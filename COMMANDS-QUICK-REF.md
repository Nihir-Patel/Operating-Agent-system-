...
These are `oas` CLI commands, not slash commands. They use one inspectable
Markdown vault across Claude, Codex, Hermes, OpenClaw, Kimi, and other
harnesses.

| Command | What it does |
|---------|-------------|
| `oas memory init` | Create project, team, or user vault directories |
| `oas memory save` | Create an unreviewed context, decision, fact, lesson, note, preference, or runbook |
| `oas memory handoff` | Transfer bounded work state from one harness to another |
| `oas memory search` | Search memories by text, scope, kind, or target harness |
| `oas memory read` | Read a memory and its backlinks by stable ID |
| `oas memory doctor` | Report malformed files, duplicate IDs, broken links, and skipped symlinks |
| `oas-memory-mcp` | Start the optional local stdio MCP server |
...
These lifecycle commands are also available through the `oas` CLI.

| Command | What it does |
|---------|-------------|
| `oas list-installed` | Show installs recorded in OAS's managed state |
| `oas doctor` | Diagnose missing or drifted managed files and point failures to the short problem form |
| `oas repair` | Restore missing or drifted managed files |
| `oas uninstall` | Remove only install-state-managed files and optionally show the 20-second exit-feedback route |
| `oas feedback` | Show the public problem, quick-feedback, and feature routes without reading files or uploading diagnostics |
...
| Command | What it does |
|---------|-------------|
| `/oas-guide` | Navigate OAS's current agents, skills, commands, hooks, install profiles, and docs from the live repository surface |
...
| `/project-init` | Detect a project's stack and produce a dry-run OAS onboarding plan |
...
| `/auto-update` | Pull the latest OAS repo changes and reinstall the current managed targets |
