...
- OpenCode home installs now use its canonical `~/.config/opencode` location, safely discover and migrate unchanged OAS-managed files from legacy `~/.opencode` installs, and preserve modified legacy files for review. Bundled agents inherit the model selected by the user instead of pinning an Anthropic provider.
...
- Discord community launch: server + GitHub PR/issue/release feed, `release-announce.yml` workflow (announce + pin + Discussions cross-post), and a dependency-free community bot (`scripts/discord/oas-bot.mjs`) with `/oas`, `/help`, `/skill`, `/docs`, `/release`.
- `orch-*` orchestrator skill family and dynamic workflow team orchestration.
- `kubernetes-patterns` skill, worktree-lifecycle service, MCP inventory (`oas.mcp.v1`), codex-worktree and opencode session adapters.
...
- Adds the public OAS 2.0 release-candidate surface for the Hermes operator story.
- Documents OAS as the reusable cross-harness substrate across Claude Code, Codex, Cursor, OpenCode, and Gemini.
...
- Added `docs/architecture/cross-harness.md` and regression coverage for the OAS/Hermes boundary.
- Kept `oas2/` versioning independent for now; it remains an alpha control-plane scaffold unless release engineering decides otherwise.

### Notes

- This is a release candidate, not a GA claim for the full OAS 2.0 control-plane roadmap.
...
- OAS 2.0 alpha control-plane binary now builds locally from `oas2/` and exposes the first usable CLI/TUI surface.
...
- `cargo build --manifest-path oas2/Cargo.toml` passes on the repository baseline.
- `oas-tui` currently exposes `dashboard`, `start`, `sessions`, `status`, `stop`, `resume`, and `daemon`.
...
- This release is a repo-surface correction and ecosystem sync, not a claim that the full OAS 2.0 roadmap is complete.
...
- `operating-agent-systems` — Self-referential OAS skill (#335)
...
- Added `OAS_HOOK_PROFILE` and `OAS_DISABLED_HOOKS` controls.
