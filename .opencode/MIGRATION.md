...
This guide helps you migrate from Claude Code to OpenCode while using the OAS configuration.
...
### OAS Plugin Hooks Included

The OAS OpenCode configuration includes translated hooks:
...
### 2. Use the OAS OpenCode Configuration
...
│   ├── oas-hooks.ts           # All OAS hooks as plugins
...
### Option 1: Use OAS Configuration Directly
...
This only loads the published OAS OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject OAS's full `agent`, `command`, or `instructions` config into your project.

If you want the full OAS OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["oas-universal"]`, note that npm plugin install does not auto-add OAS commands or agents to your project config
...
- **OAS Configuration**: Report to [github.com/neal0709/Operating-Agent-system-](https://github.com/neal0709/Operating-Agent-system-)
