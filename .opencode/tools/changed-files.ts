    ...
    changedFilesStorePromise = import("../plugins/lib/changed-files-store.js").catch(() => {
      ...
      throw new Error(
        "changed-files tool: could not load the changed-files store. " +
          ...
          "(an interrupted or partial OAS install can leave tools/ populated without plugins/). " +
          "Run `node scripts/repair.js --target opencode` (or `oas repair --target opencode`) " +
          "from the OAS repo to restore the missing files."
      )
    })
  }
