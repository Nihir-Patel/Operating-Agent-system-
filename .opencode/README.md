...
> If you installed OAS via npm (e.g. `npm install opencode-oas`), refer to the root README instead.

OAS plugin for OpenCode - agents, commands, hooks, and skills.

## Installation

## Installation Overview

There are two ways to use OAS:

1. **npm package (recommended for most users)**
   Install via npm/bun/yarn and use the `oas-install` CLI to set up rules and agents.
...
This loads the OAS OpenCode plugin module from npm:
- hook/event integrations
- bundled custom tools exported by the plugin

It does **not** auto-register the full OAS command/agent/instruction catalog in your project config. For the full OpenCode setup, either:
...
After installation, the `oas-install` CLI is also available:
...
```bash
git clone https://github.com/neal0709/Operating-Agent-system-
cd OAS
opencode
```

If you also want to apply the OAS home install
...
- `OAS_HOOK_PROFILE`: `minimal`, `standard` (default), `strict`
- `OAS_DISABLED_HOOKS`: comma-separated hook IDs to disable

## Skills

The default OpenCode config loads 11 curated OAS skills via the `instructions` array:
...
provider and select a model in OpenCode; OAS's primary agent uses that global
