...
/**
 * Read OAS version from package.json
 * Falls back to a default if package.json cannot be read
 */
function getOASVersion(): string {
  try {
...
type OASHooksPluginFn = (input: PluginInput) => Promise<Record<string, unknown>>

export const OASHooksPlugin: OASHooksPluginFn = async ({
  client,
  $,
  directory,
  worktree,
  ...
  const log = (level: "debug" | "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "oas", level, message } })
  ...
  // interrupted or partial OAS install on Termux/Android) only disables
  ...
  try {
    ...
    Promise.resolve()
      .then(() =>
        log(
          "warn",
          "[OAS] changed-files tracking disabled: could not load the changed-files store. " +
            "Run `oas repair --target opencode` to restore the missing files. Other OAS hooks are unaffected."
        )
      )
      .catch(() => {})
  }
  ...
  const currentProfile = normalizeProfile(process.env.OAS_HOOK_PROFILE)
  const disabledHooks = new Set(
    (process.env.OAS_DISABLED_HOOKS || "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
  )
  ...
  return {
    ...
    "file.edited": async (event: { path: string }) => {
      ...
      if (hookEnabled("post:edit:format", ["strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          log("info", `[OAS] Formatted: ${event.path}`)
        } catch (error: unknown) {
          // Prettier not installed or failed - log but continue
          const errorMessage = error instanceof Error ? error.message : String(error)
          log("debug", `[OAS] Prettier formatting failed for ${event.path}: ${errorMessage}`)
        }
      }
      ...
      if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[OAS] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`
            )
          }
        } catch {
    ...
    "tool.execute.after": async (
      ...
      if (
        ...
        try {
          ...
          log("info", "[OAS] TypeScript check passed")
        } catch (error: unknown) {
          const err = error as { stdout?: string }
          log("warn", "[OAS] TypeScript errors detected:")
          if (err.stdout) {
      ...
      if (
        ...
        log("info", "[OAS] PR created - check GitHub Actions status")
      }
    },
    ...
    "tool.execute.before": async (
      ...
      if (
        ...
        log(
          "info",
          "[OAS] Remember to review changes before pushing: git diff origin/main...HEAD"
        )
      }
      ...
      if (
        ...
        if (
          ...
          log(
            "warn",
            `[OAS] Creating ${filePath} - consider if this documentation is necessary`
          )
        }
      }
      ...
      if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
        ...
        if (
          ...
          log(
            "info",
            "[OAS] Long-running command detected - consider using background execution"
          )
        }
      }
    },
    ...
    "session.created": async () => {
      ...
      log("info", `[OAS] Session started - profile=${currentProfile}`)

      // Check for project-specific context files
      if (hasProjectFile("CLAUDE.md")) {
        log("info", "[OAS] Found CLAUDE.md - loading project context")
      }
    },
    ...
    "session.idle": async () => {
      ...
      log("info", "[OAS] Session idle - running console.log audit")
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[OAS] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`
        )
        filesWithConsoleLogs.forEach((f) =>
          log("warn", `  - ${f}`)
        )
        log("warn", "[OAS] Remove console.log statements before committing")
      } else {
        log("info", "[OAS] Audit passed: No console.log statements found")
      }
      ...
      try {
        if (process.platform === "darwin") {
          // macOS
          await $`osascript -e 'display notification "Task completed!" with title "OpenCode OAS"' 2>/dev/null`
        } else if (process.platform === "win32") {
          // Windows - PowerShell notification
          await $`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Task completed!', 'OpenCode OAS', 'OK', 'Information')" 2>/dev/null`
        } else if (process.platform === "linux") {
          // Linux - notify-send (requires libnotify)
          await $`notify-send "OpenCode OAS" "Task completed!" 2>/dev/null`
        }
      } catch (error: unknown) {
        // Notification not supported or failed - log but continue
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("debug", `[OAS] Desktop notification failed: ${errorMessage}`)
      }
    ...
    "session.deleted": async () => {
      ...
      log("info", "[OAS] Session ended - cleaning up")
      editedFiles.clear()
    ...
    "todo.updated": async (event: { todos: Array<{ text: string; done: boolean }> }) => {
      ...
      if (total > 0) {
        log("info", `[OAS] Progress: ${completed}/${total} tasks completed`)
      }
    },
     ...
     * Action: Sets PROJECT_ROOT, PACKAGE_MANAGER, DETECTED_LANGUAGES, OAS_VERSION
     */
    "shell.env": async () => {
      const env: Record<string, string> = {
        OAS_VERSION: getOASVersion(),
        OAS_PLUGIN: "true",
        OAS_HOOK_PROFILE: currentProfile,
        OAS_DISABLED_HOOKS: process.env.OAS_DISABLED_HOOKS || "",
     ...
     * Action: Push OAS context block and custom compaction prompt
     */
    "experimental.session.compacting": async () => {
      const contextBlock = [
        "# OAS Context (preserve across compaction)",
        "",
        "## Active Plugin: OAS v2.2.1",
    ...
    "permission.ask": async (event: PermissionEvent) => {
      log("info", `[OAS] Permission requested for: ${event.tool}`)
      ...
      try {
        ...
        if (["read", "glob", "grep", "search", "list"].includes(event.tool)) {
          log("debug", `[OAS] Auto-approved read-only tool: ${event.tool}`)
        ...
        if (event.tool === "bash" && /^(npx )?(@biomejs\/biome|prettier|black|gofmt|rustfmt|swift-format)/.test(cmd)) {
          log("debug", `[OAS] Auto-approved formatter: ${cmd}`)
        ...
        if (event.tool === "bash" && /^(npm test|npx vitest|npx jest|pytest|go test|cargo test)/.test(cmd)) {
          log("debug", `[OAS] Auto-approved test execution: ${cmd}`)
          return { approved: true, reason: "Test execution" }
        }

        // Everything else: let user decide
        log("debug", `[OAS] Permission requires user approval: ${event.tool}`)
        return { approved: undefined }
      } catch (error: unknown) {
        // Error in permission handling - log and deny for safety
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("error", `[OAS] Permission handling error for ${event.tool}: ${errorMessage}`)
