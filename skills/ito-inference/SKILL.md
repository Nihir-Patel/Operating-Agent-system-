...
`ito-inference` is the sole canonical OAS skill for inference serving on Itô
compute. Requests naming `ito-serve` route here; do not create or install a
second `ito-serve` skill. OAS never SSHes to nodes, downloads weights, launches
an engine, or exposes an endpoint; it never books, reserves, or spends.

## Current production boundary

Managed serving is unavailable today. The OAS bridge exposes only `login`,
...
probe; OAS does not invoke or depend on it. The MCP surface exposes only auth,
find, and status. The locally enforceable guarantee is that OAS rejects `serve`
...
The canonical API—not OAS—must own workload creation and return structured JSON
...
These stages describe the future backend, not code that exists in OAS:
...
4. Launch through the provider control plane, never direct root SSH from OAS.
