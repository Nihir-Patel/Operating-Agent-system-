...
OAS-native around the `/plan` confirmation gate, with zero dependencies.
...
Invoke the CLI as `oas-plan-canvas` — the bin shipped by the `oas-universal`
...
- `oas-plan-canvas pending` lists feedback queued with no listener. Check it
...
```bash
oas-plan-canvas await <file> --reply "Split Phase 2 as requested. Take a look."
```
...
oas-plan-canvas typing <file> --state thinking
# switch to "agent is typing..." just before a reply lands
oas-plan-canvas typing <file> --state typing
```
...
**4. End** when review concludes: `oas-plan-canvas end <file>`.
...
Diagrams render in the OAS dark theme with the accent palette. Mermaid loads in
...
mirror at `OAS_PLAN_CANVAS_MERMAID_URL` for air-gapped use.

## Rules

- Markdown artifacts render in OAS's plan template (including Mermaid blocks);
...
- The server is loopback-only and exits after 30 idle minutes
  (`OAS_PLAN_CANVAS_IDLE_MS`); `stop` shuts it down explicitly. State lives
  in `~/.claude/plan-canvas/` (`OAS_PLAN_CANVAS_STATE_DIR`).
...
```bash
oas-plan-canvas open .claude/plans/notifications.plan.md
oas-plan-canvas await .claude/plans/notifications.plan.md
# → {"status":"feedback","items":[{"kind":"verdict","verdict":"approve"}]}
oas-plan-canvas end .claude/plans/notifications.plan.md
...
oas-plan-canvas await <file> --reply "Reworked the risk table."
