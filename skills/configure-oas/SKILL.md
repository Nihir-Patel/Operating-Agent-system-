---
name: configure-oas
description: Guide OAS installation, update, or reconfiguration from inside Claude Code, Codex, or Kimi while respecting each harness's real plugin, scope, and hook capabilities.
metadata:
  origin: OAS
---

# Configure Operating Agent Systems
...
and show the welcome only after success. Never clone OAS into a temporary
directory or copy plugin components by hand.

For a human-operated terminal, the canonical entry points are `oas setup` and
`npx oas-universal setup`. Inside a harness, use the explicit non-interactive
...
- In Codex, use Codex's native plugin lifecycle. Do not offer Claude scopes or
  map OAS's four Claude hook profiles onto Codex.
- In Kimi, install the project surface under `./.kimi-code`. Kimi does not
  provide OAS's Claude lifecycle-hook profiles.
...
Run both commands and summarize the installed OAS scope, enabled state, and
marketplace source:
...
Treat a single existing `oas@oas` installation as a reconfiguration. Do not
interpret Claude's provider-owned "Open home page" control as installation
evidence. Stop and report the recovery returned by setup for multiple OAS
...
- `off` keeps skills and commands but disables OAS hook automation.
...
```bash
npx --yes --package oas-universal oas setup --mode claude-plugin \
  --scope <scope> --hooks <hooks> [--move-scope] --dry-run --json
```
...
Ask one yes/no question. Do not run a bare interactive `oas setup` through a
...
Continue only when exactly one enabled `oas@oas` entry exists at the selected
...
`OAS_VERSION_PATTERN` from `scripts/lib/terminal-welcome.js`. Reject unexpected
...
If the OAS marketplace is missing, add it. Otherwise refresh its snapshot:

```bash
codex plugin marketplace add neal0709/Operating-Agent-system-
codex plugin marketplace upgrade oas --json
```
...
```bash
codex plugin add oas@oas --json
codex plugin list --json
```

Continue only when the JSON reports OAS installed and provides its
...
`OAS_VERSION_PATTERN`. Invoke `node` directly with this argument array; this is
...
`./.kimi-code`; `hooks=unsupported` for OAS lifecycle hooks. Do not ask the
Claude scope or hook-mode questions. Preview first:

```bash
npx --yes --package oas-universal oas install --profile core --target kimi --dry-run
```
...
```bash
npx --yes --package oas-universal oas doctor --target kimi
```
...
```bash
npx --yes --package oas-universal oas welcome --action configured
```

Do not claim that Kimi installed or configured OAS lifecycle hooks.
