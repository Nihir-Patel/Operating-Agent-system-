...
**Opt-in (OAS):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. OAS plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during OAS install/sync with `OAS_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what OAS uses for skill auto-activation.
...
| OAS skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** OAS recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
