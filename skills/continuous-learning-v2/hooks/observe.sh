...
#   - OAS observing its own Haiku observer sessions (self-loop)
#   - OAS observing other tools' automated sessions
...
# (OAS_HOOK_PROFILE=minimal, OAS_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${OAS_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${OAS_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_OAS_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_OAS_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_OAS_SKIP_PATHS="${OAS_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _OAS_SKIP_ARRAY <<< "$_OAS_SKIP_PATHS"
  for _pattern in "${_OAS_SKIP_ARRAY[@]}"; do
...
_NOTE_OBSERVER_NOSURVIVE() {
  ...
  local warn_after="${OAS_OBSERVER_NOSURVIVE_WARN_AFTER:-3}"
  local streak
  ...
  if printf '%s\n' "$streak" > "$streak_file" 2>/dev/null &&
    ...
    printf -v message '%s\n%s\n%s\n%s' \
      ...
      "$platform_hint" \
      "[observe] Set OAS_OBSERVER_NOSURVIVE_WARN_AFTER to change this threshold (currently ${warn_after})."
...
SIGNAL_EVERY_N="${OAS_OBSERVER_SIGNAL_EVERY_N:-20}"
