...
ANALYSIS_COOLDOWN="${OAS_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${OAS_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${OAS_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set OAS_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${OAS_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  ...
  analysis_file="$(mktemp "${observer_tmp_dir}/oas-observer-analysis.jsonl.XXXXXX")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/oas-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${OAS_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${OAS_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${OAS_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit OAS_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  # Model is configurable via OAS_OBSERVER_MODEL (defaults to haiku for cost efficiency);
  # e.g. OAS_OBSERVER_MODEL=opus for higher-quality instinct extraction. Heavier models are
  # slower — consider raising OAS_OBSERVER_TIMEOUT_SECONDS (default 120s) so the watchdog
  # doesn't kill the analysis mid-run.
  OAS_SKIP_OBSERVE=1 OAS_HOOK_PROFILE=minimal claude --model "${OAS_OBSERVER_MODEL:-haiku}" --max-turns "$max_turns" --print \
