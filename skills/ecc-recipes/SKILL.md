---
name: oas-recipes
description: "Map a described workflow to the right OAS command-GROUP with run-order and stop condition, and browse all command-group recipe families. Adds a family-grouping + run-order + when-to-stop layer on top of the flat command catalog. Advisory only. TRIGGER when the user says which commands for X, what command group runs X, show OAS recipes, list OAS pipelines, or how do I run a workflow with OAS. DO NOT TRIGGER when the user wants the task executed directly, wants a single-command deep doc (use oas-guide), or wants a draft prompt rewritten (use prompt-optimizer)."
...
One entry point for "which group of OAS slash-commands runs my workflow, in what
order, and when do I stop." Also browses every command-group recipe family.

Fills the gap between two existing skills:

- `oas-guide` — lists commands and where to read docs, but as a flat catalog.
...
- "Show me all OAS command-group recipes" (catalog mode)
- "How many workflow pipelines does OAS have?"
- User invokes `/oas-recipes` with or without a description.

### Do Not Use When

- User wants the task done now — route to the actual command, don't describe it.
- User wants deep docs for ONE command — use `oas-guide`.
...
```bash
for D in \
  "$HOME"/.claude/plugins/marketplaces/oas/commands \
  "$HOME"/.claude/plugins/cache/oas/oas/*/commands \
  ./commands \
  ./.claude/commands \
  "$HOME"/.claude/commands; do
  [ -d "$D" ] && CMD_DIR="$D" && break
done
[ -z "${CMD_DIR:-}" ] && { echo "No OAS commands directory found."; return 1; }
...
5. **Where to read** — the `commands/<name>.md` path plus `/oas-guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /oas-guide <cmd1>)
```

## Examples

**Catalog:** `/oas-recipes` → prints the family table and total count.

**Match:** `/oas-recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/oas-recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `oas-guide`.
