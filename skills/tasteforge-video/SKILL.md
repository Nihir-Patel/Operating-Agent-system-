...
`tasteforge` package in the Itô video repository; OAS orchestrates and
...
This boundary is the core of the skill. Everything OAS can actually run is
**local, deterministic, and offline**:

| Operation | Deterministic? | OAS may run |
|---|---|---|
...
**Provider generation must fail closed in OAS.** Any live Fal (or other
...
separate lane with its own review. OAS never calls Fal, never reads any API
...
OAS's job is to route here, run the local deterministic commands, and
