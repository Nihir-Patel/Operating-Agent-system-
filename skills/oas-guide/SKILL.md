---
name: oas-guide
description: Guide users through OAS's current agents, skills, commands, hooks, rules, install profiles, and project onboarding by reading the live repository surface before answering.
metadata:
  origin: community
---
...
Use this skill when a user needs help understanding, navigating, installing, or choosing parts of Operating Agent Systems.

## When To Use

Use this skill when the user:

- asks what OAS includes
- wants help finding a skill, command, agent, hook, rule, or install profile
- is new to the repository and needs a guided path
- asks "how do I do X with OAS?"
- asks which OAS components fit a project
...
Answer from current files, not memory. OAS changes quickly, so hard-coded catalog counts, feature lists, and install instructions go stale.

When the OAS repository is available, inspect the relevant files before giving a concrete answer:
...
Use `/project-init` when the user wants OAS configured for a target repo. The expected sequence is:
