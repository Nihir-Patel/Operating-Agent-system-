---
name: oas-tools-cost-audit
description: Evidence-first OAS Tools burn and billing audit workflow. Use when investigating runaway PR creation, quota bypass, premium-model leakage, duplicate jobs, or GitHub App cost spikes in the OAS Tools repo.
metadata:
  origin: OAS
---
...
Use this skill when the user suspects the OAS Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [OAS-Tools](../../OAS-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these OAS-native skills into the workflow when relevant:
...
- user says OAS Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `OAS-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `OAS-Tools` repo, not in `operating-agent-systems`
...
- switch into the sibling `OAS-Tools` repo
