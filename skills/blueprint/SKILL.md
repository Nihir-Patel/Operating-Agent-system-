...
This skill ships with Operating Agent Systems. No separate installation is needed when OAS is installed.

### Full OAS install

If you are working from the OAS repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the OAS diff before updating:
...
If you are vendoring only this skill outside the full OAS install, copy the reviewed file from the OAS repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed OAS commit rather than running `git pull`.
