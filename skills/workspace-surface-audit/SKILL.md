...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value OAS-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
metadata:
  origin: OAS
---
...
This is the OAS-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against OAS-native coverage
...
- Prefer OAS-native workflows over generic "install another plugin" advice when OAS can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in OAS
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent OAS workspaces
...
- existing OAS skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but OAS lacks a billing-operator skill"
- "Google Drive is connected, but there is no OAS-native Google Workspace operator workflow"
...
2. whether OAS already has parity
3. whether OAS only has primitives
4. whether OAS is missing the workflow entirely

### Phase 3: Turn Gaps Into OAS Decisions

For every real gap, recommend the correct OAS-native shape:

| Gap Type | Preferred OAS Shape |
|----------|---------------------|
...
2. **Parity**
   - where OAS already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but OAS lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete OAS-native additions, ordered by impact
...
- If OAS already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what OAS should own next.
