...
- Never use the compute bridge for basket data: `oas ito find` submits an
  authenticated RFQ and `oas ito status` reads RFQ/procurement status, not
...
This skill never uses device authorization or `oas ito login`; those belong to
