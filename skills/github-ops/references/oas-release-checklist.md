# OAS Signed Patch Release Checklist

Use this when releasing `neal0709/Operating-Agent-system-`, especially for `OAS-031` or any follow-up
...
- Milestone: `M0` in the OAS 2.2 release train.
- Contract: ship one exact, verified artifact, keep OAS authority over release
...
## OAS-031 State To Refresh Before Mutating
...
gh run view 33429642769 --repo neal0709/Operating-Agent-system- --json status,conclusion,url
gh run view 33429641766 --repo neal0709/Operating-Agent-system- --json status,conclusion,url
gh release view v2.2.0 --repo neal0709/Operating-Agent-system- --json tagName,targetCommitish,publishedAt,url
...
```bash
gh pr list --repo neal0709/Operating-Agent-system- --state open --limit 20
gh run list --repo neal0709/Operating-Agent-system- --branch main --limit 10
git fetch origin main --tags
git switch main
...
gh release view "v${VERSION}" --repo neal0709/Operating-Agent-system-
npm view "oas-universal@${VERSION}" version
```
...
- Do **not** use `scripts/release.sh` as-is for `OAS-031`.
...
```bash
gh run list --repo neal0709/Operating-Agent-system- --branch main --limit 10
gh run view RUN_ID --repo neal0709/Operating-Agent-system- --json status,conclusion,url
git fetch origin main --tags
git switch main
git pull --ff-only origin main
git rev-parse HEAD
...
git rev-parse HEAD
git rev-parse origin/main
git tag -s "v${VERSION}" -m "OAS ${VERSION}" HEAD
git tag -v "v${VERSION}"
...
VERSION=2.2.1
npm view oas-universal dist-tags --json
npm view "oas-universal@${VERSION}" name version dist.integrity --json
gh release view "v${VERSION}" --repo neal0709/Operating-Agent-system- \
  --json tagName,name,isDraft,isPrerelease,publishedAt,url
gh api repos/neal0709/Operating-Agent-system-/releases/latest --jq .tag_name
npx --yes "oas-universal@${VERSION}" setup --help
...
### 9. Record Evidence And Close Out OAS-031
...
- Mark `OAS-031` resolved only after the signed patch release is public and
