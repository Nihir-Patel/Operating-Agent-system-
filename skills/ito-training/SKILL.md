...
`ito-training` is the canonical OAS skill for training on Itô compute. OAS
...
Managed training is unavailable today. The OAS bridge exposes only `login`,
...
backend remain scaffolds. The locally enforceable guarantee is that OAS rejects
