...
- Keep npm and npx's executable cache at `NPM_CONFIG_CACHE=/tmp/npm-cache` on
  ...
  with `OAS_TMPFS_SIZE`; `OAS_WORKSPACE_SIZE` separately controls the private
  workspace mount.
...
### Exercise the OAS Plugin Setup Harness
...
```bash
docker compose -p oas-plugin-debian-test \
  -f docker/plugin-setup/compose.yaml \
  run --rm -T real-cli dry-run

docker compose -p oas-plugin-ubuntu-test \
  -f docker/plugin-setup/compose.yaml \
...
```bash
oas install --profile core --target claude-project --dry-run --json
```
...
self-created tarball under `/tmp`, validates the `oas-universal` package name,
required install manifests, and the confined `package.json` `bin.oas` mapping,
then invokes the extracted `oas` executable. The runtime stays on
...
```bash
docker compose -p oas-plugin-session \
  -f docker/plugin-setup/compose.yaml \
  run --detach --name oas-plugin-shell real-cli shell
```
...
```bash
docker inspect --format '{{.State.Running}}' oas-plugin-shell
...
```bash
docker exec -it -w /workspace/project oas-plugin-shell bash
```
...
```bash
docker rm --force oas-plugin-shell
docker compose -p oas-plugin-session \
  -f docker/plugin-setup/compose.yaml \
  down --remove-orphans
```
...
```bash
docker image inspect oas-plugin-setup:debian oas-plugin-setup:ubuntu
```
