...
### Option A: Use the OAS hook (zero install)

The hook at `scripts/hooks/gateguard-fact-force.js` is included in this plugin. Enable it via hooks.json.

If GateGuard blocks setup or repair work, start the session with
`OAS_GATEGUARD=off`. For hook-level control, keep using
`OAS_DISABLED_HOOKS` with the GateGuard hook ID.
...
`OAS_GATEGUARD=off` (or `GATEGUARD_DISABLED=1`) turns the gate off entirely.
...
| Variable | Effect |
|---|---|
| `OAS_GATEGUARD=off` | Disables GateGuard for the session. Accepts `0`, `false`, `off`, `disabled`, or `disable`. |
| `GATEGUARD_DISABLED=1` | Same effect. Recognises `1` only — the spellings above do **not** apply here. |

For hook-level control, keep using `OAS_DISABLED_HOOKS` with the GateGuard hook ID.
