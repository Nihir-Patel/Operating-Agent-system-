...
Use the canonical Itô compute CLI or MCP server. OAS does not implement a
parallel client, local simulation, reservation, workload runner, or inference
server. OAS itself does no browser automation.
...
OAS never discovers this credential-bearing client through `PATH`.
`oas ito login` performs device authorization and never inherits `ITO_API_KEY`.
...
1. Run `oas ito login` before the first operation. OAS delegates this to the
   ...
   `oas ito login --no-browser` to suppress the page handoff. OAS itself does no
   ...
   return to the originating task and continue with `oas ito auth`.
...
2. Run `oas ito auth` to validate existing credentials; it never starts login
   and rejects `--no-browser`.
3. Before `oas ito find`, obtain explicit buyer authority to submit an RFQ.
...
5. Run `oas ito status` to inspect RFQs and procurement orders.
...
7. Run `oas ito logout` when the user explicitly asks to revoke this device.
...
`oas ito evals` exposes the canonical CLI's narrow live adapter to a separately
...
recover, repair, reset, purchase, or order resources. OAS does not forward
...
order. The passive dashboard link in OAS help is a separate user-operated web
