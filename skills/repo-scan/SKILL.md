---
name: repo-scan
description: Bootstrap pointer that installs the external repo-scan skill from a pinned, reviewable commit. Use when repo-scan must be installed before running its cross-stack source-code asset audit; this OAS pointer does not perform the audit itself.
metadata:
  origin: community
---
...
Installation completes only the bootstrap. Reload your agent harness, then invoke `repo-scan` again. This OAS pointer installs the external skill but does not run a scan itself.
