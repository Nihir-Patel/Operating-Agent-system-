...
- <https://github.com/neal0709/Operating-Agent-system-/security/advisories/new>

You can also email **<nihir@oas.tools>** (the `security@oas.tools` alias is not monitored — use `nihir@oas.tools`).
...
- the `neal0709/Operating-Agent-system-` repository
- the `oas-universal` npm package
- OAS plugin, install, repair, dashboard, hook, rule, skill, MCP, and command surfaces shipped from this repository
- GitHub Actions workflows and release automation in this repository
- the OAS Tools GitHub App integration points documented by this repository
- AgentShield usage docs when they are embedded here. AgentShield code issues belong in <https://github.com/neal0709/agentshield>

## Official Distribution Surfaces

Official OAS surfaces are:

- GitHub repo: <https://github.com/neal0709/Operating-Agent-system->
- npm package: `oas-universal`
- GitHub App: <https://github.com/apps/oas-tools>
- marketplace/plugin slug: `oas@oas`
- website: <https://oas.tools>

Official AgentShield surface:

- npm package: `oas-agentshield`
- GitHub repo: <https://github.com/neal0709/agentshield>

The following packages have been observed using OAS repository metadata but are **not maintained by OAS**:

- `@chil_ntl/oas-cli`
- `oas-100xprompt-plugin`

Treat any package not listed under official surfaces as unofficial until verified. Do not install packages named `opencode-oas`, `operating-agent-systems`, or other OAS-like aliases unless this repository explicitly documents them as official.

GitHub dependency graph may also show Go module aliases such as `github.com/neal0709/oas` or historical repository paths. OAS is not currently distributed as a supported Go module.
...
- screenshots, stale line numbers, or reports against `neal0709/operating-agent-systems` that do not reproduce on current `neal0709/Operating-Agent-system-`
- self-XSS or social engineering with no repository-controlled exploit path
- dependency graph/package metadata confusion without an install path to an official OAS package
- vulnerabilities in third-party packages unless OAS pins, installs, or executes them in a way that creates extra impact
...
OAS treats supply-chain exposure as a first-class security surface.
...
- Package metadata should point at `neal0709/Operating-Agent-system-`, not historical repo paths.
...
OAS runs inside agent harnesses that may inject ephemeral client-side system reminders into the model input on every turn. These blocks are not automatically repository-carried payloads.
...
- **AgentShield:** `npx oas-agentshield scan`
