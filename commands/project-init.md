---
description: Detect a project's stack and produce a dry-run OAS onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable OAS onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
/project-init --config oas-install.json
```
...
3. Use OAS's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- OAS config: `oas-install.json`
- optional stack map: `config/project-stack-mappings.json` in the OAS repo

When the OAS checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed OAS manifests and explicit user choices.
...
3. Resolve the smallest useful OAS plan:
   - project has an `oas-install.json`: `node scripts/install-plan.js --config oas-install.json --json`
...
- `/oas-guide` for interactive feature discovery before installing
